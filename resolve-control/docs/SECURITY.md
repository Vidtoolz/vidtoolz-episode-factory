...
* **Authenticated, replay-resistant transport.** Per-worker 32-byte secret, HMAC-SHA256 over ts+nonce+method+path+body
  hash, 120 s skew window, nonce cache, constant-time compare; 401 before any parsing. Wrong key live → `AUTHENTICATION_FAILED`.
...
* **Lifecycle containment.** No service/task/autostart/firewall change. Session anchor kills orphaned workers.
  Worker restart is visible (`worker_generation`, `worker_instance_id`) and pinnable.
