...
function buildPrompt(task) {
  ...
  return ['Plan visual coverage for the bounded Story. Local shot choices only; do not invent global style, factual authority, IDs, infrastructure, approvals, or Camera mechanics.', 'Return JSON only, using exactly the keys shown. Assess every required beat exactly once. For INTENTIONAL_NO_VISUAL use a nonempty reason and shots:[]. For PLAN_SHOTS use no_visual_reason:null and one or more shots.', 'Use exact media/mode pairs: GENERATED_STILL/STILL; GENERATED_VIDEO/DIRECT_VIDEO or IMAGE_TO_VIDEO; INFOGRAPHIC, MAP_ANIMATION, SCREEN_CAPTURE, ARCHIVAL_EXTERNAL, PRESENTER_A_ROLL, and TEXT_GRAPHIC with NOT_APPLICABLE. MAP_ANIMATION is Camera-only: set camera_required:true and supply bounded camera_intent with at least subject and purpose.', `Output schema: ${JSON.stringify(schema)}`, `Central claim: ${task.story.central_claim || ''}`, `Narrative spine: ${task.story.narrative_spine || ''}`, `Output target: ${JSON.stringify(task.output_target || {})}`, `Sections: ${JSON.stringify(task.story.sections)}`, `Required beats: ${JSON.stringify(task.required_beats)}`, `Research constraints: ${JSON.stringify(task.research?.required_constraint_ids || [])}`, `Operator instructions: ${task.operator_instructions || ''}`].join('\n');
}
