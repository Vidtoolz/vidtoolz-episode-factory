...
async function run(task, options = {}) {
  ...
  out.generation_handoffs = plan.shots.filter((s) => !['PRESENTER_A_ROLL', 'SCREEN_CAPTURE', 'ARCHIVAL_EXTERNAL'].includes(s.media_type)).map((s) => promptAdapter.generationSupervisorProjection(task, plan, s));
