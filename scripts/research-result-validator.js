...
function validateHumanException(e, options = {}) {
  ...
  const stale = (options.current_result_ref && canonicalJson(options.current_result_ref) !== canonicalJson(rr)) || (options.current_script_usage_ref && canonicalJson(options.current_script_usage_ref) !== canonicalJson(sr)) || verifyApprovalBindingForScope(e.approval_binding, options.current_exception_bytes ?? null, 'RESEARCH_EXCEPTION').verdict !== 'VALID'; return { status: stale ? 'STALE' : 'VALID', ok: !stale, reason_codes: stale ? ['HUMAN_EXCEPTION_STALE'] : [], errors: [] };
}
