...
Return 3 alternative but equally promising video candidate angles. For each, include core viewer problem, why serious creators should care, potential claim, proof path, packaging potential, overclaim risk, and VIDTOOLZ fit. Do not claim anything as proven without evidence.`;
  ...
  // plainNextAction() lives in the shared plain-next-action.js module (Fix #2), reused
  // on the Build New Video page. In the browser it's a global (loaded first); under Node
  // tests we require it. One source of truth either way.
  const plainNextAction = (globalScope && globalScope.plainNextAction)
    ? globalScope.plainNextAction
    : (typeof require !== "undefined" ? require("./plain-next-action.js").plainNextAction : null);
