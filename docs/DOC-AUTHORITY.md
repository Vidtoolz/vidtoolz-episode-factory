...
| Resolve execution subsystem (doctrine, transport, host/library, identity/timebase, capability matrix, canary authority, permissions, freeze status, evidence store) | `docs/resolve-integration/v1.9/FREEZE-MANIFEST.json` (current; self-identifying and hash-pinned by its own contents; `AUTHORITY-PRECEDENCE.json` decides conflicts; v1.8, v1.7, v1.6, v1.5, v1.4, v1.3, v1.2, v1.1 and v1.0 are immutable history — see the lineage table below) | read the current `FREEZE-MANIFEST.json` first; a document disagreeing with the manifest hash is not the frozen version; `SCHEMA-VALID != AUTHORIZED TO MUTATE`; `PERMISSION DECLARATION != ELIGIBILITY`. Nothing in the bundle is human approval of any run. v1.9 is a CANDIDATE under independent review: it is the current authority bundle for reading, and it authorizes no run, gate, refreeze or mutation. Evidence storage has exactly one authority, `docs/resolve-integration/v1.9/tools/evidence_store.py`. |
...
- `docs/resolve-integration/v1.9/` — Resolve execution subsystem authority bundle, current version (v1.9 correction of v1.8's evidence-store integrity findings; `docs/resolve-integration/v1.8/`, `v1.7/`, `v1.6/`, `v1.5/`, `v1.4/`, `v1.3/`, `v1.2/`, `v1.1/` and `v1/` are immutable history; `experiments/quarantine-2026-09-08/` holds the quarantined non-authoritative prototypes).
...
| --- | --- | --- | --- |
| 1.9.0 | `docs/resolve-integration/v1.9/` | read `docs/resolve-integration/v1.9/FREEZE-MANIFEST.json` (self-identifying; not restated here, see the note below) | **CURRENT** — candidate for independent review, correction of the v1.8 evidence-store integrity findings S19-1 … S19-4 |
...
manifest  read docs/resolve-integration/v1.9/FREEZE-MANIFEST.json
```
...
The v1.8 manifest digest is deliberately **not** restated here. `DOC-AUTHORITY.md` is pinned by that manifest's
`external_pins.doc_authority`, so a digest quoted here would depend on a manifest that depends on this file: the two
would never converge. The manifest is self-identifying; read it. This registration is committed in the same commit as
the v1.9 bundle, so the pin is fresh rather than a follow-up, and the four intake identities including the commit HEAD
are published in the v1.9 authoring report and handoff.
