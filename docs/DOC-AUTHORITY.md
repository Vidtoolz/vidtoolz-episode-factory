...
| Resolve execution subsystem (doctrine, transport, host/library, identity/timebase, capability matrix, canary authority, permissions, freeze status) | `docs/resolve-integration/v1.1/FREEZE-MANIFEST.json` (current; hash-pinned; parent v1.0 at `docs/resolve-integration/v1/` is immutable history) | read the current `FREEZE-MANIFEST.json` first; a document disagreeing with the manifest hash is not the frozen version; `SCHEMA-VALID != AUTHORIZED TO MUTATE`. Nothing in the bundle is human approval of any run. |
...
- `docs/resolve-integration/v1.1/` — Resolve execution subsystem authority bundle, current version (v1.1 correction of the 2026-09-08 v1.0 freeze; `docs/resolve-integration/v1/` is the immutable v1.0 parent; `experiments/quarantine-2026-09-08/` holds the quarantined non-authoritative prototypes).
