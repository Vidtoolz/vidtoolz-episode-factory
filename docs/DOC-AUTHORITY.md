...
| Resolve execution subsystem (doctrine, transport, host/library, identity/timebase, capability matrix, canary authority, permissions, freeze status) | `docs/resolve-integration/v1.5/FREEZE-MANIFEST.json` (current; hash-pinned; `AUTHORITY-PRECEDENCE.json` decides conflicts; v1.4 at `docs/resolve-integration/v1.4/`, v1.3 at `docs/resolve-integration/v1.3/`, v1.2 at `docs/resolve-integration/v1.2/`, v1.1 at `docs/resolve-integration/v1.1/` and v1.0 at `docs/resolve-integration/v1/` are immutable history) | read the current `FREEZE-MANIFEST.json` first; a document disagreeing with the manifest hash is not the frozen version; `SCHEMA-VALID != AUTHORIZED TO MUTATE`; `PERMISSION DECLARATION != ELIGIBILITY`. Nothing in the bundle is human approval of any run. |
...
- `docs/resolve-integration/v1.5/` — Resolve execution subsystem authority bundle, current version (v1.5 correction of v1.4; `docs/resolve-integration/v1.4/`, `docs/resolve-integration/v1.3/`, `docs/resolve-integration/v1.2/`, `docs/resolve-integration/v1.1/` and `docs/resolve-integration/v1/` are immutable history; `experiments/quarantine-2026-09-08/` holds the quarantined non-authoritative prototypes).
