...
era — `authority_lib.py#derive_attachment_state`, `#evaluate_eligibility` (both DIAGNOSTIC / NON_AUTHORIZING since
v1.14 S61), `#validate_transaction_set` or `#commit_eligibility` (both PROVISIONAL_UNTIL_M3 / NON_AUTHORIZING since
S62) — that name now reads as its **authorizing counterpart**. The four bare-evidence functions are retained — the first two as
