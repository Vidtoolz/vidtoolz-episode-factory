...
record is registered through authority_lib.make_record.
    ...
    python3 -B tools/a2_prepare.py adopt   --session <id> --uuid <uuid> --library-root <abs library path> --by "<actor>" \
                                           [--principal PREPARER:<actor>] [--journal-sha <sha256>] \
                                           [--launch-script <path> | --recipe-sha <sha256>]
    python3 -B tools/a2_prepare.py prepare --session <id> [--principal ...]
    python3 -B tools/a2_prepare.py status  --session <id>
...
def _adopt(a):
    ...
    if not L.is_abs_path(a.library_root):
        print(f"REFUSED: --root {a.library_root!r} is not an absolute traversal-free path"); return 2
    if a.recipe_sha:
        recipe = a.recipe_sha
        ...
        return 2
    p = A.create_evidence_set(a.session, a.principal)
    print(f"created OPEN      : {p}")
    prov = A.add_provisioning_record(a.session, a.principal, instance_uuid=a.uuid, root_path=a.library_root,
                                     provisioned_by=a.by)
    print(f"PROVISIONING_RECORD {prov['record_id']}")
    lr = A.add_launch_recipe(a.session, a.principal, recipe_sha256=recipe)
    print(f"LAUNCH_RECIPE       {lr['record_id']}  (version/binary/scripting derived from the contract)")
    if a.journal_sha:
        j = A.add_read_only_journal(a.session, a.principal, journal_path_sha256=a.journal_sha)
        print(f"READ_ONLY_JOURNAL   {j['record_id']}  (probe ELIGIBILITY, not attachment readiness)")
    return 0


def _prepare(a):
    d = A.mark_prepared(a.session, a.principal)
...
def main(argv=None):
    ...
    for name in ("adopt", "prepare", "status"):
        ...
        if name == "adopt":
            ...
            p.add_argument("--library-root", required=True, dest="library_root",
                           help="absolute path of the ALREADY-PROVISIONED Resolve Disk library. This is a record "
                                "field, not the evidence-set root: the governed attachment root is frozen and "
                                "cannot be chosen by any caller (V112-RP1).")
