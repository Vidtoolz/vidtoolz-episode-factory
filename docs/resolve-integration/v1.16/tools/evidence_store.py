#!/usr/bin/env python3
"""THE evidence store (v1.11). One normative, executable, append-only, closed-world, semantically reconciled
evidence-session authority with a persisted filesystem boundary and continuous metadata enforcement.

v1.11 is an extremely narrow correction of Codex's four remaining v1.10 findings. Everything v1.10 established that
passed independent review is retained byte-for-byte in behaviour and re-run as a regression: the recomputed-model
architecture, the record-key law, content addressing, the mode table, the session/root filesystem identity law, the
finalization field set, the closed world, the single-store singularity and the frozen filesystem error vocabulary.

  F110-A  BOUNDARY RECEIPT NOT COMPLETELY RECONCILED. v1.10 persisted the receipt and verified that its own digest
          recomputed, but five of its fields - schema, evidence_store_version, authority_version, platform_scope and
          created_at - were never compared to anything outside the receipt. Rewriting one of them and recomputing
          boundary_sha256 left the next put_raw() authorized. v1.11 classifies every BOUNDARY.json field with exactly
          one provenance class and independently reconciles every normative one against the frozen constant or the
          filesystem fact it claims. created_at is explicitly classified INFORMATIONAL_NON_AUTHORIZING because this
          store governs no independent creation-event authority (section 3a). Digest consistency is byte integrity
          over the stored receipt; it is not semantic truth, and authorization now requires both.

  F110-B  INVENTORY HEADER NOT COMPLETELY RECONCILED. Six normative header fields - record_count, entry_count,
          total_bytes, attempt_count, self_path and finalized_marker_path - were written into the inventory, covered by
          inventory_sha256, published in INVENTORY-FIELD-PROVENANCE.json as derived, and then never compared. An extra
          semantic top-level field also survived verification. v1.11 freezes the exact inventory field vocabulary,
          recomputes all six from the independent model and the frozen store layout, and refuses any missing field,
          unknown field, duplicate field or wrong type. The inventory schema is versioned for the new closed
          vocabulary (Inventory.v3 -> v4).

  F110-C  MARKER / RECORD CARDINALITY NOT ENFORCED. v1.10 required that every marker resolve to a record whose key
          recomputes from the marker tuple, but never required that a record have exactly ONE marker. Two distinct
          attempt ids over the same session, layer, logical identity and bytes produced two valid markers resolving to
          one content-addressed record, and finalize() and verify() both passed. v1.11 enforces the bijection in both
          directions: cardinality zero is ORPHANED_RECORD, cardinality above one is MARKER_CARDINALITY_VIOLATION, and
          the second write is refused before any ambiguous state is persisted.

  F110-D  PUBLIC AUTHORITY SURFACES DID NOT CHECK THE BOUNDARY FIRST. attempts(), expected_model(), manifest(),
          identity(), manifest_sha256(), identity_errors(), scan_tree(), boundary_receipt() and authority_errors()
          returned authority-bearing session data after the session directory had been replaced, and inventory() and
          finalization_marker() masked the boundary violation with the less important SESSION_NOT_FINALIZED. v1.11
          publishes the explicit public authority-bearing method inventory, makes every one of them check the
          persisted boundary before any other precondition, and freezes the error precedence so boundary invalidity
          can never be masked by a state, model or read error.

Inherited context: v1.10 corrected Codex's five v1.9 findings. Everything v1.7, v1.8 and v1.9 established is retained
and re-run.
...
Laws (EVIDENCE-ROOT.md is the normative prose; this file is the executable authority):
  ...
  L3  BOUNDARY RECEIPT      the established boundary is persisted as governed evidence at creation and re-checked
                            before every authorizing operation (S110-1). Every normative receipt field is
                            independently reconciled against the constant or filesystem fact it claims; the receipt
                            digest is byte integrity, never semantic truth (F110-A).
  ...
  L7  ATTEMPT BIJECTION     stored markers and records must agree one-to-one, each derived from the other (S110-3),
                            with EXACT cardinality in both directions: one marker per record and one record per
                            marker (F110-C).
  ...
  L14 CLOSED VOCABULARY     the BOUNDARY receipt, the INVENTORY header and the FINALIZATION marker each have an
                            EXACT frozen top-level field set: a missing, unknown, duplicate or mistyped field is a
                            refusal, never a tolerated extension. Extension requires a schema version (F110-B).
  L15 BOUNDARY DOMINANCE    every public authority-bearing method checks the persisted boundary BEFORE any state,
                            model or read precondition, so a substituted session can never be reported as a
                            lesser status error (F110-D).
...
INVENTORY_DOMAIN = "vidtoolz.resolveEvidenceInventory.v4"
...
ERROR_CODES = (
    ...
    "INVENTORY_FIELD_SET_INVALID", "MARKER_CARDINALITY_VIOLATION",
    # metadata
...
# ---- F110-B section 10. THE exact top-level INVENTORY field vocabulary. Verification refuses a missing normative
# field, an unknown extra semantic field, a duplicate field (refused at the strict parse boundary) or a wrong type.
# Semantic extension is a SCHEMA VERSION change, never a tolerated additional key: that is why Inventory.v3 became v4.
INVENTORY_FIELDS = ("schema", "evidence_store_version", "authority_version", "platform_scope", "session_id",
                    "session_dir_basename", "session_identity_sha256", "boundary_identity_sha256", "boundary_sha256",
                    "session_manifest_sha256", "closed_world", "semantic_reconciliation", "recomputed_model",
                    "record_count", "entry_count", "total_bytes", "attempt_count", "directories", "mode_table",
                    "self_path", "finalized_marker_path", "entries")
INVENTORY_FIELD_SET = tuple(sorted(INVENTORY_FIELDS + ("inventory_sha256",)))
# The exact frozen top-level field set of the FINALIZATION marker (unchanged from v1.10; L14 names it explicitly).
FINALIZATION_FIELDS = ("schema", "evidence_store_version", "authority_version", "platform_scope", "session_id",
                       "session_identity_sha256", "boundary_identity_sha256", "inventory_sha256", "state")
# F110-B section 13. THE frozen total_bytes scope: every governed REGULAR file entry of the closed world EXCEPT
# INVENTORY.json and FINALIZED, which finalization creates and which therefore cannot be inside their own count.
TOTAL_BYTES_SCOPE = "GOVERNED_ENTRIES_EXCLUDING_INVENTORY_AND_FINALIZATION_MARKER"
...
# ---- F110-A section 3. THE boundary field provenance table. Every BOUNDARY.json field has EXACTLY ONE class; there
# is no ambiguous or unclassified field, and INVENTORY-FIELD-PROVENANCE.json republishes this table verbatim.
#   DERIVED_FROM_FILESYSTEM         re-lstat'd against the live filesystem on every authorizing operation
#   DERIVED_FROM_SESSION_AUTHORITY  recomputed from the session this store instance was opened as, or from the
#                                   receipt's own canonical bytes
#   FROZEN_CONSTANT                 compared to a constant of THIS evidence-store authority, not to itself
#   INFORMATIONAL_NON_AUTHORIZING   structurally validated, covered by boundary_sha256 for byte integrity, and
#                                   deliberately NOT authorizing: changing it consistently does not refuse
PROV_FS = "DERIVED_FROM_FILESYSTEM"
PROV_SESSION = "DERIVED_FROM_SESSION_AUTHORITY"
PROV_CONST = "FROZEN_CONSTANT"
PROV_INFO = "INFORMATIONAL_NON_AUTHORIZING"
BOUNDARY_PROVENANCE = {
    "schema": PROV_CONST,
    "evidence_store_version": PROV_CONST,
    "authority_version": PROV_CONST,
    "platform_scope": PROV_CONST,
    "session_id": PROV_SESSION,
    "root_path": PROV_FS,
    "root_device": PROV_FS,
    "root_inode": PROV_FS,
    "session_basename": PROV_FS,
    "session_device": PROV_FS,
    "session_inode": PROV_FS,
    "created_at": PROV_INFO,
    "boundary_sha256": PROV_SESSION,
}
# The four FROZEN_CONSTANT receipt fields, each compared to the constant of THIS authority. A receipt written by an
# earlier evidence-store version therefore does not authorize under v1.11: the store law changed (L7 cardinality, L14
# closed vocabulary, L15 boundary dominance), so v4 sessions are deliberately not v5 sessions.
BOUNDARY_FROZEN_CONSTANTS = {
    "schema": EVIDENCE_BOUNDARY_SCHEMA,
    "evidence_store_version": EVIDENCE_STORE_VERSION,
    "authority_version": L.AUTHORITY_VERSION,
    "platform_scope": PLATFORM_SCOPE,
}
BOUNDARY_STR_FIELDS = ("session_id", "root_path", "session_basename")
BOUNDARY_INT_FIELDS = ("root_device", "root_inode", "session_device", "session_inode")
# created_at law (F110-A section 6, DELIBERATE DECISION): INFORMATIONAL_NON_AUTHORIZING. This store records no
# governed timestamp anywhere - MODE_TABLE deliberately excludes mtime, owner and inode age - so there is no
# independent creation-event authority to bind it to. Comparing it to the session manifest's created_at would compare
# one caller-supplied value to another caller-supplied value written by the same call in the same trust domain, which
# is exactly the self-assertion F110-A objects to. It is therefore NOT authorizing: it is type-checked, it is covered
# by boundary_sha256 so an unrecomputed edit is still caught as byte tampering, it is excluded from boundary_identity
# and so from the session identity, and a consistent rewrite of it does NOT refuse. No claim that every BOUNDARY field
# is normative survives in v1.11.
CREATED_AT_CLASSIFICATION = PROV_INFO
...
# ================================================================ L15 the public authority surface (F110-D sections 27, 33)
# THE explicit inventory of every public EvidenceStore surface, classified by discovery of what it returns, never by
# naming convention. validate_v1_16.py asserts this inventory is COMPLETE against dir(EvidenceStore) and that every
# authority-bearing member refuses a replaced session before any other semantic result.
#
# RAISES: check_boundary() runs first and its EvidenceStoreError propagates.
# REPORTS: the method's contract is to RETURN a refusal list or a state label, so the boundary failure is expressed
#          FIRST in that method's own vocabulary rather than raised.
PUBLIC_AUTHORITY_READERS_RAISING = ("attempts", "boundary_receipt", "check_boundary", "expected_model",
                                    "finalization_marker", "get", "identity", "inventory", "manifest",
                                    "manifest_sha256", "scan_tree")
PUBLIC_AUTHORITY_READERS_REPORTING = ("active_integrity", "authority_errors", "identity_errors", "session_state",
                                      "verify", "verify_summary")
PUBLIC_AUTHORITY_WRITERS = ("finalize", "put", "put_derived", "put_json", "put_promotion", "put_raw", "put_review")
# Authority-bearing state, boundary-checked because the finalization state of a substituted session is not this
# session's finalization state.
PUBLIC_AUTHORITY_PROPERTIES = ("finalized",)
# NOT authority-bearing: pure derivation under frozen store law and the caller's own construction arguments. These
# read no session content and can influence no downstream authorization on their own.
PUBLIC_NON_AUTHORITY = ("record_path", "root", "dir", "session_id", "boundary")
PUBLIC_AUTHORITY_METHODS = tuple(sorted(PUBLIC_AUTHORITY_READERS_RAISING + PUBLIC_AUTHORITY_READERS_REPORTING
                                        + PUBLIC_AUTHORITY_WRITERS + PUBLIC_AUTHORITY_PROPERTIES))
# ---- L15 THE frozen error precedence for every public authority-bearing surface. A lower-priority condition may
# never be reported while a higher-priority one holds, which is what made inventory() answer SESSION_NOT_FINALIZED
# for a substituted session in v1.10.
ERROR_PRECEDENCE = (
    "BOUNDARY",        # 1. persisted receipt validity, frozen constants, root/session filesystem identity and mode
    "SESSION_STATE",   # 2. finalization state, SESSION_FINALIZED / SESSION_NOT_FINALIZED, SESSION_INVALID
    "MODEL",           # 3. the recomputed semantic model: entries, markers, records, modes, inventory reconciliation
    "READ",            # 4. individual read preconditions: RECORD_MISSING, MANIFEST_MISSING
)
ERROR_PRECEDENCE_CLASS = {
    "BOUNDARY": ("ROOT_SYMLINK_REFUSED", "ROOT_NOT_A_DIRECTORY", "ROOT_REPLACED", "ROOT_IDENTITY_MISMATCH",
                 "SESSION_SYMLINK_REFUSED", "SESSION_BOUNDARY_CHANGED", "BOUNDARY_RECEIPT_MISSING",
                 "BOUNDARY_RECEIPT_INVALID", "SESSION_PATH_MISMATCH"),
    "SESSION_STATE": ("SESSION_FINALIZED", "SESSION_NOT_FINALIZED", "SESSION_INVALID", "SESSION_NOT_FOUND",
                      "SESSION_EXISTS"),
    "MODEL": ("MARKER_CARDINALITY_VIOLATION", "ORPHANED_RECORD", "INVENTORY_FIELD_SET_INVALID", "INVENTORY_TAMPERED",
              "FINALIZATION_MARKER_INVALID", "MODE_MISMATCH", "UNEXPECTED_ENTRY", "EVIDENCE_REPLACED"),
    "READ": ("RECORD_MISSING", "MANIFEST_MISSING"),
}
...
class EvidenceStore:
    ...
    L15 (F110-D): every member named in PUBLIC_AUTHORITY_METHODS checks the persisted boundary BEFORE any other
    precondition. The only unchecked internals are private (_boundary_receipt, _scan_tree, _put, _require_can_write),
    because check_boundary() is built from them."""
    ...
    def _boundary_receipt(self):
        """L3/L14 (sections 1, 2, 7). Read the persisted receipt and reconcile EVERY normative field independently.

        PRIVATE by L15: this is the primitive check_boundary() itself is built from, so it cannot boundary-check first
        without unbounded recursion. The checked public surface is boundary_receipt(), which is check_boundary().

        Three layers, in this order:
          1  EXACT field vocabulary  - the top-level field set is the canonical one, no more and no less (L14).
          2  BYTE INTEGRITY          - boundary_sha256 is the digest of these stored bytes. Integrity only: a
                                       consistent rewrite recomputes it, which is exactly why layer 3 exists (F110-A
                                       section 7: digest consistency != semantic truth).
          3  SEMANTIC RECONCILIATION - every FROZEN_CONSTANT field is compared to the constant of THIS authority and
                                       every field is type-checked. The filesystem fields are reconciled against the
                                       live filesystem by check_boundary(); the informational field is not
                                       authorizing.
        """
        ...
        # ---- F110-A: the four FROZEN_CONSTANT fields, each compared OUTSIDE the receipt. Rewriting one of these and
        # recomputing boundary_sha256 is the v1.10 defect; it now refuses here, before any authorizing operation.
        for k in ("schema", "evidence_store_version", "authority_version", "platform_scope"):
            want = BOUNDARY_FROZEN_CONSTANTS[k]
            if b[k] != want:
                raise EvidenceStoreError("BOUNDARY_RECEIPT_INVALID",
                                         f"{k}={b[k]!r} is not the frozen {BOUNDARY_PROVENANCE[k]} "
                                         f"{want!r} of this evidence-store authority")
        # ---- structural law for every remaining field, so no class can be attacked through its type
        for k in BOUNDARY_STR_FIELDS:
            if not isinstance(b[k], str) or not b[k]:
                raise EvidenceStoreError("BOUNDARY_RECEIPT_INVALID", f"{k} is not a non-empty string")
        for k in BOUNDARY_INT_FIELDS:
            if not isinstance(b[k], int) or isinstance(b[k], bool):
                raise EvidenceStoreError("BOUNDARY_RECEIPT_INVALID", f"{k} is not an integer device/inode number")
        if b["created_at"] is not None and not isinstance(b["created_at"], str):
            raise EvidenceStoreError("BOUNDARY_RECEIPT_INVALID", "created_at is neither a string nor null")
        # ---- L5 internal coherence: the basename IS the session id, inside the receipt as well as on disk
        if b["session_id"] != b["session_basename"]:
            raise EvidenceStoreError("BOUNDARY_RECEIPT_INVALID",
                                     f"receipt session_id {b['session_id']!r} != session_basename {b['session_basename']!r}")
        return b

    def boundary_receipt(self):
        """L15. The CHECKED public surface over the receipt: the full boundary is established first, so this can never
        hand a caller the receipt of a session that has been replaced (F110-D)."""
    ...
    def check_boundary(self):
        ...
        DERIVED_FROM_FILESYSTEM reconciliation lives here; FROZEN_CONSTANT and structural reconciliation live in
        _boundary_receipt(). Together they cover every field of BOUNDARY_PROVENANCE except the one deliberately
        classified INFORMATIONAL_NON_AUTHORIZING (F110-A)."""
        b = self._boundary_receipt()
    ...
    @property
    def finalized(self):
        """L15. Authority-bearing state: the finalization state of a substituted directory is not this session's."""
        self.check_boundary()
        return os.path.exists(os.path.join(self.dir, FINALIZED_FILE))

    def _finalized(self):
        """The unchecked primitive, for the callers that have already established the boundary."""
        return os.path.exists(os.path.join(self.dir, FINALIZED_FILE))

    def manifest(self):
        self.check_boundary()                                    # L15 BOUNDARY before READ (F110-D)
    ...
    def _manifest_sha256(self):
        return sha256_bytes(_read_exact(os.path.join(self.dir, SESSION_FILE)))

    def identity(self):
        b = self.check_boundary()                                # L15 BOUNDARY before MODEL (F110-D)
        return session_identity(self._manifest(), os.path.basename(self.dir), boundary_identity(b))

    def identity_errors(self):
        """L15. Reports rather than raises: the boundary refusal is the FIRST element of its own vocabulary."""
        try:
            self.check_boundary()
        except EvidenceStoreError as e:
            return [f"{e.code}: {e.detail}"]
        return self._identity_errors()
    ...
    def scan_tree(self):
        """L15. The CHECKED public enumeration surface: the record map of a replaced session is never returned."""
        self.check_boundary()                                    # L15 BOUNDARY before MODEL (F110-D)
    ...
    def expected_model(self):
        ...
        L15 (F110-D): the boundary is established BEFORE the model is derived, so a caller can never receive a model
        built from a substituted session directory."""
        self.check_boundary()                                    # L15 BOUNDARY before MODEL (F110-D)
        errs = []
        files, dirs, partials, bad = self._scan_tree()
        for rel in partials:
        ...
        for rel in sorted(files):
            ...
            if kind == KIND_RECORD:
                ...
                # ---- L7 F110-C section 21: EXACT cardinality. For every authoritative record the set of valid
                # attempt markers resolving to it must have size exactly one. Zero is ORPHANED_RECORD; more than one
                # is the v1.10 defect - two distinct attempt ids over the same session, layer, logical identity and
                # bytes produced two valid markers pointing at one content-addressed record and everything passed.
                if len(claims) > 1:
                    errs.append(f"MARKER_CARDINALITY_VIOLATION: {rel} is claimed by {len(claims)} attempt markers "
                                f"({', '.join(sorted(c[0] for c in claims))}); store law is exactly one marker per "
                ...
                base.update(layer=path_layer, record_key=key, logical_identity=t["logical_identity"],
                            ...
                            attempt_ids=[t["attempt_id"]],
                            attempt_keys=[attempt_key(self.session_id, t["layer"], t["logical_identity"], t["attempt_id"])])
            entries[rel] = base
    ...
    def session_state(self):
        ...
        if not os.path.isdir(self.dir) or not os.path.exists(os.path.join(self.dir, SESSION_FILE)):
            return "INVALID"
        _f, _d, partials, _b = self._scan_tree()
        if self._finalized():
    ...
    def _require_can_write(self):
        ...
        self.check_boundary()                                    # L15 BOUNDARY before SESSION_STATE (F110-D)
        if self._finalized():
    ...
    def _put(self, layer, attempt_id, raw_bytes, logical_identity, declared_digest=None):
        ...
        # ---- L7 F110-C section 23: refuse the SECOND attempt id for an already-recorded (session, layer, logical
        # identity, content) BEFORE anything is written, so no ambiguous many-markers-to-one-record state is ever
        # persisted. _require_can_write() has already established that every existing record has exactly one marker,
        # so an existing record here means another attempt already owns it. The v1.10 idempotence law is untouched:
        # when THIS attempt id already has its marker, the marker path exists and the reconciliation below decides.
        if os.path.exists(final) and not os.path.exists(marker):
            raise EvidenceStoreError("MARKER_CARDINALITY_VIOLATION",
                                     f"attempt {attempt_id!r} would be a second marker for the record already stored "
                                     f"at {os.path.relpath(final, self.dir)}; store law is exactly one marker per "
                                     f"authoritative record")
    ...
    def finalize(self):
        ...
        assert tuple(sorted(inv)) == tuple(sorted(INVENTORY_FIELDS)), "finalize() must write the exact L14 vocabulary"
    ...
    def inventory(self):
        """L15/L33 (F110-D section 29). BOUNDARY dominates SESSION_STATE: in v1.10 this method tested the
        finalization file first, so a replaced session answered SESSION_NOT_FINALIZED and MASKED the substitution."""
        self.check_boundary()                                    # L15 BOUNDARY before SESSION_STATE (F110-D)
    ...
    def finalization_marker(self):
        """L15. Same precedence correction as inventory(): boundary before finalization state (F110-D)."""
        self.check_boundary()                                    # L15 BOUNDARY before SESSION_STATE (F110-D)
        return self._finalization_marker()

    def _finalization_marker(self):
    ...
    def verify_summary(self, require_finalized=True):
        ...
        try:
            ...
            return sorted(set(errs)), counts
        # ---- L14 F110-B section 10/17: the EXACT top-level field vocabulary, before any field is interpreted. A
        # missing normative field, an unknown extra semantic field or a duplicate field is a refusal. In v1.10 an
        # injected top-level field was simply covered by a recomputed inventory_sha256 and verification passed.
        # (A duplicate top-level key never reaches here: L.strict_loads refuses it at the parse boundary.)
        if not isinstance(inv, dict):
            add("changed", "INVENTORY_FIELD_SET_INVALID: the inventory is not a JSON object")
            return sorted(set(errs)), counts
        if tuple(sorted(inv)) != INVENTORY_FIELD_SET:
            miss = sorted(set(INVENTORY_FIELD_SET) - set(inv))
            extra = sorted(set(inv) - set(INVENTORY_FIELD_SET))
            add("changed", f"INVENTORY_FIELD_SET_INVALID: missing {miss} / unknown {extra}; the inventory field set is "
                           f"exact and semantic extension requires an inventory schema version")
            return sorted(set(errs)), counts
        if inv["schema"] != EVIDENCE_INVENTORY_SCHEMA or inv["closed_world"] is not True or inv["semantic_reconciliation"] is not True or inv["recomputed_model"] is not True:
            add("changed", "INVENTORY_TAMPERED: wrong schema or missing a frozen inventory property")
            return sorted(set(errs)), counts
        # ---- L14 the type law for the fields verified by value below, so no comparison can be attacked by type
        for f_, ty in (("record_count", int), ("entry_count", int), ("total_bytes", int), ("attempt_count", int),
                       ("self_path", str), ("finalized_marker_path", str), ("entries", list), ("directories", list),
                       ("mode_table", dict)):
            if not isinstance(inv[f_], ty) or isinstance(inv[f_], bool):
                add("changed", f"INVENTORY_FIELD_SET_INVALID: {f_} is not a {ty.__name__}")
                return sorted(set(errs)), counts
        if inv["inventory_sha256"] != inventory_digest(inv):
        ...
        # ---- F110-B sections 11-16. The six normative header fields v1.10 wrote and then trusted, each RECOMPUTED
        # from the independent model and the frozen store layout, never read from the inventory. `expected` is the
        # governed closed world minus the two files finalization itself creates, which is also exactly the set
        # finalize() counted, so all four counters and both paths are exact equalities, not tolerances.
        expected = {p: e for p, e in model["entries"].items() if p not in (INVENTORY_FILE, FINALIZED_FILE)}
        derived_header = (
            ("evidence_store_version", EVIDENCE_STORE_VERSION), ("authority_version", L.AUTHORITY_VERSION),
            ...
            ("session_manifest_sha256", self._manifest_sha256()),
            # F110-B 11: recomputed from the actual governed record set
            ("record_count", sum(1 for e in expected.values() if e["kind"] == KIND_RECORD)),
            # F110-B 12: recomputed from the actual expected inventory entries
            ("entry_count", len(expected)),
            # F110-B 13: recomputed over the frozen TOTAL_BYTES_SCOPE
            ("total_bytes", sum(e["byte_count"] for e in expected.values())),
            # F110-B 14: recomputed from the authoritative attempt-marker set of the model, never from the inventory
            ("attempt_count", sum(1 for e in expected.values() if e["kind"] == KIND_ATTEMPT_MARKER)),
            # F110-B 15/16: derived from frozen store layout, never trusted from the stored value
            ("self_path", INVENTORY_FILE),
            ("finalized_marker_path", FINALIZED_FILE),
        )
        for field, want in derived_header:
            if inv[field] != want:
                add("semantic_mismatch", f"INVENTORY_TAMPERED: {field} is not the value derived from this session")
        if inv["mode_table"] != {k: v for k, v in sorted(MODE_TABLE.items()) if v is not None}:
        ...
        try:
            mk = self._finalization_marker()
        except (EvidenceStoreError, L.ParseError, ValueError, UnicodeDecodeError) as e:
            add("changed", f"FINALIZATION_MARKER_INVALID: unreadable ({e})")
            mk = None
        if mk is not None:
            if not isinstance(mk, dict) or sorted(mk) != sorted(FINALIZATION_FIELDS):
    ...
    def authority_errors(self, env, active, rp):
        """L15. Reports rather than raises: the boundary refusal is the FIRST element (F110-D)."""
        try:
            self.check_boundary()                                # L15 BOUNDARY before READ (F110-D)
        except EvidenceStoreError as e:
            return [f"{e.code}: {e.detail}"]
