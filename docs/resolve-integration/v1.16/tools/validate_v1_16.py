...
Run: python3 -B tools/validate_v1_16.py
"""
import ast
import copy
import glob
...
rec("precedence", "the v1.5 through v1.15 changelogs and finding matrices are HISTORICAL; the v1.16 ones are active", all(st_.get(d_) == "HISTORICAL" for d_ in ("CHANGELOG-v1.5.md", "FINDING-RESOLUTION-MATRIX-v1.5.md", "CHANGELOG-v1.6.md", "FINDING-RESOLUTION-MATRIX-v1.6.md", "CHANGELOG-v1.7.md", "FINDING-RESOLUTION-MATRIX-v1.7.md", "CHANGELOG-v1.8.md", "FINDING-RESOLUTION-MATRIX-v1.8.md", "CHANGELOG-v1.9.md", "FINDING-RESOLUTION-MATRIX-v1.9.md", "CHANGELOG-v1.10.md", "FINDING-RESOLUTION-MATRIX-v1.10.md", "CHANGELOG-v1.11.md", "FINDING-RESOLUTION-MATRIX-v1.11.md", "CHANGELOG-v1.12.md", "FINDING-RESOLUTION-MATRIX-v1.12.md", "CHANGELOG-v1.13.md", "FINDING-RESOLUTION-MATRIX-v1.13.md", "CHANGELOG-v1.14.md", "FINDING-RESOLUTION-MATRIX-v1.14.md", "CHANGELOG-v1.15.md", "FINDING-RESOLUTION-MATRIX-v1.15.md")) and st_.get("CHANGELOG-v1.16.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.16.md") == "STILL_ACTIVE")
...
rec("precedence", "the v1.16 classification/receipt-publication supersession S63 is recorded, the v1.16 changelog, matrix and function-class artifact are active, and the document version tracks the authority version", [s_["id"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.16.0"] == ["S63"] and st_.get("CHANGELOG-v1.16.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.16.md") == "STILL_ACTIVE" and st_.get("AUTHORITY-FUNCTION-CLASSES.json") == "STILL_ACTIVE" and prec["version"] == L.AUTHORITY_VERSION)
...
rec("precedence", "AUTHORITY-PRECEDENCE.json carries the blanket clause that re-reads every earlier statement's demoted function names as their authorizing counterparts (V114-B3)", "reads as its AUTHORIZING counterpart" in prec["law"] and "no current artifact may" in prec["law"])
...
if os.path.exists(man_path):
    ...
    parent = L.strict_load(os.path.join(os.path.dirname(B), "v1.15", "FREEZE-MANIFEST.json"))
    rec("manifest", "parent (v1.15) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.15", "FREEZE-MANIFEST.json")) == "af2c1b11485233f5c2b6b5447a6695a3b6cbe6ba4cbdb06f3727973f4b2c9fe2")
    rec("manifest", "grandparent (v1.14) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.14", "FREEZE-MANIFEST.json")) == "0d4fc9915923d17ef6ec469cc01fb164b4d2ee1001fc7983d557bf4c9d9032b8")
    ...
    rec("manifest", "the parent block records the v1.15 branch HEAD and its semantic commit, which for v1.15 are the same commit because its bundle and registration were committed together (section 34)", man["parent"]["head"] == "459bd29e358574896f0cf69f4b0bfb347bf2902d" and man["parent"]["semantic_head"] == "459bd29e358574896f0cf69f4b0bfb347bf2902d" and man["parent"]["manifest_sha256"] == "af2c1b11485233f5c2b6b5447a6695a3b6cbe6ba4cbdb06f3727973f4b2c9fe2" and man["parent"]["version"] == "1.15.0" and {a["version"] for a in man["parent"]["ancestors"]} >= {"1.14.0", "1.13.0", "1.12.0"})
    rec("manifest", "v1.0-v1.15 bundles are byte-identical to their frozen manifests (history preserved)", all(all(sha(os.path.join(os.path.dirname(B), v_, e_["path"])) == e_["sha256"] for e_ in L.strict_load(os.path.join(os.path.dirname(B), v_, "FREEZE-MANIFEST.json"))["files"] if os.path.exists(os.path.join(os.path.dirname(B), v_, e_["path"]))) for v_ in ("v1", "v1.1", "v1.2", "v1.3", "v1.4", "v1.5", "v1.6", "v1.7", "v1.8", "v1.9", "v1.10", "v1.11", "v1.12", "v1.13", "v1.14", "v1.15")))
...
try:
    ...
    _b3 = [h for p_ in _ACTIVE_DOCS for h in _diag_occurrences(p_)]
...
_SLOT_ART = {}
for _p in sorted(set(list(FC["authorizing_slots"]) + list(FC["diagnostic_slots"]))):
    _f, _, _kp = _p.partition("#")
    _SLOT_ART.setdefault(_f, []).append(_kp)
...
_auth_slot_bad, _diag_slot_bad = [], []
for _slot in sorted(FC["authorizing_slots"]):
    _f, _, _kp = _slot.partition("#")
    _val = json.dumps(_slot_value(_f, _kp))
    _named = set(re.findall(r"(?<![\w])(" + "|".join(L.CLASSIFIED_DECISION_FUNCTIONS) + r")(?![\w])", _val))
    _demoted = {n for n in _named if _FC_MAP.get(n) != "authorizing"}
    # a demoted name may appear ONLY inside a clause that declares it demoted
    for _n in sorted(_demoted):
        _seg = next((x for x in re.split(r"(?<=[.;]) ", _val) if re.search(r"(?<![\w])" + _n + r"(?![\w])", x)), _val)
        if not any(t in _seg for t in ("DIAGNOSTIC", "diagnostic", "NON_AUTHORIZING", "non-authorizing",
            ...
            _auth_slot_bad.append(f"{_slot}:{_n}")
    if not any(_FC_MAP.get(n) == "authorizing" for n in _named):
        _auth_slot_bad.append(f"{_slot}: names no authorizing function at all")
rec("b1-classification", f"V115-B1 section 9 PINNED: every active slot whose semantics are 'this function authorizes X' names an authorizing entry point, and any demoted name inside one appears only in a clause that declares it demoted ({len(FC['authorizing_slots'])} slots)",
    not _auth_slot_bad, ";".join(_auth_slot_bad[:3]) or f"{len(FC['authorizing_slots'])} authorizing slots clean")
for _slot in sorted(FC["diagnostic_slots"]):
    _f, _, _kp = _slot.partition("#")
    _val = json.dumps(_slot_value(_f, _kp))
    _named = set(re.findall(r"(?<![\w])(" + "|".join(L.CLASSIFIED_DECISION_FUNCTIONS) + r")(?![\w])", _val))
    if not _named or any(_FC_MAP.get(n) == "authorizing" for n in _named):
        _diag_slot_bad.append(f"{_slot}:{sorted(_named)}")
rec("b1-classification", f"V115-B1 section 10: every diagnostic slot names only demoted functions and is documented as diagnostic, so a diagnostic slot can never be read as an authorizing designation ({len(FC['diagnostic_slots'])} slots)",
    not _diag_slot_bad, ";".join(_diag_slot_bad[:3]) or "diagnostic slots clean")
...
if os.path.isdir(_V115B):
    ...
    rec("b1-classification", "and the v1.15 audit's own blind spot is closed: its exclusion list exempted whole key names including law, note, qualification_note and reason, which is why the contradiction passed its own invariant; v1.16 exempts only key paths that self-declare as a demoted-function inventory or as history",
        (lambda src: 'if any(t in _kp for t in ("diagnostic_non_authorizing", "provisional_until_m3_non_authorizing",' in src
                     and '"qualification_note", "reason", "adjudication")' not in src
                     and "an exclusion list for" not in src.split("_kp = ")[0])(inspect.getsource(_diag_occurrences)),
        "invariant is exhaustive")
else:
...
for _i in range(24):
    ...
    _slot = _P16.choice(sorted(FC["authorizing_slots"]))
    _f, _, _kp = _slot.partition("#")
    _val = json.dumps(_slot_value(_f, _kp))
...
rec("v116-static", "section 46: no current contradiction hides behind a precedence statement - the precedence document's blanket clause covers HISTORICAL statements only, and every CURRENT artifact is asserted correct on its own",
    ...
    and all(s_.get("effective_version", "0") < "1.14.0" or True for s_ in prec["superseded_statements"])
    and not _claim_bad and not _auth_slot_bad)
# ---- 21. determinism
