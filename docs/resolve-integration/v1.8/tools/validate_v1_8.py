#!/usr/bin/env python3
"""Layered, deterministic authority-validation suite for bundle v1.8.
...
Run: python3 -B tools/validate_v1_8.py
"""
import copy
import glob
import hashlib
...
rec("precedence", "the v1.5, v1.6 and v1.7 changelogs and finding matrices are HISTORICAL; the v1.8 ones are active", st_.get("CHANGELOG-v1.5.md") == "HISTORICAL" and st_.get("FINDING-RESOLUTION-MATRIX-v1.5.md") == "HISTORICAL" and st_.get("CHANGELOG-v1.6.md") == "HISTORICAL" and st_.get("FINDING-RESOLUTION-MATRIX-v1.6.md") == "HISTORICAL" and st_.get("CHANGELOG-v1.7.md") == "HISTORICAL" and st_.get("FINDING-RESOLUTION-MATRIX-v1.7.md") == "HISTORICAL" and st_.get("CHANGELOG-v1.8.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.8.md") == "STILL_ACTIVE")
...
rec("precedence", "the v1.8 evidence-store supersessions S46-S47 are recorded and EVIDENCE-ROOT.md is the single STILL_ACTIVE store document", sorted(s_["id"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.8.0") == ["S46", "S47"] and st_.get("EVIDENCE-ROOT.md") == "STILL_ACTIVE")
...
if os.path.exists(man_path):
    ...
    parent = L.strict_load(os.path.join(os.path.dirname(B), "v1.7", "FREEZE-MANIFEST.json"))
    rec("manifest", "parent (v1.7) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.7", "FREEZE-MANIFEST.json")) == "8da668fcb3df771b645e7feb092e88ce381d9fa5a4f9de931bfaf1bc2f55fb5d")
    rec("manifest", "grandparent (v1.6) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.6", "FREEZE-MANIFEST.json")) == "9f8a1a2e51f205409f8cb3f175144d21c59658221a42398612f982746a04ac29")
    rec("manifest", "the parent block records BOTH the v1.7 branch HEAD it was cut from and the inherited v1.7 SEMANTIC commit (section 26)", man["parent"]["head"] == "6a805181b14617c1b6259847b0905e21362a935c" and man["parent"]["semantic_head"] == "ebc2dd4db2462f73db20054ba60cf36691ab29b3" and man["parent"]["manifest_sha256"] == "8da668fcb3df771b645e7feb092e88ce381d9fa5a4f9de931bfaf1bc2f55fb5d")
    rec("manifest", "v1.0-v1.7 bundles are byte-identical to their frozen manifests (history preserved)", all(all(sha(os.path.join(os.path.dirname(B), v_, e_["path"])) == e_["sha256"] for e_ in L.strict_load(os.path.join(os.path.dirname(B), v_, "FREEZE-MANIFEST.json"))["files"] if os.path.exists(os.path.join(os.path.dirname(B), v_, e_["path"]))) for v_ in ("v1", "v1.1", "v1.2", "v1.3", "v1.4", "v1.5", "v1.6", "v1.7")))
    ...
    rec("manifest", "validation tools listed as TOOL", all(any(e["path"] == p and e["authority_class"] == "TOOL" for e in man["files"]) for p in ("tools/authority_lib.py", "tools/validate_v1_8.py", "tools/build_v1_8.py", "tools/build_manifest.py", "tools/fixture_evidence.py", "tools/capture_shim_reference.py")))
...
STORE_ROOT = tempfile.mkdtemp(prefix="resolve-v18-evidence-")


def _plant(st, rel, data=b"planted\n", directory=False):
    """Write an entry into a FINALIZED session the way a hostile local process would: straight onto the filesystem,
    bypassing the store API entirely. Returns the absolute path so the test can remove it again."""
    p = os.path.join(st.dir, *rel.split("/"))
    os.makedirs(os.path.dirname(p), mode=0o700, exist_ok=True) if not directory else None
    if directory:
...
def _unplant(p, session_dir=None):
    """Undo a plant completely, including any directory the plant caused to exist. An empty leftover directory would
    itself be an UNEXPECTED_ENTRY, which is correct behaviour but would mask the next attack."""
...
def _fresh_session(name, finalize=True, records=3):
    """A valid session with one record per layer, finalized unless asked otherwise."""
    sm = ES_MOD.session_manifest(name, "probe-" + name, "adapter-operator (M0A driver)", ENVC, ACTIVE_HYP, rp, "2026-09-09T10:00:00Z")
    st = ES_MOD.create_session(STORE_ROOT, name, sm)
    st.put_raw(name + ":0001", SHIM.raw_frame_bytes(_good_cap))
    if records > 1:
        st.put_derived(name + ":1001", {"derived": 1}, logical_identity="GetStartFrame")
        st.put_review(name + ":1002", {"review": 1}, logical_identity="review-1")
        st.put_promotion(name + ":1003", {"promotion": 1}, logical_identity="refreeze-1")
    if finalize:
        st.finalize()
    return st
...
try:
    # ---- section 22 / 21: the pinned session manifest
    SM = ES_MOD.session_manifest("sess-v18-selftest", "probe-v18", "adapter-operator (M0A driver)", ENVC, ACTIVE_HYP, rp, "2026-09-09T10:00:00Z")
    rec("evidence-store", "the session manifest pins the active authority, registry, matrix, trusted shim, parser and primitive spec",
        L.session_manifest_errors(SM, ENVC, ACTIVE_HYP, rp) == [], "; ".join(L.session_manifest_errors(SM, ENVC, ACTIVE_HYP, rp)[:2])[:160])
    for _field, _bad in (("authority_manifest_sha256", "0" * 64), ("trusted_shim_sha256", "0" * 64), ("parser_sha256", "0" * 64),
                         ("primitive_spec_sha256", "0" * 64), ("schema_registry_sha256", "0" * 64), ("codec_version", "other"),
                         ("evidence_store_version", "vidtoolz.resolveEvidenceStore.v1"),
                         ("evidence_inventory_schema", "vidtoolz.resolveEvidenceHashes.v1"), ("stdout_retention", "KEEP_EVERYTHING")):
        rec("evidence-store", f"a session manifest with a wrong {_field} is refused", bool(L.session_manifest_errors(dict(SM, **{_field: _bad}), ENVC, ACTIVE_HYP, rp)))
    rec("evidence-store", "the session manifest names the v1.8 store, session and inventory schemas",
        SM["evidence_store_version"] == "vidtoolz.resolveEvidenceStore.v2" and SM["evidence_session_schema"] == "vidtoolz.resolveEvidenceSession.v2" and SM["evidence_inventory_schema"] == "vidtoolz.resolveEvidenceInventory.v1")

    # ---- normal workflow (section 29)
    ST = ES_MOD.create_session(STORE_ROOT, "sess-v18-selftest", SM)
    rec("evidence-store", "a session directory is created once and can never be created again",
        os.path.isdir(ST.dir) and _try_store(lambda: ES_MOD.create_session(STORE_ROOT, "sess-v18-selftest", SM)).code == "SESSION_EXISTS")
    rec("evidence-store", "a brand new session reports state ACTIVE", ST.session_state() == "ACTIVE")
    _f1 = SHIM.raw_frame_bytes(_good_cap)
    _p1 = ST.put_raw("probe-v18:0001", _f1, logical_identity="GetStartFrame")
    rec("evidence-store", "a record is stored at a path derived from the sha256 of the bytes the store hashed itself",
        _p1["digest"] == hashlib.sha256(_f1).hexdigest() and _p1["digest"] in _p1["path"] and not _p1["idempotent"] and ST.get("RAW", _p1["digest"]) == _f1)
    rec("evidence-store", "re-putting byte-identical content under the same attempt id is idempotent, not an overwrite",
        ST.put_raw("probe-v18:0001", _f1, logical_identity="GetStartFrame")["idempotent"] is True)
    rec("evidence-store", "a caller-declared digest is compared and never trusted",
        _try_store(lambda: ST.put("RAW", "probe-v18:0002", _f1 + b" ", declared_digest=_p1["digest"])).code == "DECLARED_DIGEST_MISMATCH")
    rec("evidence-store", "the same attempt id with different bytes is refused (section 16)",
        _try_store(lambda: ST.put_raw("probe-v18:0001", _f1 + b"\n")).code == "ATTEMPT_ID_REUSED")
    rec("evidence-store", "an unknown layer is refused", _try_store(lambda: ST.put("NOT_A_LAYER", "probe-v18:0003", _f1)).code == "LAYER_UNKNOWN")
    _pd = ST.put_derived("probe-v18:1001", {"derived": True}, logical_identity="GetStartFrame")
    _pr = ST.put_review("probe-v18:1002", {"review": True}, logical_identity="review-1")
    _pp = ST.put_promotion("probe-v18:1003", {"promotion": True}, logical_identity="refreeze-1")
    rec("evidence-store", "DERIVED, HUMAN_REVIEW and AUTHORITY_PROMOTION take content plus a logical identity and the store derives the content-addressed destination (sections 5/6)",
        all(r["digest"] in r["path"] and r["path"].startswith(r["layer"] + os.sep) and r["logical_identity"] for r in (_pd, _pr, _pp)))
    _inv_sha = ST.finalize()
    _inv = ST.inventory()
    rec("evidence-store", "finalization writes a self-digesting closed-world inventory and a FINALIZED marker naming it",
        L.is_sha(_inv_sha) and _inv["inventory_sha256"] == _inv_sha and _inv["closed_world"] is True and ST.verify() == [], "; ".join(ST.verify()[:2])[:150])
    rec("evidence-store", "a finalized session reports state FINALIZED", ST.session_state() == "FINALIZED")
    rec("evidence-store", "after finalization the session is immutable: every write API and re-finalization refuse (section 10)",
        all(_try_store(fn).code == "SESSION_FINALIZED" for fn in (
            lambda: ST.put_raw("probe-v18:9001", b'{"late":1}\n'),
            lambda: ST.put_derived("probe-v18:9002", {"late": 1}),
            lambda: ST.put_review("probe-v18:9003", {"late": 1}),
            lambda: ST.put_promotion("probe-v18:9004", {"late": 1}),
            lambda: ST.finalize())))
    rec("evidence-store", "the pinned authority of a finalized session is re-checkable and refuses a mismatched active authority",
        ST.authority_errors(ENVC, ACTIVE_HYP, rp) == [] and bool(ST.authority_errors(ENVC, dict(ACTIVE_HYP, manifest_sha256="0" * 64), rp)))
    rec("evidence-store", "the store never opens an existing record for writing, truncates, renames over one or unlinks a final record",
    ...
    rec("evidence-store", "the store implements the four EVIDENCE-ROOT layers and a closed refusal vocabulary",
        ES_MOD.LAYERS == ("RAW", "DERIVED", "HUMAN_REVIEW", "AUTHORITY_PROMOTION") and len(ES_MOD.ERROR_CODES) >= 24 and "UNEXPECTED_ENTRY" in ES_MOD.ERROR_CODES)
    rec("evidence-store", "content addressing is retained: every verification recomputes the bytes and compares to the name (section 15)",
        all(hashlib.sha256(ST.get(e["layer"], e["digest"])).hexdigest() == e["digest"] for e in _inv["entries"] if e["kind"] == "RECORD"))

    # ---- section 18: crash / partial state
    STP = _fresh_session("sess-v18-partial", finalize=False)
    rec("evidence-store", "an unfinalized session reports ACTIVE and has no inventory",
        STP.session_state() == "ACTIVE" and _try_store(lambda: STP.inventory()).code == "SESSION_NOT_FINALIZED" and any("SESSION_NOT_FINALIZED" in e for e in STP.verify()))
    _part = _plant(STP, "RAW/%s/.partial-deadbeef" % _p1["digest"][:2], b"half")
    rec("evidence-store", "a partially written record makes the session PARTIAL, is reported, and blocks finalization (section 18)",
        STP.session_state() == "PARTIAL" and any("PARTIAL_RECORD" in e for e in STP.verify(require_finalized=False)) and _try_store(lambda: STP.finalize()).code == "PARTIAL_RECORD")
    _unplant(_part, STP.dir)
    rec("evidence-store", "a crashed session can never report FINALIZED, and the four states are closed",
        STP.session_state() in ("ACTIVE", "PARTIAL") and tuple(ES_MOD.SESSION_STATES) == ("ACTIVE", "PARTIAL", "FINALIZED", "INVALID"))

    # ================================================================ ES-1: the closed world (sections 7-11, 23)
    CW = _fresh_session("sess-v18-closedworld")
    rec("evidence-closed-world", "control: the untouched finalized session verifies clean", CW.verify() == [], "; ".join(CW.verify()[:2])[:150])
    _cw_inv = CW.inventory()
    rec("evidence-closed-world", "the inventory records path, kind, layer, digest, byte count, schema type, logical identity and attempt ids for every entry, plus the expected directory set (section 8/9)",
        all(set(e) == {"path", "kind", "layer", "digest", "byte_count", "schema_type", "logical_identity", "attempt_ids"} for e in _cw_inv["entries"])
        and any(e["kind"] == "RECORD" and e["logical_identity"] for e in _cw_inv["entries"])
        and any(e["kind"] == "SESSION_MANIFEST" for e in _cw_inv["entries"])
        and any(e["kind"] == "ATTEMPT_MARKER" for e in _cw_inv["entries"])
        and set(ES_MOD.LAYERS) <= set(_cw_inv["directories"]))

    # ---- the pinned STORE-14 regression (section 23)
    _s14 = _plant(CW, "PLANTED-BY-ATTACKER.json", b'{"forged":"authority"}\n')
    _s14v = CW.verify()
    rec("evidence-closed-world", "STORE-14 (Codex, pinned regression): finalize a valid session, plant an unexpected file in the session ROOT, verify -> UNEXPECTED_ENTRY. v1.7 ignored this file and returned no errors.",
        any("UNEXPECTED_ENTRY" in e and "PLANTED-BY-ATTACKER.json" in e for e in _s14v) and CW.session_state() == "INVALID", "; ".join(_s14v[:2])[:170])
    _unplant(_s14, CW.dir)
    rec("evidence-closed-world", "removing the planted file restores a clean verification (the refusal was the file, not a latch)", CW.verify() == [])

    POST_FINALIZATION_ATTACKS = [
        ("an unexpected file in the session root", lambda st: _plant(st, "EXTRA-ROOT.json"), "UNEXPECTED_ENTRY"),
        ("an unexpected nested file inside a layer", lambda st: _plant(st, "RAW/EXTRA-NESTED.json"), "UNEXPECTED_ENTRY"),
        ("an unexpected file in a record shard directory", lambda st: _plant(st, "RAW/%s/EXTRA-SHARD.json" % _p1["digest"][:2]), "UNEXPECTED_ENTRY"),
        ("an unexpected directory", lambda st: _plant(st, "EXTRA-DIR", directory=True), "UNEXPECTED_ENTRY"),
        ("an unexpected nested directory", lambda st: _plant(st, "DERIVED/EXTRA-DIR", directory=True), "UNEXPECTED_ENTRY"),
        ("an unexpected file in the attempts index", lambda st: _plant(st, "ATTEMPTS/forged-attempt", b"0" * 64 + b"\n"), "UNEXPECTED_ENTRY"),
    ]
    for _label, _mk, _code in POST_FINALIZATION_ATTACKS:
        _p = _mk(CW)
        _v = CW.verify()
        rec("evidence-closed-world", f"post-finalization: {_label} invalidates the session ({_code})",
            any(_code in e for e in _v) and CW.session_state() == "INVALID", "; ".join(_v[:1])[:150])
        _unplant(_p, CW.dir)
    rec("evidence-closed-world", "after every planted entry is removed the session verifies clean again", CW.verify() == [])

    # extra RAW / DERIVED records planted as genuine-looking content-addressed files
    for _layer in ("RAW", "DERIVED"):
        _extra = b'{"planted":"%s"}\n' % _layer.encode()
        _ed = hashlib.sha256(_extra).hexdigest()
        _p = _plant(CW, f"{_layer}/{_ed[:2]}/{_ed}.json", _extra)
        _v = CW.verify()
        rec("evidence-closed-world", f"post-finalization: an extra {_layer} record that is correctly content-addressed is still UNEXPECTED_ENTRY",
            any("UNEXPECTED_ENTRY" in e for e in _v), "; ".join(_v[:1])[:150])
        _unplant(_p, CW.dir)

    _rec_entry = next(e for e in _cw_inv["entries"] if e["kind"] == "RECORD")
    _rec_abs = os.path.join(CW.dir, *_rec_entry["path"].split("/"))
    _orig = open(_rec_abs, "rb").read()
    os.chmod(_rec_abs, 0o600)
    open(_rec_abs, "wb").write(_orig[:-1] + b" \n")
    rec("evidence-closed-world", "post-finalization: altering the bytes of a stored record is EVIDENCE_REPLACED",
        any("EVIDENCE_REPLACED" in e for e in CW.verify()), "; ".join(CW.verify()[:1])[:150])
    open(_rec_abs, "wb").write(_orig)
    os.chmod(_rec_abs, 0o400)
    rec("evidence-closed-world", "restoring the exact bytes makes the session verify again", CW.verify() == [])

    os.chmod(os.path.dirname(_rec_abs), 0o700)
    os.unlink(_rec_abs)
    rec("evidence-closed-world", "post-finalization: deleting a stored record is RECORD_MISSING",
        any("RECORD_MISSING" in e for e in CW.verify()), "; ".join(CW.verify()[:1])[:150])
    _renamed = _rec_abs + ".renamed"
    open(_renamed, "wb").write(_orig)
    _v = CW.verify()
    rec("evidence-closed-world", "post-finalization: renaming a stored record is BOTH a missing entry and an unexpected one",
        any("RECORD_MISSING" in e for e in _v) and any("UNEXPECTED_ENTRY" in e for e in _v), "; ".join(_v[:2])[:170])
    os.unlink(_renamed)
    open(_rec_abs, "wb").write(_orig)
    os.chmod(_rec_abs, 0o400)
    rec("evidence-closed-world", "restoring the record under its own name verifies again", CW.verify() == [])

    _ip = os.path.join(CW.dir, ES_MOD.INVENTORY_FILE)
    _iorig = open(_ip, "rb").read()
    os.chmod(_ip, 0o600)
    open(_ip, "wb").write(_iorig.replace(b'"record_count"', b'"record_countX"', 1))
    rec("evidence-closed-world", "post-finalization: altering the inventory is INVENTORY_TAMPERED",
        any("INVENTORY_TAMPERED" in e for e in CW.verify()), "; ".join(CW.verify()[:1])[:150])
    open(_ip, "wb").write(_iorig)
    os.chmod(_ip, 0o444)
    _fp = os.path.join(CW.dir, ES_MOD.FINALIZED_FILE)
    _forig = open(_fp, "rb").read()
    os.chmod(_fp, 0o600)
    open(_fp, "wb").write(b"0" * 64 + b"\n")
    rec("evidence-closed-world", "post-finalization: altering the finalization receipt is INVENTORY_TAMPERED",
        any("INVENTORY_TAMPERED" in e for e in CW.verify()), "; ".join(CW.verify()[:1])[:150])
    open(_fp, "wb").write(_forig)
    os.chmod(_fp, 0o444)
    rec("evidence-closed-world", "the finalized session verifies clean after every attack is reverted, and the inventory digest is unchanged",
        CW.verify() == [] and CW.inventory()["inventory_sha256"] == _cw_inv["inventory_sha256"])

    # ---- section 13/14: symlink and file-type law
    _sym = os.path.join(CW.dir, "PLANTED-SYMLINK.json")
    os.symlink(_rec_abs, _sym)
    _v = CW.verify()
    rec("evidence-closed-world", "a symlink planted inside a finalized session is SYMLINK_REJECTED (section 13: no symlink may exist anywhere in an evidence session)",
        any("SYMLINK_REJECTED" in e for e in _v), "; ".join(_v[:1])[:150])
    os.unlink(_sym)
    _symdir = os.path.join(CW.dir, "PLANTED-SYMLINK-DIR")
    os.symlink(os.path.join(CW.dir, "RAW"), _symdir)
    rec("evidence-closed-world", "a symlinked DIRECTORY planted inside a finalized session is rejected and never followed",
        any("SYMLINK_REJECTED" in e for e in CW.verify()))
    os.unlink(_symdir)
    _fifo = os.path.join(CW.dir, "PLANTED-FIFO")
    try:
        os.mkfifo(_fifo)
        _v = CW.verify()
        rec("evidence-closed-world", "a FIFO planted inside a finalized session is FILE_TYPE_REJECTED (section 14)",
            any("FILE_TYPE_REJECTED" in e for e in _v), "; ".join(_v[:1])[:150])
        os.unlink(_fifo)
    except (AttributeError, OSError) as _ex:
        rec("evidence-closed-world", "a FIFO planted inside a finalized session is FILE_TYPE_REJECTED (section 14)", False, f"mkfifo unavailable: {_ex!r}")
    rec("evidence-closed-world", "the session verifies clean once every non-regular entry is removed", CW.verify() == [])
    rec("evidence-closed-world", "verification walks the tree without following symlinks", "followlinks=False" in inspect.getsource(ES_MOD.EvidenceStore.scan_tree))

    # ================================================================ ES-2: one authority, no caller paths (sections 1-3, 20, 24)
    rec("evidence-single-authority", "exactly one module in this bundle is classified EVIDENCE_STORE_AUTHORIZING",
        ES_MOD.AUTHORITY_CLASS == "EVIDENCE_STORE_AUTHORIZING" and L.EVIDENCE_STORE_AUTHORITY_CLASS == ES_MOD.AUTHORITY_CLASS
        and L.EVIDENCE_STORE_MODULE == "tools/evidence_store.py"
        and sum(1 for _m in (SHIM, L, F) if getattr(_m, "AUTHORITY_CLASS", None) == "EVIDENCE_STORE_AUTHORIZING") == 0)
    rec("evidence-single-authority", "the legacy EvidenceRoot surface is GONE from the capture shim (Codex ES-2)",
        not hasattr(SHIM, "EvidenceRoot") and "EvidenceRoot" not in [n for n in dir(SHIM)] and "class EvidenceRoot" not in inspect.getsource(SHIM))
    _shim_src = inspect.getsource(SHIM)
    rec("evidence-single-authority", "the capture shim contains no storage primitive at all: no directory creation, no exclusive create, no hard link, no layer path",
        not re.search(r"^\s*os\.makedirs\(", _shim_src, re.M) and not re.search(r"^\s*os\.open\(", _shim_src, re.M)
        and "O_EXCL" not in _shim_src.split("STORAGE_AUTHORITY_NOTE")[0] and "os.link(" not in _shim_src
        and not re.search(r'"(RAW|DERIVED|HUMAN_REVIEW|AUTHORITY_PROMOTION)/', _shim_src))
    rec("evidence-single-authority", "the capture shim names the one store module and disclaims storage authority (section 19)",
        SHIM.EVIDENCE_STORE_MODULE == "tools/evidence_store.py" and "no storage authority" in SHIM.STORAGE_AUTHORITY_NOTE)
    rec("evidence-single-authority", "no module other than the store defines a session-creating, record-writing, finalizing or verifying entry point",
        all(not any(hasattr(_m, _fn) for _fn in ("create_session", "finalize", "put_raw", "put_derived", "add_derived", "add_capture", "add_review", "add_refreeze")) for _m in (SHIM, F))
        and all(hasattr(ES_MOD, _fn) for _fn in ("create_session", "open_session")) and all(hasattr(ES_MOD.EvidenceStore, _fn) for _fn in ("put_raw", "put_derived", "put_review", "put_promotion", "finalize", "verify")))
    rec("evidence-single-authority", "the store's write API accepts no filesystem path: every signature takes a layer/attempt id/logical identity and bytes",
    ...
    rec("evidence-single-authority", "only the store module contains the append-only write primitive",
        "O_EXCL" in inspect.getsource(ES_MOD) and "O_EXCL" not in inspect.getsource(F) and "O_EXCL" not in _shim_src.split("STORAGE_AUTHORITY_NOTE")[0])

    # ---- the exact legacy attack (section 24) and the traversal battery (section 12)
    TRAVERSALS = ["../x", "../../x", "../../ESCAPED-DERIVED", "/tmp/x", "/etc/passwd", "..", ".", "", "a/b", "a\\b",
                  "./x", "..%2Fx", "x/../../y", "\x00evil", "-" * 200, "RAW/../../x", "~/x", " x", "x ", ".hidden"]
    TR = _fresh_session("sess-v18-traversal", finalize=False)
    _before = set()
    for dp, dn, fn in os.walk(STORE_ROOT):
        for f in fn:
            _before.add(os.path.join(dp, f))
    _refused, _accepted = 0, []
    for _bad in TRAVERSALS:
        for _api, _call in (("put_raw", lambda b=_bad: TR.put_raw(b, b"x\n")),
                            ("put_derived", lambda b=_bad: TR.put_derived("ok-attempt-" + hashlib.sha256(b.encode("utf-8", "surrogatepass")).hexdigest()[:8], {"x": 1}, logical_identity=b)),
                            ("put_review", lambda b=_bad: TR.put_review(b, {"x": 1})),
                            ("put_promotion", lambda b=_bad: TR.put_promotion(b, {"x": 1})),
                            ("put", lambda b=_bad: TR.put("RAW", b, b"x\n")),
                            ("record_path", lambda b=_bad: TR.record_path("RAW", b)),
                            ("create_session", lambda b=_bad: ES_MOD.create_session(STORE_ROOT, b, SM)),
                            ("open_session", lambda b=_bad: ES_MOD.open_session(STORE_ROOT, b))):
            try:
                _call()
                _accepted.append(f"{_api}({_bad!r})")
            except ES_MOD.EvidenceStoreError:
                _refused += 1
    ...
    for dp, dn, fn in os.walk(STORE_ROOT):
        for f in fn:
            _after.add(os.path.join(dp, f))
    _outside = sorted(p for p in (_after - _before) if not os.path.realpath(p).startswith(os.path.realpath(TR.dir) + os.sep))
    rec("evidence-traversal", f"every one of {len(TRAVERSALS)} traversal / absolute / separator / dot / empty / over-long identifier shapes is refused by all eight store entry points ({_refused} refusals)",
        not _accepted, "accepted: " + "; ".join(_accepted[:3]))
    rec("evidence-traversal", "no file appeared anywhere outside the session root during the whole traversal battery (section 24)",
        not _outside, "; ".join(_outside[:3]))
    rec("evidence-traversal", "the exact legacy attack EvidenceRoot.add_derived('../../...') has no API left to call, and the canonical equivalent refuses before any write",
        not hasattr(SHIM, "EvidenceRoot")
        and _try_store(lambda: TR.put_derived("legacy-attempt", {"raw_capture_sha256": "../../ESCAPED-DERIVED"}, logical_identity="../../ESCAPED-DERIVED")).code == "LOGICAL_IDENTITY_INVALID"
        and not os.path.exists(os.path.join(STORE_ROOT, "ESCAPED-DERIVED.derived.json")))
    rec("evidence-traversal", "a derived payload whose CONTENT contains a traversal string is stored harmlessly, because content never becomes a path",
        TR.put_derived("content-traversal", {"raw_capture_sha256": "../../ESCAPED-DERIVED"}, logical_identity="GetStartFrame")["path"].startswith("DERIVED" + os.sep)
        and not os.path.exists(os.path.join(STORE_ROOT, "ESCAPED-DERIVED.derived.json")))
    rec("evidence-traversal", "containment is proven on the RESOLVED path, not by a string prefix check",
        "realpath" in inspect.getsource(ES_MOD._safe_join) and "PATH_ESCAPE" in inspect.getsource(ES_MOD._safe_join))
    rec("evidence-traversal", "the logical-name gate rejects empty, dot, dot-dot, separators, NUL, leading dot and over-long names",
        all(not ES_MOD.NAME_RE.match(x) for x in ("", ".", "..", "a/b", "a\\b", ".hidden", "-lead", "x" * 200)) and bool(ES_MOD.NAME_RE.match("probe-0001:0001")))
    _symroot = os.path.join(STORE_ROOT, "symlinked-session")
    os.symlink(ST.dir, _symroot)
    rec("evidence-traversal", "a symlinked session name is refused at open (section 13)",
        _try_store(lambda: ES_MOD.open_session(STORE_ROOT, "symlinked-session")).code == "SYMLINK_REJECTED")
    os.unlink(_symroot)
    _escape_dir = os.path.join(TR.dir, "RAW", "aa")
    os.makedirs(_escape_dir, mode=0o700, exist_ok=True)
    _sl = os.path.join(_escape_dir, "escape")
    os.symlink(STORE_ROOT, _sl)
    rec("evidence-traversal", "a symlink planted inside a layer cannot be traversed through, and blocks finalization",
        _try_store(lambda: TR.finalize()).code in ("SYMLINK_REJECTED", "FILE_TYPE_REJECTED"))
    os.unlink(_sl)
finally:
...
CODEX_MATRIX = L.strict_load(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.8.json"))
...
rec("codex-matrix", "the inherited v1.6 finding matrix is retained unchanged as history and its seven ids are still enumerated",
    ...
    and CODEX_MATRIX["inherited_matrices"][0] == "FINDING-RESOLUTION-MATRIX-v1.7.json")
rec("codex-matrix", "the ACTIVE v1.8 finding-resolution matrix enumerates exactly the two Codex v1.7 evidence-store ids",
    sorted(f["id"] for f in CODEX_MATRIX["findings"]) == ["ES-1", "ES-2"])
