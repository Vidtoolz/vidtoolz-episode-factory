...
Run from the bundle dir: python3 -B tools/build_v1_8.py
...
tc["schema"] = "vidtoolz.resolveTargetContract.v1.8"; tc["version"] = "1.8.0"
...
tcs["$id"] = "vidtoolz.resolveTargetContract.v1.8"; tcs["title"] = "Resolve target contract v1.8 (attachment gate; derived state; envelope law)"
tcs["properties"]["schema"] = {"const": "vidtoolz.resolveTargetContract.v1.8"}
tcs["properties"]["version"] = {"type": "string", "pattern": "^1\\.8\\.[0-9]+$"}
...
rp["schema"] = "vidtoolz.resolveReadPrimitives.v1.8"; rp["version"] = "1.8.0"
...
TRUSTED_SHIM_S = S("vidtoolz.resolveTrustedCaptureShim.v1", "Trusted capture shim authority v1.8 (the one shim identity a promotable capture may carry)",
...
rp_schema["$id"] = "vidtoolz.resolveReadPrimitives.v1.8"; rp_schema["properties"]["schema"] = {"const": "vidtoolz.resolveReadPrimitives.v1.8"}
...
caps["schema"] = "vidtoolz.resolveCapabilityMatrix.v1.8"; caps["version"] = "1.8.0"
...
cap_schema["$id"] = "vidtoolz.resolveCapabilityMatrix.v1.8"; cap_schema["title"] = "Capability matrix v1.8 (content-bound; exact stored-chain evidence entries)"
cap_schema["properties"]["schema"] = {"const": "vidtoolz.resolveCapabilityMatrix.v1.8"}
...
caps_hyp = copy.deepcopy(caps); caps_hyp["version"] = "1.8.0-HYPOTHETICAL"; caps_hyp["qualification_note"] = "HYPOTHETICAL_NOT_AUTHORITY: illustrates a future reviewed refreeze (rows QUALIFIED_READ citing raw/derived/review digests of fixture captures); never load as CAPABILITIES.json"
...
ev_schema = S("vidtoolz.resolveEvidenceSet.v1.8", "Linked evidence set (content-addressed, envelope-bound records)", {"schema": {"const": "vidtoolz.resolveEvidenceSet.v1.8"}, "current_session_id": STR_OR_NULL, "evaluated_at": {"type": "string", "minLength": 20}, "records": {"type": "object", "propertyNames": {"pattern": "^[a-f0-9]{64}$"}, "additionalProperties": {"type": "object", "required": ["record_type", "record_id", "envelope", "recorded_at"], "properties": record_common, "allOf": RECORD_RULES}}}, ["schema", "current_session_id", "evaluated_at", "records"],
...
req_schema["$id"] = "vidtoolz.resolveEligibilityRequest.v1.8"; req_schema["properties"]["schema"] = {"const": "vidtoolz.resolveEligibilityRequest.v1.8"}
...
snap_schema = S("vidtoolz.resolveSnapshot.v1.8", "Canonical Resolve readback snapshot v1.8", {
    "schema": {"const": "vidtoolz.resolveSnapshot.v1.8"}, "collector_version": {"type": "string", "minLength": 1}, "canonicalization_version": {"const": "1.5"}, "target_epoch": {"type": "string", "minLength": 1}, "resolve_build": {"type": "string", "minLength": 1},
...
policy = {"target_contract_sha256": sha_of("TARGET-CONTRACT.json"), "timebase_sha256": sha_of("TIMEBASE.json"), "track_policy_sha256": sha_of("schemas/resolveTrackPolicy.v1.json"), "capabilities_version": "1.8.0", "capability_matrix_sha256": CAPS_SHA, "collector_version": "0.0.0-fixture", "canonicalization_version": "1.5"}
policy_hyp = dict(policy, capabilities_version="1.8.0-HYPOTHETICAL", capability_matrix_sha256=HYP_SHA)
...
dump("fixtures/canonicalization/vectors.json", {"schema": "vidtoolz.resolveCanonicalizationVectors.v1.8", "spec": "CANONICALIZATION.md v1.8", "digest_rule": "sha256(utf8(domain) + 0x0A + canonical_bytes)", "registered_domains": sorted(L.HASH_DOMAINS), "reference_implementation": "tools/authority_lib.py", "vectors": vectors})
...
dump("fixtures/canonicalization/rejections.json", {"schema": "vidtoolz.resolveCanonicalizationRejections.v1.8", "cases": cases})

# ============================================================ PERMISSIONS v1.5 (unchanged law; schema/version bump) + ELIGIBILITY cases
perms = load("PERMISSIONS.json")
perms["schema"] = "vidtoolz.resolvePermissions.v1.8"; perms["version"] = "1.8.0"
...
perm_schema["$id"] = "vidtoolz.resolvePermissions.v1.8"; perm_schema["properties"]["schema"] = {"const": "vidtoolz.resolvePermissions.v1.8"}
...
matrix = load("MILESTONE-MATRIX.json"); matrix["schema"] = "vidtoolz.resolveMilestoneMatrix.v1.8"; matrix["version"] = "1.8.0"; dump("MILESTONE-MATRIX.json", matrix)
probes = load("M3-PROBES.json"); probes["schema"] = "vidtoolz.resolveM3Probes.v1.8"; dump("M3-PROBES.json", probes)


def REQ(m, op, project=None, timeline=None, refs=None, plan_target=None, **kw):
    r = {"schema": "vidtoolz.resolveEligibilityRequest.v1.8", "milestone": m, "operation": op, "scope": SCOPE, "refs": refs or {}}
...
h0_other_matrix = copy.deepcopy(s0); h0_other_matrix["policy"] = dict(policy_hyp, capability_matrix_sha256=CAPS_SHA, capabilities_version="1.8.0"); resign(h0_other_matrix)
...
p = copy.deepcopy(plan_m3); p["authority_version"] = "1.4.0"; fixture("plan-authority-version-old", "schema", "provisional/resolveMutationPlan", p, expect_contains="1.8.0")
...
man_schema["$id"] = "vidtoolz.resolveFreezeManifest.v1.8"; man_schema["title"] = "Freeze manifest v1.8"
man_schema["properties"]["schema"] = {"const": "vidtoolz.resolveFreezeManifest.v1.8"}; man_schema["properties"]["bundle"] = {"const": "docs/resolve-integration/v1.8"}; man_schema["properties"]["version"] = {"const": "1.8.0"}
man_schema["properties"]["parent"] = {"type": "object", "additionalProperties": False, "required": ["version", "branch", "head", "semantic_head", "manifest_sha256", "immutable", "ancestors"], "properties": {
    "version": {"const": "1.7.0"}, "branch": {"const": "docs/resolve-authority-freeze-v1.7"}, "head": {"const": "6a805181b14617c1b6259847b0905e21362a935c"}, "semantic_head": {"const": "ebc2dd4db2462f73db20054ba60cf36691ab29b3"}, "manifest_sha256": {"const": "8da668fcb3df771b645e7feb092e88ce381d9fa5a4f9de931bfaf1bc2f55fb5d"}, "immutable": {"const": True},
    "ancestors": {"type": "array", "minItems": 7, "items": {"type": "object", "additionalProperties": False, "required": ["version", "branch", "head", "manifest_sha256"], "properties": {"version": {"type": "string"}, "branch": {"type": "string"}, "head": {"type": "string", "pattern": "^[0-9a-f]{40}$"}, "manifest_sha256": SHA, "baseline": {"type": "string"}}}}}}
...
reg_schema = S("vidtoolz.resolveSchemaRegistry.v1", "Schema registry v1.8 (the exact schema bytes this authority version validates against)",
...
CODEX_FINDINGS = [
    {"id": "C16-B1", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_8.py section cache-mutation: nine in-place mutations of a qualified chain (raw->RAISED, primitive spec, review->REJECT, refreeze un-reviewed, matrix row demoted, stored derived edited, host_name, build, matrix evidence reference), each asserting warm == cold and NOT QUALIFIED, plus parser replacement, callable-set mutation and cross-evidence-set contamination.",
    ...
    {"id": "C16-B2", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_8.py section validator-injection: five injected validators x (positional, keyword) x both entry points, five validator-named keywords, the diagnostic surface, and one changed byte of one pinned schema file making the whole registry untrusted.",
    ...
    {"id": "C16-B3", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_8.py section shim-trust: wrong sha, stale build, unknown shim, same version with changed code, other codec, other raw schema, a method outside the trusted allowlist, an active authority missing or misnaming the trusted shim, and a whole chain over a wrong-shim capture.",
    ...
    {"id": "C16-M1", "severity": "MAJOR",
     ...
     "negative_fixture": "validate_v1_8.py section identity-duplicates: duplicate path, path with two handle tokens, path with two identity values, duplicate attempt id, two paths returning one id, and a pass-set mismatch.",
    ...
    {"id": "C16-M2", "severity": "MAJOR",
     ...
     "negative_fixture": "validate_v1_8.py section raw-ingestion: thirteen malformed frames each asserting its closed RAW_INGEST_CODES refusal, plus records with a missing, mis-bound or non-strict receipt.",
    ...
    {"id": "C16-M3", "severity": "MAJOR",
     ...
     "negative_fixture": "validate_v1_8.py section evidence-store: session re-creation, caller-declared digest mismatch, attempt-id reuse, path traversal, unknown layer, symlinked session, write after finalization, re-finalization, replaced record bytes, tampered HASHES, removed record, unfinalized session, partially written record, mismatched pinned authority, and a static audit that no code path opens a record for writing, truncates, renames over one or unlinks one.",
...
ES_FINDINGS = [
    ...
    {"id": "ES-2", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_8.py sections evidence-single-authority (static audit: exactly one EVIDENCE_STORE_AUTHORIZING module, EvidenceRoot absent, no storage primitives in the shim) and evidence-traversal (the exact legacy attack plus twelve traversal, absolute-path, separator, symlink, dot, empty and over-long identifier shapes against every write API, each proving no file appears outside the session root).",
     "expected_layer": "evidence store (path law) / authority surface",
     "validation_sections": ["evidence-single-authority", "evidence-traversal"]},
]
dump("FINDING-RESOLUTION-MATRIX-v1.8.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.8", "authority_version": L.AUTHORITY_VERSION,
                                             ...
                                             "law": "each finding names the v1.7 defect, the v1.8 correction, at least one negative fixture, the layer the attack must fail at, and the validate_v1_8.py sections that prove it. validate_v1_8.py refuses to pass unless every named section exists and every check in it passes.",
...
dump("FINDING-RESOLUTION-MATRIX-v1.7.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.8", "authority_version": L.AUTHORITY_VERSION,
                                             "adjudication": "Codex final v1.6 forensic adjudication (4 BLOCKER, 3 M0A MAJOR, 1 operational)",
                                             "law": "each finding names the v1.6 defect, the v1.7 correction, at least one negative fixture, the layer the attack must fail at, and the validate_v1_8.py sections that prove it. validate_v1_8.py refuses to pass unless every named section exists and every check in it passes.",
                                             "operational_closure": "C16-OP1 (Hermes M0A cannot bind to v1.6 because the trusted shim, parser, spec and raw identities are not operationally closed) is closed by M0A-BINDING-VALUES.json, which publishes every value with no placeholder and is verified against the running code by validate_v1_8.py section m0a-binding.",
                                             "findings": CODEX_FINDINGS})
frm_schema = S("vidtoolz.resolveFindingResolutionMatrix.v1.8", "Finding resolution matrix v1.7 (Codex v1.6 adjudication)",
               {"schema": {"const": "vidtoolz.resolveFindingResolutionMatrix.v1.8"}, "authority_version": {"const": L.AUTHORITY_VERSION}, "adjudication": {"type": "string"}, "law": {"type": "string"}, "operational_closure": {"type": "string"},
                ...
                "findings": {"type": "array", "minItems": 2, "items": {"type": "object", "additionalProperties": False, "required": ["id", "severity", "codex_finding", "correction", "negative_fixture", "expected_layer", "validation_sections"],
                                                                       "properties": {"id": {"pattern": "^(C16-(B|M)[0-9]|ES-[0-9])$"}, "severity": {"enum": ["BLOCKER", "MAJOR", "OPERATIONAL"]}, "codex_finding": {"type": "string", "minLength": 20}, "v16_defect": {"type": "string", "minLength": 20}, "prior_defect": {"type": "string", "minLength": 20}, "correction": {"type": "string", "minLength": 20}, "negative_fixture": {"type": "string", "minLength": 20}, "expected_layer": {"type": "string", "minLength": 3}, "validation_sections": {"type": "array", "minItems": 1, "items": {"type": "string"}}}}}},
...
dump("fixtures/raw-ingestion/CASES.json", {
    "schema": "vidtoolz.resolveRawIngestionCases.v1.8",
...
ing_schema = S("vidtoolz.resolveRawIngestionCases.v1.8", "Strict raw-ingestion negative fixtures v1.8",
               {"schema": {"const": "vidtoolz.resolveRawIngestionCases.v1.8"}, "authority_version": {"const": L.AUTHORITY_VERSION}, "law": {"type": "string"}, "codes": {"type": "array", "items": {"type": "string"}},
...
dump("M0A-BINDING-VALUES.json", {
    "schema": "vidtoolz.resolveM0ABindingValues.v1.8",
    "authority_version": L.AUTHORITY_VERSION,
    "purpose": "The exact identities an M0A probe package must record and be bound to under authority v1.7. Codex's v1.6 operational finding was that Hermes could not bind its M0A package because these values were not operationally closed. Every value here is the value the running code produces; validate_v1_8.py section m0a-binding re-derives each one and fails if any differs or reads as a placeholder.",
...
m0a_schema = S("vidtoolz.resolveM0ABindingValues.v1.8", "M0A binding values v1.8 (the identities a probe package must bind to)",
               {"schema": {"const": "vidtoolz.resolveM0ABindingValues.v1.8"}, "authority_version": {"const": L.AUTHORITY_VERSION}, "purpose": {"type": "string"}, "active_authority_shape": {"type": "array", "items": {"type": "string"}}, "retention_rule": {"type": "string"}, "manifest_sha256_note": {"type": "string"},
