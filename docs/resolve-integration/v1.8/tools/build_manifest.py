...
Run after validate_v1_8.py (so VALIDATION-REPORT.md is current): python3 -B tools/build_manifest.py
...
PARENT_HEAD = "6a805181b14617c1b6259847b0905e21362a935c"            # v1.7 branch HEAD (metadata-registration commit) that v1.8 was cut from
PARENT_SEMANTIC_HEAD = "ebc2dd4db2462f73db20054ba60cf36691ab29b3"   # the v1.7 SEMANTIC commit that froze the v1.7 bundle bytes
PARENT_MANIFEST = "8da668fcb3df771b645e7feb092e88ce381d9fa5a4f9de931bfaf1bc2f55fb5d"
...
parent = L.strict_load(os.path.join(V17, "FREEZE-MANIFEST.json"))
...
NOTES = {
    ...
    "fixtures/evidence/ACTIVE-AUTHORITY-PLACEHOLDER.json": ("fixture evidence is minted against this PLACEHOLDER manifest sha; validate_v1_8.py proves placeholder-bound records do not derive against the real FREEZE-MANIFEST.json sha and that re-minted records do", None),
...
m = {"schema": "vidtoolz.resolveFreezeManifest.v1.8", "bundle": "docs/resolve-integration/v1.8", "version": "1.8.0", "frozen_at": "2026-09-09", "prepared_by": "Claude Code (Fable 5.1)", "approval_status": "CANDIDATE_FOR_INDEPENDENT_REVIEW", "approver": None,
     "parent": {"version": "1.7.0", "branch": "docs/resolve-authority-freeze-v1.7", "head": PARENT_HEAD, "semantic_head": PARENT_SEMANTIC_HEAD, "manifest_sha256": PARENT_MANIFEST, "immutable": True,
                "ancestors": [
     ...
     "rules": ["a file whose sha256 or byte count differs from this manifest is not the frozen version", "v1.0 through v1.7 bytes and manifests are never modified; v1.8 supersedes by reference", "SCHEMA-VALID != AUTHORIZED TO MUTATE; PERMISSION DECLARATION != ELIGIBILITY; PROBE_ALLOWED != QUALIFIED_READ; A CAPTURE != EVIDENCE THAT SOMETHING WORKS; attachment state is derived from a coherent, current, envelope-bound evidence set against the ACTIVE reviewed manifest, never declared; contradictory evidence is CONFLICT", "a BUNDLE_VERIFICATION, MILESTONE_EXIT, M3_AUTHORIZATION or REFREEZE_RECORD counts only when bound to this manifest's sha256 and authority version 1.8.0; records for other manifests are historical", "mutation-capable schemas remain PROVISIONAL_UNTIL_M3; verification is derived from S0/S1 over the protected surface (verify_transaction) and never declared; commit_eligibility -> validate_transaction_set is the ONLY authorizing commit path (every per-document helper is INTERNAL_NON_AUTHORIZING); capability meaning is DERIVED by the versioned reference parser from an immutable facts-only raw capture (never from a probe-written flag), the ACTIVE capability authority is the matrix's own content digest, a degraded H0 can never be a write precondition, and schema enforcement in the composed path is mandatory; no adapter mutation implementation exists; no Resolve write was executed; M0 has not begun",
