...
v1.5 was corrected by making the schema validator a **required** parameter of `validate_transaction_set` and
`commit_eligibility`, so that no default could make schema enforcement optional. That closed the wrong half of the
...
1. **The authority owns the law; the caller supplies artifacts.** `validate_transaction_set` and `commit_eligibility`
