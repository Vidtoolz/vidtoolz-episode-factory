...
derive_attachment_state.
    ...
    python3 -B tools/a2_verify.py check  --session <id> [--principal VERIFIER:<actor>] [--evidence-root <path>]
    python3 -B tools/a2_verify.py verify --session <id> [--principal ...] [--evidence-root ...]
"""
import argparse
import hashlib
import json
...
def _independent_checks(a, quiet=False):
    ...
    try:
        es, wf = A._load(a.session, a.evidence_root)
    except A.AuthoringError as e:
        fails.append(f"{e.code}: {e.detail}")
        return fails, None, None
    say(f"document state      {wf['state']}  at {A.set_path(a.session, a.evidence_root)}")
...
def main(argv=None):
    ...
    try:
        r = A.add_bundle_verification(a.session, a.principal, root=a.evidence_root)
    except A.AuthoringError as e:
        print()
