#!/usr/bin/env python3
"""Layered, deterministic authority-validation suite for bundle v1.7.
...
Run: python3 -B tools/validate_v1_7.py
"""
import copy
import glob
import hashlib
...
rec("precedence", "the v1.5 and v1.6 changelogs and finding matrices are HISTORICAL; the v1.7 ones are active", st_.get("CHANGELOG-v1.5.md") == "HISTORICAL" and st_.get("FINDING-RESOLUTION-MATRIX-v1.5.md") == "HISTORICAL" and st_.get("CHANGELOG-v1.6.md") == "HISTORICAL" and st_.get("FINDING-RESOLUTION-MATRIX-v1.6.md") == "HISTORICAL" and st_.get("CHANGELOG-v1.7.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.7.md") == "STILL_ACTIVE")
rec("precedence", "the four v1.7 correction laws are STILL_ACTIVE documents with supersession statements S39-S45", all(st_.get(d_) == "STILL_ACTIVE" for d_ in ("AUTHORITY-CACHING.md", "SCHEMA-REGISTRY.md", "TRUSTED-SHIM.md", "STORED-CHAIN.md")) and sorted(s_["id"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.7.0") == ["S39", "S40", "S41", "S42", "S43", "S44", "S45"])
...
rec("precedence", "SCORECRAFT-EXTRACTION.md cites v1.7 active schemas, not earlier ones", "resolveTargetContract.v1.7" in scx and "resolveSnapshot.v1.7" in scx and not any(x in scx for x in ("resolveTargetContract.v1.1", "resolveSnapshot.v1.1", "resolveTargetContract.v1.4", "resolveSnapshot.v1.4", "resolveTargetContract.v1.5", "resolveSnapshot.v1.5", "resolveTargetContract.v1.6", "resolveSnapshot.v1.6")))
...
if os.path.exists(man_path):
    ...
    parent = L.strict_load(os.path.join(os.path.dirname(B), "v1.6", "FREEZE-MANIFEST.json"))
    rec("manifest", "parent (v1.6) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.6", "FREEZE-MANIFEST.json")) == "9f8a1a2e51f205409f8cb3f175144d21c59658221a42398612f982746a04ac29")
    rec("manifest", "grandparent (v1.5) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.5", "FREEZE-MANIFEST.json")) == "a40c6954fbe7f72d48eaf3957c0ac036c471a7a92496e277660530386e7aba5a")
    rec("manifest", "v1.0-v1.6 bundles are byte-identical to their frozen manifests (history preserved)", all(all(sha(os.path.join(os.path.dirname(B), v_, e_["path"])) == e_["sha256"] for e_ in L.strict_load(os.path.join(os.path.dirname(B), v_, "FREEZE-MANIFEST.json"))["files"] if os.path.exists(os.path.join(os.path.dirname(B), v_, e_["path"]))) for v_ in ("v1", "v1.1", "v1.2", "v1.3", "v1.4", "v1.5", "v1.6")))
    ...
    rec("manifest", "validation tools listed as TOOL", all(any(e["path"] == p and e["authority_class"] == "TOOL" for e in man["files"]) for p in ("tools/authority_lib.py", "tools/validate_v1_7.py", "tools/build_v1_7.py", "tools/build_manifest.py", "tools/fixture_evidence.py", "tools/capture_shim_reference.py")))
...
STORE_ROOT = tempfile.mkdtemp(prefix="resolve-v17-evidence-")
try:
    SM = ES_MOD.session_manifest("sess-v17-selftest", "probe-v17", "adapter-operator (M0A driver)", ENVC, ACTIVE_HYP, rp, "2026-09-09T10:00:00Z")
    ...
    for _field, _bad in (("authority_manifest_sha256", "0" * 64), ("trusted_shim_sha256", "0" * 64), ("parser_sha256", "0" * 64), ("primitive_spec_sha256", "0" * 64), ("schema_registry_sha256", "0" * 64), ("codec_version", "other"), ("stdout_retention", "KEEP_EVERYTHING")):
    ...
    ST = ES_MOD.create_session(STORE_ROOT, "sess-v17-selftest", SM)
    rec("evidence-store", "a session directory is created once and can never be created again",
        os.path.isdir(ST.dir) and (lambda: [_e.code for _e in [_try_store(lambda: ES_MOD.create_session(STORE_ROOT, "sess-v17-selftest", SM))]][0] == "SESSION_EXISTS")())
    _f1 = SHIM.raw_frame_bytes(_good_cap)
    _p1 = ST.put("RAW", "probe-v17:0001", _f1)
    ...
    rec("evidence-store", "re-putting byte-identical content under the same attempt id is idempotent, not an overwrite",
        ST.put("RAW", "probe-v17:0001", _f1)["idempotent"] is True)
    rec("evidence-store", "a caller-declared digest is compared and never trusted",
        _try_store(lambda: ST.put("RAW", "probe-v17:0002", _f1 + b" ", declared_digest=_p1["digest"])).code == "DECLARED_DIGEST_MISMATCH")
    rec("evidence-store", "the same attempt id with different bytes is refused",
        _try_store(lambda: ST.put("RAW", "probe-v17:0001", _f1 + b"\n")).code == "ATTEMPT_ID_REUSED")
    rec("evidence-store", "a path traversal in the attempt id is refused before any file is touched",
        _try_store(lambda: ST.put("RAW", "../../escape", _f1)).code == "NAME_INVALID" and _try_store(lambda: ST.put("NOT_A_LAYER", "probe-v17:0003", _f1)).code == "LAYER_UNKNOWN")
    _sym = os.path.join(STORE_ROOT, "symlinked-session")
    os.symlink(ST.dir, _sym)
    rec("evidence-store", "a symlinked session name is refused",
        _try_store(lambda: ES_MOD.open_session(STORE_ROOT, "symlinked-session")).code == "SYMLINK_REJECTED")
    for _i, _payload in enumerate([b'{"derived":1}\n', b'{"review":1}\n', b'{"promotion":1}\n']):
        ST.put(("DERIVED", "HUMAN_REVIEW", "AUTHORITY_PROMOTION")[_i], f"probe-v17:1{_i:03d}", _payload)
    _hashes_sha = ST.finalize()
    rec("evidence-store", "finalization writes a HASHES manifest that is the digest of itself and a FINALIZED marker naming it",
        L.is_sha(_hashes_sha) and ST.hashes()["hashes_sha256"] == _hashes_sha and ST.verify() == [], "; ".join(ST.verify()[:2])[:150])
    rec("evidence-store", "after finalization the session is immutable: every further write is refused",
        _try_store(lambda: ST.put("RAW", "probe-v17:9999", b'{"late":1}\n')).code == "SESSION_FINALIZED" and _try_store(lambda: ST.finalize()).code == "SESSION_FINALIZED")
    ...
    # tampering attacks against the finalized session
    _rec_path = os.path.join(ST.dir, "RAW", _p1["digest"][:2], _p1["digest"] + ".json")
    _orig = open(_rec_path, "rb").read()
    os.chmod(_rec_path, 0o600)
    open(_rec_path, "wb").write(_orig[:-1] + b" \n")
    rec("evidence-store", "replacing the bytes of a stored record is detected as EVIDENCE_REPLACED", any("EVIDENCE_REPLACED" in e for e in ST.verify()), "; ".join(ST.verify()[:1])[:140])
    open(_rec_path, "wb").write(_orig)
    os.chmod(_rec_path, 0o400)
    rec("evidence-store", "restoring the exact bytes makes the session verify again", ST.verify() == [])
    _hp = os.path.join(ST.dir, "HASHES.json")
    _horig = open(_hp, "rb").read()
    os.chmod(_hp, 0o600)
    open(_hp, "wb").write(_horig.replace(b'"record_count"', b'"record_countX"', 1))
    rec("evidence-store", "tampering with HASHES.json is detected", any("HASHES_TAMPERED" in e for e in ST.verify()))
    open(_hp, "wb").write(_horig)
    os.chmod(_hp, 0o444)
    _del = os.path.join(ST.dir, "DERIVED")
    _dfile = [os.path.join(dp, f_) for dp, _dn, fn in os.walk(_del) for f_ in fn][0]
    os.chmod(os.path.dirname(_dfile), 0o700)
    _dbytes = open(_dfile, "rb").read()
    os.unlink(_dfile)
    rec("evidence-store", "removing a record that HASHES lists is detected as RECORD_MISSING", any("RECORD_MISSING" in e for e in ST.verify()))
    open(_dfile, "wb").write(_dbytes)
    os.chmod(_dfile, 0o400)
    rec("evidence-store", "the restored record verifies again", ST.verify() == [])
    # an unfinalized session and a partially written record
    ST2 = ES_MOD.create_session(STORE_ROOT, "sess-v17-unfinalized", ES_MOD.session_manifest("sess-v17-unfinalized", "probe-v17b", "adapter-operator (M0A driver)", ENVC, ACTIVE_HYP, rp, "2026-09-09T10:05:00Z"))
    ST2.put("RAW", "probe-v17b:0001", _f1)
    rec("evidence-store", "an unfinalized session is reported as not finalized and has no HASHES manifest",
        any("SESSION_NOT_FINALIZED" in e for e in ST2.verify()) and _try_store(lambda: ST2.hashes()).code == "SESSION_NOT_FINALIZED")
    open(os.path.join(ST2.dir, "RAW", _p1["digest"][:2], ".partial-deadbeef"), "wb").write(b"half")
    rec("evidence-store", "a partially written temp file is reported and blocks finalization",
        any("PARTIAL_RECORD" in e for e in ST2.verify(require_finalized=False)) and _try_store(lambda: ST2.finalize()).code == "PARTIAL_RECORD")
    rec("evidence-store", "the store never opens an existing record for writing, truncates, renames over one or unlinks one",
    ...
    rec("evidence-store", "the executable store implements the four EVIDENCE-ROOT layers and a closed refusal vocabulary",
        ES_MOD.LAYERS == ("RAW", "DERIVED", "HUMAN_REVIEW", "AUTHORITY_PROMOTION") and len(ES_MOD.ERROR_CODES) >= 20)
finally:
...
CODEX_MATRIX = L.strict_load(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.7.json"))
rec("codex-matrix", "the v1.7 finding-resolution matrix enumerates exactly the seven Codex v1.6 ids",
    sorted(f["id"] for f in CODEX_MATRIX["findings"]) == ["C16-B1", "C16-B2", "C16-B3", "C16-B4", "C16-M1", "C16-M2", "C16-M3"])
for _f in CODEX_MATRIX["findings"]:
    ...
    rec("codex-matrix", f"{_f['id']}: correction, negative fixture and expected layer are named, and its validation sections all pass",
        bool(_f["correction"]) and bool(_f["negative_fixture"]) and bool(_f["expected_layer"]) and len(_checks) >= 3 and all(ok_ for _s, _n, ok_ in _checks),
