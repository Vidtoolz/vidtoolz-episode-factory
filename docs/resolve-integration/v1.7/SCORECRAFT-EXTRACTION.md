...
Active extraction targets are the **v1.6** authorities (`resolveTargetContract.v1.7`, `resolveSnapshot.v1.7`, `TARGET-ATTACHMENT-GATE.md`, `ELIGIBILITY.md`, `SEMANTIC-VALIDATION.md`). References to earlier schema versions in `../v1/`, `../v1.1/`, `../v1.2/`, `../v1.3/`, `../v1.4/`, `../v1.5/` are historical only.
...
|---|---|---|
| explicit target contract shape (`normalizeTarget`) | QUALIFIED_PATTERN | PRESERVE as concept → `resolveTargetContract.v1.7` (attachment state derived from envelope-bound, current evidence against the active manifest, not declared) |
...
| readback | EXPERIMENTAL | REWRITE for `resolveSnapshot.v1.7` (all tracks, provenance variants, observation status on every timeline/project/track/item field, field provenance (`method_provenance`), unique occurrence identity, failure ledger, no rounding, plural APIs) |
