...
## v1.7: raw evidence enters as BYTES (Codex C16-M2)

v1.6 let a raw capture enter the authority as an already-parsed dictionary. That is not equivalent to parsing a frame,
because the standard JSON parser silently keeps the **last** of two duplicate keys: a frame carrying `"method"` twice
would have been accepted with the attacker's value, and nothing would have recorded that a strict parse never happened.

`authority_lib.ingest_raw_frame(raw_bytes, env, active, rp)` is the one canonical boundary:

1. **bytes** — a parsed object is refused outright (`NOT_BYTES`).
2. **strict single-object parse** — refuses a duplicate JSON key, malformed UTF-8, a byte-order mark, a raw control
   character, invalid number syntax, the non-finite constants, trailing bytes after the object, a second JSON value in
   one frame, a top-level value that is not an object, an empty frame and an oversized frame. Each refusal carries one
   code from the closed `RAW_INGEST_CODES` vocabulary.
3. **frame schema** — the frame must declare the trusted raw schema version (`UNKNOWN_SCHEMA_VERSION`).
4. **structure** — the facts-only capture rules of this document, unchanged.
5. **digest** — `raw_digest` must re-hash the frame content.
6. **trusted shim** — the capture must come from the shim pinned by `TRUSTED-SHIM.json` (see `TRUSTED-SHIM.md`).
7. **receipt** — the record carries a strict-parse receipt naming the exact frame bytes, their sha256 and byte count,
   the ingest, codec, raw schema and authority versions, the four strictness assertions, the capture digest those
   bytes produced, and its own digest.

A `RAW_CAPABILITY_CAPTURE` record without an admissible receipt is inadmissible
(`PARSED_OBJECT_NOT_AUTHORITY`). The wire format of a frame is
`capture_shim_reference.raw_frame_bytes(capture)`: sorted-key, minimal-separator UTF-8 JSON with a trailing newline.
Those exact bytes are what the shim writes into the RAW layer, what sha256 addresses that record by, and what the
strict parser re-reads.
