...
Run: python3 -B tools/validate_v1_11.py
"""
import copy
import glob
import hashlib
...
rec("precedence", "the v1.5 through v1.10 changelogs and finding matrices are HISTORICAL; the v1.11 ones are active", all(st_.get(d_) == "HISTORICAL" for d_ in ("CHANGELOG-v1.5.md", "FINDING-RESOLUTION-MATRIX-v1.5.md", "CHANGELOG-v1.6.md", "FINDING-RESOLUTION-MATRIX-v1.6.md", "CHANGELOG-v1.7.md", "FINDING-RESOLUTION-MATRIX-v1.7.md", "CHANGELOG-v1.8.md", "FINDING-RESOLUTION-MATRIX-v1.8.md", "CHANGELOG-v1.9.md", "FINDING-RESOLUTION-MATRIX-v1.9.md", "CHANGELOG-v1.10.md", "FINDING-RESOLUTION-MATRIX-v1.10.md")) and st_.get("CHANGELOG-v1.11.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.11.md") == "STILL_ACTIVE")
...
rec("precedence", "the v1.11 evidence-store supersessions S52-S55 are recorded and the v1.11 changelog and matrix are active", sorted(s_["id"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.11.0") == ["S52", "S53", "S54", "S55"] and st_.get("CHANGELOG-v1.11.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.11.md") == "STILL_ACTIVE" and st_.get("CHANGELOG-v1.10.md") == "HISTORICAL" and st_.get("EVIDENCE-ROOT.md") == "STILL_ACTIVE")
...
if os.path.exists(man_path):
    ...
    parent = L.strict_load(os.path.join(os.path.dirname(B), "v1.10", "FREEZE-MANIFEST.json"))
    rec("manifest", "parent (v1.10) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.10", "FREEZE-MANIFEST.json")) == "46491cee979dc59a306993de8cc66308884967dca46304168096f935939f6156")
    rec("manifest", "grandparent (v1.9) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.9", "FREEZE-MANIFEST.json")) == "5aac34dae6c2ec9842aaa3499d7c20ffd104f65966ba7cb8b0c79d32d42eb9fe")
    ...
    rec("manifest", "the parent block records the v1.10 branch HEAD and its semantic commit, which for v1.10 are the same commit because its bundle and registration were committed together (section 38)", man["parent"]["head"] == "9c6643430d2dc66741afe9852668d8e6e19c0630" and man["parent"]["semantic_head"] == "9c6643430d2dc66741afe9852668d8e6e19c0630" and man["parent"]["manifest_sha256"] == "46491cee979dc59a306993de8cc66308884967dca46304168096f935939f6156")
    rec("manifest", "v1.0-v1.10 bundles are byte-identical to their frozen manifests (history preserved)", all(all(sha(os.path.join(os.path.dirname(B), v_, e_["path"])) == e_["sha256"] for e_ in L.strict_load(os.path.join(os.path.dirname(B), v_, "FREEZE-MANIFEST.json"))["files"] if os.path.exists(os.path.join(os.path.dirname(B), v_, e_["path"]))) for v_ in ("v1", "v1.1", "v1.2", "v1.3", "v1.4", "v1.5", "v1.6", "v1.7", "v1.8", "v1.9", "v1.10")))
    ...
    rec("manifest", "validation tools listed as TOOL", all(any(e["path"] == p and e["authority_class"] == "TOOL" for e in man["files"]) for p in ("tools/authority_lib.py", "tools/validate_v1_11.py", "tools/build_v1_11.py", "tools/build_manifest.py", "tools/fixture_evidence.py", "tools/capture_shim_reference.py")))
...
CODEX_MATRIX = L.strict_load(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.11.json"))
...
rec("codex-matrix", "the inherited v1.6 finding matrix is retained unchanged as history and its seven ids are still enumerated",
    ...
    and CODEX_MATRIX["inherited_matrices"] == ["FINDING-RESOLUTION-MATRIX-v1.10.json", "FINDING-RESOLUTION-MATRIX-v1.9.json", "FINDING-RESOLUTION-MATRIX-v1.8.json", "FINDING-RESOLUTION-MATRIX-v1.7.json"])
...
rec("codex-matrix", "the ACTIVE v1.11 finding-resolution matrix enumerates exactly the four release-relevant Codex v1.10 ids",
    sorted(f["id"] for f in CODEX_MATRIX["findings"]) == ["F110-A", "F110-B", "F110-C", "F110-D"])
rec("codex-matrix", "the v1.11 matrix pins the reproduction against the frozen v1.10 head and records the deliberate filesystem-normalization exclusion",
    all("9c66434" in f_["prior_defect"] and "frozen v1.10 bundle" in f_["prior_defect"] for f_ in CODEX_MATRIX["findings"])
    and "9c6643430d2dc66741afe9852668d8e6e19c0630" in CODEX_MATRIX["law"]
    and "filesystem-normalization" in CODEX_MATRIX["operational_closure"]
    and "1911/1911" in CODEX_MATRIX["adjudication"])
