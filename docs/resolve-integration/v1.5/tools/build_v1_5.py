#!/usr/bin/env python3
"""Generate v1.5 machine artifacts as a narrow correction of v1.5. Deterministic; offline; no Resolve.
Run from the bundle dir: python3 -B tools/build_v1_5.py
Inputs: the v1.5 instances/schemas already seeded in this directory (byte-identical copies of the frozen v1.5 bundle)."""
import copy
import hashlib
import json
import os
...
tc["schema"] = "vidtoolz.resolveTargetContract.v1.5"; tc["version"] = "1.5.0"
...
tcs["$id"] = "vidtoolz.resolveTargetContract.v1.5"; tcs["title"] = "Resolve target contract v1.5 (attachment gate; derived state; envelope law)"
tcs["properties"]["schema"] = {"const": "vidtoolz.resolveTargetContract.v1.5"}
tcs["properties"]["version"] = {"type": "string", "pattern": "^1\\.5\\.[0-9]+$"}
...
caps["refreeze"] = {"kind": "M0_READ_REQUALIFICATION", "reviewed": False, "review_decision_ref": None, "promoted_probe_ids": [], "promoted_evidence_sha256": [], "note": "no probe has run; nothing promoted"}
caps["qualification_pipeline"] = ["RAW_EVIDENCE (content + content_sha256) captured by the probe", "PARSE: parse block {parser, succeeded, error, getter_exception, observed_type, shape_ok} + parsed_observation; the classification label MUST equal the classification derived from these facts", "PARSED_RESULT: SUCCESS only when parse succeeded, no error, no getter exception, expected receiver and expected_type, non-null shape", "REVIEW: independent decision per record (ACCEPT/REJECT) whose review_raw_evidence_sha256 names the exact raw evidence hash; a failed observation can never be ACCEPTed", "REFREEZE: new CAPABILITIES.json with reviewed refreeze block listing promoted_probe_ids AND promoted_evidence_sha256, rows QUALIFIED_READ citing (method, probe_id, raw_evidence_sha256, result SUCCESS)", "REFREEZE_RECORD in the evidence set naming the new matrix sha256", "ACTIVE_CAPABILITY: records' qualification.promoted_by = {authority_version, capability_matrix_sha256, raw_evidence_sha256} of the active matrix; the evaluator derives success from raw+parse and compares every link to the ACTIVE matrix; a label, a review or a refreeze can never override a parsed failure"]
for r in caps["rows"]:
...
cap_schema["$id"] = "vidtoolz.resolveCapabilityMatrix.v1.5"; cap_schema["title"] = "Capability matrix v1.5"
cap_schema["properties"]["schema"] = {"const": "vidtoolz.resolveCapabilityMatrix.v1.5"}
cap_schema["properties"]["refreeze"] = {"type": "object", "additionalProperties": False, "required": ["kind", "reviewed", "review_decision_ref", "promoted_probe_ids", "promoted_evidence_sha256"], "properties": {"kind": {"const": "M0_READ_REQUALIFICATION"}, "reviewed": {"type": "boolean"}, "review_decision_ref": STR_OR_NULL, "promoted_probe_ids": {"type": "array", "items": {"type": "string"}}, "promoted_evidence_sha256": {"type": "array", "items": SHA}, "note": {"type": "string"}}}
...
neg = copy.deepcopy(caps); neg["rows"][0]["evidence_class"] = "QUALIFIED_READ"; neg["rows"][0]["evidence_records"] = [{"host": HOST, "resolve_version": "21.1.0", "build": 14, "run_ref": "probe", "method": "GetVersionString", "observed_result": "x", "evidence_path": "/x", "evidence_sha256": "a" * 64, "version_match": True, "reviewed_refreeze_version": "1.5.0", "probe_id": "probe-0001", "raw_evidence_sha256": "b" * 64, "result": "CAPABILITY_FAILURE"}]; fixture("capabilities-qualified-read-on-failed-evidence", "semantic", "resolveCapabilityMatrix", neg, check="semantic_capabilities", expect_contains="SUCCESS")
neg = copy.deepcopy(caps); neg["rows"][0]["evidence_class"] = "QUALIFIED_READ"; neg["rows"][0]["evidence_records"] = [{"host": HOST, "resolve_version": "21.1.0", "build": 14, "run_ref": "probe", "method": "GetVersionString", "observed_result": "x", "evidence_path": "/x", "evidence_sha256": "a" * 64, "version_match": True, "reviewed_refreeze_version": "1.4.0", "probe_id": "probe-0001", "raw_evidence_sha256": "b" * 64, "result": "SUCCESS"}]; fixture("capabilities-qualified-read-promoted-by-other-refreeze", "semantic", "resolveCapabilityMatrix", neg, check="semantic_capabilities", expect_contains="refreeze")
neg = copy.deepcopy(caps); neg["rows"][0]["evidence_class"] = "QUALIFIED_READ"; neg["rows"][0]["evidence_records"] = [{"host": HOST, "resolve_version": "21.1.0", "build": 14, "run_ref": "probe", "method": "GetVersionString", "observed_result": "x", "evidence_path": "/x", "evidence_sha256": "a" * 64, "version_match": True, "reviewed_refreeze_version": "1.5.0", "probe_id": "probe-0001", "raw_evidence_sha256": "b" * 64, "result": "SUCCESS"}]; fixture("capabilities-qualified-read-unreviewed-refreeze-block", "semantic", "resolveCapabilityMatrix", neg, check="semantic_capabilities", expect_contains="reviewed refreeze block")
...
neg = copy.deepcopy(caps); neg["rows"][0]["evidence_class"] = "QUALIFIED_READ"; neg["refreeze"].update(reviewed=True, promoted_probe_ids=["probe-0001"], promoted_evidence_sha256=[]); neg["rows"][0]["evidence_records"] = [{"host": HOST, "resolve_version": "21.1.0", "build": 14, "run_ref": "probe", "method": "GetVersionString", "observed_result": "x", "evidence_path": "/x", "evidence_sha256": "a" * 64, "version_match": True, "reviewed_refreeze_version": "1.5.0", "probe_id": "probe-0001", "raw_evidence_sha256": "b" * 64, "result": "SUCCESS"}]; fixture("capabilities-qualified-read-evidence-not-promoted-by-refreeze", "semantic", "resolveCapabilityMatrix", neg, check="semantic_capabilities", expect_contains="not explicitly promoted")
...
rp["schema"] = "vidtoolz.resolveReadPrimitives.v1.5"; rp["version"] = "1.5.0"
for op, spec in rp["logical_operations"].items():
    for p in spec["primitives"]:
        p["receiver"] = F.receiver_of(p["method"])
        p["expected_type"] = F.expected_type_of(p["method"])
...
rp_schema["$id"] = "vidtoolz.resolveReadPrimitives.v1.5"; rp_schema["properties"]["schema"] = {"const": "vidtoolz.resolveReadPrimitives.v1.5"}
...
pi["required"] = sorted(set(pi["required"]) | {"receiver", "expected_type"})
rp["expected_type_note"] = "expected_type is the documented 21.1 return type of the getter (provisional expectation); the probe parser records observed_type and shape_ok, and SUCCESS is derived only when they match"
...
HYP_RAWS = sorted({L.sha256_text(F.raw_content(m)) for r in caps_hyp["rows"] if r["operation"].startswith("read:") for m in r["primitives"]})
caps_hyp["refreeze"] = {"kind": "M0_READ_REQUALIFICATION", "reviewed": True, "review_decision_ref": "HYPOTHETICAL review decision", "promoted_probe_ids": ["probe-0001"], "promoted_evidence_sha256": HYP_RAWS, "note": "hypothetical"}
for r in caps_hyp["rows"]:
    if r["operation"].startswith("read:"):
        r["evidence_class"] = "QUALIFIED_READ"; r.pop("probe_candidate", None)
        r["evidence_records"] = [{"host": HOST, "resolve_version": "21.1.0", "build": 14, "run_ref": "HYPOTHETICAL M0 probe run probe-0001", "method": m, "observed_result": "hypothetical non-null", "evidence_path": "/HYPOTHETICAL", "evidence_sha256": hashlib.sha256(m.encode()).hexdigest(), "version_match": True, "reviewed_refreeze_version": "1.5.0-HYPOTHETICAL", "probe_id": "probe-0001", "raw_evidence_sha256": L.sha256_text(F.raw_content(m)), "result": "SUCCESS"} for m in r["primitives"]]
...
dump("fixtures/evidence/ACTIVE-AUTHORITY-PLACEHOLDER.json", {"schema": "vidtoolz.resolveFixtureActiveAuthority.v1.5", "note": "fixtures are minted against this PLACEHOLDER manifest sha; the validator proves they do not derive against the real FREEZE-MANIFEST.json sha and that records re-minted against the real sha do", "placeholder_manifest_sha256": F.PLACEHOLDER_MANIFEST, "capability_matrix_sha256": CAPS_SHA, "hypothetical_capability_matrix_sha256": HYP_SHA})

# ============================================================ EVIDENCE SET schema v1.5 (envelope law) + fixture evidence sets
...
QUAL_S = {"type": "object", "additionalProperties": False, "required": ["reviewed", "decision", "review_raw_evidence_sha256", "promoted_by"], "properties": {"reviewed": {"type": "boolean"}, "decision": {"anyOf": [{"enum": ["ACCEPT", "REJECT"]}, {"type": "null"}]}, "review_raw_evidence_sha256": SHA_OR_NULL, "promoted_by": {"anyOf": [{"type": "object", "additionalProperties": False, "required": ["authority_version", "capability_matrix_sha256", "raw_evidence_sha256"], "properties": {"authority_version": {"type": "string"}, "capability_matrix_sha256": SHA, "raw_evidence_sha256": SHA}}, {"type": "null"}]}}}
PARSE_S = {"type": "object", "additionalProperties": False, "required": ["parser", "succeeded", "error", "getter_exception", "observed_type", "shape_ok"], "properties": {"parser": {"type": "string", "minLength": 1}, "succeeded": {"type": "boolean"}, "error": STR_OR_NULL, "getter_exception": STR_OR_NULL, "observed_type": {"enum": list(L.EXPECTED_TYPES)}, "shape_ok": {"type": "boolean"}}}
RECORD_RULES = [
    ...
    rec_schema("CAPABILITY_EVIDENCE", ["method", "receiver_type", "probe_id", "probe_authority_version", "raw_evidence_sha256", "parse", "parsed_observation", "result", "qualification"], {"method": {"type": "string", "minLength": 1}, "receiver_type": {"type": "string", "minLength": 1}, "probe_id": {"type": "string", "minLength": 1}, "probe_authority_version": {"type": "string", "minLength": 1}, "raw_evidence_sha256": SHA, "parse": PARSE_S, "parsed_observation": {"type": "object", "required": ["type", "non_null"], "properties": {"type": {"enum": list(L.EXPECTED_TYPES)}, "non_null": {"type": "boolean"}}}, "result": RES_S, "qualification": QUAL_S}),
...
ev_schema = S("vidtoolz.resolveEvidenceSet.v1.5", "Linked evidence set (content-addressed, envelope-bound records)", {"schema": {"const": "vidtoolz.resolveEvidenceSet.v1.5"}, "current_session_id": STR_OR_NULL, "evaluated_at": {"type": "string", "minLength": 20}, "records": {"type": "object", "propertyNames": {"pattern": "^[a-f0-9]{64}$"}, "additionalProperties": {"type": "object", "required": ["record_type", "record_id", "envelope", "recorded_at"], "properties": record_common, "allOf": RECORD_RULES}}}, ["schema", "current_session_id", "evaluated_at", "records"],
              comment="v1.5: CAPABILITY_EVIDENCE success is DERIVED from RAW_EVIDENCE + parse (never from the label); the review names the exact raw hash; the promotion names the exact raw hash. Every record carries the identity envelope (authority_version, manifest_sha256, host, product/version/build, library name/uuid/root, session_id, provisioning_id, project/timeline identity, target_epoch, sequence, captured_at, record_type_version). Level law: BUNDLE records need authority+manifest+host; LIBRARY records add library name/uuid/root; SESSION records add product/version/build, session_id and provisioning_id. record_id = sha256(domain vidtoolz.resolveEvidenceRecord.v1 + canonical body without record_id). Inconsistent records never compose (envelope_conflicts); the CURRENT record of a type is the highest envelope.sequence within current_session_id; map order is irrelevant.")
dump("schemas/resolveEvidenceSet.schema.json", ev_schema)
req_schema = load("schemas/resolveEligibilityRequest.schema.json")
req_schema["$id"] = "vidtoolz.resolveEligibilityRequest.v1.5"; req_schema["properties"]["schema"] = {"const": "vidtoolz.resolveEligibilityRequest.v1.5"}
...
# ============================================================ SNAPSHOT v1.5: observation status on timeline/project/track fields; capability coupling; GUARD v2
...
PROV_ENTRY_S = {"type": "object", "additionalProperties": False, "required": ["observation_class", "capability_evidence_record_id", "capability_matrix_sha256"], "properties": {"observation_class": {"enum": list(L.OBSERVATION_CLASSES)}, "capability_evidence_record_id": SHA_OR_NULL, "capability_matrix_sha256": SHA_OR_NULL}}
...
snap_schema = S("vidtoolz.resolveSnapshot.v1.5", "Canonical Resolve readback snapshot v1.5", {
    "schema": {"const": "vidtoolz.resolveSnapshot.v1.5"}, "collector_version": {"type": "string", "minLength": 1}, "canonicalization_version": {"const": "1.5"}, "target_epoch": {"type": "string", "minLength": 1}, "resolve_build": {"type": "string", "minLength": 1},
    ...
    comment="v1.5: the honest observation model (OBSERVED|UNAVAILABLE|UNSUPPORTED|ERROR|NOT_REQUESTED + reason) covers timeline fields (unique_id, name, start_frame, end_frame, start_timecode, fps, width, height, is_current, settings), project fields (unique_id, name, last_modified_time), track fields (name, enabled, locked) and the ten item fields. Every value is nullable; null only when status permits; OBSERVED requires a value AND a callable producing primitive cited in collection.method_provenance as a QUALIFIED_OBSERVATION backed by qualifying CAPABILITY_EVIDENCE under the active matrix (field provenance law); CANDIDATE_OBSERVATION (probe output) never backs an OBSERVED field; occurrence unique_id must be unique within the timeline (DUPLICATE_OCCURRENCE_IDENTITY). coverage.complete is a function of the profile's mandatory domains, item fields, timeline fields, identity, locks, guard and an empty failure ledger; otherwise incomplete_reasons must name what is missing.")
...
guard_schema = S("vidtoolz.resolveGuard.v3", "Composite pre-write / revalidation guard v3 (identity with observation status + reason; provenance binding)", {"hash_domain": {"const": "vidtoolz.resolveGuard.v3"}, "guard_version": {"const": 3}, "library": library_s, "project": ident_s, "timeline": ident_s, "target_epoch": {"type": "string"}, "coverage": coverage, "policy": policy_s, "provenance_sha256": SHA, "payload_sha256": SHA}, ["hash_domain", "guard_version", "library", "project", "timeline", "target_epoch", "coverage", "policy", "provenance_sha256", "payload_sha256"],
                 comment="Guard digest = sha256(domain vidtoolz.resolveGuard.v3 + canonical guard object). v3 carries project/timeline identity WITH observation status and reason, and provenance_sha256 = digest(collection.method_provenance, vidtoolz.resolveProvenance.v1) so the guard binds which capability evidence produced the OBSERVED fields. Payload digest alone is never a version token.")
dump("schemas/resolveGuard.schema.json", guard_schema)
policy = {"target_contract_sha256": sha_of("TARGET-CONTRACT.json"), "timebase_sha256": sha_of("TIMEBASE.json"), "track_policy_sha256": sha_of("schemas/resolveTrackPolicy.v1.json"), "capabilities_version": "1.5.0", "capability_matrix_sha256": CAPS_SHA, "collector_version": "0.0.0-fixture", "canonicalization_version": "1.5"}
...
CAPEV_ID = {r["method"]: r["record_id"] for r in H["caps_ok"] if r["record_type"] == "CAPABILITY_EVIDENCE"}
CAPEV_UNREV_ID = {r["method"]: r["record_id"] for r in H["caps_unrev"] if r["record_type"] == "CAPABILITY_EVIDENCE"}
HYP_PROV = {m: {"observation_class": "QUALIFIED_OBSERVATION", "capability_evidence_record_id": CAPEV_ID[m], "capability_matrix_sha256": HYP_SHA} for m in PROBE_METHODS}
FROZEN_PROV = {m: {"observation_class": "NOT_CALLABLE", "capability_evidence_record_id": None, "capability_matrix_sha256": None} for m in PROBE_METHODS}
CANDIDATE_PROV = {m: {"observation_class": "CANDIDATE_OBSERVATION", "capability_evidence_record_id": CAPEV_UNREV_ID[m], "capability_matrix_sha256": None} for m in PROBE_METHODS}
...
deg["collection"]["method_provenance"]["GetStartFrame"] = {"observation_class": "NOT_CALLABLE", "capability_evidence_record_id": None, "capability_matrix_sha256": None}
...
sneg("snapshot-claims-callable-not-in-authority", lambda d: d["collection"]["method_provenance"].update(GetStart={"observation_class": "QUALIFIED_OBSERVATION", "capability_evidence_record_id": CAPEV_ID["GetStart"], "capability_matrix_sha256": CAPS_SHA}), "claims GetStart QUALIFIED_OBSERVATION", base=m0_snap, extra=SX_FROZEN)
...
sneg("snapshot-provenance-cites-unreviewed-evidence", lambda d: d["collection"]["method_provenance"]["GetStartFrame"].update(capability_evidence_record_id=CAPEV_UNREV_ID["GetStartFrame"]), "does not qualify", extra={"capabilities": HYP, "evidence_set": "attached-candidate-evidence-unreviewed"})
sneg("snapshot-provenance-cites-adjacent-getter", lambda d: d["collection"]["method_provenance"]["GetStartFrame"].update(capability_evidence_record_id=CAPEV_ID["GetEndFrame"]), "adjacent getter")
...
sneg("snapshot-provenance-evidence-not-in-set", lambda d: d["collection"]["method_provenance"]["GetStartFrame"].update(capability_evidence_record_id="1" * 64), "does not resolve")
sneg("snapshot-provenance-qualified-without-evidence-id", lambda d: d["collection"]["method_provenance"]["GetStartFrame"].update(capability_evidence_record_id=None), "requires capability_evidence_record_id")
...
sneg("snapshot-provenance-cites-failed-evidence", lambda d: None, "does not qualify", extra={"capabilities": HYP, "evidence_set": "attached-evidence-label-success-parse-failed"})
...
dump("fixtures/canonicalization/vectors.json", {"schema": "vidtoolz.resolveCanonicalizationVectors.v1.5", "spec": "CANONICALIZATION.md v1.5", "digest_rule": "sha256(utf8(domain) + 0x0A + canonical_bytes)", "registered_domains": sorted(L.HASH_DOMAINS), "reference_implementation": "tools/authority_lib.py", "vectors": vectors})
...
dump("fixtures/canonicalization/rejections.json", {"schema": "vidtoolz.resolveCanonicalizationRejections.v1.5", "cases": cases})

# ============================================================ PERMISSIONS v1.5 (unchanged law; schema/version bump) + ELIGIBILITY cases
perms = load("PERMISSIONS.json")
perms["schema"] = "vidtoolz.resolvePermissions.v1.5"; perms["version"] = "1.5.0"
...
perm_schema["$id"] = "vidtoolz.resolvePermissions.v1.5"; perm_schema["properties"]["schema"] = {"const": "vidtoolz.resolvePermissions.v1.5"}
...
matrix = load("MILESTONE-MATRIX.json"); matrix["schema"] = "vidtoolz.resolveMilestoneMatrix.v1.5"; matrix["version"] = "1.5.0"; dump("MILESTONE-MATRIX.json", matrix)
probes = load("M3-PROBES.json"); probes["schema"] = "vidtoolz.resolveM3Probes.v1.5"; dump("M3-PROBES.json", probes)


def REQ(m, op, project=None, timeline=None, refs=None, **kw):
    r = {"schema": "vidtoolz.resolveEligibilityRequest.v1.5", "milestone": m, "operation": op, "scope": SCOPE, "refs": refs or {}}
...
case("m0-connect-deny-hyp-parse-failed-accepted", REQ("M0", "CONNECT", refs=ROJ), "attached-evidence-parse-failed-accepted", False, "REFUSE", capabilities=HYP)
case("m0-connect-deny-hyp-getter-exception-accepted", REQ("M0", "CONNECT", refs=ROJ), "attached-evidence-getter-exception-accepted", False, "REFUSE", capabilities=HYP)
case("m0-connect-deny-hyp-label-success-parse-failed", REQ("M0", "CONNECT", refs=ROJ), "attached-evidence-label-success-parse-failed", False, "REFUSE", capabilities=HYP)
case("m0-connect-deny-hyp-wrong-observed-type", REQ("M0", "CONNECT", refs=ROJ), "attached-evidence-wrong-observed-type", False, "REFUSE", capabilities=HYP)
case("m0-connect-deny-hyp-review-references-other-raw", REQ("M0", "CONNECT", refs=ROJ), "attached-evidence-review-other-raw", False, "REFUSE", capabilities=HYP)
case("m0-connect-deny-hyp-promotion-names-other-raw", REQ("M0", "CONNECT", refs=ROJ), "attached-evidence-promoted-other-raw", False, "REFUSE", capabilities=HYP)
case("m0-connect-deny-hyp-null-result", REQ("M0", "CONNECT", refs=ROJ), "attached-evidence-null-result", False, "REFUSE", capabilities=HYP)
...
dump("fixtures/eligibility/cases.json", {"schema": "vidtoolz.resolveEligibilityFixtures.v1.5", "law": "expect_eligible/expect_attachment_state are computed by evaluate_eligibility/derive_attachment_state over (PERMISSIONS, request, READ-PRIMITIVES, capabilities file, TARGET-CONTRACT, fixtures/evidence/<evidence_set>.json, active authority = {1.5.0, placeholder manifest sha, sha256 of the capabilities file}); results must be identical under randomized record order", "cases": elig})
...
vs["required"] = sorted(set(vs["required"]) | {"applied_operation_ids", "session_id"})
vs["$comment"] = "v1.5: the verifier DERIVES added/removed/changed/creation_identity_map/missing_expected/unrelated/verdict from S0, the bound plan, the journal's applied operations and S1 (authority_lib.verify_transaction); a declared result must equal the derived truth. h0/s1 digests must resolve to supplied snapshots; readback_snapshot_sha256 = digest of the S1 snapshot object. VERIFIED is impossible for operations whose effect law is NOT_YET_SPECIFIED (EFFECT_NOT_SPECIFIED). Never human approval."
...
p = copy.deepcopy(plan_m3); p["authority_version"] = "1.4.0"; fixture("plan-authority-version-old", "schema", "provisional/resolveMutationPlan", p, expect_contains="1.5.0")
...
fixture("journal-recovery-committed-recovered", "none", "provisional/resolveTransactionJournal", chain(["PREPARED", "LEASED", "PREFLIGHT_OK", "OP_STARTED", "APPLIED", "RECOVERY_RECONCILING", "COMMITTED_RECOVERED"]), check="semantic_journal", extra={"plan": plan_m3})
...
fixture("journal-operation-failed-aborted", "none", "provisional/resolveTransactionJournal", chain(["PREPARED", "LEASED", "PREFLIGHT_OK", "OP_STARTED", "OP_FAILED", "ABORTED"]), check="semantic_journal", extra={"plan": plan_m3})
...
b = chain(["PREPARED", "LEASED", "PREFLIGHT_OK", "OP_STARTED", "APPLIED", "OP_STARTED", "APPLIED"], ops_=("op-1", "op-1")); fixture("journal-operation-applied-twice", "semantic", "provisional/resolveTransactionJournal", b, check="semantic_journal", expect_contains="applied twice", extra={"plan": plan_m3})
b = chain(["PREPARED", "LEASED", "PREFLIGHT_OK", "READBACK_S1"], s1_=s1); fixture("journal-readback-before-all-applied", "semantic", "provisional/resolveTransactionJournal", b, check="semantic_journal", expect_contains="READBACK_S1 before all plan operations applied", extra=JX)
...
b = chain(["PREPARED", "LEASED", "PREFLIGHT_OK", "OP_STARTED", "APPLIED", "READBACK_S1", "READBACK_S1"], s1_=s1); fixture("journal-duplicate-readback-events", "semantic", "provisional/resolveTransactionJournal", b, check="semantic_journal", expect_contains="multiple READBACK_S1", extra=JX)
...
fixture("journal-in-flight-no-readback-with-s1-supplied", "semantic", "provisional/resolveTransactionJournal", chain(["PREPARED", "LEASED", "PREFLIGHT_OK", "OP_STARTED", "APPLIED"]), check="semantic_journal", expect_contains="no READBACK_S1 event", extra=JX)
...
fixture("verification-without-readback-event", "semantic", "provisional/resolveVerificationResult", vr_ok, check="semantic_verification_result", expect_contains="no single READBACK_S1", extra=vx(s1, chain(["PREPARED", "LEASED", "PREFLIGHT_OK", "OP_STARTED", "APPLIED"])))
...
linked("linked-set-missing-readback-event-with-verification", "linked-set", dict(TS_OK, journal=chain(["PREPARED", "LEASED", "PREFLIGHT_OK", "OP_STARTED", "APPLIED"]), commit=None), "no single READBACK_S1")
...
man_schema["$id"] = "vidtoolz.resolveFreezeManifest.v1.5"; man_schema["title"] = "Freeze manifest v1.5"
man_schema["properties"]["schema"] = {"const": "vidtoolz.resolveFreezeManifest.v1.5"}; man_schema["properties"]["bundle"] = {"const": "docs/resolve-integration/v1.5"}; man_schema["properties"]["version"] = {"const": "1.5.0"}
man_schema["properties"]["parent"]["properties"].update({"version": {"const": "1.4.0"}, "branch": {"const": "docs/resolve-authority-freeze-v1.4"}, "head": {"const": "d2d77680047221f2dac52adce6938dd410e38b47"}, "manifest_sha256": {"const": "34a7d0507a5d7f31769a46561d97219869e3525889f121424623eab4ff1dd84f"}})
dump("schemas/resolveFreezeManifest.schema.json", man_schema)
print("build_v1_5: OK; layered fixtures:", FIXTURE_COUNT[0], "; evidence sets:", len(EVSETS))
