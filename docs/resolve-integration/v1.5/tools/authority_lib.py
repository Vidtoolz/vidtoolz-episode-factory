"""VIDTOOLZ Resolve authority bundle v1.5 — reference canonicalization, evidence-envelope binding, deterministic freshness,
evidence-derived attachment/eligibility, DERIVED capability success (raw -> parse -> review -> refreeze -> active), occurrence
identity uniqueness, protected-surface delta with the unrelated-change law, journal readback -> S1 binding, snapshot field
provenance (QUALIFIED_OBSERVATION vs CANDIDATE_OBSERVATION), operation-specific S1 verify profiles and ONE authorizing
commit path (commit_eligibility -> validate_transaction_set).
...
AUTHORITY_VERSION = "1.5.0"
CANONICALIZATION_VERSION = "1.5"
RECORD_TYPE_VERSION = "1.5"
...
def primitive_status(caps, rp_entry, method, es, env, active):
    """QUALIFIED_CALLABLE only when the ACTIVE capability matrix row is QUALIFIED_READ under a reviewed refreeze block whose
    promoted_probe_ids / promoted_evidence_sha256 name the exact evidence, the row's evidence_records entry cites
    (method, probe_id, raw_evidence_sha256), and the linked CAPABILITY_EVIDENCE record passes capability_record_qualifies
    (DERIVED success from raw + parse, exact receiver/type/host/product/version/build, review referencing the exact raw hash,
    promotion by the active authority version + matrix sha). PROBE_ALLOWED only inside the probe. Otherwise UNQUALIFIED."""
    ...
    if row["evidence_class"] == "QUALIFIED_READ":
        ...
        for x in row.get("evidence_records", []):
            if x.get("method") != method or x.get("version_match") is not True or not x.get("reviewed_refreeze_version") or not is_sha(x.get("raw_evidence_sha256")) or not x.get("probe_id") or x.get("result") != "SUCCESS":
                continue
            if x["probe_id"] not in (rf.get("promoted_probe_ids") or []) or x["raw_evidence_sha256"] not in (rf.get("promoted_evidence_sha256") or []):
                continue
            for rec in find_records(es, "CAPABILITY_EVIDENCE", method=method, probe_id=x["probe_id"], raw_evidence_sha256=x["raw_evidence_sha256"]):
                if capability_record_qualifies(rec, rp_entry, env, active, es)[0]:
...
def capability_success(rec, rp_entry, es):
    """INTERNAL_NON_AUTHORIZING. The ONE canonical capability-success predicate: success is DERIVED from the raw evidence
    and its parse, never read from a label. Returns (ok, reasons)."""
    reasons = []
    e = rec.get("envelope") or {}
    res, parse, po = rec.get("result") or {}, rec.get("parse") or {}, rec.get("parsed_observation") or {}
    raws = [r for r in find_records(es, "RAW_EVIDENCE", content_sha256=rec.get("raw_evidence_sha256")) if is_sha(r.get("content_sha256")) and sha256_text(r.get("content", "")) == r["content_sha256"] and (r.get("envelope") or {}).get("session_id") == e.get("session_id")]
    if not raws:
        reasons.append("raw evidence object missing, not re-hashing or from another session")
    if parse.get("succeeded") is not True:
        reasons.append("parser did not succeed")
    if parse.get("error") is not None:
        reasons.append(f"parse error: {parse.get('error')}")
    if parse.get("getter_exception") is not None:
        reasons.append(f"getter exception: {parse.get('getter_exception')}")
    if rp_entry is not None:
        if rec.get("receiver_type") != rp_entry.get("receiver"):
            reasons.append(f"receiver {rec.get('receiver_type')} != expected {rp_entry.get('receiver')}")
        if parse.get("observed_type") != rp_entry.get("expected_type"):
            reasons.append(f"observed type {parse.get('observed_type')} != expected {rp_entry.get('expected_type')}")
    if parse.get("shape_ok") is not True or po.get("non_null") is not True:
        reasons.append("parsed output shape/type not acceptable (shape_ok/non_null)")
    if not (res.get("classification") == "SUCCESS" and res.get("success") is True and res.get("code") is None):
        reasons.append("result label is not SUCCESS")
    derived = classify_capability_record(rec)
    if derived != "SUCCESS":
        reasons.append(f"derived classification {derived} contradicts qualification")
    return (not reasons, reasons)


def classify_capability_record(rec):
    """INTERNAL_NON_AUTHORIZING. Derive the classification from parse/raw facts (never from the label)."""
    parse = rec.get("parse") or {}
    res = rec.get("result") or {}
    if res.get("classification") == "FATAL_TARGET_FAILURE":
        return "FATAL_TARGET_FAILURE"
    if parse.get("getter_exception") is not None:
        return "CAPABILITY_FAILURE"
    if parse.get("succeeded") is not True or parse.get("error") is not None or parse.get("shape_ok") is not True or not (rec.get("parsed_observation") or {}).get("non_null"):
        return "CAPABILITY_FAILURE"
    return "SUCCESS"


def capability_record_qualifies(rec, rp_entry, env, active, es):
    """INTERNAL_NON_AUTHORIZING (composed by primitive_status). Returns (ok, reasons). Requires capability_success plus:
    exact host/product/version/build; reviewed ACCEPT whose review references THIS raw evidence hash; promotion by the active
    authority version AND active capability matrix sha; probe authority version and parsed observation present."""
    ok, reasons = capability_success(rec, rp_entry, es)
    e = rec.get("envelope") or {}
    ...
    if not (e.get("host_name") == env["host_name"] and e.get("product") == env["product"] and e.get("resolve_version") == env["resolve_version"] and e.get("build") == env["build"]):
        reasons.append("host/product/version/build differ from the contract environment")
    if not (q.get("reviewed") is True and q.get("decision") == "ACCEPT"):
        reasons.append("not reviewed ACCEPT")
    if q.get("review_raw_evidence_sha256") != rec.get("raw_evidence_sha256"):
        reasons.append("review does not reference this exact raw evidence hash")
    if pb.get("authority_version") != active.get("authority_version") or pb.get("capability_matrix_sha256") != active.get("capability_matrix_sha256"):
        reasons.append("not promoted by the active authority version + capability matrix")
    if pb.get("raw_evidence_sha256") != rec.get("raw_evidence_sha256"):
        reasons.append("refreeze promotion does not name this exact raw evidence")
    if rec.get("probe_authority_version") is None or not isinstance(rec.get("parsed_observation"), dict):
        reasons.append("probe authority version / parsed observation missing")
    return (not reasons, reasons)
...
def semantic_capability_record(rec, rp_entry=None):
    """INTERNAL_NON_AUTHORIZING. Structural/semantic law for one CAPABILITY_EVIDENCE record: the classification label must
    equal the classification DERIVED from parse/raw facts; a failed observation can never be ACCEPTed; unreviewed candidates
    carry no decision/promotion; ACCEPT requires promoted_by and a review reference to this raw hash."""
    errs = []
    res = rec.get("result") or {}
    q = rec.get("qualification") or {}
    parse = rec.get("parse") or {}
    ...
    derived = classify_capability_record(rec)
    if cls not in ("SUCCESS", "CAPABILITY_FAILURE", "FATAL_TARGET_FAILURE"):
        errs.append(f"unknown classification {cls}")
    elif cls != derived:
        errs.append(f"label {cls} contradicts derived classification {derived} (parse/raw facts win)")
    if cls == "SUCCESS":
    ...
    for k in ("parser", "succeeded", "error", "getter_exception", "observed_type", "shape_ok"):
        if k not in parse:
            errs.append(f"parse block missing {k}")
    if rp_entry is not None and parse.get("succeeded") is True and parse.get("observed_type") != rp_entry.get("expected_type") and cls == "SUCCESS":
        errs.append("SUCCESS with observed type different from the primitive's expected type")
    if q.get("reviewed") is False and (q.get("decision") is not None or q.get("promoted_by") is not None or q.get("review_raw_evidence_sha256") is not None):
        errs.append("unreviewed candidate cannot carry a decision, promotion or review reference")
    if q.get("reviewed") is True and q.get("review_raw_evidence_sha256") != rec.get("raw_evidence_sha256"):
        errs.append("reviewed record must reference its own raw evidence hash")
    if q.get("decision") == "ACCEPT" and not isinstance(q.get("promoted_by"), dict):
        errs.append("ACCEPT requires promoted_by {authority_version, capability_matrix_sha256, raw_evidence_sha256}")
    if isinstance(q.get("promoted_by"), dict) and q["promoted_by"].get("raw_evidence_sha256") != rec.get("raw_evidence_sha256"):
        errs.append("promotion names another raw evidence hash than this record's")
    return errs
...
def provenance_errors(snap, ctx):
    """Field provenance law: every method an OBSERVED field/domain relies on must appear in collection.method_provenance as a
    QUALIFIED_OBSERVATION whose capability_evidence_record_id names a CAPABILITY_EVIDENCE record in the evidence set for that
    method that qualifies under the ACTIVE authority (capability_record_qualifies) and whose capability_matrix_sha256 is the
    active matrix. CANDIDATE_OBSERVATION methods can never back an OBSERVED field. ctx = {caps, rp, es, active, tc}."""
    errs = []
    ...
    entries = {p["method"]: p for spec in rp.get("logical_operations", {}).values() for p in spec.get("primitives", [])}
    ...
    for m in sorted(_observed_methods(snap)):
        ...
        rid = pv.get("capability_evidence_record_id")
        rec = (es or {}).get("records", {}).get(rid) if is_sha(rid) else None
        if rec is None or rec.get("record_type") != "CAPABILITY_EVIDENCE" or record_id(rec) != rid:
            errs.append(f"method_provenance[{m}]: capability_evidence_record_id does not resolve to a CAPABILITY_EVIDENCE record")
            continue
        if rec.get("method") != m:
            errs.append(f"method_provenance[{m}]: cited evidence is for {rec.get('method')} (adjacent getter)")
            continue
        ok, why = capability_record_qualifies(rec, entries.get(m), env, active, es)
        if not ok:
            errs.append(f"method_provenance[{m}]: cited evidence does not qualify ({why[0]})")
        if primitive_status(caps, entries.get(m), m, es, env, active) != "QUALIFIED_CALLABLE":
            errs.append(f"method_provenance[{m}]: primitive is not QUALIFIED_CALLABLE under the active capability matrix")
    return errs
...
def semantic_snapshot(snap, ctx=None):
    ...
    if not isinstance(coll.get("method_provenance"), dict):
        ...
        for m, pv in coll["method_provenance"].items():
            if not isinstance(pv, dict) or pv.get("observation_class") not in OBSERVATION_CLASSES:
                errs.append(f"method_provenance[{m}]: unknown observation_class")
            elif pv["observation_class"] == "QUALIFIED_OBSERVATION" and (not is_sha(pv.get("capability_evidence_record_id")) or not is_sha(pv.get("capability_matrix_sha256"))):
                errs.append(f"method_provenance[{m}]: QUALIFIED_OBSERVATION requires capability_evidence_record_id and capability_matrix_sha256")
            elif pv["observation_class"] != "QUALIFIED_OBSERVATION" and (pv.get("capability_evidence_record_id") is not None and pv["observation_class"] == "NOT_CALLABLE"):
                errs.append(f"method_provenance[{m}]: NOT_CALLABLE cannot cite capability evidence")
...
def semantic_capabilities(caps, contract_version="21.1.0", contract_build=14):
    ...
    for r in caps["rows"]:
        if r["evidence_class"] == "QUALIFIED_READ":
            ...
            for x in recs:
                if x["probe_id"] not in (rf.get("promoted_probe_ids") or []) or x["raw_evidence_sha256"] not in (rf.get("promoted_evidence_sha256") or []):
                    errs.append(f"row '{r['operation']}': evidence {x['method']} is not explicitly promoted by the refreeze block (probe id + raw evidence hash)")
...
def validate_transaction_set(ts, perms, rp, caps, tc, es, active, schema_validate=None, _stages=None):
...
def commit_eligibility(ts, perms, rp, caps, tc, es, active, schema_validate=None):
    """THE ONE AUTHORIZING commit entry point. A commit is eligible only when the complete linked set (S0, plan, journal, S1,
    verification, conflicts, commit manifest) passes validate_transaction_set with zero errors. Returns
    {eligible, errors, stages_completed}. A verification object, a hash or a receipt is never sufficient on its own."""
    stages = []
    errs = []
    ...
    return {"eligible": not errs and "commit" in stages, "errors": errs, "stages_completed": stages}
...
def semantic_manifest(m, bundle_dir=None, sha_fn=None, parent_manifest=None, size_fn=None):
    errs = []
    if m.get("schema") != "vidtoolz.resolveFreezeManifest.v1.5":
    ...
    par = m.get("parent", {})
    if par.get("version") != "1.4.0" or par.get("head") != "d2d77680047221f2dac52adce6938dd410e38b47" or par.get("manifest_sha256") != "34a7d0507a5d7f31769a46561d97219869e3525889f121424623eab4ff1dd84f":
