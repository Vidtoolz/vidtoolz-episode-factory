# Implementation lineage — Scorecraft extraction map v1.3 (FROZEN_NOW)

Active extraction targets are the **v1.3** authorities (`resolveTargetContract.v1.3`, `resolveSnapshot.v1.3`, `TARGET-ATTACHMENT-GATE.md`, `ELIGIBILITY.md`). References to v1.1 or v1.2 schemas in `../v1/`, `../v1.1/`, `../v1.2/` are historical only.
...
|---|---|---|
| explicit target contract shape (`normalizeTarget`) | QUALIFIED_PATTERN | PRESERVE as concept → `resolveTargetContract.v1.3` (attachment state derived from evidence, not declared) |
...
| readback | EXPERIMENTAL | REWRITE for `resolveSnapshot.v1.3` (all tracks, provenance variants, per-field observation status, failure ledger, no rounding, plural APIs) |
