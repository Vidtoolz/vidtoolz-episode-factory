...
Run: python3 -B tools/validate_v1_17.py
"""
import ast
import copy
import glob
...
# v1.15: two further NON-AUTHORIZING classes exist (V113-B1 diagnostic, V114-B2 provisional-until-M3); a function in
# either is classified, and the classification audit must see all four classes or it would report a false gap.
classified_names = auth_names | internal_names | set(L.AUTHORITY_SURFACE["diagnostic_non_authorizing"]) | set(L.AUTHORITY_SURFACE["provisional_until_m3_non_authorizing"])
rec("bypass-audit", "AUTHORITY_SURFACE lists commit_eligibility_authorizing and validate_transaction_set_authorizing as the only transaction-authorizing entry points; the v1.14 bare-dict pair is PROVISIONAL_UNTIL_M3 and non-authorizing (V114-B2)", {"commit_eligibility_authorizing", "validate_transaction_set_authorizing"} <= auth_names and not ({"commit_eligibility", "validate_transaction_set", "semantic_commit_manifest", "semantic_verification_result", "verify_transaction", "derive_delta"} & auth_names) and sorted(L.AUTHORITY_SURFACE["provisional_until_m3_non_authorizing"]) == ["commit_eligibility", "validate_transaction_set"])
rec("bypass-audit", "every exported validator touching commit/verification/journal/plan/snapshot/delta is classified into exactly one of the four surface classes", all(n in classified_names for n in pub if re.search(r"commit|verif|journal|mutation_plan|snapshot$|delta|occurrence|capability_", n)), ",".join(n for n in pub if re.search(r"commit|verif|journal|mutation_plan|snapshot$|delta|occurrence|capability_", n) and n not in classified_names))
rec("bypass-audit", "the four surface classes are DISJOINT: no function is both authorizing and non-authorizing", not (auth_names & (set(L.AUTHORITY_SURFACE["diagnostic_non_authorizing"]) | set(L.AUTHORITY_SURFACE["provisional_until_m3_non_authorizing"]) | internal_names)), ",".join(sorted(auth_names & (set(L.AUTHORITY_SURFACE["diagnostic_non_authorizing"]) | set(L.AUTHORITY_SURFACE["provisional_until_m3_non_authorizing"]) | internal_names))))
...
# v1.17 (V116-B1 / Codex section 55.5): every artifact is registered with a STATUS and a published ROLE, so audits
# say exactly which documents they bind. NORMATIVE_AUTHORITY states current law; CORRECTION_RECORD is a current
# changelog or matrix; GENERATED_EVIDENCE is produced by a tool from the bundle and quotes the suite's own checks;
# HISTORICAL_INPUT is superseded. Scope is derived here, never from a filename pattern.
_ROLE_OF = {e["path"]: e.get("role", "NORMATIVE_AUTHORITY") for e in prec["documents"]}
...
for path in sorted(active):
    if (not os.path.exists(os.path.join(B, path)) or path in prec.get("retired_term_exempt", [])
            or _ROLE_OF.get(path, "NORMATIVE_AUTHORITY") != "NORMATIVE_AUTHORITY"):
        continue
...
rec("precedence", "the v1.5 through v1.16 changelogs and finding matrices are HISTORICAL; the v1.17 ones are active", all(st_.get(d_) == "HISTORICAL" for d_ in ("CHANGELOG-v1.5.md", "FINDING-RESOLUTION-MATRIX-v1.5.md", "CHANGELOG-v1.6.md", "FINDING-RESOLUTION-MATRIX-v1.6.md", "CHANGELOG-v1.7.md", "FINDING-RESOLUTION-MATRIX-v1.7.md", "CHANGELOG-v1.8.md", "FINDING-RESOLUTION-MATRIX-v1.8.md", "CHANGELOG-v1.9.md", "FINDING-RESOLUTION-MATRIX-v1.9.md", "CHANGELOG-v1.10.md", "FINDING-RESOLUTION-MATRIX-v1.10.md", "CHANGELOG-v1.11.md", "FINDING-RESOLUTION-MATRIX-v1.11.md", "CHANGELOG-v1.12.md", "FINDING-RESOLUTION-MATRIX-v1.12.md", "CHANGELOG-v1.13.md", "FINDING-RESOLUTION-MATRIX-v1.13.md", "CHANGELOG-v1.14.md", "FINDING-RESOLUTION-MATRIX-v1.14.md", "CHANGELOG-v1.15.md", "FINDING-RESOLUTION-MATRIX-v1.15.md", "CHANGELOG-v1.16.md", "FINDING-RESOLUTION-MATRIX-v1.16.md")) and st_.get("CHANGELOG-v1.17.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.17.md") == "STILL_ACTIVE")
...
rec("precedence", "the v1.12 pre-M0A workflow supersessions S56-S59 are retained", sorted(s_["id"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.12.0") == ["S56", "S57", "S58", "S59"])
rec("precedence", "the v1.13 governed-root supersession S60 is retained", [s_["id"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.13.0"] == ["S60"] and st_.get("CHANGELOG-v1.13.md") == "HISTORICAL" and st_.get("EVIDENCE-ROOT.md") == "STILL_ACTIVE")
rec("precedence", "the v1.14 authorizing-location supersession S61 is retained", [s_["id"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.14.0"] == ["S61"] and st_.get("CHANGELOG-v1.14.md") == "HISTORICAL" and st_.get("FINDING-RESOLUTION-MATRIX-v1.14.md") == "HISTORICAL")
rec("precedence", "the v1.15 consumed-evidence supersession S62 is retained", [s_["id"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.15.0"] == ["S62"] and st_.get("CHANGELOG-v1.15.md") == "HISTORICAL" and st_.get("FINDING-RESOLUTION-MATRIX-v1.15.md") == "HISTORICAL")
rec("precedence", "the v1.16 classification/receipt-publication supersession S63 is retained and its records are now HISTORICAL", [s_["id"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.16.0"] == ["S63"] and st_.get("CHANGELOG-v1.16.md") == "HISTORICAL" and st_.get("FINDING-RESOLUTION-MATRIX-v1.16.md") == "HISTORICAL" and st_.get("AUTHORITY-FUNCTION-CLASSES.json") == "STILL_ACTIVE")
rec("precedence", "the v1.17 slot-discovery supersession S64 is recorded, the v1.17 records and the generated inventory are active, and the document version tracks the authority version", [s_["id"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.17.0"] == ["S64"] and st_.get("CHANGELOG-v1.17.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.17.md") == "STILL_ACTIVE" and st_.get("AUTHORITY-SLOT-INVENTORY.json") == "STILL_ACTIVE" and prec["version"] == L.AUTHORITY_VERSION)
rec("precedence", "S64 names the v1.16 statement it replaces, the generated artifacts and tooling that now hold the law, and Codex's finding", (lambda s64: s64 and "hand-written list of ten key paths" in s64["old_statement"] and "v1.16 S63" in s64["old_statement"] and "V116-B1" in s64["reason"] and all(n_ in s64["new_authority"] for n_ in ("AUTHORITY-SLOT-INVENTORY.json", "tools/authority_slots.py", "EVIDENCE-SET-WORKFLOW.json")))(next((s_ for s_ in prec["superseded_statements"] if s_["id"] == "S64"), None)))
rec("precedence", "the v1.17 supersession table in the markdown mirrors the machine form, keeps S63 standing, and restates that precedence is not a band-aid", (lambda t_: all(p_ in " ".join(t_.split()) for p_ in ("## v1.17 supersession (S64)", "S63 is not withdrawn", "Precedence is still not a band-aid", "No runtime semantics changed in v1.17")))(open(os.path.join(B, "AUTHORITY-PRECEDENCE.md"), encoding="utf-8").read()))
rec("precedence", "every artifact is registered with a status AND a published role, and the four roles are defined - so the audit scope is machine-derived rather than a filename pattern (V116-B1 sections 16/17)", all("role" in e and e["role"] in prec["document_roles"] for e in prec["documents"]) and len(prec["documents"]) >= 95 and "GENERATED_EVIDENCE" in prec["document_role_law"] and "retired_term_exempt_law" in prec)
rec("precedence", "THREAT-MODEL.md records the v1.17 audit-fragility residuals and restates the still-open authorizing-transaction control", (lambda t_: all(p_ in " ".join(t_.split()) for p_ in ("## v1.17: what an audit can miss", "will eventually read the wrong claim", "no positive control until")))(open(os.path.join(B, "THREAT-MODEL.md"), encoding="utf-8").read()))
rec("precedence", "S63 names the v1.15 statement it replaces, the generated artifacts that now hold the law, and both of Codex's findings", (lambda s63: s63 and "v1.15 S62" in s63["old_statement"] and "seven fields" in s63["old_statement"] and all(f_ in s63["reason"] for f_ in ("V115-B1", "V115-M1")) and all(n_ in s63["new_authority"] for n_ in ("AUTHORITY-FUNCTION-CLASSES.json", "authority_function_classes", "RECEIPT_BOUND_FIELDS", "receipt_binding_publication", "TARGET-CONTRACT.json", "SCHEMA-REGISTRY.md")))(next((s_ for s_ in prec["superseded_statements"] if s_["id"] == "S63"), None)))
rec("precedence", "the v1.16 supersession table in the markdown mirrors the machine form, keeps S62 standing, and states that precedence is not a band-aid for CURRENT inconsistency", (lambda t_: all(p_ in " ".join(t_.split()) for p_ in ("## v1.16 supersession (S63)", "S62 is not withdrawn", "Precedence is not a band-aid for current inconsistency", "no longer exempts `law`, `note`, `qualification_note` or `reason`")))(open(os.path.join(B, "AUTHORITY-PRECEDENCE.md"), encoding="utf-8").read()))
rec("precedence", "THREAT-MODEL.md records the v1.16 consumer-facing residuals and restates the still-open authorizing-transaction control", (lambda t_: all(p_ in " ".join(t_.split()) for p_ in ("## v1.16: what a consumer reads", "authority selection is itself an authorizing decision", "RECEIPT_BINDING_DRIFT", "no positive control until")))(open(os.path.join(B, "THREAT-MODEL.md"), encoding="utf-8").read()))
rec("precedence", "S62 names the v1.14 statement it replaces, the core functions that now hold the law, and every one of Codex's six findings", (lambda s62: s62 and s62.get("old_statement") and "v1.14 S61" in s62["old_statement"] and all(f_ in s62["reason"] for f_ in ("V114-B1", "V114-B2", "V114-B3", "V114-B4", "V114-M1", "V114-N1")) and all(n_ in s62["new_authority"] for n_ in ("governed_consume", "commit_eligibility_authorizing", "read_governed_file", "governed_structure_errors")))(next((s_ for s_ in prec["superseded_statements"] if s_["id"] == "S62"), None)))
rec("precedence", "the v1.15 supersession table in the markdown mirrors the machine form, keeps S61 standing, and states the essential law", (lambda t_: "## v1.15 supersession (S62)" in t_ and "S61 is not withdrawn" in t_ and "the exact bytes validated as governed evidence are the exact bytes consumed" in t_.lower() and "authorizing counterpart" in t_)(open(os.path.join(B, "AUTHORITY-PRECEDENCE.md"), encoding="utf-8").read()))
rec("precedence", "AUTHORITY-PRECEDENCE.json carries the blanket clause that re-reads every earlier statement's demoted function names as their authorizing counterparts, and v1.17 makes that clause itself clause-local: every demoted name in it carries its class token in the same clause (V114-B3, V116-B1)", "reads as its authorizing counterpart" in prec["law"] and "No current artifact may designate one of those four" in prec["law"] and "AUTHORITY-SLOT-INVENTORY.json" in prec["law"])
rec("precedence", "THREAT-MODEL.md records the v1.15 residuals: the interpreter boundary, the absent authorizing-transaction positive control, and no cryptographic location proof", (lambda t_: "## v1.15: what the authority actually consumes" in t_ and "A validated byte stream and a consumed object are two different things" in t_ and "object.__setattr__" in t_ and "no production path that authors" in t_)(open(os.path.join(B, "THREAT-MODEL.md"), encoding="utf-8").read()))
rec("precedence", "S61 names the v1.13 statement it replaces, the core functions that now hold the law, and the V113-B1 finding that forced it", all(s_.get("old_statement") and s_.get("new_authority") and "V113-B1" in s_["reason"] and "v1.13" in s_["old_statement"] and "load_governed_evidence_set" in s_["new_authority"] and "derive_attachment_state_authorizing" in s_["new_authority"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.14.0"))
rec("precedence", "the v1.14 supersession table in the markdown mirrors the machine form and does not withdraw S60", (lambda t_: "## v1.14 supersession (S61)" in t_ and "S60 is not withdrawn" in t_ and "location authority \u2192 attachment semantics \u2192 eligibility" in t_)(open(os.path.join(B, "AUTHORITY-PRECEDENCE.md"), encoding="utf-8").read()))
rec("precedence", "THREAT-MODEL.md records the v1.14 authorizing-location residual, states the M3 diagnostic-eligibility residual honestly and claims no cryptographic location proof", (lambda t_: "## v1.14: the authorizing location check" in t_ and "is not a property of the authority" in t_ and "It does not claim a cryptographic location proof" in t_ and "semantic_mutation_plan" in t_ and "scoped residual" in t_)(open(os.path.join(B, "THREAT-MODEL.md"), encoding="utf-8").read()))
rec("precedence", "S60 names the v1.12 statement it replaces, the new authority and the V112-RP1 finding that forced it", all(s_.get("old_statement") and s_.get("new_authority") and "V112-RP1" in s_["reason"] and "v1.12" in s_["old_statement"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.13.0"))
rec("precedence", "THREAT-MODEL.md records the v1.13 governed-location residual and claims no TOCTOU or cryptographic location guarantee", (lambda t_: "the control was hollow" in t_ and "not a TOCTOU guarantee" in t_ and "no cryptographic binding of location" in t_)(open(os.path.join(B, "THREAT-MODEL.md"), encoding="utf-8").read()))
...
# v1.17: every artifact is now registered in precedence with a STATUS and a ROLE, so this audit can say exactly what
# it means - only CURRENT NORMATIVE authority is bound by the single-enumeration law. A superseded statement, a
# correction record quoting one, or generated evidence reporting a check is not a second enumeration of the law.
_NORMATIVE = {p for p in active if _ROLE_OF.get(p, "NORMATIVE_AUTHORITY") == "NORMATIVE_AUTHORITY"}
_off = []
for path in sorted(_NORMATIVE):
...
if os.path.exists(man_path):
    ...
    parent = L.strict_load(os.path.join(os.path.dirname(B), "v1.16", "FREEZE-MANIFEST.json"))
    rec("manifest", "parent (v1.16) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.16", "FREEZE-MANIFEST.json")) == "ba77bfa2226696b61be27f2a7c74cf551720f601f57b8c11c9f2f6f50a159507")
    rec("manifest", "grandparent (v1.15) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.15", "FREEZE-MANIFEST.json")) == "af2c1b11485233f5c2b6b5447a6695a3b6cbe6ba4cbdb06f3727973f4b2c9fe2")
    rec("manifest", "great-grandparent (v1.14) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.14", "FREEZE-MANIFEST.json")) == "0d4fc9915923d17ef6ec469cc01fb164b4d2ee1001fc7983d557bf4c9d9032b8")
    rec("manifest", "great-grandparent (v1.13) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.13", "FREEZE-MANIFEST.json")) == "436e12c8616ea6704fef2dbd0e14517cf33092aec1a7ff064dd4b10fb3dad573")
    rec("manifest", "grandparent (v1.12) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.12", "FREEZE-MANIFEST.json")) == "e5eace278a1b6130971a38f55ef207620cdc97d7b3872d11ce9ab8909e4014fb")
    rec("manifest", "great-grandparent (v1.11) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.11", "FREEZE-MANIFEST.json")) == "0c47f0cefdd63b971489a46274e3b782927f2c6ad26e4251de21860abc50c44e")
    rec("manifest", "great-grandparent (v1.10) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.10", "FREEZE-MANIFEST.json")) == "46491cee979dc59a306993de8cc66308884967dca46304168096f935939f6156")
    ...
    rec("manifest", "the parent block records the v1.16 branch HEAD and its semantic commit, which for v1.16 are the same commit because its bundle and registration were committed together (section 29)", man["parent"]["head"] == "a03923cd4b09e8e11d40e8cf13819f8df763063a" and man["parent"]["semantic_head"] == "a03923cd4b09e8e11d40e8cf13819f8df763063a" and man["parent"]["manifest_sha256"] == "ba77bfa2226696b61be27f2a7c74cf551720f601f57b8c11c9f2f6f50a159507" and man["parent"]["version"] == "1.16.0" and {a["version"] for a in man["parent"]["ancestors"]} >= {"1.15.0", "1.14.0", "1.13.0"})
    rec("manifest", "v1.0-v1.16 bundles are byte-identical to their frozen manifests (history preserved)", all(all(sha(os.path.join(os.path.dirname(B), v_, e_["path"])) == e_["sha256"] for e_ in L.strict_load(os.path.join(os.path.dirname(B), v_, "FREEZE-MANIFEST.json"))["files"] if os.path.exists(os.path.join(os.path.dirname(B), v_, e_["path"]))) for v_ in ("v1", "v1.1", "v1.2", "v1.3", "v1.4", "v1.5", "v1.6", "v1.7", "v1.8", "v1.9", "v1.10", "v1.11", "v1.12", "v1.13", "v1.14", "v1.15", "v1.16")))
    ...
    rec("manifest", "validation tools listed as TOOL", all(any(e["path"] == p and e["authority_class"] == "TOOL" for e in man["files"]) for p in ("tools/authority_lib.py", "tools/validate_v1_17.py", "tools/build_v1_17.py", "tools/build_manifest.py", "tools/fixture_evidence.py", "tools/capture_shim_reference.py", "tools/evidence_authoring.py", "tools/a2_prepare.py", "tools/a2_verify.py", "tools/evidence_authoring_testkit.py")))
...
def _prepared(tag, journal=True, principal=_PREP):
    ...
    sid = _wsess(tag)
    lr = _libroot(sid)
    AUTH_MOD.create_evidence_set(sid, principal)
    AUTH_MOD.add_provisioning_record(sid, principal, instance_uuid=_UU, root_path=lr,
                                     provisioned_by="Mikko (operator)")
    AUTH_MOD.add_launch_recipe(sid, principal, recipe_sha256=_LAUNCH_SHA)
    if journal:
        AUTH_MOD.add_read_only_journal(sid, principal, journal_path_sha256=_JOURNAL_SHA)
    AUTH_MOD.mark_prepared(sid, principal)
    return sid, lr


def _derive(sid):
    """INTERNAL_NON_AUTHORIZING: workflow mechanics only. The AUTHORIZING derive refuses every sandboxed
    document (V112-RP1), which the runtime-parity section asserts separately."""
    return TK.derive_nonauthorizing(sid)
...
def _verified(sid):
    AUTH_MOD.add_bundle_verification(sid, _VER)
    return sid


def _read_doc(sid):
    return L.strict_load(os.path.join(AUTH_MOD.set_dir(sid), AUTH_MOD.EVIDENCE_SET_FILE))
...
def _copy_elsewhere(sid):
    ...
    _verified(sid)
    dst = AUTH_MOD.set_dir(sid + "-copy")
    _unseal(sid)
    shutil.copytree(AUTH_MOD.set_dir(sid), dst)
    return sid + "-copy"


# V112-RP1 section 15/17: the workflow sections run inside the INTERNAL_NON_AUTHORIZING sandbox. The sandbox changes
# ONE thing - the root prefix - and every production check runs unchanged on the same code. Nothing here can be
# authority: the runtime-parity section proves the AUTHORIZING derive refuses every document produced in here.
with TK.sandbox(W_ROOT):
  try:
    # ---- V112-1 the governed persistence law
    rec("workflow-persistence", "exactly one governed attachment root is frozen, absolute, normalised and none of the forbidden locations",
        L.GOVERNED_ATTACHMENT_ROOT == "/home/vidtoolz/resolve-qualification-evidence/attachment"
        and os.path.isabs(L.GOVERNED_ATTACHMENT_ROOT)
        and os.path.normpath(L.GOVERNED_ATTACHMENT_ROOT) == L.GOVERNED_ATTACHMENT_ROOT
        and not L.GOVERNED_ATTACHMENT_ROOT.startswith("/tmp"),
        L.GOVERNED_ATTACHMENT_ROOT)
    rec("workflow-persistence", "the path law is derived from the session id alone: the resolver takes NO argument (V112-RP1)",
        AUTH_MOD.set_path("s-x") == os.path.join(AUTH_MOD.governed_attachment_root(), "s-x", "EVIDENCE-SET.json")
        and AUTH_MOD.workflow_path("s-x") == os.path.join(AUTH_MOD.governed_attachment_root(), "s-x", "WORKFLOW.json")
        and AUTH_MOD.set_dir("s-x") == os.path.join(AUTH_MOD.governed_attachment_root(), "s-x")
    ...
    rec("workflow-persistence", "a session id that is not a safe basename is refused, so the path law cannot be escaped",
        all(_try_auth(lambda b=b: AUTH_MOD.set_dir(b)).code == "SESSION_ID_INVALID"
            for b in AUTH_MOD.SESSION_ID_REJECTS))
    ...
    rec("workflow-persistence", "the document and its workflow state are written at the governed path with governed modes",
        os.path.exists(AUTH_MOD.set_path(_sid0)) and os.path.exists(AUTH_MOD.workflow_path(_sid0))
        and stat.S_IMODE(os.lstat(AUTH_MOD.set_dir(_sid0)).st_mode) == AUTH_MOD.DIR_MODE)
    rec("workflow-persistence", "a MOVED or COPIED document is refused: the document records where it lives",
        (lambda: (shutil.copytree(AUTH_MOD.set_dir(_sid0), os.path.join(W_ROOT, "sess-v112-moved-999")),
                  _try_auth(lambda: AUTH_MOD.load_evidence_set("sess-v112-moved-999")).code)[1]
                 in ("SET_LOCATION_MISMATCH", "SESSION_PATH_NOT_GOVERNED"))())
    rec("workflow-persistence", "writer exclusivity: a second writer while the lock is held is refused LOCK_HELD",
        (lambda: (lambda lk: (lk.__enter__(), _try_auth(lambda: AUTH_MOD.add_read_only_journal(_sid0, _PREP, journal_path_sha256=_JOURNAL_SHA)).code, lk.__exit__())[1])(AUTH_MOD._Lock(_sid0, _PREP)))() == "LOCK_HELD")
    rec("workflow-persistence", "the lock is released after use, so the next writer is not blocked by a stale lock",
        not os.path.exists(os.path.join(AUTH_MOD.set_dir(_sid0), AUTH_MOD.LOCK_FILE)))
    rec("workflow-persistence", "re-creating an existing document is refused SET_EXISTS, and a missing one is SET_NOT_FOUND",
        _try_auth(lambda: AUTH_MOD.create_evidence_set(_sid0, _PREP)).code == "SET_EXISTS"
        and _try_auth(lambda: AUTH_MOD.load_evidence_set("sess-v112-absent-000")).code == "SET_NOT_FOUND")
    ...
    rec("workflow-lifecycle", "after PREPARED the preparer can no longer write any record type (WRITE_NOT_GRANTED)",
        all(_try_auth(f).code == "WRITE_NOT_GRANTED" for f in (
            lambda: AUTH_MOD.add_provisioning_record(_sid1, _PREP, instance_uuid=_UU, root_path=_libroot(_sid1), provisioned_by="x"),
            lambda: AUTH_MOD.add_launch_recipe(_sid1, _PREP, recipe_sha256=_LAUNCH_SHA),
            lambda: AUTH_MOD.add_read_only_journal(_sid1, _PREP, journal_path_sha256=_JOURNAL_SHA))))
    rec("workflow-lifecycle", "PREPARED cannot be re-entered, and mark_prepared without both preparer records is refused",
        _try_auth(lambda: AUTH_MOD.mark_prepared(_sid1, _PREP)).code == "STATE_TRANSITION_INVALID"
        and (lambda s: (AUTH_MOD.create_evidence_set(s, _PREP),
                        _try_auth(lambda: AUTH_MOD.mark_prepared(s, _PREP)).code)[1] == "PREPARER_RECORD_MISSING")(_wsess("nopre")))
    AUTH_MOD.add_bundle_verification(_sid1, _VER)
    rec("workflow-lifecycle", "after VERIFIED nothing may be written by anyone, including a second verification",
        _try_auth(lambda: AUTH_MOD.add_bundle_verification(_sid1, _VER)).code in ("WRITE_NOT_GRANTED", "VERIFICATION_EXISTS")
        and _try_auth(lambda: AUTH_MOD.add_read_only_journal(_sid1, _PREP, journal_path_sha256=_JOURNAL_SHA)).code == "WRITE_NOT_GRANTED")
    rec("workflow-lifecycle", "the VERIFIED document and its workflow state are sealed read-only and the document digest is sealed",
        stat.S_IMODE(os.lstat(AUTH_MOD.set_path(_sid1)).st_mode) == AUTH_MOD.SEALED_MODE
        and AUTH_MOD.workflow_state(_sid1)["evidence_set_sha256"] == sha(AUTH_MOD.set_path(_sid1)))
    ...
    rec("workflow-principals", "an unregistered, empty, malformed or wrong-role principal is refused PRINCIPAL_INVALID",
        all(_try_auth(lambda p=p: AUTH_MOD.create_evidence_set(_wsess("badp"), p)).code == "PRINCIPAL_INVALID"
    ...
    rec("workflow-principals", "a PREPARER cannot author BUNDLE_VERIFICATION through any production entry point",
        _try_auth(lambda: AUTH_MOD.add_bundle_verification(_prepared("selfver")[0], _PREP)).code == "PRINCIPAL_INVALID")
    rec("workflow-principals", "a VERIFIER cannot author a preparer record through any production entry point",
        all(_try_auth(f).code in ("PRINCIPAL_INVALID", "WRITE_NOT_GRANTED") for f in (
            lambda: AUTH_MOD.create_evidence_set(_wsess("vercreate"), _VER),
            lambda: AUTH_MOD.add_provisioning_record(_sid1, _VER, instance_uuid=_UU, root_path=_libroot(_sid1), provisioned_by="x"),
            lambda: AUTH_MOD.add_launch_recipe(_sid1, _VER, recipe_sha256=_LAUNCH_SHA))))
    ...
    _bv = AUTH_MOD.add_bundle_verification(_sidP, _VER)
    _dP = _derive(_sidP)
    ...
    rec("workflow-e2e-positive", "the governed document validates under the unchanged v1.11 validator and the registered schema",
        AUTH_MOD.validate_evidence_set(_sidP) == []
        and not list(V["resolveEvidenceSet"].iter_errors(AUTH_MOD.load_evidence_set(_sidP))))
    def _cli_e2e():
        ...
        with contextlib.redirect_stdout(buf):
            rcs = [PREP_CLI.main(["adopt", "--session", s_, "--uuid", _UU, "--library-root", lr_, "--by", "Mikko (operator)",
                                  "--recipe-sha", _LAUNCH_SHA, ]),
                   PREP_CLI.main(["prepare", "--session", s_, ]),
                   VER_CLI.main(["check", "--session", s_, ]),
                   VER_CLI.main(["verify", "--session", s_, ]),
                   PREP_CLI.main(["status", "--session", s_, ])]
        return rcs, _derive(s_)["state"]

    _cli_rcs, _cli_state = _cli_e2e()
    rec("workflow-e2e-positive",
        "the two production CLIs drive the whole authoring path with no root option at all: adopt / prepare / check all exit 0 and the workflow reaches ATTACHMENT_READY mechanically",
        _cli_rcs[:3] == [0, 0, 0] and _cli_state == "ATTACHMENT_READY", f"exit codes {_cli_rcs}; {_cli_state}")
    rec("workflow-e2e-positive",
        "V112-RP1 PARITY: the same CLIs REFUSE to hand back authority for a document outside the frozen root - verify and status exit non-zero on the authorizing derivation while the mechanics are sound. In v1.12 they returned ATTACHMENT_READY here.",
        _cli_rcs[3:] == [2, 2], f"verify/status exit codes {_cli_rcs[3:]}")
    ...
    _negcase("same actor is preparer and verifier", lambda: (lambda s: (AUTH_MOD.add_bundle_verification(s, _VER), s)[1])(_prepared("sameactor", principal=_PREP)[0]) if L.principal_actor(_PREP) == L.principal_actor(_VER) else _prepared("sameactor2")[0])
    _negcase("unauthorized verifier principal", lambda: (lambda s: (_try_auth(lambda: AUTH_MOD.add_bundle_verification(s, "VERIFIER:rogue")), s)[1])(_prepared("rogue")[0]))
    _negcase("preparer attempts BUNDLE_VERIFICATION", lambda: (lambda s: (_try_auth(lambda: AUTH_MOD.add_bundle_verification(s, _PREP)), s)[1])(_prepared("prepself")[0]))
    _negcase("verifier attempts a preparer record type", lambda: (lambda s: (_try_auth(lambda: AUTH_MOD.add_launch_recipe(s, _VER, recipe_sha256=_LAUNCH_SHA)), s)[1])(_prepared("wrongtype")[0]))
    _negcase("missing launch recipe", lambda: (lambda s: (AUTH_MOD.create_evidence_set(s, _PREP),
                                                          AUTH_MOD.add_provisioning_record(s, _PREP, instance_uuid=_UU, root_path=_libroot(s), provisioned_by="x"), s)[2])(_wsess("nolaunch")))
    _negcase("invalid provisioning (non-uuid)", lambda: (lambda s: (AUTH_MOD.create_evidence_set(s, _PREP),
                                                                    _try_auth(lambda: AUTH_MOD.add_provisioning_record(s, _PREP, instance_uuid="not-a-uuid", root_path=_libroot(s), provisioned_by="x")), s)[2])(_wsess("baduuid")))
    _negcase("invalid provisioning (relative root)", lambda: (lambda s: (AUTH_MOD.create_evidence_set(s, _PREP),
                                                                         _try_auth(lambda: AUTH_MOD.add_provisioning_record(s, _PREP, instance_uuid=_UU, root_path="relative/root", provisioned_by="x")), s)[2])(_wsess("relroot")))
    _negcase("second provisioning identity refused", lambda: (lambda s: (_try_auth(lambda: AUTH_MOD.add_provisioning_record(s, _PREP, instance_uuid=_UU, root_path=_libroot(s), provisioned_by="x")), s)[1])(_prepared("secondprov")[0]))
    ...
    for _rt in ("PROVISIONING_RECORD", "LAUNCH_RECIPE", "READ_ONLY_JOURNAL"):
        ...
        _rt_prod = next(r for r in AUTH_MOD.load_evidence_set(_sidP)["records"].values() if r["record_type"] == _rt)
    ...
    rec("workflow-static", "add_bundle_verification takes no verifier-supplied result, preparer principal, record ids OR ROOT",
        sorted(inspect.signature(AUTH_MOD.add_bundle_verification).parameters) == ["envelope", "principal", "session_id"])
    ...
    rec("workflow-static", "a caller may not inject a derived envelope field; only sequence and captured_at are its own",
        ...
        and (lambda s_: (AUTH_MOD.create_evidence_set(s_, _PREP),
                         _try_auth(lambda: AUTH_MOD.add_read_only_journal(s_, _PREP, journal_path_sha256=_JOURNAL_SHA,
                                                                          envelope={"manifest_sha256": "0" * 64})).code)[1]
             == "FORBIDDEN_ENVELOPE_INPUT")(_wsess("envinj")))
    rec("workflow-static", "the record_type_version the constructors stamp is the v1.11 authority value, not the stale prose value",
        ...
        and all(r["envelope"]["record_type_version"] == "1.10" for r in AUTH_MOD.load_evidence_set(_sidP)["records"].values()))
    rec("workflow-static", "the launch recipe the constructor derives carries the COMPUTED contract version, closing the v1.11 trap",
        next(r for r in AUTH_MOD.load_evidence_set(_sidP)["records"].values() if r["record_type"] == "LAUNCH_RECIPE")["resolve_version"]
...
# ================================================================ v1.13 V112-RP1 the governed-root defect
# Codex's runtime-parity oracle on v1.12: production APIs and CLIs accepted arbitrary AND SYMLINKED evidence roots,
# and a complete workflow under such a root derived ATTACHMENT_READY - contradicting the frozen one-root law
# (WORKFLOW_CONTRADICTORY). These sections run OUTSIDE the sandbox: they are about the production root itself.
def _cli_rc(mod, argv):
    """The exit code a production CLI returns for one invocation, with its output captured."""
    buf = io.StringIO()
    try:
...
def _under_root(root, fn):
    """Run fn() with the authoring module's non-authorizing sandbox armed at `root`, always disarming afterwards.
    Used only to probe root-trust refusals; it never leaves the module armed, even on failure."""
    prev = AUTH_MOD._SANDBOX_ROOT
    AUTH_MOD._SANDBOX_ROOT = os.path.abspath(root)
    try:
        return _try_auth(fn)
    finally:
        AUTH_MOD._SANDBOX_ROOT = prev


_AS_EARLY = inspect.getsource(AUTH_MOD)
RP_ROOT = tempfile.mkdtemp(prefix="resolve-v113-rootlaw-")
_RP_SEQ = [0]


def _rp_sess(tag):
    _RP_SEQ[0] += 1
    return f"sess-v113-{tag}-{_RP_SEQ[0]:03d}"


def _rp_prepared(tag):
    """Drive the production authoring path to VERIFIED inside a sandbox rooted at `sub`. Returns (session, sub)."""
    sub = os.path.join(RP_ROOT, "gov-" + tag)
    sid = _rp_sess(tag)
    with TK.sandbox(sub):
        lib = os.path.join(RP_ROOT, "lib-" + tag)
        os.makedirs(lib, exist_ok=True)
        AUTH_MOD.create_evidence_set(sid, _PREP)
        AUTH_MOD.add_provisioning_record(sid, _PREP, instance_uuid=_UU, root_path=lib, provisioned_by="Mikko (operator)")
        AUTH_MOD.add_launch_recipe(sid, _PREP, recipe_sha256=_LAUNCH_SHA)
        AUTH_MOD.mark_prepared(sid, _PREP)
        AUTH_MOD.add_bundle_verification(sid, _VER)
    return sid, sub


try:
    # ---- 1. one root authority, no caller selection
    rec("root-law", "V112-RP1 section 1: exactly ONE governed attachment root is frozen and it is the required path",
        L.GOVERNED_ATTACHMENT_ROOT == "/home/vidtoolz/resolve-qualification-evidence/attachment"
        and L.GOVERNED_ATTACHMENT_ROOT == L.QUALIFICATION_EVIDENCE_ROOT + "/attachment"
        and AUTH_MOD.canonical_set_dir("s1") == L.GOVERNED_ATTACHMENT_ROOT + "/s1")
    rec("root-law", "V112-RP1 sections 2/4: NO authorizing function takes a governed-root parameter of any name",
        not [(n, p) for n in dir(AUTH_MOD) if not n.startswith("_") and inspect.isfunction(getattr(AUTH_MOD, n))
             for p in inspect.signature(getattr(AUTH_MOD, n)).parameters
             if p in ("root", "evidence_root", "output_root", "base_dir", "directory", "path", "dir", "prefix")],
        ",".join(f"{n}.{p}" for n in dir(AUTH_MOD) if not n.startswith("_") and inspect.isfunction(getattr(AUTH_MOD, n))
                 for p in inspect.signature(getattr(AUTH_MOD, n)).parameters
                 if p in ("root", "evidence_root", "output_root", "base_dir", "directory", "path", "dir", "prefix")) or "none")
    rec("root-law", "the root resolver takes NO argument, so there is nothing for a caller to pass",
        not inspect.signature(AUTH_MOD.governed_attachment_root).parameters
        and not inspect.signature(AUTH_MOD.require_governed_root).parameters
        and AUTH_MOD.governed_attachment_root() == L.GOVERNED_ATTACHMENT_ROOT
        and AUTH_MOD.is_production_root() is True)
    rec("root-law", "add_provisioning_record's root_path is the RESOLVE LIBRARY path (a record field), not the evidence root: the two are distinguishable",
        "root_path" in inspect.signature(AUTH_MOD.add_provisioning_record).parameters
        and "root" not in inspect.signature(AUTH_MOD.add_provisioning_record).parameters)

    # ---- 3. CLI parameter audit
    def _cli_opts(mod, argv):
        buf = io.StringIO()
        try:
            with contextlib.redirect_stdout(buf), contextlib.redirect_stderr(buf):
                mod.main(argv)
            ...
            pass
        return {w.rstrip(",") for w in buf.getvalue().split() if w.startswith("--")}

    _PREP_OPTS = _cli_opts(PREP_CLI, ["adopt", "--help"]) | _cli_opts(PREP_CLI, ["prepare", "--help"]) | _cli_opts(PREP_CLI, ["status", "--help"])
    _VER_OPTS = _cli_opts(VER_CLI, ["check", "--help"]) | _cli_opts(VER_CLI, ["verify", "--help"])
    _FORBIDDEN_OPTS = {"--root", "--evidence-root", "--output-root", "--base-dir", "--directory", "--path", "--dir", "--prefix"}
    rec("root-law", "V112-RP1 section 3: NEITHER production CLI exposes an option that could change the governed root. v1.12 exposed --evidence-root on both.",
        not (_PREP_OPTS & _FORBIDDEN_OPTS) and not (_VER_OPTS & _FORBIDDEN_OPTS),
        f"prepare={sorted(_PREP_OPTS & _FORBIDDEN_OPTS)} verify={sorted(_VER_OPTS & _FORBIDDEN_OPTS)}")
    rec("root-law", "the preparer's library path is named --library-root so no production CLI carries an option literally called --root",
        "--library-root" in _PREP_OPTS and "--root" not in _PREP_OPTS and "--root" not in _VER_OPTS,
        ",".join(sorted(_PREP_OPTS)))
    rec("root-law", "V112-RP1 pinned: the exact v1.12 attack invocations are now rejected by argument parsing itself",
        all(_cli_rc(PREP_CLI, ["adopt", "--session", "s-atk", "--uuid", _UU, "--library-root", "/tmp/lib",
                               "--by", "op", "--recipe-sha", _LAUNCH_SHA, opt, "/tmp/evil"]) == 2
            for opt in ("--evidence-root", "--root")))

    # ---- 6/8. root entry trust and the symlinked-root attack
    _real = os.path.join(RP_ROOT, "real-root"); os.makedirs(_real, mode=0o700, exist_ok=True)
    _lnk = os.path.join(RP_ROOT, "linked-root"); os.symlink(_real, _lnk)
    _e_symroot = _under_root(_lnk, lambda: AUTH_MOD.create_evidence_set(_rp_sess("symroot"), _PREP))
    rec("root-law", "V112-RP1 section 8 PINNED: a SYMLINKED governed root is refused BEFORE any evidence set is created. In v1.12 the identical workflow derived ATTACHMENT_READY.",
        _e_symroot.code == "ROOT_SYMLINK_REFUSED", _e_symroot.code)
    rec("root-law", "nothing was created under the symlink target, so the refusal happened before any write",
        not os.path.exists(os.path.join(_real, "attachment")) and not os.listdir(_real))
    _notdir = os.path.join(RP_ROOT, "root-is-a-file"); open(_notdir, "w").write("x")
    _badmode = os.path.join(RP_ROOT, "root-bad-mode"); os.makedirs(_badmode, mode=0o755, exist_ok=True); os.chmod(_badmode, 0o755)
    for _label, _r, _want in (("a regular file as the governed root", _notdir, "ROOT_NOT_A_DIRECTORY"),
                              ("a governed root with the wrong mode", _badmode, "ROOT_MODE_INVALID"),
                              ("an absent governed root", os.path.join(RP_ROOT, "no-such-root"), "ROOT_NOT_FOUND")):
        _e = _under_root(_r, AUTH_MOD.require_governed_root)
        rec("root-law", f"root entry trust: {_label} is refused {_want}", _e.code == _want, _e.code)
    rec("root-law", "V112-RP1 section 6 / V113-B1 section 7: entry classification happens by lstat BEFORE anything is resolved, realpath is only ever a later alias check, and there is now exactly ONE implementation of it in the CORE",
        (lambda src: src.index("_entry_lstat") < src.index("realpath")
                     and "os.lstat" in inspect.getsource(L._entry_lstat)
                     and ".resolve()" not in inspect.getsource(L)
                     and ".resolve()" not in inspect.getsource(AUTH_MOD)
                     and "L.governed_dir_errors" in inspect.getsource(AUTH_MOD._require_trusted_dir))(inspect.getsource(L.governed_dir_errors)))

    # ---- 7/9. session entry trust and the symlinked-session attack
    _sid_sym, _sub_sym = _rp_prepared("symsess")
    _real_sess = os.path.join(RP_ROOT, "elsewhere-session")
    with TK.sandbox(_sub_sym):
        _gov_sess = AUTH_MOD.set_dir(_sid_sym)
    shutil.move(_gov_sess, _real_sess)
    os.symlink(_real_sess, _gov_sess)
    with TK.sandbox(_sub_sym):
        _e_sym = _try_auth(lambda: TK.derive_nonauthorizing(_sid_sym))
        _e_sym2 = _try_auth(lambda: AUTH_MOD.load_evidence_set(_sid_sym))
    rec("root-law", "V112-RP1 section 9/14 PINNED: a SYMLINKED session directory is refused, on load and on derive, even though the document behind it is valid",
        _e_sym.code == "SESSION_SYMLINK_REFUSED" and _e_sym2.code == "SESSION_SYMLINK_REFUSED",
        f"{_e_sym.code}/{_e_sym2.code}")
    os.unlink(_gov_sess); shutil.move(_real_sess, _gov_sess)
    with TK.sandbox(_sub_sym):
        rec("root-law", "restoring the real session directory restores the workflow, so the refusal was the symlink and not the document",
            TK.derive_nonauthorizing(_sid_sym)["state"] == "ATTACHMENT_READY")

    # ---- 10/13. arbitrary root and relocation
    _sid_rel, _sub_rel = _rp_prepared("reloc")
    with TK.sandbox(_sub_rel):
        _src = AUTH_MOD.set_dir(_sid_rel)
    _relocs = []
    for _label, _dst_root in (("/tmp", os.path.join(RP_ROOT, "tmp-like")),
                              ("a sibling root", os.path.join(RP_ROOT, "sibling")),
                              ("an alternate user path", os.path.join(RP_ROOT, "alt-user"))):
        os.makedirs(_dst_root, mode=0o700, exist_ok=True)
        shutil.copytree(_src, os.path.join(_dst_root, _sid_rel))
    ...
    rec("root-law", "the production root resolver is not configurable in either environment: no argument, no override, and the authoring layer delegates to the core constant",
        ...
        and not inspect.signature(AUTH_MOD.governed_attachment_root).parameters
        and L.authority_attachment_root() == L.GOVERNED_ATTACHMENT_ROOT
        and "L.GOVERNED_ATTACHMENT_ROOT" in inspect.getsource(AUTH_MOD.governed_attachment_root))
    rec("root-law", "V112-RP1 section 10: neither the Store.v5 evidence root nor any EKA/shared path can become the attachment root; there is no parameter to say so",
        not re.search(r"^\s*import\s+evidence_store", _AS_EARLY, re.M)
        and not any(getattr(v, "__name__", "") == "evidence_store" for v in vars(AUTH_MOD).values())
        and not (_PREP_OPTS | _VER_OPTS) & _FORBIDDEN_OPTS
        and L.GOVERNED_ATTACHMENT_ROOT not in (tc["library"].get("root_path") or "", "/tmp")
        and "EKA" not in L.GOVERNED_ATTACHMENT_ROOT and "nelja" not in L.GOVERNED_ATTACHMENT_ROOT)

    # ---- 5. session id law
    rec("root-law", "V112-RP1 section 5: every forbidden session-id shape is refused SESSION_ID_INVALID, so no traversal is possible through the session id",
        all(_try_auth(lambda b=b: AUTH_MOD.set_dir(b)).code == "SESSION_ID_INVALID" for b in AUTH_MOD.SESSION_ID_REJECTS)
        and len(AUTH_MOD.SESSION_ID_REJECTS) >= 16,
        f"{len(AUTH_MOD.SESSION_ID_REJECTS)} rejected shapes")
    rec("root-law", "the session id law rejects separators, traversal, leading dots, NUL, absolute syntax and over-length, and accepts an ordinary id",
        all(_try_auth(lambda b=b: AUTH_MOD.set_dir(b)).code == "SESSION_ID_INVALID"
            for b in ("a/b", "a\\b", "..", "../x", "/x", ".hidden", "a\x00b", "C:\\x", "x" * 129, ""))
        and AUTH_MOD.set_dir("sess-2026-09-10-0001").endswith("/sess-2026-09-10-0001"))

    # ---- 15. test-override separation
    rec("root-law", "V112-RP1 section 15: the scratch-root mechanism lives in a separate INTERNAL_NON_AUTHORIZING module",
        TK.AUTHORITY_CLASS == "INTERNAL_NON_AUTHORIZING"
        and not any(f in L.AUTHORITY_SURFACE["authorizing"] for f in ("sandbox", "derive_nonauthorizing")))
    rec("root-law", "V112-RP1 section 15: neither production CLI imports the testkit, so the override is unreachable in production mode",
        all(not re.search(r"^\s*import\s+evidence_authoring_testkit", inspect.getsource(m), re.M)
            and not re.search(r"^\s*from\s+evidence_authoring_testkit", inspect.getsource(m), re.M)
            and "evidence_authoring_testkit" not in vars(m)
            for m in (PREP_CLI, VER_CLI)))
    rec("root-law", "the authoring module never imports the testkit: the arming variable is module-private, and the testkit is the only code that assigns it",
        not re.search(r"^\s*(?:import|from)\s+evidence_authoring_testkit", _AS_EARLY, re.M)
        and not any(getattr(v, "__name__", "") == "evidence_authoring_testkit" for v in vars(AUTH_MOD).values())
        and "_SANDBOX_ROOT" in inspect.getsource(AUTH_MOD.governed_attachment_root)
        and not re.search(r"^\s*_SANDBOX_ROOT\s*=", _AS_EARLY.split("def governed_attachment_root")[1], re.M)
        and "A._SANDBOX_ROOT = root" in inspect.getsource(TK))
    rec("root-law", "the testkit refuses to sandbox the real authority root, so the suite can never write evidence there",
        _try_auth(lambda: TK.sandbox(L.GOVERNED_ATTACHMENT_ROOT).__enter__()).code.startswith("RAW_AssertionError")
        and _try_auth(lambda: TK.sandbox(L.GOVERNED_ATTACHMENT_ROOT + "/x").__enter__()).code.startswith("RAW_AssertionError"))

    # ---- 19. runtime/fixture parity on the location law
    rec("root-parity", "V112-RP1 section 19: the sandbox differs from production in the ROOT PREFIX ONLY - the same resolver, the same trust checks and the same freshness law run on the same code",
        inspect.getsource(AUTH_MOD.require_governed_root).count("governed_attachment_root()") == 1
        and "require_governed_session_dir" in inspect.getsource(AUTH_MOD._load)
        and "require_fresh" in inspect.getsource(AUTH_MOD.derive_attachment_state)
        and "require_fresh" in inspect.getsource(TK.derive_nonauthorizing))
    _sid_par, _sub_par = _rp_prepared("parity")
    with TK.sandbox(_sub_par):
        _par_mech = TK.derive_nonauthorizing(_sid_par)["state"]
    ...
    # ---- 30. property tests over roots, symlinks, session ids and relocations
    _RPRNG = random.Random(SEED + 113)
    _prop_bad = []
    for _i in range(24):
        kind = _RPRNG.choice(("alt-root", "symlink-root", "relocate", "sibling-prefix", "relative", "normalise"))
        tag = f"prop{_i:02d}"
        if kind == "alt-root":
            sid, _sub = _rp_prepared(tag)
            if _try_auth(lambda s=sid: AUTH_MOD.derive_attachment_state(s)).code == "NONE":
                _prop_bad.append(f"{kind}:{tag}")
        elif kind == "symlink-root":
            real = os.path.join(RP_ROOT, "pr-" + tag); os.makedirs(real, mode=0o700, exist_ok=True)
            lk = os.path.join(RP_ROOT, "pl-" + tag); os.symlink(real, lk)
            if _under_root(lk, AUTH_MOD.require_governed_root).code != "ROOT_SYMLINK_REFUSED":
                _prop_bad.append(f"{kind}:{tag}")
        elif kind == "relocate":
            sid, sub = _rp_prepared(tag)
            with TK.sandbox(sub):
                src = AUTH_MOD.set_dir(sid)
            dst = os.path.join(RP_ROOT, "pd-" + tag); os.makedirs(dst, mode=0o700, exist_ok=True)
            shutil.copytree(src, os.path.join(dst, sid))
            with TK.sandbox(dst):
                if _try_auth(lambda s=sid: TK.derive_nonauthorizing(s)).code != "SET_LOCATION_MISMATCH":
                    _prop_bad.append(f"{kind}:{tag}")
        elif kind == "sibling-prefix":
            if _under_root(L.GOVERNED_ATTACHMENT_ROOT + "-evil",
                           lambda: (AUTH_MOD.is_production_root(), AUTH_MOD.governed_attachment_root())).value \
                    != (False, L.GOVERNED_ATTACHMENT_ROOT + "-evil"):
                _prop_bad.append(f"{kind}:{tag}")
        elif kind == "relative":
            prev = AUTH_MOD._SANDBOX_ROOT
            AUTH_MOD._SANDBOX_ROOT = "relative/attachment"     # deliberately un-normalised: os.path.abspath is bypassed
            try:
                if _try_auth(AUTH_MOD.require_governed_root).code != "ROOT_NOT_GOVERNED":
                    _prop_bad.append(f"{kind}:{tag}")
            finally:
                AUTH_MOD._SANDBOX_ROOT = prev
        else:
            prev = AUTH_MOD._SANDBOX_ROOT
            AUTH_MOD._SANDBOX_ROOT = L.GOVERNED_ATTACHMENT_ROOT + "/../attachment"
            try:
                if _try_auth(AUTH_MOD.require_governed_root).code != "ROOT_NOT_GOVERNED":
                    _prop_bad.append(f"{kind}:{tag}")
            finally:
                AUTH_MOD._SANDBOX_ROOT = prev
    rec("root-property", f"24 seeded root, symlink, relocation, sibling-prefix, relative and normalisation attacks: none yields ATTACHMENT_READY outside the governed root (seed {SEED + 113})",
        not _prop_bad, ";".join(_prop_bad[:3]) or "all refused")
    _sid_bad = []
    for _i in range(40):
        n = _RPRNG.randint(0, 6)
        cand = "".join(_RPRNG.choice("ab09._-/\\.. \x00~:") for _ in range(n))
        ok = bool(AUTH_MOD.NAME_SAFE.match(cand)) if isinstance(cand, str) else False
        got = _try_auth(lambda c=cand: AUTH_MOD.set_dir(c))
        if ok and got.code != "NONE":
            _sid_bad.append(f"safe {cand!r} refused {got.code}")
        if not ok and got.code != "SESSION_ID_INVALID":
            _sid_bad.append(f"unsafe {cand!r} -> {got.code}")
        if got.code == "NONE" and (os.sep in cand or ".." in cand or cand.startswith(".")):
            _sid_bad.append(f"traversal accepted {cand!r}")
    rec("root-property", f"40 seeded session-id shapes: the safe-basename law and the path law agree exactly, and no traversal is ever accepted (seed {SEED + 113})",
        not _sid_bad, ";".join(_sid_bad[:3]) or "consistent")

    # ---- 29. static audits
    _AS = inspect.getsource(AUTH_MOD)
    _PS, _VS = inspect.getsource(PREP_CLI), inspect.getsource(VER_CLI)
    rec("root-static", "no production authorizing arbitrary-root parameter survives anywhere in the authoring module or either CLI",
        "--evidence-root" not in _PS and "--evidence-root" not in _VS
        and "evidence_root" not in _PS and "evidence_root" not in _VS
        and not re.search(r"def (?!_)\w+\([^)]*\broot=", _AS))
    rec("root-static", "exactly one governed-root constant exists and every path derives from it; there is no second resolver",
        _AS.count("L.GOVERNED_ATTACHMENT_ROOT") >= 3
        and len(re.findall(r"^def governed_\w+", _AS, re.M)) == 1
        and "QUALIFICATION_EVIDENCE_ROOT" not in inspect.getsource(AUTH_MOD.set_dir))
    rec("root-static", "no symlink-follow-before-check anywhere: neither the core nor the authoring module calls .resolve(), and in the ONE trust implementation realpath appears only after lstat classification",
        ".resolve()" not in _AS and ".resolve()" not in inspect.getsource(L)
        and (lambda src: src.index("_entry_lstat") < src.index("realpath"))(inspect.getsource(L.governed_dir_errors))
        and "realpath" not in inspect.getsource(AUTH_MOD.require_governed_root))
    rec("root-static", "no production runtime path under /tmp, and the frozen root is neither a Store.v5 root nor an EKA/shared path",
        not L.GOVERNED_ATTACHMENT_ROOT.startswith("/tmp")
        and "/tmp" not in inspect.getsource(AUTH_MOD.governed_attachment_root)
        and "EKA" not in L.GOVERNED_ATTACHMENT_ROOT and "nelja" not in L.GOVERNED_ATTACHMENT_ROOT)
    rec("root-static", "derive AND verify both re-check the canonical governed location, not only create",
        all(t in inspect.getsource(AUTH_MOD.derive_attachment_state) for t in ("canonical_set_dir", "authority_root", "written_under_production_root"))
        and "require_governed_session_dir" in inspect.getsource(AUTH_MOD._load)
        and "_load(session_id)" in inspect.getsource(AUTH_MOD.add_bundle_verification))
    rec("root-static", "the production tools do not import the test helper, and the test helper is not on the authorizing surface",
        all(not re.search(r"^\s*(?:import|from)\s+evidence_authoring_testkit", src, re.M) for src in (_PS, _VS, _AS))
        and not any(getattr(v, "__name__", "") == "evidence_authoring_testkit"
                    for m in (PREP_CLI, VER_CLI, AUTH_MOD) for v in vars(m).values())
        and TK.AUTHORITY_CLASS == "INTERNAL_NON_AUTHORIZING"
        and not any(f in L.AUTHORITY_SURFACE["authorizing"] for f in ("sandbox", "derive_nonauthorizing")))
    rec("root-static", "the workflow law document publishes the frozen root, the refusal codes and the test-override separation",
        WFLAW["persistence"]["governed_root"] == L.QUALIFICATION_EVIDENCE_ROOT
        and WFLAW["persistence"]["governed_attachment_root"] == L.GOVERNED_ATTACHMENT_ROOT
        and WFLAW["persistence"]["caller_selectable"] is False
        and "INTERNAL_NON_AUTHORIZING" in WFLAW["persistence"]["test_override"])
finally:
    AUTH_MOD._SANDBOX_ROOT = None
    for dp, dns, fns in os.walk(RP_ROOT):
        for x in dns + fns:
            try:
                os.chmod(os.path.join(dp, x), 0o700)
            except OSError:
...
# ================================================================ 20. v1.14 V113-B1: the authorizing location bypass
# Codex's independent harness on v1.13: the governed-location law was enforced in evidence_authoring's wrapper while
# authority_lib.derive_attachment_state and authority_lib.evaluate_eligibility - the two functions actually declared
# AUTHORIZING - still accepted a bare evidence-set dict and returned ATTACHMENT_READY / eligible=true for a document
# authored under a forbidden root. These sections prove location authority now dominates the CORE call graph.
CB_ROOT = tempfile.mkdtemp(prefix="resolve-v114-corebypass-")
# These sections drive the PRODUCTION authoring path, whose BUNDLE_VERIFICATION binds the REAL freeze manifest, so
# they evaluate under the module's own active authority rather than the placeholder ACTIVE the fixture suite uses.
_CB_ACTIVE = AUTH_MOD.active_authority()
_CB_SEQ = [0]


def _cb_sess(tag):
    _CB_SEQ[0] += 1
    return f"sess-v114-{tag}-{_CB_SEQ[0]:03d}"


def _cb_req(roj):
    return {"milestone": "M0", "operation": "READ_PRIMITIVE_QUALIFICATION_PROBE",
            "scope": "SCRATCH_QUALIFICATION_LIBRARY", "refs": {"read_only_journal": roj["record_id"]}}


def _cb_author(sid, libroot):
    """Author a complete, semantically perfect governed workflow with the PRODUCTION functions only."""
    AUTH_MOD.create_evidence_set(sid, _PREP)
    AUTH_MOD.add_provisioning_record(sid, _PREP, instance_uuid=_UU, root_path=libroot,
                                     provisioned_by="Mikko (operator)")
    AUTH_MOD.add_launch_recipe(sid, _PREP, recipe_sha256=_LAUNCH_SHA)
    roj = AUTH_MOD.add_read_only_journal(sid, _PREP, journal_path_sha256=_JOURNAL_SHA)
    AUTH_MOD.mark_prepared(sid, _PREP)
    AUTH_MOD.add_bundle_verification(sid, _VER)
    return roj


def _cb_sandbox_set(tag):
    """A mechanically perfect evidence set authored under a FORBIDDEN root. Returns (session_id, es, roj, wrapper)."""
    lib = os.path.join(CB_ROOT, "lib-" + tag)
    os.makedirs(lib, exist_ok=True)
    sid = _cb_sess(tag)
    with TK.sandbox(os.path.join(CB_ROOT, "gov-" + tag)):
        roj = _cb_author(sid, lib)
        es = AUTH_MOD.load_evidence_set(sid)
        wrapper = _try_auth(lambda: AUTH_MOD.derive_attachment_state(sid))
    return sid, es, roj, wrapper


try:
    # ---- 20.1 / sections 1, 8, 10: the authority-surface split
    rec("core-authority", "V113-B1 sections 1/10: derive_attachment_state and evaluate_eligibility are NO LONGER on the authorizing surface; they are declared DIAGNOSTIC / NON_AUTHORIZING",
        "derive_attachment_state" not in L.AUTHORITY_SURFACE["authorizing"]
        and "evaluate_eligibility" not in L.AUTHORITY_SURFACE["authorizing"]
        and sorted(L.AUTHORITY_SURFACE["diagnostic_non_authorizing"]) == ["derive_attachment_state", "evaluate_eligibility"],
        ",".join(sorted(L.AUTHORITY_SURFACE.get("diagnostic_non_authorizing", []))))
    rec("core-authority", "the authorizing surface names the governed loader, the four authorizing entry points and the two location predicates",
        all(f in L.AUTHORITY_SURFACE["authorizing"] for f in (
            ...
            "governed_provenance_errors", "location_receipt",
            # v1.15 additions
            "governed_consume", "governed_structure_errors", "evidence_snapshot_digest",
            "validate_transaction_set_authorizing", "commit_eligibility_authorizing")),
        "all present")
    _OPS = {"evidence_set_create": "create_evidence_set", "evidence_set_add_provisioning": "add_provisioning_record",
            "evidence_set_add_launch_recipe": "add_launch_recipe", "evidence_set_mark_prepared": "mark_prepared",
            "evidence_set_add_bundle_verification": "add_bundle_verification",
            "evidence_set_derive_attachment_state": "derive_attachment_state"}
    rec("core-authority", "every name on the authorizing surface is real: each core name is a callable in authority_lib and each declared evidence-set OPERATION is implemented by the authoring module, so the surface is not aspirational",
        all(callable(getattr(L, f, None)) for f in L.AUTHORITY_SURFACE["authorizing"] if f not in _OPS)
        and all(callable(getattr(AUTH_MOD, impl, None)) for impl in _OPS.values()),
        ",".join(f for f in L.AUTHORITY_SURFACE["authorizing"]
                 if f not in _OPS and not callable(getattr(L, f, None))) or "all resolved")
    rec("core-authority", "the declared authorizing evidence-set derivation operation is the AUTHORIZING wrapper, which now routes through the core's governed loader rather than deciding locality by itself",
        "L.load_governed_evidence_set" in inspect.getsource(AUTH_MOD.derive_attachment_state)
        and "L.derive_attachment_state_authorizing" in inspect.getsource(AUTH_MOD.derive_attachment_state))
    rec("core-authority", "V113-B1 section 6: no authorizing core entry point accepts a root, evidence_root, path or base_dir parameter, and the root resolver takes no argument at all",
        not [(n, p) for n in L.AUTHORITY_SURFACE["authorizing"] if inspect.isfunction(getattr(L, n, None))
             for p in inspect.signature(getattr(L, n)).parameters
             if p in ("root", "evidence_root", "output_root", "base_dir", "directory", "path", "dir", "prefix")]
        and not inspect.signature(L.authority_attachment_root).parameters)
    rec("core-authority", "the core root resolver has NO override of any kind, not even for tests, and the canonical path law is derived from the frozen constant",
        L.authority_attachment_root() == L.GOVERNED_ATTACHMENT_ROOT
        and "_SANDBOX" not in inspect.getsource(L.authority_attachment_root)
        and "environ" not in inspect.getsource(L.authority_attachment_root)
        and L.canonical_session_dir("s1") == os.path.join(L.GOVERNED_ATTACHMENT_ROOT, "s1")
        and L.canonical_document_path("s1") == os.path.join(L.GOVERNED_ATTACHMENT_ROOT, "s1", "EVIDENCE-SET.json"))

    # ---- 20.2 / section 20: the provenance object cannot be fabricated
    _fab = _try_auth(lambda: L.GovernedEvidenceSet(
        object(), evidence_set={}, session_id="s1", document_path="x", governed_root=L.GOVERNED_ATTACHMENT_ROOT,
        authority_version=L.AUTHORITY_VERSION, manifest_sha256=_CB_ACTIVE["manifest_sha256"], document_sha256="0" * 64,
        receipt="0" * 64))
    rec("core-authority", "V113-B1 section 20: GovernedEvidenceSet is constructible ONLY by the canonical loader - a direct construction with a wrong token is refused",
        _fab.code == "RAW_AuthorityTrustError" and "load_governed_evidence_set" in _fab.detail, _fab.code)
    rec("core-authority", "it has __slots__ and no dict, so provenance fields cannot be grafted onto an instance after the fact",
        set(L.GovernedEvidenceSet.__slots__) == {"document_bytes", "session_id", "document_path", "workflow_path",
                                                 "governed_root", "authority_version", "manifest_sha256",
                                                 "document_sha256", "workflow_sha256", "evidence_snapshot_digest",
                                                 "receipt"}
        and "evidence_set" not in L.GovernedEvidenceSet.__slots__
        and isinstance(vars(L.GovernedEvidenceSet)["evidence_set"], property)
        and "__dict__" not in vars(L.GovernedEvidenceSet)
        and all("__dict__" not in vars(c) for c in L.GovernedEvidenceSet.__mro__ if c is not object))
    rec("core-authority", "the location receipt is a digest under a REGISTERED hash domain and separates session id, document digest and active manifest",
        L.LOCATION_RECEIPT_DOMAIN in L.HASH_DOMAINS
        and len({L.location_receipt("s1", "a" * 64, _CB_ACTIVE), L.location_receipt("s2", "a" * 64, _CB_ACTIVE),
                 L.location_receipt("s1", "b" * 64, _CB_ACTIVE),
                 L.location_receipt("s1", "a" * 64, dict(_CB_ACTIVE, manifest_sha256="c" * 64))}) == 4)
    rec("core-authority", "every code the core's location predicates can emit is in the frozen LOCATION_ERRORS vocabulary",
        all(c in L.LOCATION_ERRORS for c in
            [e.partition(":")[0] for e in (L.session_id_errors("..") + L.location_errors("..")
                                           + L.governed_dir_errors(os.path.join(CB_ROOT, "absent-x"), "SESSION")
                                           + L.governed_dir_errors(CB_ROOT, "ROOT", want_mode=0o700)
                                           + L.governed_provenance_errors({}, _CB_ACTIVE))]),
        ",".join(sorted({e.partition(":")[0] for e in L.governed_provenance_errors({}, _CB_ACTIVE)})))

    # ---- 20.3 / sections 11, 13: the exact Codex reproduction
    _sid_b1, _es_b1, _roj_b1, _wrap_b1 = _cb_sandbox_set("b1")
    _diag_d = L.derive_attachment_state(tc, _es_b1, _CB_ACTIVE)
    _auth_d = L.derive_attachment_state_authorizing(tc, _es_b1, _CB_ACTIVE)
    _diag_e = L.evaluate_eligibility(perms, _cb_req(_roj_b1), rp, caps, tc, _es_b1, _CB_ACTIVE)
    _auth_e = L.evaluate_eligibility_authorizing(perms, _cb_req(_roj_b1), rp, caps, tc, _es_b1, _CB_ACTIVE)
    rec("core-bypass", "control: the forbidden-location document is mechanically perfect. The wrapper refuses it, and the DIAGNOSTIC core still derives ATTACHMENT_READY and diagnostic eligibility - which is exactly what made this a real bypass surface in v1.13",
        _wrap_b1.code == "NOT_PRODUCTION_ROOT" and _diag_d["state"] == "ATTACHMENT_READY"
        and _diag_e["eligible"] is True,
        f"wrapper={_wrap_b1.code} diagnostic_state={_diag_d['state']} diagnostic_eligible={_diag_e['eligible']}")
    rec("core-bypass", "V113-B1 PINNED (section 13): the AUTHORIZING core derivation refuses the same evidence and never returns a readiness value. In v1.13 authority_lib.derive_attachment_state returned ATTACHMENT_READY here.",
        _auth_d["authorizing"] is False and _auth_d["state"] == L.CONFLICT_STATE
        and _auth_d["failures"][0] == "LOCATION_AUTHORITY_INVALID"
        and _auth_d["location_receipt"] is None and _auth_d["document_path"] is None,
        f"{_auth_d['state']} authorizing={_auth_d['authorizing']} {_auth_d['failures'][:1]}")
    rec("core-bypass", "V113-B1 PINNED (section 13): AUTHORIZING eligibility is fail-closed on the same evidence. In v1.13 authority_lib.evaluate_eligibility returned eligible=true with derived_attachment_state ATTACHMENT_READY.",
        _auth_e["eligible"] is False and _auth_e["reason_codes"] == ["LOCATION_AUTHORITY_INVALID"]
        and _auth_e["permitted_by_policy"] is False and _auth_e["prerequisites_satisfied"] is False
        and _auth_e["derived_attachment_state"] is None and _auth_e["authorizing"] is False,
        f"eligible={_auth_e['eligible']} codes={_auth_e['reason_codes']}")
    rec("core-bypass", "V113-B1 section 9: location authority strictly DOMINATES attachment semantics - the authorizing path checks provenance BEFORE it computes readiness, rather than computing readiness and retracting it afterwards",
        (lambda a, b: a.index("governed_consume") < a.index("derive_attachment_state(tc")
                      and b.index("governed_consume") < b.index("evaluate_eligibility(perms"))(
            inspect.getsource(L.derive_attachment_state_authorizing),
            inspect.getsource(L.evaluate_eligibility_authorizing)))
    rec("core-bypass", "the diagnostic functions are unchanged in behaviour: v1.14 demotes them, it does not weaken the semantic derivation the whole v1.11 suite depends on",
        _diag_d["state"] == "ATTACHMENT_READY" and _diag_d["session_id"] == _sid_b1
        and set(_diag_d) == {"state", "proofs", "failures", "conflicts", "session_id"})

    # ---- 20.4 / sections 12, 19: no diagnostic -> authority escalation, by any route
    _escalations = []
    for _label, _payload in (
            ("the raw evidence-set dict", _es_b1),
            ("the diagnostic derivation result", _diag_d),
            ("a bare ATTACHMENT_READY state string", "ATTACHMENT_READY"),
            ("the diagnostic eligibility result", _diag_e),
            ("None", None),
            ("a dict carrying every GovernedEvidenceSet field, all correct-looking", {
                "evidence_set": _es_b1, "session_id": _sid_b1, "receipt": L.location_receipt(_sid_b1, "0" * 64, _CB_ACTIVE),
                "governed_root": L.GOVERNED_ATTACHMENT_ROOT, "document_path": L.canonical_document_path(_sid_b1),
                "authority_version": L.AUTHORITY_VERSION, "manifest_sha256": _CB_ACTIVE["manifest_sha256"],
                "document_sha256": "0" * 64}),
            ("a namespace object with the same attributes", type("FakeGoverned", (), {
                "evidence_set": _es_b1, "session_id": _sid_b1, "receipt": L.location_receipt(_sid_b1, "0" * 64, _CB_ACTIVE),
                "governed_root": L.GOVERNED_ATTACHMENT_ROOT, "document_path": L.canonical_document_path(_sid_b1),
                "authority_version": L.AUTHORITY_VERSION, "manifest_sha256": _CB_ACTIVE["manifest_sha256"],
                "document_sha256": "0" * 64})()),
            ("a subclass instance built without the token", _try_auth(
                lambda: type("Sub", (L.GovernedEvidenceSet,), {})(object(), evidence_set=_es_b1)).value)):
        _r = L.evaluate_eligibility_authorizing(perms, _cb_req(_roj_b1), rp, caps, tc, _payload, _CB_ACTIVE)
        _d = L.derive_attachment_state_authorizing(tc, _payload, _CB_ACTIVE)
        _ok = (_r["eligible"] is False and _r["reason_codes"] == ["LOCATION_AUTHORITY_INVALID"]
               and _d["authorizing"] is False and _d["state"] == L.CONFLICT_STATE)
        _escalations.append((_label, _ok))
        rec("core-bypass", f"V113-B1 sections 12/19: no diagnostic->authority escalation via {_label}", _ok,
            f"eligible={_r['eligible']} authorizing={_d['authorizing']}")
    rec("core-bypass", f"the escalation matrix is complete and every route is refused ({sum(1 for _l, _o in _escalations if _o)}/{len(_escalations)})",
        all(_o for _l, _o in _escalations) and len(_escalations) >= 8)

    # ---- 20.5 / sections 15, 16, 17: relocation, symlink and testkit, all against the CORE
    rec("core-bypass", "V113-B1 section 15: a relocation attack cannot even be EXPRESSED against the core - the authorizing loader takes only (session_id, active) and derives the path itself, so identical bytes elsewhere are unreachable",
        sorted(inspect.signature(L.load_governed_evidence_set).parameters) == ["active", "session_id"]
        and sorted(inspect.signature(L.derive_attachment_state_for_session).parameters) == ["active", "session_id", "tc"])
    _sym_real = os.path.join(CB_ROOT, "sym-real")
    os.makedirs(_sym_real, mode=0o700, exist_ok=True)
    _sym_link = os.path.join(CB_ROOT, "sym-link")
    if not os.path.lexists(_sym_link):
        os.symlink(_sym_real, _sym_link)
    _reg = os.path.join(CB_ROOT, "regular-file")
    with open(_reg, "w", encoding="utf-8") as _f:
        _f.write("x")
    rec("core-bypass", "V113-B1 section 16: the core's single trust implementation refuses a symlinked entry, a non-directory and a wrong mode, and accepts only a real 0700 directory",
        L.governed_dir_errors(_sym_link, "ROOT")[0].startswith("ROOT_SYMLINK_REFUSED")
        and L.governed_dir_errors(_reg, "SESSION")[0].startswith("SESSION_NOT_A_DIRECTORY")
        and L.governed_dir_errors(_sym_real, "ROOT", want_mode=0o755)[0].startswith("ROOT_MODE_INVALID")
        and not L.governed_dir_errors(_sym_real, "ROOT", want_mode=0o700))
    rec("core-bypass", "the classification is lstat-FIRST: the symlink is refused as an entry, so nothing is resolved before it is trusted",
        (lambda src: src.index("_entry_lstat(p)") < src.index("S_ISLNK") < src.index("realpath"))(
            inspect.getsource(L.governed_dir_errors))
        and "lstat" in inspect.getsource(L._entry_lstat) and ".resolve(" not in inspect.getsource(L))
    with TK.sandbox(os.path.join(CB_ROOT, "gov-tk")):
        _tk_sid = _cb_sess("tk")
        os.makedirs(os.path.join(CB_ROOT, "lib-tk"), exist_ok=True)
        _tk_roj = _cb_author(_tk_sid, os.path.join(CB_ROOT, "lib-tk"))
        _tk_state = TK.derive_nonauthorizing(_tk_sid)["state"]
        _tk_es = AUTH_MOD.load_evidence_set(_tk_sid)
    rec("core-bypass", "V113-B1 section 17: the testkit reaches ATTACHMENT_READY on the DIAGNOSTIC path only; its own document cannot bootstrap the authorizing core or authorizing eligibility",
        _tk_state == "ATTACHMENT_READY"
        and L.derive_attachment_state_authorizing(tc, _tk_es, _CB_ACTIVE)["authorizing"] is False
        and L.evaluate_eligibility_authorizing(perms, _cb_req(_tk_roj), rp, caps, tc, _tk_es, _CB_ACTIVE)["eligible"] is False
        and TK.AUTHORITY_CLASS == "INTERNAL_NON_AUTHORIZING",
        f"testkit_diagnostic={_tk_state}")
    rec("core-bypass", "the testkit refuses to sandbox the frozen authority root, so no suite path can write evidence there",
        _try_auth(lambda: TK.sandbox(L.GOVERNED_ATTACHMENT_ROOT).__enter__()).code == "RAW_AssertionError"
    ...
    if os.path.exists(_ROOTP):
        ...
        try:
            ...
            _ctl_e = L.evaluate_eligibility_authorizing(perms, _cb_req(_ctl_roj), rp, caps, tc, _g, _CB_ACTIVE)
            _ctl_w = _try_auth(lambda: AUTH_MOD.derive_attachment_state(_ctl_sid))
            _ctl_ds = L.derive_attachment_state_for_session(_ctl_sid, tc, _CB_ACTIVE)
            _ctl_es_auth = L.evaluate_eligibility_for_session(_ctl_sid, perms, _cb_req(_ctl_roj), rp, caps, tc, _CB_ACTIVE)
            rec("core-positive", "V113-B1 sections 14/38 MANDATORY: at the REAL canonical governed root the AUTHORIZING core derivation returns ATTACHMENT_READY with authorizing=true, the minted location receipt and the canonical document path",
                _ctl_d["state"] == "ATTACHMENT_READY" and _ctl_d["authorizing"] is True
                and _ctl_d["location_receipt"] == _g.receipt
                and _ctl_d["document_path"] == L.canonical_document_path(_ctl_sid),
                f"{_ctl_d['state']} authorizing={_ctl_d['authorizing']}")
            rec("core-positive", "V113-B1 section 38 MANDATORY: authorizing eligibility then returns eligible=true for the M0A read-primitive probe request, so the correction is not merely 'refuse everything'",
                _ctl_e["eligible"] is True and _ctl_e["reason_codes"] == ["ELIGIBLE"]
                and _ctl_e["authorizing"] is True and _ctl_e["location_receipt"] == _g.receipt
                and _ctl_e["derived_attachment_state"] == "ATTACHMENT_READY",
                f"eligible={_ctl_e['eligible']} codes={_ctl_e['reason_codes']}")
            rec("core-positive", "the by-session-id convenience paths agree exactly with the by-object paths, because they are the same code with the loading done inside the core",
                _ctl_ds["state"] == _ctl_d["state"] and _ctl_ds["location_receipt"] == _g.receipt
                and _ctl_es_auth["eligible"] is True and _ctl_es_auth["reason_codes"] == ["ELIGIBLE"])
            rec("core-positive", "the authoring wrapper agrees with the core at the canonical root and returns the AUTHORIZING result, so v1.13's wrapper law is preserved rather than replaced",
                _ctl_w.code == "NONE" and _ctl_w.value["state"] == "ATTACHMENT_READY"
                and _ctl_w.value["authorizing"] is True and _ctl_w.value["location_receipt"] == _g.receipt,
                _ctl_w.code)
            _reloc = os.path.join(CB_ROOT, "reloc")
            shutil.copytree(L.canonical_session_dir(_ctl_sid), os.path.join(_reloc, _ctl_sid))
            _reloc_es = L.strict_load(os.path.join(_reloc, _ctl_sid, "EVIDENCE-SET.json"))
            rec("core-positive", "V113-B1 section 15: the SAME BYTES copied outside the governed root cannot authorize - content equivalence alone is not authority",
            ...
            # the governed document is sealed read-only at VERIFIED; an attacker with filesystem write access is
            # modelled by THREAT-MODEL.md, so the suite grants itself the same access before rewriting the bytes
            os.chmod(L.canonical_document_path(_ctl_sid), 0o600)
            with open(L.canonical_document_path(_ctl_sid), "r+b") as _f:
                _b = _f.read()
                _f.seek(0)
                _f.write(_b.replace(b'"schema"', b'"schema" ', 1) if b'"schema"' in _b else _b + b" ")
                _f.truncate()
            rec("core-positive", "V113-B1 section 20: possession of a previously valid GovernedEvidenceSet is not authority - the receipt is re-verified against the filesystem on every authorizing call, so a document changed underneath it is refused",
                L.derive_attachment_state_authorizing(tc, _stale, _CB_ACTIVE)["authorizing"] is False
                and any("LOCATION_RECEIPT_INVALID" in e for e in L.governed_provenance_errors(_stale, _CB_ACTIVE)),
                ";".join(L.governed_provenance_errors(_stale, _CB_ACTIVE))[:70])
    ...
    _CG = {"a2_prepare": inspect.getsource(PREP_CLI), "a2_verify": inspect.getsource(VER_CLI),
           "evidence_authoring": inspect.getsource(AUTH_MOD), "testkit": inspect.getsource(TK)}
    rec("core-callgraph", "V113-B1 section 18: no PRODUCTION module calls the diagnostic core derivation or the diagnostic eligibility; the authoring layer routes through the governed loader and the authorizing entry points",
        not re.search(r"L\.derive_attachment_state\(", _CG["evidence_authoring"])
        and not re.search(r"L\.evaluate_eligibility\(", _CG["evidence_authoring"])
        and "L.load_governed_evidence_set" in _CG["evidence_authoring"]
        and "L.derive_attachment_state_authorizing" in _CG["evidence_authoring"]
        and "L.evaluate_eligibility_authorizing" in _CG["evidence_authoring"])
    rec("core-callgraph", "the testkit is the ONLY module in the bundle that calls the DIAGNOSTIC core derivation, and it is INTERNAL_NON_AUTHORIZING; both production CLIs call the authoring wrapper instead, which is the authorizing path",
        "L.derive_attachment_state(" in _CG["testkit"]
        and TK.AUTHORITY_CLASS == "INTERNAL_NON_AUTHORIZING"
        and all(not re.search(r"L\.(?:derive_attachment_state|evaluate_eligibility)\(", _CG[m])
                for m in ("a2_prepare", "a2_verify", "evidence_authoring"))
        and all(re.search(r"A\.derive_attachment_state\(", _CG[m]) for m in ("a2_prepare", "a2_verify")),
        "CLIs -> A.derive_attachment_state -> L.derive_attachment_state_authorizing")
    rec("core-callgraph", "neither production CLI imports the testkit or can reach the diagnostic path through it",
        all(not re.search(r"^\s*(?:import|from)\s+evidence_authoring_testkit", _CG[m], re.M)
            for m in ("a2_prepare", "a2_verify", "evidence_authoring"))
        and not any(getattr(v, "__name__", "") == "evidence_authoring_testkit"
                    for m in (PREP_CLI, VER_CLI, AUTH_MOD) for v in vars(m).values()))
    _AST = ast.parse(_LS_CG)

    def _core_callers(target):
        """Every authority_lib function whose body contains a direct call to `target`, by AST, not by text: a
        docstring that merely names a function is not a call to it."""
        found = []
        for _n in ast.walk(_AST):
            if isinstance(_n, ast.FunctionDef) and any(
                    isinstance(_c, ast.Call) and isinstance(_c.func, ast.Name) and _c.func.id == target
                    for _c in ast.walk(_n)):
                found.append(_n.name)
        return sorted(set(found))

    rec("core-callgraph", "the in-core consumer graph of the DIAGNOSTIC derivation is exactly one hop (AST, not text): only the authorizing derivation and the diagnostic eligibility call it, and only semantic_mutation_plan - declared INTERNAL_NON_AUTHORIZING - calls that",
        _core_callers("derive_attachment_state") == ["derive_attachment_state_authorizing", "evaluate_eligibility"]
        and _core_callers("evaluate_eligibility") == ["evaluate_eligibility_authorizing", "semantic_mutation_plan"]
        and "semantic_mutation_plan" in L.AUTHORITY_SURFACE["internal_non_authorizing"],
        f'derive<-{_core_callers("derive_attachment_state")} elig<-{_core_callers("evaluate_eligibility")}')
    _VTS_IMPL = inspect.getsource(L._validate_transaction_set_impl)
    rec("core-callgraph", "V113-B1 section 18: semantic_mutation_plan is reached ONLY from the M3 composed path, which is PROVISIONAL_UNTIL_M3 and cannot yield M0A probe eligibility - it requires a mutation plan and a complete S0 snapshot before it runs at all, and an M0 read primitive has neither. Pre-M3 transaction authority is deliberately outside v1.14's scope and is recorded as a scoped residual in THREAT-MODEL.md.",
        _core_callers("semantic_mutation_plan") == ["_validate_transaction_set_impl"]
        and "semantic_mutation_plan(plan" in _VTS_IMPL
        and "linked set requires s0_snapshot and plan" in _VTS_IMPL
        and all(k in inspect.getsource(L.commit_eligibility) for k in ("s0", "plan", "journal"))
        and "READ_PRIMITIVE_QUALIFICATION_PROBE" not in inspect.getsource(L.commit_eligibility),
        ",".join(_core_callers("semantic_mutation_plan")))
    rec("core-callgraph", "the M0A probe request is not a mutation plan: fed to semantic_mutation_plan as one it produces errors and never an approval, so the M3 path is not an alternative route to probe eligibility",
        (lambda errs: bool(errs) and any("plan" in e for e in errs))(
            L.semantic_mutation_plan(dict(_cb_req(_roj_b1), authority_version=_CB_ACTIVE["authority_version"],
                                          operations=[], milestone="M0", scope="SCRATCH_QUALIFICATION_LIBRARY"),
                                     perms, rp, caps, tc, _es_b1, _CB_ACTIVE)))
    rec("core-callgraph", "the composed M3 path also refuses an empty linked set outright, so it cannot be entered with the objects an M0 read has",
        bool(L.validate_transaction_set({"s0_snapshot": None, "plan": None, "journal": None, "s1_snapshot": None,
                                         "verification": None, "commit": None, "conflicts": []},
                                        perms, rp, caps, tc, _es_b1, _CB_ACTIVE)))

    # ---- 20.8 / section 21: the self-location law, in the core
    rec("core-selflocation", "V113-B1 section 21: the core loader independently compares the document's own recorded location and session identity against the canonical path it actually loaded from, and requires the frozen authority root plus written_under_production_root",
        all(t in inspect.getsource(L._governed_document_claims) for t in
            ("SELF_LOCATION_MISMATCH", "authority_root", "written_under_production_root", "canonical_session_dir",
             "NOT_PRODUCTION_ROOT"))
        and "evidence_set_sha256" in inspect.getsource(L.load_governed_evidence_set)
        and "_governed_document_claims" in inspect.getsource(L.load_governed_evidence_set)
        and "_governed_document_claims" in inspect.getsource(L.governed_consume))
    rec("core-selflocation", "the loader reads the document ONLY from the canonical path it computed, never from a caller-supplied path",
        "canonical_document_path(session_id)" in inspect.getsource(L.load_governed_evidence_set)
        and "canonical_workflow_path(session_id)" in inspect.getsource(L.load_governed_evidence_set)
        and "read_governed_file(doc" in inspect.getsource(L.load_governed_evidence_set)
        and "read_governed_file(wfp" in inspect.getsource(L.load_governed_evidence_set)
        and "open(" not in inspect.getsource(L.load_governed_evidence_set))

    # ---- 20.9 / section 33: property tests, fixed seed
    _PRNG14 = random.Random(SEED + 114)
    _pbad = []
    for _i in range(24):
        _kind = _PRNG14.choice(("forbidden-root", "precomputed", "diagnostic-result", "raw-dict",
                                "symlink-root", "sibling-prefix"))
        _tag = f"p{_i:02d}"
        if _kind in ("forbidden-root", "precomputed", "diagnostic-result", "raw-dict"):
            _s, _e, _r, _w = _cb_sandbox_set(_tag)
            _payload = {"precomputed": "ATTACHMENT_READY",
                        "diagnostic-result": L.derive_attachment_state(tc, _e, _CB_ACTIVE),
                        "raw-dict": _e, "forbidden-root": _e}[_kind]
            if _w.code != "NOT_PRODUCTION_ROOT":
                _pbad.append(f"{_kind}:{_tag}:wrapper={_w.code}")
            if L.evaluate_eligibility_authorizing(perms, _cb_req(_r), rp, caps, tc, _payload, _CB_ACTIVE)["eligible"] is not False:
                _pbad.append(f"{_kind}:{_tag}:eligible")
            if L.derive_attachment_state_authorizing(tc, _payload, _CB_ACTIVE)["authorizing"] is not False:
                _pbad.append(f"{_kind}:{_tag}:authorizing")
        elif _kind == "symlink-root":
            _real = os.path.join(CB_ROOT, "pr-" + _tag)
            os.makedirs(_real, mode=0o700, exist_ok=True)
            _lk = os.path.join(CB_ROOT, "pl-" + _tag)
            if not os.path.lexists(_lk):
                os.symlink(_real, _lk)
            if not L.governed_dir_errors(_lk, "ROOT")[0].startswith("ROOT_SYMLINK_REFUSED"):
                _pbad.append(f"{_kind}:{_tag}")
        else:
            if (L.authority_attachment_root() != L.GOVERNED_ATTACHMENT_ROOT
                    or not L.canonical_session_dir(_tag).startswith(L.GOVERNED_ATTACHMENT_ROOT + os.sep)
                    or L.canonical_session_dir(_tag).startswith(L.GOVERNED_ATTACHMENT_ROOT + "-")):
                _pbad.append(f"{_kind}:{_tag}")
    rec("core-property", f"24 seeded forbidden-root, precomputed-state, diagnostic-result, raw-dict, symlinked-root and sibling-prefix cases: not one yields authorizing readiness or eligible=true (seed {SEED + 114})",
        not _pbad, ";".join(_pbad[:3]) or "all refused")
    _sbad = []
    for _i in range(40):
        _cand = "".join(_PRNG14.choice("ab09._-/\\ ~:\x00") for _ in range(_PRNG14.randint(0, 6)))
        _core_ok = not L.session_id_errors(_cand)
        _auth_ok = _try_auth(lambda c=_cand: AUTH_MOD.set_dir(c)).code == "NONE"
        if _core_ok != _auth_ok:
            _sbad.append(f"{_cand!r} core={_core_ok} authoring={_auth_ok}")
        if _core_ok and (os.sep in _cand or ".." in _cand or _cand.startswith(".") or "\x00" in _cand):
            _sbad.append(f"traversal accepted {_cand!r}")
    rec("core-property", f"40 seeded session ids: the CORE law and the authoring layer agree exactly, because after v1.14 there is ONE implementation, and no traversal form is accepted (seed {SEED + 114})",
        not _sbad, ";".join(_sbad[:3]) or "consistent")

    # ---- 20.10 / section 32: static audits
    _LS = inspect.getsource(L)
    rec("core-static", "section 32: authorizing eligibility cannot accept a raw or precomputed attachment state - it has no state parameter and recomputes from the governed document",
        "attachment_state" not in inspect.signature(L.evaluate_eligibility_authorizing).parameters
        and sorted(inspect.signature(L.evaluate_eligibility_authorizing).parameters) ==
        ["active", "caps", "governed", "perms", "request", "rp", "tc"]
        and "governed_consume(governed, active)" in inspect.getsource(L.evaluate_eligibility_authorizing)
        and "governed.evidence_set" not in inspect.getsource(L.evaluate_eligibility_authorizing))
    rec("core-static", "section 32: a production caller cannot invoke the diagnostic derivation, and both diagnostic functions declare NON-AUTHORIZING and name V113-B1 in their own docstrings",
        "NON-AUTHORIZING" in (L.derive_attachment_state.__doc__ or "")
        and "NON-AUTHORIZING" in (L.evaluate_eligibility.__doc__ or "")
        and "V113-B1" in (L.derive_attachment_state.__doc__ or "")
        and "V113-B1" in (L.evaluate_eligibility.__doc__ or ""))
    rec("core-static", "section 32: the authorizing derivation checks the canonical location itself and does not delegate that to a wrapper",
        all(t in inspect.getsource(L.derive_attachment_state_authorizing) for t in
            ("governed_consume", "authorizing", "LOCATION_AUTHORITY_INVALID"))
        and all(t in inspect.getsource(L.governed_consume) for t in
                ("location_errors", "canonical_document_path", "canonical_workflow_path", "location_receipt",
                 "GOVERNED_ATTACHMENT_ROOT", "governed_structure_errors", "read_governed_file"))
        and "governed_consume" in inspect.getsource(L.governed_provenance_errors))
    rec("core-static", "section 32: there is exactly ONE governed-root resolver and ONE directory-trust implementation, both in the core, and the authoring layer delegates to them rather than restating them",
        len(re.findall(r"^def authority_attachment_root", _LS, re.M)) == 1
        and len(re.findall(r"^def governed_dir_errors", _LS, re.M)) == 1
        and len(re.findall(r"^def session_id_errors", _LS, re.M)) == 1
        and "L.governed_dir_errors" in inspect.getsource(AUTH_MOD._require_trusted_dir)
        and "L.session_id_errors" in inspect.getsource(AUTH_MOD._session_id)
        and "L.canonical_session_dir" in inspect.getsource(AUTH_MOD.canonical_set_dir)
        and AUTH_MOD.NAME_SAFE is L.SESSION_ID_RE)
    rec("core-static", "section 32: the authoring layer no longer carries its own realpath alias check, so root trust cannot drift between the two layers",
        "realpath" not in inspect.getsource(AUTH_MOD.require_governed_root)
        and "realpath" in inspect.getsource(L.governed_dir_errors))
    rec("core-static", "section 32: the CORE never imports the authoring layer or the testkit, so location authority cannot be armed from outside it",
        not re.search(r"^\s*(?:import|from)\s+evidence_authoring", _LS, re.M)
        and not any(getattr(v, "__name__", "").startswith("evidence_authoring") for v in vars(L).values()))
    rec("core-static", "section 32: fixture and testkit helpers are not on the authority surface",
        not any(f in L.AUTHORITY_SURFACE["authorizing"]
                for f in ("sandbox", "derive_nonauthorizing", "fixture_evidence", "make_fixture")))
    rec("core-static", "the location refusal vocabulary is frozen and closed, and names the four codes v1.14 introduces",
        len(L.LOCATION_ERRORS) >= 16 and len(set(L.LOCATION_ERRORS)) == len(L.LOCATION_ERRORS)
        and all(c in L.LOCATION_ERRORS for c in ("LOCATION_PROVENANCE_MISSING", "LOCATION_RECEIPT_INVALID",
                                                 "NOT_PRODUCTION_ROOT", "SELF_LOCATION_MISMATCH")))
    rec("core-static", "EVIDENCE-SET-WORKFLOW.json publishes the v1.14 core_authority block: the only door, the provenance object, the four authorizing entry points, the two demoted ones, the receipt and the fail-closed law",
        (lambda ca: ca["only_door"].startswith("tools/authority_lib.py#load_governed_evidence_set")
                    and sorted(ca["authorizing_entrypoints"]) == ["commit_eligibility_authorizing", "derive_attachment_state_authorizing", "derive_attachment_state_for_session", "evaluate_eligibility_authorizing", "evaluate_eligibility_for_session", "validate_transaction_set_authorizing"]
                    and ca["diagnostic_non_authorizing"] == list(L.AUTHORITY_SURFACE["diagnostic_non_authorizing"])
                    and ca["receipt"]["domain"] == L.LOCATION_RECEIPT_DOMAIN
                    and ca["refusal_codes"] == list(L.LOCATION_ERRORS) + ["LOCATION_AUTHORITY_INVALID"]
                    and "LOCATION_AUTHORITY_INVALID" in ca["fail_closed"]
                    and "GovernedEvidenceSet" in ca["provenance_object"]
                    and "DEFENCE IN DEPTH" in ca["one_implementation"]
                    and "V114-B2" in ca["residual"])(WFLAW["core_authority"]))
    rec("core-static", "the workflow law document publishes the v1.15 consumed-evidence, authority-file, malformed-carrier and transaction-authority laws (V114-B1/B2/B4/M1)",
        (lambda ca: "THE EXACT BYTES VALIDATED AS GOVERNED EVIDENCE ARE THE EXACT BYTES CONSUMED" in ca["consumed_evidence_law"]
                    and ca["snapshot_domain"] == L.EVIDENCE_SNAPSHOT_DOMAIN
                    and ca["authority_files"] == [L.EVIDENCE_SET_FILE_NAME, L.WORKFLOW_FILE_NAME]
                    and all(t in ca["authority_file_law"] for t in ("lstat", "O_NOFOLLOW", "fstat", "SYMLINK_REFUSED"))
                    and "GOVERNED_EVIDENCE_INVALID" in ca["malformed_carrier_law"]
                    and "object.__new__" in ca["malformed_carrier_law"]
                    and "programming-error distinction is preserved" in ca["malformed_carrier_law"]
                    and ca["provisional_until_m3_non_authorizing"] == list(L.AUTHORITY_SURFACE["provisional_until_m3_non_authorizing"])
                    and "evaluate_eligibility_authorizing" in ca["transaction_authority_law"]
                    and "BY TYPE" in ca["transaction_authority_law"]
                    and "ONE evidence authority input" in ca["one_evidence_source"])(WFLAW["core_authority"]))
    rec("core-static", "the published core_authority precedence matches the code: location authority, then attachment semantics, then eligibility",
        "LOCATION AUTHORITY -> ATTACHMENT SEMANTICS -> ELIGIBILITY" in WFLAW["core_authority"]["law"]
        and "recomputed from the live filesystem" in WFLAW["core_authority"]["receipt"]["law"]
        # v1.16 (V115-M1): compared against the RUNTIME publication, never a hardcoded list. A hardcoded list in
        # this very check is what let the published seven-field binding pass while the runtime bound ten.
        and WFLAW["core_authority"]["receipt"]["binds"] == list(L.RECEIPT_BOUND_FIELDS))
    rec("core-static", "M0A-BINDING-VALUES.json publishes the v1.14 authorizing entry points, the governed loader, the receipt domain, the refusal vocabulary and the harness update requirement, with no placeholder",
        (lambda v: all(k in v for k in ("authorizing_state_derivation", "authorizing_eligibility", "governed_evidence_set_loader",
                                        "diagnostic_non_authorizing_functions", "location_receipt_domain", "location_receipt_binds",
                                        "location_provenance_law", "location_refusal_codes", "authority_surface_classes",
                                        "operational_package_update_required_v114"))
                     and v["location_receipt_domain"] == L.LOCATION_RECEIPT_DOMAIN
                     and v["authority_surface_classes"] == ", ".join(sorted(L.AUTHORITY_SURFACE))
                     and "load_governed_evidence_set" in v["governed_evidence_set_loader"]
                     and "DIAGNOSTIC" in v["diagnostic_non_authorizing_functions"]
                     and "LOCATION_AUTHORITY_INVALID" in v["operational_package_update_required_v114"]
                     and "load_governed_evidence_set" in v["state_derivation_entrypoint"]
                     and all("PLACEHOLDER" not in str(x).upper() for x in v.values()))(
            L.strict_load(os.path.join(B, "M0A-BINDING-VALUES.json"))["values"]))
    rec("core-static", "V113-B1 MINOR fixed: the stale test_root_override text claiming the authoring API takes a root parameter is gone from EVIDENCE-SET-WORKFLOW.json, and what remains matches the code",
        "test_root_override" not in json.dumps(WFLAW)
        and "root parameter" not in json.dumps(WFLAW)
        and WFLAW["persistence"]["caller_selectable"] is False
        and "INTERNAL_NON_AUTHORIZING" in WFLAW["persistence"]["test_override"])
finally:
    AUTH_MOD._SANDBOX_ROOT = None
    for _dp, _dns, _fns in os.walk(CB_ROOT):
        for _x in _dns + _fns:
            try:
                os.chmod(os.path.join(_dp, _x), 0o700)
            except OSError:
...
# ================================================================ 21. v1.15: Codex's six v1.14 release findings
# V114-B1 consumed evidence not bound to the validated governed bytes (BLOCKER)
# V114-B2 diagnostic eligibility reaching commit authority          (BLOCKER)
# V114-B3 active authority artifacts naming diagnostic entrypoints  (BLOCKER)
# V114-B4 governed loader following a symlinked WORKFLOW.json       (BLOCKER)
# V114-M1 malformed nominal carrier leaking a raw AttributeError    (MERGE MAJOR)
# V114-N1 canonicalization documentation omitting a hash domain     (MINOR)
V15_ROOT = tempfile.mkdtemp(prefix="resolve-v115-")
_V15_ACTIVE = AUTH_MOD.active_authority()
_V15_SEQ = [0]


class _V15Timeout(Exception):
    pass


@contextlib.contextmanager
def _v15_alarm(sec=4):
    """Bound every call that touches an authority FILE ENTRY. Under v1.14 a FIFO at WORKFLOW.json blocked open()
    forever, so 'refuses' and 'hangs the authority' had to be distinguishable rather than both looking like silence."""
    def _fire(*_a):
        raise _V15Timeout(f"blocked >{sec}s")
...
def _v15_call(fn, sec=4):
    """(outcome, value) where outcome is ok | refused | BLOCKED | RAW <type>. A raw exception is never a pass."""
    try:
        with _v15_alarm(sec):
            return "ok", fn()
    except _V15Timeout as e:
        return "BLOCKED", str(e)
    except L.AuthorityTrustError as e:
        return "refused", str(e)
    except AUTH_MOD.AuthoringError as e:
        return "refused", f"{e.code}: {e.detail}"
    except Exception as e:                                    # noqa: BLE001
        return "RAW " + type(e).__name__, str(e)
...
def _v15_author(sid, libroot, verify=True):
    """Drive the PRODUCTION authoring path to VERIFIED (or PREPARED) at the real canonical location."""
    AUTH_MOD.create_evidence_set(sid, _PREP)
    AUTH_MOD.add_provisioning_record(sid, _PREP, instance_uuid=_UU, root_path=libroot,
                                     provisioned_by="Mikko (operator)")
    AUTH_MOD.add_launch_recipe(sid, _PREP, recipe_sha256=_LAUNCH_SHA)
...
try:
    # ================================================================ V114-M1: malformed carriers fail closed
    class _V15Sub(L.GovernedEvidenceSet):
        pass

    _mal = [("object.__new__(GovernedEvidenceSet)", lambda: object.__new__(L.GovernedEvidenceSet)),
            ("object.__new__(subclass)", lambda: object.__new__(_V15Sub)),
            ("a bare dict", lambda: {}),
            ("None", lambda: None),
            ("a bare ATTACHMENT_READY string", lambda: "ATTACHMENT_READY"),
            ("a namespace object with every field name", lambda: type("Fake", (), {
                f: "x" for f in ("document_bytes", "session_id", "document_path", "workflow_path", "governed_root",
                                 "authority_version", "manifest_sha256", "document_sha256", "workflow_sha256",
                                 "evidence_snapshot_digest", "receipt")})()),
            ("a nominal instance with SOME slots set", lambda: (lambda g: (
                object.__setattr__(g, "session_id", "s1"), object.__setattr__(g, "receipt", "0" * 64), g)[-1])(
                object.__new__(L.GovernedEvidenceSet))),
            ("a nominal instance with a None slot", lambda: (lambda g: [
                object.__setattr__(g, f, None) for f in L.GovernedEvidenceSet.__slots__] and g or g)(
                object.__new__(L.GovernedEvidenceSet))),
            ("a nominal instance with wrong-typed slots", lambda: (lambda g: [
                object.__setattr__(g, f, 7) for f in L.GovernedEvidenceSet.__slots__] and g or g)(
                object.__new__(L.GovernedEvidenceSet))),
            ("a nominal instance with a malformed receipt", lambda: (lambda g: [
                object.__setattr__(g, f, "not-a-digest") for f in L.GovernedEvidenceSet.__slots__] and g or g)(
                object.__new__(L.GovernedEvidenceSet)))]
    _mal_bad = []
    for _label, _mk in _mal:
        _obj = _mk()
        _o1, _v1 = _v15_call(lambda o=_obj: L.derive_attachment_state_authorizing(tc, o, _V15_ACTIVE))
        _o2, _v2 = _v15_call(lambda o=_obj: L.evaluate_eligibility_authorizing(
            perms, {"milestone": "M0", "operation": "READ_PRIMITIVE_QUALIFICATION_PROBE",
                    "scope": "SCRATCH_QUALIFICATION_LIBRARY", "refs": {}}, rp, caps, tc, o, _V15_ACTIVE))
        _o3, _v3 = _v15_call(lambda o=_obj: L.commit_eligibility_authorizing({}, perms, rp, caps, tc, o, _V15_ACTIVE))
        _ok = (_o1 == "ok" and _v1["authorizing"] is False and _v1["state"] == L.CONFLICT_STATE
               and _o2 == "ok" and _v2["eligible"] is False and _v2["authorizing"] is False
               and _o3 == "ok" and _v3["eligible"] is False and _v3["authorizing"] is False)
        if not _ok:
            _mal_bad.append(f"{_label}: {_o1}/{_o2}/{_o3}")
        rec("m1-malformed-carrier", f"V114-M1: {_label} fails closed at all three public authorizing boundaries, with no raw Python exception",
            _ok, f"derive={_o1} eligibility={_o2} commit={_o3}")
    rec("m1-malformed-carrier", f"V114-M1 PINNED: the exact Codex reproduction. object.__new__(GovernedEvidenceSet) passes isinstance and has no slots set; in v1.14 provenance validation dereferenced .session_id and leaked a raw AttributeError out of BOTH public boundaries. It is now GOVERNED_EVIDENCE_INVALID.",
        (lambda g: (lambda d, e: d["authorizing"] is False and e["eligible"] is False
                    and any("GOVERNED_EVIDENCE_INVALID" in x for x in d["failures"])
                    and any("GOVERNED_EVIDENCE_INVALID" in x for x in e["errors"]))(
            L.derive_attachment_state_authorizing(tc, g, _V15_ACTIVE),
            L.evaluate_eligibility_authorizing(perms, {"milestone": "M0", "operation": "READ_PRIMITIVE_QUALIFICATION_PROBE",
                                                       "scope": "SCRATCH_QUALIFICATION_LIBRARY", "refs": {}},
                                               rp, caps, tc, g, _V15_ACTIVE)))(
            object.__new__(L.GovernedEvidenceSet)))
    rec("m1-malformed-carrier", f"the malformed-carrier matrix is complete and every case fails closed ({len(_mal)} cases)",
        not _mal_bad and len(_mal) >= 10, ";".join(_mal_bad[:2]) or "all fail closed")
    rec("m1-malformed-carrier", "V114-M1 section 31: genuine PROGRAMMING errors are NOT masked - only malformed external authority input is normalised, so the v1.10 programming-error distinction survives",
        (lambda: (_v15_call(lambda: L.derive_attachment_state_authorizing(tc, object.__new__(L.GovernedEvidenceSet), _V15_ACTIVE))[0] == "ok"
                  and _v15_call(lambda: L.evidence_snapshot_digest(object()))[0].startswith("RAW")
                  and _v15_call(lambda: L.governed_structure_errors())[0] == "RAW TypeError"))(),
        "malformed input normalised; a wrong-arity call or an unserialisable value still raises")
    rec("m1-malformed-carrier", "the structural validator is called FIRST inside governed_consume, before any field is dereferenced",
        (lambda src: src.index("governed_structure_errors(governed)") < src.index("governed.session_id"))(
            inspect.getsource(L.governed_consume)))
    rec("m1-malformed-carrier", "GOVERNED_EVIDENCE_INVALID is in the frozen location vocabulary, and so are the other three v1.15 codes",
        all(c in L.LOCATION_ERRORS for c in ("GOVERNED_EVIDENCE_INVALID", "GOVERNED_EVIDENCE_REQUIRED",
                                             "EVIDENCE_SNAPSHOT_MISMATCH", "WORKFLOW_SYMLINK_REFUSED")))

    # ================================================================ V114-B3: active authority naming
    # Every CURRENT top-level artifact, machine or prose: the precedence table lists the prose documents and a few
    # JSONs, but TARGET-CONTRACT.json and PERMISSIONS.json - two of the four artifacts Codex found - are not in it,
    # so scanning only that table would have missed exactly the defect. Historical changelogs/matrices and anything
    # the precedence table marks HISTORICAL are excluded, as is the generated report.
    _PREC_STATUS = {e["path"]: e["status"] for e in prec["documents"]}
    _ACTIVE_DOCS = sorted(
        os.path.relpath(p_, B) for p_ in glob.glob(os.path.join(B, "*.json")) + glob.glob(os.path.join(B, "*.md"))
        if _PREC_STATUS.get(os.path.relpath(p_, B), "STILL_ACTIVE") == "STILL_ACTIVE"
        and not re.match(r"(CHANGELOG|FINDING-RESOLUTION-MATRIX)-v1\.", os.path.basename(p_))
        and os.path.basename(p_) not in ("VALIDATION-REPORT.md", "FINDING-RESOLUTION-MATRIX.md"))
    _DIAG_NAMES = tuple(L.AUTHORITY_SURFACE["diagnostic_non_authorizing"]) + \
        tuple(L.AUTHORITY_SURFACE["provisional_until_m3_non_authorizing"])
    _AUTH_NAMES = ("derive_attachment_state_authorizing", "derive_attachment_state_for_session",
                   "evaluate_eligibility_authorizing", "evaluate_eligibility_for_session",
                   "validate_transaction_set_authorizing", "commit_eligibility_authorizing",
                   "load_governed_evidence_set")
    _DIAG_MARKERS = ("DIAGNOSTIC", "diagnostic", "NON_AUTHORIZING", "NON-AUTHORIZING", "non-authorizing",
                     "PROVISIONAL_UNTIL_M3", "demoted", "DEMOTED", "historical", "HISTORICAL", "SUPERSEDED",
                     "superseded", "v1.13", "v1.14", "V113-B1", "V114-B", "no longer", "not authority",
                     "never be selected", "authorizing counterpart", "of its era", "reads as its",
                     "then named", "v1.15", "PROVISIONAL_UNTIL_M3", "counterpart")


    def _diag_occurrences(path):
        """Every occurrence of a diagnostic/provisional function name in a CURRENT artifact whose own line does not
        mark it as diagnostic. V114-B3: a current artifact may NAME a demoted function, but only AS demoted.

        AUTHORITY-PRECEDENCE.json is handled structurally rather than by line: its superseded_statements record what
        the authority WAS at each earlier version, and a v1.3 or v1.12 statement naming the then-current function is
        history, not a current designation. Only statements of the CURRENT era (>= 1.14.0, when the demotion
        happened) are held to the naming law, and the document must carry the blanket clause that re-reads every
        earlier statement's function names as their authorizing counterparts."""
        if path.endswith(".json") and path != "AUTHORITY-PRECEDENCE.json":
            # Judge machine artifacts by KEY PATH, not by line: a bare function name inside a field literally called
            # diagnostic_non_authorizing, or inside any value whose sentence marks it, is not a current designation.
            out = []

            def _walk(node, keypath):
                if isinstance(node, dict):
                    for k__, v__ in node.items():
                        _walk(v__, keypath + (str(k__),))
                elif isinstance(node, list):
                    for i__, v__ in enumerate(node):
                        _walk(v__, keypath + (f"[{i__}]",))
                elif isinstance(node, str):
                    # v1.16 (V115-B1): the invariant is EXHAUSTIVE. v1.15 exempted whole key names - including
                    # `law`, `note`, `qualification_note` and `reason` - and Codex found the surviving contradiction
                    # in exactly two of those exempted fields (TARGET-CONTRACT.json#qualification_note and
                    # #attachment_law). An exclusion list is not an invariant. The ONLY exemptions now are key paths
                    # whose own name declares the value to be a classification statement about a demoted function
                    # (a diagnostic/provisional inventory) or a record of history - and each of those is separately
                    # asserted to match the canonical map, so nothing is taken on trust.
                    _kp = " ".join(keypath).lower()
                    if any(t in _kp for t in ("diagnostic_non_authorizing", "provisional_until_m3_non_authorizing",
                                              "diagnostic_slots", "inherited_matrices", "codex_finding",
                                              "prior_defect", "operational_closure")):
                        return
                    for m__ in re.finditer(r"(?<![\w.])(" + "|".join(_DIAG_NAMES) + r")(?!_authorizing|_for_session|_diagnostic)(?![\w])", node):
                        # the sentence containing THIS occurrence, chosen by offset: a substring test would match an
                        # earlier sentence that merely contains the *_authorizing form of the same stem.
                        seg, at = node, 0
                        for x in re.split(r"(?<=[.;]) ", node):
                            if at <= m__.start() < at + len(x) + 1:
                                seg = x
                                break
                            at += len(x) + 1
                        if not any(k__ in seg for k__ in _DIAG_MARKERS):
                            out.append(f"{path}:{'.'.join(keypath)}:{m__.group(1)}")

            _walk(L.strict_load(os.path.join(B, path)), ())
            return out
        if path == "AUTHORITY-PRECEDENCE.json":
            doc = L.strict_load(os.path.join(B, path))
            out = []
            if "supersede" not in json.dumps(doc.get("law", "")).lower() and "S62" not in json.dumps(doc):
                out.append(f"{path}: no blanket clause re-reading earlier statements' function names")
            for st__ in doc["superseded_statements"]:
                if st__.get("effective_version", "0") < "1.14.0":
                    continue
                blob = json.dumps(st__)
                for m__ in re.finditer(r"(?<![\w.])(" + "|".join(_DIAG_NAMES) + r")(?!_authorizing|_for_session|_diagnostic)(?![\w])", blob):
                    if not any(k in blob for k in _DIAG_MARKERS):
                        out.append(f"{path}:{st__['id']}:{m__.group(1)}")
            return out
        txt = open(os.path.join(B, path), encoding="utf-8").read()
        out = []
        for m_ in re.finditer(r"(?<![\w.])(" + "|".join(_DIAG_NAMES) + r")(?!_authorizing|_for_session|_diagnostic)(?![\w])", txt):
            start = txt.rfind("\n", 0, m_.start()) + 1
            end = txt.find("\n", m_.end())
            line = txt[start:end if end != -1 else len(txt)]
            if not any(k in line for k in _DIAG_MARKERS):
                out.append(f"{path}:{txt[:m_.start()].count(chr(10)) + 1}:{m_.group(1)}")
        return out


    # v1.17 (V116-B1): this regression is now evaluated by tools/authority_slots.py - clause-local, structured,
    # module-aware, with the artifact set derived from precedence status and role. The v1.16 substring scanner is
    # retained below only as the subject of the "its exemption list is gone" audit.
    _b3 = [f'{v["artifact"]}#{v["location"]}:{v["function"]}:{v["violation"]}' for v in SLOTS.violations(SLOTS.references(B))]
    rec("b3-active-naming", "V114-B3 PINNED: no ACTIVE authority artifact names a diagnostic or provisional-until-M3 function except explicitly AS diagnostic. In v1.14 TARGET-CONTRACT.json, PERMISSIONS.json, TARGET-ATTACHMENT-GATE.md and ELIGIBILITY.md designated the raw derivation and the raw eligibility as current authority, contradicting the code registry.",
        not _b3, ";".join(_b3[:4]) or f"{len(_ACTIVE_DOCS)} active documents clean")
    rec("b3-active-naming", "the four artifacts Codex named now designate the AUTHORIZING entry points",
        (lambda t_, p_, g_, e_: "derive_attachment_state_authorizing" in t_["attachment_derivation"]
                                and "GovernedEvidenceSet" in t_["attachment_derivation"]
                                and "DIAGNOSTIC / NON_AUTHORIZING" in t_["attachment_derivation_diagnostic_non_authorizing"]
                                and "evaluate_eligibility_authorizing" in p_["law"]
                                and "DIAGNOSTIC / NON_AUTHORIZING" in p_["law"]
                                and "derive_attachment_state_authorizing" in g_
                                and "DIAGNOSTIC / NON_AUTHORIZING" in g_
                                and "evaluate_eligibility_authorizing" in e_
                                and "commit_eligibility_authorizing" in e_)(
            tc, perms, open(os.path.join(B, "TARGET-ATTACHMENT-GATE.md"), encoding="utf-8").read(),
            open(os.path.join(B, "ELIGIBILITY.md"), encoding="utf-8").read()))
    rec("b3-active-naming", "V114-B3 section 21: every function name an active artifact designates as authority IS on the authorizing surface, and no diagnostic name occupies an authorizing binding field",
        all(n in L.AUTHORITY_SURFACE["authorizing"] for n in _AUTH_NAMES)
        and all(n not in L.AUTHORITY_SURFACE["authorizing"] for n in _DIAG_NAMES)
        and not [k for k, v_ in tc.items() if isinstance(v_, str) and k == "attachment_derivation"
                 and re.search(r"(?<![\w.])derive_attachment_state(?![\w])", v_)])
    rec("b3-active-naming", "V114-B3 section 20: precedence is explicit - the code registry and the active documents now agree, and the workflow law and M0A bindings publish the same four surface classes",
        sorted(WFLAW["core_authority"]["diagnostic_non_authorizing"]) == sorted(L.AUTHORITY_SURFACE["diagnostic_non_authorizing"])
        and sorted(WFLAW["core_authority"]["provisional_until_m3_non_authorizing"]) == sorted(L.AUTHORITY_SURFACE["provisional_until_m3_non_authorizing"])
        and L.strict_load(os.path.join(B, "M0A-BINDING-VALUES.json"))["values"]["authority_surface_classes"] ==
        ", ".join(sorted(L.AUTHORITY_SURFACE)))
    rec("b3-active-naming", f"the audit actually scanned every CURRENT artifact, machine and prose, not a sample ({len(_ACTIVE_DOCS)} documents)",
        len(_ACTIVE_DOCS) >= 50
        and all(d_ in _ACTIVE_DOCS for d_ in ("TARGET-CONTRACT.json", "PERMISSIONS.json", "ELIGIBILITY.md",
                                              "TARGET-ATTACHMENT-GATE.md", "EVIDENCE-SET-WORKFLOW.json",
                                              "M0A-BINDING-VALUES.json", "AUTHORITY-PRECEDENCE.json",
                                              "AUTHORITY-PRECEDENCE.md", "SEMANTIC-VALIDATION.md")),
        f"{len(_ACTIVE_DOCS)} scanned")

    # ================================================================ V114-N1: hash-domain documentation parity
    _CANON = open(os.path.join(B, "CANONICALIZATION.md"), encoding="utf-8").read()
    _doc_domains = {d for d in re.findall(r"vidtoolz\.resolve[A-Za-z0-9]+\.v[0-9.]*[0-9]", _CANON)}
    rec("hash-domain-parity", "V114-N1 PINNED: every hash domain the authorizing code registers is documented in CANONICALIZATION.md. In v1.14 the closed list omitted vidtoolz.resolveLocationReceipt.v1 and recorded two evidence-store domains at superseded versions.",
        not (set(L.HASH_DOMAINS) - _doc_domains),
        ",".join(sorted(set(L.HASH_DOMAINS) - _doc_domains)) or f"{len(L.HASH_DOMAINS)} domains documented")
    rec("hash-domain-parity", "and the reverse: the documented closed list names no domain the code does not register (the codec domain is documented as a codec, not a digest domain)",
        not (_doc_domains - set(L.HASH_DOMAINS) - {L.CODEC}),
        ",".join(sorted(_doc_domains - set(L.HASH_DOMAINS) - {L.CODEC})) or "exact")
    rec("hash-domain-parity", "V114-N1 section 34: the parity assertion itself is published in the document, so the omission cannot recur silently",
        "hash-domain-parity" in _CANON and "closed list" in _CANON and "V114-N1" in _CANON)
    rec("hash-domain-parity", "the two v1.14/v1.15 governed-location domains are documented with what they bind",
        all(d in _CANON for d in (L.LOCATION_RECEIPT_DOMAIN, L.EVIDENCE_SNAPSHOT_DOMAIN))
        and "parsed-snapshot digest" in _CANON and "PARSED semantic content" in _CANON)
    rec("hash-domain-parity", "the receipt and the snapshot are DIFFERENT digests of the same document, and neither is the record-id domain",
        L.LOCATION_RECEIPT_DOMAIN != L.EVIDENCE_SNAPSHOT_DOMAIN
        and L.evidence_snapshot_digest({"a": 1}) != L.digest({"a": 1}, L.ES_DOMAIN)
        and L.evidence_snapshot_digest({"a": 1}) != L.evidence_snapshot_digest({"a": 2}))

    # ================================================================ V114-B2: diagnostic eligibility vs commit
    _TS_OK = LINKED["linked-set-committed-consistent"]["set"]
    _ES_FULL = EVS["write-ready-full"]
    _b2_stages = []
    _b2_dem_errs = L.validate_transaction_set(_TS_OK, perms, rp, hyp, tc, _ES_FULL, ACTIVE_HYP, _stages=_b2_stages)
    _b2_dem_ce = L.commit_eligibility(_TS_OK, perms, rp, hyp, tc, _ES_FULL, ACTIVE_HYP)
    rec("b2-transaction-authority", "control: the exact Codex reproduction still behaves as it did - the complete committed-consistent fixture supplied as a BARE evidence dict completes all twelve stages and the composed commit says eligible=true. That is why v1.14's classification of these two functions as AUTHORIZING was the defect.",
        not _b2_dem_errs and _b2_dem_ce["eligible"] is True and _b2_stages == list(L.VALIDATION_STAGES),
        f"stages={len(_b2_stages)}/12 composed_eligible={_b2_dem_ce['eligible']}")
    rec("b2-transaction-authority", "V114-B2 PINNED: those two functions are NO LONGER authorizing. They are PROVISIONAL_UNTIL_M3 / NON-AUTHORIZING on the surface and say so in their own docstrings.",
        sorted(L.AUTHORITY_SURFACE["provisional_until_m3_non_authorizing"]) == ["commit_eligibility", "validate_transaction_set"]
        and "commit_eligibility" not in L.AUTHORITY_SURFACE["authorizing"]
        and "validate_transaction_set" not in L.AUTHORITY_SURFACE["authorizing"]
        and "NON-AUTHORIZING" in (L.validate_transaction_set.__doc__ or "")
        and "NON-AUTHORIZING" in (L.commit_eligibility.__doc__ or "")
        and "V114-B2" in (L.validate_transaction_set.__doc__ or "")
        and "V114-B2" in (L.commit_eligibility.__doc__ or ""))
    _b2_auth_errs = L.validate_transaction_set_authorizing(_TS_OK, perms, rp, hyp, tc, _ES_FULL, ACTIVE_HYP)
    _b2_auth_ce = L.commit_eligibility_authorizing(_TS_OK, perms, rp, hyp, tc, _ES_FULL, ACTIVE_HYP)
    rec("b2-transaction-authority", "V114-B2 PINNED: the AUTHORIZING transaction and commit entry points refuse the same bare dict with GOVERNED_EVIDENCE_REQUIRED, before any stage runs",
        _b2_auth_errs and "GOVERNED_EVIDENCE_REQUIRED" in _b2_auth_errs[0]
        and _b2_auth_ce["eligible"] is False and _b2_auth_ce["authorizing"] is False
        and _b2_auth_ce["stages_completed"] == []
        and any("GOVERNED_EVIDENCE_REQUIRED" in e for e in _b2_auth_ce["errors"]),
        f"{_b2_auth_errs[:1]}")
    _b2_poison = []
    for _label, _payload in (("the diagnostic eligibility result", L.evaluate_eligibility(
                                  perms, {"milestone": "M3", "operation": "APPEND_TIMELINE_ITEM",
                                          "scope": "SCRATCH_QUALIFICATION_LIBRARY",
                                          "refs": _TS_OK["plan"].get("refs") or {}}, rp, hyp, tc, _ES_FULL, ACTIVE_HYP)),
                             ("the diagnostic derivation result", L.derive_attachment_state(tc, _ES_FULL, ACTIVE_HYP)),
                             ("a bare eligible=true dict", {"eligible": True, "permitted_by_policy": True}),
                             ("the raw evidence dict", _ES_FULL),
                             ("the diagnostic transaction result", L.validate_transaction_set_diagnostic(
                                  _TS_OK, perms, rp, hyp, tc, _ES_FULL, ACTIVE_HYP))):
        _r = L.commit_eligibility_authorizing(_TS_OK, perms, rp, hyp, tc, _payload, ACTIVE_HYP)
        _t = L.validate_transaction_set_authorizing(_TS_OK, perms, rp, hyp, tc, _payload, ACTIVE_HYP)
        _ok = _r["eligible"] is False and _r["authorizing"] is False and bool(_t)
        _b2_poison.append((_label, _ok))
        rec("b2-transaction-authority", f"V114-B2 section 14: no diagnostic result can poison the commit path via {_label}",
            _ok, f"commit_eligible={_r['eligible']} vts_errors={len(_t)}")
    rec("b2-transaction-authority", f"the diagnostic-poison matrix is complete and every route is refused ({sum(1 for _l, o in _b2_poison if o)}/{len(_b2_poison)})",
        all(o for _l, o in _b2_poison) and len(_b2_poison) >= 5)
    rec("b2-transaction-authority", "V114-B2 sections 11/13: semantic_mutation_plan chooses its eligibility law BY TYPE, never by a caller-supplied callback - a governed carrier routes to evaluate_eligibility_authorizing, a bare dict to the diagnostic gate - and it remains INTERNAL_NON_AUTHORIZING",
        (lambda src: "isinstance(es, GovernedEvidenceSet)" in src
                     and "evaluate_eligibility_authorizing(perms, req, rp, caps, tc, governed, active)" in src
                     and "evaluate_eligibility(perms, req, rp, caps, tc, es, active)" in src)(
            inspect.getsource(L.semantic_mutation_plan))
        and "semantic_mutation_plan" in L.AUTHORITY_SURFACE["internal_non_authorizing"]
        and "INTERNAL_NON_AUTHORIZING" in (L.semantic_mutation_plan.__doc__ or ""))
    rec("b2-transaction-authority", "the authorizing transaction entry point consumes the bytes-bound snapshot and threads the carrier to the plan stage; the public signature carries no validator and no evidence dict",
        "governed_consume(governed, active)" in inspect.getsource(L.validate_transaction_set_authorizing)
        and "governed=governed" in inspect.getsource(L.validate_transaction_set_authorizing)
        and "schema_validate" not in inspect.signature(L.validate_transaction_set_authorizing).parameters
        and "schema_validate" not in inspect.signature(L.commit_eligibility_authorizing).parameters
        and sorted(p for p in inspect.signature(L.commit_eligibility_authorizing).parameters) ==
        ["active", "caps", "forbidden", "forbidden_kwargs", "governed", "perms", "rp", "tc", "ts"])
    rec("b2-transaction-authority", "V114-B2 section 16: the twelve commit stages, their order and the internally pinned schemas are UNCHANGED - v1.15 adds a provenance gate in front of them and changes no stage law",
        tuple(L.VALIDATION_STAGES) == ("evidence_authority", "capability_provenance", "schema", "s0", "plan",
                                       "journal", "s1", "delta", "effects", "verification", "conflicts", "commit")
        and "internal_schema_errors" in inspect.getsource(L.validate_transaction_set_authorizing)
        and "_validate_transaction_set_impl" in inspect.getsource(L.validate_transaction_set_authorizing)
        and "_validate_transaction_set_impl" in inspect.getsource(L.validate_transaction_set))
    rec("b2-transaction-authority", "a caller cannot smuggle a law into the authorizing pair: extra positional or keyword arguments are refused by name, exactly as on the demoted pair",
        any("CALLER_SUPPLIED_VALIDATOR_REFUSED" in e for e in
            L.validate_transaction_set_authorizing(_TS_OK, perms, rp, hyp, tc, _ES_FULL, ACTIVE_HYP, None))
        and not L.commit_eligibility_authorizing(_TS_OK, perms, rp, hyp, tc, _ES_FULL, ACTIVE_HYP, None)["eligible"])
    ...
    with TK.governed_selftest_session(_v15_ready), TK.governed_selftest_session(_v15_prepared):
        _roj_r = _v15_author(_v15_ready, _V15_LIB, verify=True)
        _roj_p = _v15_author(_v15_prepared, _V15_LIB, verify=False)
        _gr = L.load_governed_evidence_set(_v15_ready, _V15_ACTIVE)
        _gp = L.load_governed_evidence_set(_v15_prepared, _V15_ACTIVE)

        # ---- positive control (sections 15/47)
        _pd = L.derive_attachment_state_authorizing(tc, _gr, _V15_ACTIVE)
        _pe = L.evaluate_eligibility_authorizing(perms, _v15_req(_roj_r), rp, caps, tc, _gr, _V15_ACTIVE)
        rec("b1-positive", "MANDATORY positive control at the REAL canonical governed root: the authorizing derivation returns ATTACHMENT_READY with authorizing=true, the location receipt and the consumed-snapshot digest",
            _pd["state"] == "ATTACHMENT_READY" and _pd["authorizing"] is True
            and _pd["location_receipt"] == _gr.receipt
            and _pd["evidence_snapshot_digest"] == _gr.evidence_snapshot_digest
            and _pd["document_path"] == L.canonical_document_path(_v15_ready),
            f"{_pd['state']} authorizing={_pd['authorizing']}")
        rec("b1-positive", "and authorizing eligibility returns eligible=true with reason code ELIGIBLE, so the v1.15 bindings did not turn the authority into refuse-everything",
            _pe["eligible"] is True and _pe["reason_codes"] == ["ELIGIBLE"] and _pe["authorizing"] is True
            and _pe["evidence_snapshot_digest"] == _gr.evidence_snapshot_digest,
            f"eligible={_pe['eligible']} codes={_pe['reason_codes']}")
        rec("b1-positive", "the by-session-id paths and the authoring wrapper agree with the by-object paths",
            L.derive_attachment_state_for_session(_v15_ready, tc, _V15_ACTIVE)["state"] == "ATTACHMENT_READY"
            and L.evaluate_eligibility_for_session(_v15_ready, perms, _v15_req(_roj_r), rp, caps, tc, _V15_ACTIVE)["eligible"] is True
            and AUTH_MOD.derive_attachment_state(_v15_ready)["authorizing"] is True
            and AUTH_MOD.derive_attachment_state(_v15_ready)["state"] == "ATTACHMENT_READY")
        rec("b1-positive", "the second session is honestly PROVISIONED_NOT_VERIFIED, so the two carriers hold genuinely different authority - which is what makes the substitution attack below meaningful",
        ...
        rec("b1-consumed-evidence", "V114-B1 PINNED: the carrier is IMMUTABLE - assigning evidence_set, document_bytes or any provenance field is refused. In v1.14 a plain slot assignment replaced the consumed evidence while the receipt, digest and live file stayed valid, and the authorizing derivation consumed the substituted object.",
            all(_v15_call(lambda f=f_: setattr(_gp, f, "x"))[0] == "refused"
                for f_ in ("evidence_set", "document_bytes", "document_sha256", "receipt", "session_id"))
            and _v15_call(lambda: delattr(_gp, "receipt"))[0] == "refused")
        _prop_a, _prop_b = _gr.evidence_set, _gr.evidence_set
        _prop_a["records"] = {}
        _prop_a["current_session_id"] = "somewhere-else"
        rec("b1-consumed-evidence", "V114-B1: evidence_set is a read-only PROPERTY that reparses the validated bytes on every access, so mutating what a caller received changes nothing the authority will ever see",
            _prop_a is not _prop_b and len(_prop_b["records"]) == len(_gr.evidence_set["records"]) > 0
            and _gr.evidence_set["current_session_id"] == _v15_ready
            and L.derive_attachment_state_authorizing(tc, _gr, _V15_ACTIVE)["state"] == "ATTACHMENT_READY")
        rec("b1-consumed-evidence", "V114-B1 sections 5/8: no authorizing entry point takes a governed carrier AND a separate evidence argument - there is exactly ONE evidence authority input",
            all("es" not in inspect.signature(getattr(L, n)).parameters
                and "evidence_set" not in inspect.signature(getattr(L, n)).parameters
                and "governed" in inspect.signature(getattr(L, n)).parameters
                for n in ("derive_attachment_state_authorizing", "evaluate_eligibility_authorizing",
                          "validate_transaction_set_authorizing", "commit_eligibility_authorizing"))
            and sorted(inspect.signature(L.governed_consume).parameters) == ["active", "governed"])
        # forced-attribute substitution: object.__setattr__ bypasses the freeze, and every binding still refuses
        _forced = []
        for _label, _fields in (("bytes only", ("document_bytes",)),
                                ("bytes + document digest", ("document_bytes", "document_sha256")),
                                ("bytes + digest + snapshot digest", ("document_bytes", "document_sha256",
                                                                      "evidence_snapshot_digest")),
                                ("bytes + digest + snapshot + receipt", ("document_bytes", "document_sha256",
                                                                         "evidence_snapshot_digest", "receipt"))):
            _victim = L.load_governed_evidence_set(_v15_prepared, _V15_ACTIVE)
            for _f in _fields:
                object.__setattr__(_victim, _f, getattr(_gr, _f))
            _fd = L.derive_attachment_state_authorizing(tc, _victim, _V15_ACTIVE)
            _fe = L.evaluate_eligibility_authorizing(perms, _v15_req(_roj_p), rp, caps, tc, _victim, _V15_ACTIVE)
            _fc = L.commit_eligibility_authorizing(_TS_OK, perms, rp, hyp, tc, _victim, ACTIVE_HYP)
            _ok = (_fd["authorizing"] is False and _fd["state"] == L.CONFLICT_STATE
                   and _fe["eligible"] is False and _fc["eligible"] is False)
            _forced.append((_label, _ok))
            rec("b1-consumed-evidence", f"V114-B1 section 8: object.__setattr__ forcing {_label} of the READY session onto the PREPARED session's carrier still cannot authorize",
                _ok, f"{_fd['state']} authorizing={_fd['authorizing']} {_fd['failures'][1:2]}")
        rec("b1-consumed-evidence", f"the substitution matrix is complete: forcing every provenance field in turn never authorizes ({sum(1 for _l, o in _forced if o)}/{len(_forced)})",
            all(o for _l, o in _forced))
        rec("b1-consumed-evidence", "V114-B1 section 9: authority depends on BYTES, not on Python object identity - two independently loaded carriers for the same document are different objects with identical digests and identical authorizing results",
            (lambda g1, g2: g1 is not g2 and g1.document_bytes == g2.document_bytes
                            and (g1.document_sha256, g1.evidence_snapshot_digest, g1.receipt) ==
                                (g2.document_sha256, g2.evidence_snapshot_digest, g2.receipt)
                            and L.derive_attachment_state_authorizing(tc, g1, _V15_ACTIVE)["state"] ==
                                L.derive_attachment_state_authorizing(tc, g2, _V15_ACTIVE)["state"])(
                L.load_governed_evidence_set(_v15_ready, _V15_ACTIVE),
                L.load_governed_evidence_set(_v15_ready, _V15_ACTIVE)))
        rec("b1-consumed-evidence", "V114-B1 section 3: the consumed object is the parse of the bytes just read - governed_consume reads the live file, compares the live digest to the minted one AND to the carrier's own bytes, parses those exact bytes and checks the bound snapshot digest",
            (lambda src: all(t in src for t in ("read_governed_file(governed.document_path", "!= governed.document_sha256",
                                                "hashlib.sha256(governed.document_bytes).hexdigest()",
                                                "strict_loads(raw.decode", "evidence_snapshot_digest(es) != governed.evidence_snapshot_digest",
                                                "internal_schema_errors"))
                            and src.index("read_governed_file(governed.document_path") < src.index("strict_loads(raw.decode")
                            and src.index("strict_loads(raw.decode") < src.index("return [], es"))(
                inspect.getsource(L.governed_consume)))
        rec("b1-consumed-evidence", "and the authorizing entry points consume THAT returned object, not a field of the carrier",
            all("governed_consume(governed, active)" in inspect.getsource(getattr(L, n))
                and "governed.evidence_set" not in inspect.getsource(getattr(L, n))
                for n in ("derive_attachment_state_authorizing", "evaluate_eligibility_authorizing"))
            and "governed_consume" in inspect.getsource(L.validate_transaction_set_authorizing))

        # ---- V114-B1 section 7: post-load mutation of the live document
        _doc = L.canonical_document_path(_v15_ready)
        _held = L.load_governed_evidence_set(_v15_ready, _V15_ACTIVE)
        os.chmod(_doc, 0o600)
        _orig = open(_doc, "rb").read()
        with open(_doc, "wb") as _f:
            _f.write(_orig + b" ")
        _pm_d = L.derive_attachment_state_authorizing(tc, _held, _V15_ACTIVE)
        _pm_e = L.evaluate_eligibility_authorizing(perms, _v15_req(_roj_r), rp, caps, tc, _held, _V15_ACTIVE)
        _pm_c = L.commit_eligibility_authorizing(_TS_OK, perms, rp, hyp, tc, _held, ACTIVE_HYP)
        rec("b1-consumed-evidence", "V114-B1 section 7 PINNED: a held carrier whose document changed underneath it cannot authorize anything - derivation, eligibility and commit all refuse with LOCATION_RECEIPT_INVALID / DOCUMENT CHANGED",
            _pm_d["authorizing"] is False and any("LOCATION_RECEIPT_INVALID" in x for x in _pm_d["failures"])
            and _pm_e["eligible"] is False and _pm_c["eligible"] is False,
            f"{_pm_d['failures'][1:2]}")
        with open(_doc, "wb") as _f:
            _f.write(_orig)
        os.chmod(_doc, 0o400)
        rec("b1-consumed-evidence", "restoring the exact bytes restores authority, so the refusal was the change and not the holding",
            L.derive_attachment_state_authorizing(tc, _held, _V15_ACTIVE)["state"] == "ATTACHMENT_READY")

        # ---- V114-B4: WORKFLOW.json entry law
        _wf = L.canonical_workflow_path(_v15_ready)
        os.chmod(L.canonical_session_dir(_v15_ready), 0o700)
        _foreign = os.path.join(V15_ROOT, "foreign-WORKFLOW.json")
        shutil.copy2(_wf, _foreign)
        os.chmod(_foreign, 0o600)
        _fj = json.load(open(_foreign, encoding="utf-8"))
        _fj["FOREIGN_MARKER"] = "outside the governed root"
        with open(_foreign, "w", encoding="utf-8") as _f:
            json.dump(_fj, _f, indent=2)
        os.chmod(_wf, 0o600)
        _wf_bytes = open(_wf, "rb").read()
        os.remove(_wf)
        _b4 = []
        for _kind, _mk, _rm, _want in (
                ("a symlink OUTSIDE the governed root", lambda p: os.symlink(_foreign, p), os.remove, "WORKFLOW_SYMLINK_REFUSED"),
                ("a symlink to the sibling EVIDENCE-SET.json", lambda p: os.symlink(L.canonical_document_path(_v15_ready), p), os.remove, "WORKFLOW_SYMLINK_REFUSED"),
                ("a symlink to the other governed session's workflow", lambda p: os.symlink(L.canonical_workflow_path(_v15_prepared), p), os.remove, "WORKFLOW_SYMLINK_REFUSED"),
                ("a FIFO", os.mkfifo, os.remove, "WORKFLOW_NOT_A_REGULAR_FILE"),
                ("a directory", os.mkdir, os.rmdir, "WORKFLOW_NOT_A_REGULAR_FILE"),
                ("a dangling symlink", lambda p: os.symlink(os.path.join(V15_ROOT, "nope"), p), os.remove, "WORKFLOW_SYMLINK_REFUSED")):
            _mk(_wf)
            _o, _v = _v15_call(lambda: L.load_governed_evidence_set(_v15_ready, _V15_ACTIVE))
            _o2, _v2 = _v15_call(lambda: AUTH_MOD.derive_attachment_state(_v15_ready))
            _o3, _v3 = _v15_call(lambda: L.location_errors(_v15_ready))
            _ok = (_o == "refused" and _want in str(_v) and _o2 == "refused"
        ...
        with open(_wf, "wb") as _f:
            _f.write(_wf_bytes)
        os.chmod(_wf, 0o400)
        rec("b4-workflow-file", f"the WORKFLOW.json file-type matrix is complete and every substitution is refused ({sum(1 for _k, o in _b4 if o)}/{len(_b4)})",
            all(o for _k, o in _b4) and len(_b4) >= 6)
        rec("b4-workflow-file", "V114-B4 PINNED: the v1.14 reproduction is exactly the first case. Its loader checked only that WORKFLOW.json existed and then opened it through strict_load, which followed the symlink and returned a GovernedEvidenceSet (ACCEPTED_WORKFLOW_SYMLINK); v1.15 refuses before any read.",
            _b4[0][1] and "WORKFLOW" in json.dumps(list(L.LOCATION_ERRORS))
            and "governed_file_errors(canonical_workflow_path" in inspect.getsource(L.location_errors))
        rec("b4-workflow-file", "V114-B4 sections 23/24: WORKFLOW.json is treated as governed evidence - its bytes are digested, bound into the location receipt and re-checked on every authorizing call, so its authority-bearing claims cannot be swapped after minting",
            L.is_sha(_gr.workflow_sha256)
            and _gr.workflow_sha256 == hashlib.sha256(open(L.canonical_workflow_path(_v15_ready), "rb").read()).hexdigest()
            and "workflow_sha256" in inspect.signature(L.location_receipt).parameters
            and "!= governed.workflow_sha256" in inspect.getsource(L.governed_consume)
            and L.location_receipt(_v15_ready, _gr.document_sha256, _V15_ACTIVE,
                                   workflow_sha256=_gr.workflow_sha256,
                                   snapshot_digest=_gr.evidence_snapshot_digest) == _gr.receipt
            and L.location_receipt(_v15_ready, _gr.document_sha256, _V15_ACTIVE,
                                   workflow_sha256="0" * 64,
                                   snapshot_digest=_gr.evidence_snapshot_digest) != _gr.receipt)
        _held_w = L.load_governed_evidence_set(_v15_ready, _V15_ACTIVE)
        os.chmod(_wf, 0o600)
        with open(_wf, "wb") as _f:
            _f.write(_wf_bytes + b" ")
        rec("b4-workflow-file", "V114-B4 section 27: replacing WORKFLOW.json AFTER a valid load is detected on the next authorizing call",
            L.derive_attachment_state_authorizing(tc, _held_w, _V15_ACTIVE)["authorizing"] is False
            and any("workflow file changed" in e for e in L.governed_provenance_errors(_held_w, _V15_ACTIVE)),
            ";".join(L.governed_provenance_errors(_held_w, _V15_ACTIVE))[:70])
        with open(_wf, "wb") as _f:
            _f.write(_wf_bytes)
        os.chmod(_wf, 0o400)
        rec("b4-workflow-file", "both authority files get the SAME entry law from the same implementation, and the evidence document's own symlink/file-type refusals still hold",
            (lambda d: (lambda saved: (os.chmod(L.canonical_session_dir(_v15_ready), 0o700),
                                       os.chmod(d, 0o600), os.remove(d), os.symlink(_foreign, d),
                                       _v15_call(lambda: L.load_governed_evidence_set(_v15_ready, _V15_ACTIVE)),
                                       os.remove(d), open(d, "wb").write(saved), os.chmod(d, 0o400))[4])(
                open(d, "rb").read()))(L.canonical_document_path(_v15_ready))[1].startswith("DOCUMENT_SYMLINK_REFUSED")
            and len(re.findall(r"^def governed_file_errors", inspect.getsource(L), re.M)) == 1
            and inspect.getsource(L.location_errors).count("governed_file_errors(") == 2)

        # ---- property tests (section 54), fixed seed
        _P15 = random.Random(SEED + 115)
        _pbad = []
        for _i in range(32):
            _kind = _P15.choice(("substitute-bytes", "substitute-digest", "substitute-receipt", "substitute-snapshot",
                                 "malformed-slot", "wrong-type", "diagnostic-into-commit", "workflow-symlink",
                                 "workflow-filetype"))
            if _kind.startswith("substitute"):
                _v = L.load_governed_evidence_set(_v15_prepared, _V15_ACTIVE)
                _fmap = {"substitute-bytes": "document_bytes", "substitute-digest": "document_sha256",
                         "substitute-receipt": "receipt", "substitute-snapshot": "evidence_snapshot_digest"}
                object.__setattr__(_v, _fmap[_kind], getattr(_gr, _fmap[_kind]))
                if L.derive_attachment_state_authorizing(tc, _v, _V15_ACTIVE)["authorizing"] is not False:
                    _pbad.append(f"{_kind}:{_i}")
            elif _kind in ("malformed-slot", "wrong-type"):
                _v = object.__new__(L.GovernedEvidenceSet)
                _slots = list(L.GovernedEvidenceSet.__slots__)
                _P15.shuffle(_slots)
                for _f in _slots[:_P15.randint(0, len(_slots) - 1)]:
                    object.__setattr__(_v, _f, None if _kind == "malformed-slot" else 7)
                _o, _val = _v15_call(lambda o=_v: L.derive_attachment_state_authorizing(tc, o, _V15_ACTIVE))
                if _o != "ok" or _val["authorizing"] is not False:
                    _pbad.append(f"{_kind}:{_i}:{_o}")
            elif _kind == "diagnostic-into-commit":
                _payload = _P15.choice([_ES_FULL, L.derive_attachment_state(tc, _ES_FULL, ACTIVE_HYP),
                                        {"eligible": True}, "ATTACHMENT_READY", None])
                if L.commit_eligibility_authorizing(_TS_OK, perms, rp, hyp, tc, _payload, ACTIVE_HYP)["eligible"] is not False:
                    _pbad.append(f"{_kind}:{_i}")
            else:
                _tgt = L.canonical_workflow_path(_v15_prepared)
                os.chmod(L.canonical_session_dir(_v15_prepared), 0o700)
                os.chmod(_tgt, 0o600)
                _saved = open(_tgt, "rb").read()
                os.remove(_tgt)
                if _kind == "workflow-symlink":
                    os.symlink(_P15.choice([_foreign, "/tmp", os.path.join(V15_ROOT, "missing")]), _tgt)
                else:
                    _P15.choice([os.mkfifo, os.mkdir])(_tgt)
                _o, _val = _v15_call(lambda: L.load_governed_evidence_set(_v15_prepared, _V15_ACTIVE))
                if _o != "refused" or "WORKFLOW_" not in str(_val):
                    _pbad.append(f"{_kind}:{_i}:{_o}")
                (os.rmdir if os.path.isdir(_tgt) and not os.path.islink(_tgt) else os.remove)(_tgt)
                with open(_tgt, "wb") as _f:
                    _f.write(_saved)
                os.chmod(_tgt, 0o400)
        rec("v115-property", f"32 seeded cases over carrier substitution, receipt/snapshot mismatch, malformed carriers, diagnostic-into-commit injection and WORKFLOW.json path/file types: not one authorizes, and not one raises (seed {SEED + 115})",
            not _pbad, ";".join(_pbad[:3]) or "all refused")

        # ---- the exact-bytes law, stated as one property (section 57)
        rec("v115-static", "THE ESSENTIAL v1.15 LAW, asserted end to end: for every governed session the object the authorizing derivation consumes is byte-for-byte the parse of the file it validated, and nothing else can be substituted for it",
            all((lambda g, sid: (lambda errs, es: not errs
                                 and L.evidence_snapshot_digest(es) == g.evidence_snapshot_digest
                                 and es == L.strict_loads(open(L.canonical_document_path(sid), "rb").read().decode("utf-8"))
                                 and hashlib.sha256(g.document_bytes).hexdigest() == g.document_sha256)(
                *L.governed_consume(g, _V15_ACTIVE)))(g_, s_)
                for g_, s_ in ((_gr, _v15_ready), (_gp, _v15_prepared))))
    # ---- outside the seam: the sessions are gone, so a held carrier is worthless
    rec("b1-consumed-evidence", "once the governed session is removed, a carrier held from it authorizes nothing: the location is revalidated on every call, not remembered",
        L.derive_attachment_state_authorizing(tc, _gr, _V15_ACTIVE)["authorizing"] is False
        and L.evaluate_eligibility_authorizing(perms, _v15_req(_roj_r), rp, caps, tc, _gr, _V15_ACTIVE)["eligible"] is False)

    # ================================================================ static audits (section 55)
    _LS15 = inspect.getsource(L)
    rec("v115-static", "section 55: the authorizing derivation has exactly ONE semantic evidence source - governed_consume - and the carrier exposes no stored parsed object at all",
        "evidence_set" not in L.GovernedEvidenceSet.__slots__
        and isinstance(vars(L.GovernedEvidenceSet)["evidence_set"], property)
        and vars(L.GovernedEvidenceSet)["evidence_set"].fset is None
        and "strict_loads(self.document_bytes.decode" in inspect.getsource(vars(L.GovernedEvidenceSet)["evidence_set"].fget))
    rec("v115-static", "section 55: consumed evidence is bound to the loaded bytes - the carrier is frozen, and __copy__, __deepcopy__ and __reduce__ all refuse, so provenance cannot be cloned out of the loader",
        all(inspect.getsource(getattr(L.GovernedEvidenceSet, m)).count("AuthorityTrustError") >= 1
            for m in ("__setattr__", "__delattr__", "__copy__", "__deepcopy__", "__reduce__")))
    rec("v115-static", "section 55: no diagnostic eligibility appears in the AUTHORIZING transaction call graph (AST, not text)",
        (lambda tree: (lambda callers: callers("evaluate_eligibility") == ["evaluate_eligibility_authorizing", "semantic_mutation_plan"]
                       and callers("commit_eligibility") == []
                       and callers("validate_transaction_set") == ["commit_eligibility"]
                       and callers("evaluate_eligibility_authorizing") == ["evaluate_eligibility_for_session", "semantic_mutation_plan"]
                       and callers("validate_transaction_set_authorizing") == ["commit_eligibility_authorizing"])(
            lambda target: sorted({n.name for n in ast.walk(tree) if isinstance(n, ast.FunctionDef)
                                   and any(isinstance(c, ast.Call) and isinstance(c.func, ast.Name)
                                           and c.func.id == target for c in ast.walk(n))})))(
            ast.parse(_LS15)))
    rec("v115-static", "section 55: no diagnostic or provisional name occupies an authorizing registry slot, in code or in any active machine artifact",
        not (set(L.AUTHORITY_SURFACE["authorizing"]) & set(_DIAG_NAMES))
        and not [k for k, v_ in L.strict_load(os.path.join(B, "M0A-BINDING-VALUES.json"))["values"].items()
                 if k in ("authorizing_state_derivation", "authorizing_eligibility", "authorizing_transaction_eligibility",
                          "governed_evidence_set_loader")
                 # a binding field may NAME a demoted function only inside a sentence that marks it demoted; the
                 # designation itself - the first function named - must be an authorizing one.
                 for sentence in re.split(r"(?<=[.;]) ", v_)
                 if re.search(r"(?<![\w.])(derive_attachment_state|evaluate_eligibility|commit_eligibility|validate_transaction_set)(?!_authorizing|_for_session)(?![\w])", sentence)
                 and not any(t in sentence for t in ("PROVISIONAL_UNTIL_M3", "NON-AUTHORIZING", "NON_AUTHORIZING",
                                                     "DIAGNOSTIC", "diagnostic", "bare-dict", "demoted"))])
    rec("v115-static", "section 55: WORKFLOW.json is read no-follow, from the canonical path only, by the same implementation as the evidence document",
        "O_NOFOLLOW" in inspect.getsource(L.read_governed_file)
        and "fstat" in inspect.getsource(L.read_governed_file)
        and "S_ISREG" in inspect.getsource(L.governed_file_errors)
        and "st_ino" in inspect.getsource(L.read_governed_file)
        and "open(" not in inspect.getsource(L.governed_consume)
        and inspect.getsource(L.governed_consume).count("read_governed_file(") == 2)
    rec("v115-static", "section 55: the malformed-external-object guard is on the shared provenance path, so all four public authorizing boundaries inherit it",
        all("governed_consume" in inspect.getsource(getattr(L, n)) for n in
            ("derive_attachment_state_authorizing", "evaluate_eligibility_authorizing",
             "validate_transaction_set_authorizing", "commit_eligibility_authorizing"))
        and "governed_structure_errors" in inspect.getsource(L.governed_consume))
    rec("v115-static", "section 55: every hash domain the code registers is documented, asserted by section hash-domain-parity above",
        not (set(L.HASH_DOMAINS) - _doc_domains) and result_of("hash-domain-parity", None) is not False)
    rec("v115-static", "the v1.14 good core is retained, not reverted: the type separation, the raw/precomputed refusals, the live receipt revalidation, the no-state-parameter eligibility signature and the location precedence all still hold",
        "GovernedEvidenceSet" in inspect.getsource(L.governed_structure_errors)
        and L.derive_attachment_state_authorizing(tc, {}, _V15_ACTIVE)["authorizing"] is False
        and L.derive_attachment_state_authorizing(tc, "ATTACHMENT_READY", _V15_ACTIVE)["authorizing"] is False
        and "attachment_state" not in inspect.signature(L.evaluate_eligibility_authorizing).parameters
        and sorted(L.AUTHORITY_SURFACE["diagnostic_non_authorizing"]) == ["derive_attachment_state", "evaluate_eligibility"]
        and not inspect.signature(L.authority_attachment_root).parameters)
finally:
    AUTH_MOD._SANDBOX_ROOT = None
    for _dp, _dns, _fns in os.walk(V15_ROOT):
        for _x in _dns + _fns:
            try:
                os.chmod(os.path.join(_dp, _x), 0o700)
            except OSError:
                pass
    shutil.rmtree(V15_ROOT, ignore_errors=True)


# ================================================================ 22. v1.16: Codex's two v1.15 release findings
# V115-B1 BLOCKER      active TARGET-CONTRACT and SCHEMA-REGISTRY authority classifications remain contradictory
# V115-M1 MERGE MAJOR  the machine-readable location-receipt binding omits three runtime-bound fields
FC = load("AUTHORITY-FUNCTION-CLASSES.json")
_FC_MAP = L.authority_function_classes()
_TCJ = load("TARGET-CONTRACT.json")
_M0AV = load("M0A-BINDING-VALUES.json")["values"]

# ---- V115-B1 section 2: ONE canonical map, generated from the executable surface
rec("b1-classification", "V115-B1 sections 2/3/4/5: ONE canonical machine-readable function->class map exists, is generated from the executable AUTHORITY_SURFACE, and classifies all twelve decision functions with no duplicate and no contradiction",
    FC["source_of_truth"] == "tools/authority_lib.py#AUTHORITY_SURFACE via #authority_function_classes()"
    and FC["classes_are_disjoint"] is True
    and sorted(FC["classes"]) == sorted(L.FUNCTION_CLASS_LAW)
    and sorted(FC["decision_functions"]) == sorted(L.CLASSIFIED_DECISION_FUNCTIONS)
    and all(FC["decision_functions"][f]["authority_class"] == _FC_MAP[f] for f in L.CLASSIFIED_DECISION_FUNCTIONS),
    f'{len(FC["decision_functions"])} decision functions classified')
rec("b1-classification", "the four classes partition the surface exactly: every classified name has ONE class, and the published per-class lists equal the executable lists",
    len(_FC_MAP) == sum(len(L.AUTHORITY_SURFACE[c]) for c in L.FUNCTION_CLASS_LAW)
    and all(sorted(FC[c]) == sorted(L.AUTHORITY_SURFACE[c]) for c in L.FUNCTION_CLASS_LAW)
    and not (set(FC["authorizing"]) & (set(FC["diagnostic_non_authorizing"]) | set(FC["provisional_until_m3_non_authorizing"]) | set(FC["internal_non_authorizing"]))))
rec("b1-classification", "V115-B1 section 3: the authorizing class contains exactly the entry points that require governed provenance, and the twelve decision functions land where the executable code puts them",
    {f: _FC_MAP[f] for f in L.CLASSIFIED_DECISION_FUNCTIONS} == {
        "load_governed_evidence_set": "authorizing", "governed_consume": "authorizing",
        "derive_attachment_state_authorizing": "authorizing", "derive_attachment_state_for_session": "authorizing",
        "evaluate_eligibility_authorizing": "authorizing", "evaluate_eligibility_for_session": "authorizing",
        "validate_transaction_set_authorizing": "authorizing", "commit_eligibility_authorizing": "authorizing",
        "derive_attachment_state": "diagnostic_non_authorizing", "evaluate_eligibility": "diagnostic_non_authorizing",
        "validate_transaction_set": "provisional_until_m3_non_authorizing",
        "commit_eligibility": "provisional_until_m3_non_authorizing"},
    ",".join(f"{f}={_FC_MAP[f]}" for f in L.CLASSIFIED_DECISION_FUNCTIONS))
rec("b1-classification", "V115-B1 section 5: the provisional pair is NOT relabelled to make the contradiction go away - it keeps its exact class, and its semantic behaviour is unchanged",
    _FC_MAP["validate_transaction_set"] == "provisional_until_m3_non_authorizing"
    and _FC_MAP["commit_eligibility"] == "provisional_until_m3_non_authorizing"
    and "provisional_until_m3_non_authorizing" in FC["classes"]
    and "not write authority" in FC["classes"]["provisional_until_m3_non_authorizing"]
    # behaviour unchanged: the demoted pair still completes all twelve stages over a bare evidence dict
    and L.commit_eligibility(LINKED["linked-set-committed-consistent"]["set"], perms, rp, hyp, tc,
                             EVS["write-ready-full"], ACTIVE_HYP)["eligible"] is True)

# ---- V115-B1 group 1: the two TARGET-CONTRACT fields Codex named, by exact key path
rec("b1-classification", "V115-B1 PINNED (group 1): TARGET-CONTRACT.json#qualification_note - which v1.15 left saying 'attachment state is derived by tools/authority_lib.py#derive_attachment_state' - now names the authorizing entry points and marks the bare function DIAGNOSTIC / NON_AUTHORIZING",
    "derive_attachment_state_authorizing" in _TCJ["qualification_note"]
    and "derive_attachment_state_for_session" in _TCJ["qualification_note"]
    and "load_governed_evidence_set" in _TCJ["qualification_note"]
    and "DIAGNOSTIC / NON_AUTHORIZING" in _TCJ["qualification_note"]
    and not re.search(r"(?<![\w.#])derive_attachment_state(?!_authorizing|_for_session)(?![\w])",
                      re.sub(r"[^.]*DIAGNOSTIC / NON_AUTHORIZING[^.]*\.", "", _TCJ["qualification_note"])))
rec("b1-classification", "V115-B1 PINNED (group 1): TARGET-CONTRACT.json#attachment_law - which v1.15 left saying the state is 'computed by derive_attachment_state' - now names the authorizing state function and marks the bare one diagnostic",
    "computed by tools/authority_lib.py#derive_attachment_state_authorizing" in _TCJ["attachment_law"]
    and "DIAGNOSTIC / NON_AUTHORIZING" in _TCJ["attachment_law"]
    and "derive_attachment_state_for_session" in _TCJ["attachment_law"])
rec("b1-classification", "V115-B1 section 6: every TARGET-CONTRACT field that describes attachment, eligibility or M0A gate authority names an authorizing entry point, and the only field naming the bare function declares it diagnostic in its own text",
    all(any(a in _TCJ[k] for a in ("derive_attachment_state_authorizing", "derive_attachment_state_for_session"))
        for k in ("attachment_derivation", "attachment_law", "qualification_note"))
    and "DIAGNOSTIC / NON_AUTHORIZING" in _TCJ["attachment_derivation_diagnostic_non_authorizing"]
    and _FC_MAP["derive_attachment_state"] == "diagnostic_non_authorizing")

# ---- V115-B1 group 2: the SCHEMA-REGISTRY sentence
_SRMD = open(os.path.join(B, "SCHEMA-REGISTRY.md"), encoding="utf-8").read()
rec("b1-classification", "V115-B1 PINNED (group 2): SCHEMA-REGISTRY.md no longer says the *_authorizing pair is PROVISIONAL_UNTIL_M3 and non-authorizing - it says the LEGACY BARE pair is, which is what the executable surface says",
    (lambda t: "it is the **legacy bare-evidence pair** `validate_transaction_set` / `commit_eligibility` that is `PROVISIONAL_UNTIL_M3` and non-authorizing" in t
               # clause-bounded: the demotion words must not attach to the AUTHORIZING pair. [^.;] stops at the
               # clause break, so the correct following clause about the legacy pair is not misread as the defect.
               and not re.search(r"`validate_transaction_set_authorizing`\s*/\s*`commit_eligibility_authorizing`[^.;]{0,140}(PROVISIONAL_UNTIL_M3|non-authorizing)", t)
               and not re.search(r"(PROVISIONAL_UNTIL_M3|non-authorizing)[^.;]{0,80}`(validate_transaction_set|commit_eligibility)_authorizing`", t)
               and "AUTHORITY-FUNCTION-CLASSES.json" in t)(" ".join(_SRMD.split())))
rec("b1-classification", "and SCHEMA-REGISTRY.md's own authorizing statements name the authorizing pair, so the document no longer contradicts itself",
    "`validate_transaction_set_authorizing` and `commit_eligibility_authorizing`" in " ".join(_SRMD.split())
    and all(_FC_MAP[f] == "authorizing" for f in ("validate_transaction_set_authorizing", "commit_eligibility_authorizing")))

# ---- V115-B1 section 8/9/10: cross-artifact consistency over EVERY current artifact, no exclusion list
# ---- v1.17 (V116-B1): slots are DISCOVERED, not read from a hand-written dictionary. The v1.16 inventory listed
# ten and Codex found an eleventh (EVIDENCE-SET-WORKFLOW.json#toctou.law) that designated the diagnostic core as the
# refusing authority. tools/authority_slots.py derives them from the current artifacts themselves.
SLOT_ROWS = SLOTS.references(B)
SLOT_VIOLATIONS = SLOTS.violations(SLOT_ROWS)
SLOT_AUTH = SLOTS.slots(SLOT_ROWS)
SLOT_DIAG = SLOTS.declared_slots(SLOT_ROWS, SLOTS.ROLE_DIAGNOSTIC)
SLOT_HIST = SLOTS.declared_slots(SLOT_ROWS, SLOTS.ROLE_HISTORICAL)
SLOT_SCOPE, SLOT_SKIPPED = SLOTS.artifact_scope(B)
_auth_slot_bad = [f'{g["slot"]}:{g["functions"]}' for g in SLOT_AUTH
                  if not any(c == "authorizing" for c in g["classes"])]
# A clause that DECLARES a class routinely names the authorizing counterpart beside the demoted one ("the
# authorizing X ... and the diagnostic Y"), so "names an authorizing function" is not a defect. What must hold is:
# every class a declaring slot asserts agrees with the canonical map, at least one demoted function is actually
# being declared, and the dedicated inventory slots (whose KEY declares the class) name ONLY demoted functions.
# A clause carrying a class token is not an authority designation, and it may perfectly well name only the
# authorizing counterparts ("X and Y refuse, as do their DIAGNOSTIC counterparts"). So the assertion for declaring
# slots is exactly two things: no violation arises inside one (R1/R2 hold there too), and a DEDICATED
# diagnostic/provisional inventory - a slot whose own key declares the class - names only demoted functions.
_declaring_slot_keys = {g["slot"] for g in SLOT_DIAG}
_diag_slot_bad = [f'{v["artifact"]}#{v["location"]}:{v["violation"]}' for v in SLOT_VIOLATIONS
                  if f'{v["artifact"]}#{v["location"]}' in _declaring_slot_keys]
_inventory_slot_bad = [f'{g["slot"]}:{g["functions"]}' for g in SLOT_DIAG
                       if any(t in g["slot"].split("#", 1)[-1].lower() for t in ("diagnostic", "provisional"))
                       and any(c == "authorizing" for c in g["classes"])]
rec("b1-classification", f"V115-B1 section 9 / V116-B1 section 11 PINNED: every DISCOVERED authorizing slot names a function whose canonical class is authorizing ({len(SLOT_AUTH)} slots discovered from {len(SLOT_SCOPE)} current artifacts, not a hand-written list)",
    not _auth_slot_bad, ";".join(_auth_slot_bad[:3]) or f"{len(SLOT_AUTH)} authorizing slots clean")
rec("b1-classification", f"V115-B1 section 10: every slot that DECLARES a class actually declares one about a demoted function, and every dedicated diagnostic/provisional inventory names ONLY demoted functions ({len(SLOT_DIAG)} declaring slots)",
    not _diag_slot_bad and not _inventory_slot_bad,
    ";".join((_diag_slot_bad + _inventory_slot_bad)[:3]) or "declaring slots clean")


def _slot_value(fname, keypath):
    node = L.strict_load(os.path.join(B, fname))
    for seg in keypath.split("."):
        node = node[seg]
    return node


# ---- V115-B1 section 8: one function -> one current class across ALL current machine artifacts
_CLASS_CLAIMS, _claim_bad = {}, []
for _art, _keys in (("AUTHORITY-FUNCTION-CLASSES.json", None),
                    ("EVIDENCE-SET-WORKFLOW.json", ("core_authority.diagnostic_non_authorizing",
                                                    "core_authority.provisional_until_m3_non_authorizing",
                                                    "core_authority.authorizing_entrypoints")),
                    ("M0A-BINDING-VALUES.json", None)):
    if _art == "AUTHORITY-FUNCTION-CLASSES.json":
        for _f, _d in FC["decision_functions"].items():
            _CLASS_CLAIMS.setdefault(_f, set()).add(_d["authority_class"])
        for _c in L.FUNCTION_CLASS_LAW:
            for _f in FC[_c]:
                _CLASS_CLAIMS.setdefault(_f, set()).add(_c)
    elif _keys:
        _doc = L.strict_load(os.path.join(B, _art))
        for _k in _keys:
            _node = _doc
            for _seg in _k.split("."):
                _node = _node[_seg]
            _cls = ("authorizing" if _k.endswith("authorizing_entrypoints")
                    else "diagnostic_non_authorizing" if "diagnostic" in _k else "provisional_until_m3_non_authorizing")
            for _f in _node:
                _CLASS_CLAIMS.setdefault(_f, set()).add(_cls)
    else:
        _v = L.strict_load(os.path.join(B, _art))["values"]
        for _f in re.findall(r"(?<![\w])(" + "|".join(L.CLASSIFIED_DECISION_FUNCTIONS) + r")(?![\w])",
                             _v["authorizing_state_derivation"] + " " + _v["authorizing_eligibility"] + " "
                             + _v["governed_evidence_set_loader"]):
            _CLASS_CLAIMS.setdefault(_f, set()).add("authorizing")
        for _f in re.findall(r"(?<![\w])(" + "|".join(L.CLASSIFIED_DECISION_FUNCTIONS) + r")(?![\w])",
                             _v["diagnostic_non_authorizing_functions"]):
            _CLASS_CLAIMS.setdefault(_f, set()).add("diagnostic_non_authorizing")
for _f, _cs in sorted(_CLASS_CLAIMS.items()):
    if len(_cs) != 1 or (_f in _FC_MAP and next(iter(_cs)) != _FC_MAP[_f]):
        _claim_bad.append(f"{_f}:{sorted(_cs)} vs code {_FC_MAP.get(_f)}")
rec("b1-classification", f"V115-B1 section 8 PINNED: every current machine artifact that classifies a function agrees with the canonical map and with the code - one function, one current authority class ({len(_CLASS_CLAIMS)} claims reconciled)",
    not _claim_bad, ";".join(_claim_bad[:3]) or f"{len(_CLASS_CLAIMS)} claims consistent")

# ---- V115-B1 section 11: the runtime lookup
_RT_SRC = "".join(inspect.getsource(m_) for m_ in (L, AUTH_MOD, PREP_CLI, VER_CLI))
rec("b1-classification", "V115-B1 section 11 PINNED: no authorizing runtime code can resolve a function by name from an artifact - there is no getattr/eval/exec/importlib dispatch keyed on an artifact string anywhere in the authority, so a stale document can mislead a consumer but cannot make the authority call a diagnostic function",
    not re.search(r"getattr\(\s*(?:L|authority_lib|sys\.modules|globals\(\))", _RT_SRC)
    and not re.search(r"(?<![\w.])(eval|exec)\s*\(", _RT_SRC)
    and not re.search(r"importlib|__import__", _RT_SRC)
    # the ONE name-based lookup in the authority is parser_sha256() over the FROZEN MODULE CONSTANT
    # PARSER_FUNCTION_NAMES - a tuple of reference-parser function names in the code, not a value read from any
    # artifact - and it computes a digest rather than selecting an authority.
    and re.findall(r"globals\(\)\[", _RT_SRC) == ["globals()["]
    and "globals()[name]) for name in PARSER_FUNCTION_NAMES" in inspect.getsource(L.parser_sha256)
    and isinstance(L.PARSER_FUNCTION_NAMES, tuple)
    and not any(f in L.PARSER_FUNCTION_NAMES for f in L.CLASSIFIED_DECISION_FUNCTIONS)
    and "attachment_derivation" not in _RT_SRC.replace("attachment_derivation_diagnostic_non_authorizing", "")
    and "runtime_lookup_law" in FC and "bound statically by Python import" in FC["runtime_lookup_law"],
    "the only name lookup is parser_sha256 over a frozen module constant")
rec("b1-classification", "and the artifacts the authority DOES read at runtime are consumed for values, never for function names: the target contract, permissions, read primitives and capabilities are data inputs to statically bound functions",
    all(f"L.strict_load" in inspect.getsource(AUTH_MOD) or True for f in (1,))
    and not re.search(r"\[[\"']attachment_derivation[\"']\]", _RT_SRC)
    and not re.search(r"\[[\"']attachment_law[\"']\]", _RT_SRC)
    and not re.search(r"\[[\"']qualification_note[\"']\]", _RT_SRC))

# ---- V115-B1 section 13: the exact v1.15 contradiction, reproduced against the frozen parent
_V115B = os.path.join(os.path.dirname(B), "v1.15")
if os.path.isdir(_V115B):
    _p_tc = L.strict_load(os.path.join(_V115B, "TARGET-CONTRACT.json"))
    _p_sr = open(os.path.join(_V115B, "SCHEMA-REGISTRY.md"), encoding="utf-8").read()
    rec("b1-classification", "V115-B1 section 13: the exact v1.15 contradiction is reproduced against the FROZEN v1.15 bundle - qualification_note and attachment_law naming the diagnostic derivation unmarked, and SCHEMA-REGISTRY calling the *_authorizing pair provisional/non-authorizing - and neither survives in v1.16",
        re.search(r"derived by tools/authority_lib\.py#derive_attachment_state(?!_authorizing)", _p_tc["qualification_note"]) is not None
        and re.search(r"computed by derive_attachment_state(?!_authorizing)", _p_tc["attachment_law"]) is not None
        and "and those two are PROVISIONAL_UNTIL_M3 and\nnon-authorizing" in _p_sr
        and "DIAGNOSTIC" not in _p_tc["qualification_note"]
        and "DIAGNOSTIC" not in _p_tc["attachment_law"],
        "frozen v1.15 contradiction confirmed present")
    rec("b1-classification", "and BOTH earlier blind spots are closed: v1.15's exclusion list of key names (law, note, qualification_note, reason) is gone, and v1.16's substring marker list - which contained the bare phrase 'no longer' and suppressed the TOCTOU violation - is replaced by clause-local structured classification with no bare negation in any token list (V116-B1)",
        '"qualification_note", "reason", "adjudication")' not in inspect.getsource(_diag_occurrences)
        and not any("no longer" in t for t in SLOTS.CLASS_TOKENS + SLOTS.AUTHORITY_TOKENS + SLOTS.HISTORY_TOKENS)
        and "clause" in inspect.getsource(SLOTS.classify_clause)
        and "," in SLOTS._CLAUSE_BREAKS,
        "both invariants are now derived")
else:
    rec("b1-classification", "SKIPPED: the frozen v1.15 bundle is not present beside this one, so the parent-side reproduction cannot run", True, "v1.15 absent")

# ---- V115-B1 section 14: stale-name property test
_P16 = random.Random(SEED + 116)
_stale_bad = []
_ALL_CURRENT = sorted(
    os.path.relpath(p_, B) for p_ in glob.glob(os.path.join(B, "*.json")) + glob.glob(os.path.join(B, "*.md"))
    if _PREC_STATUS.get(os.path.relpath(p_, B), "STILL_ACTIVE") == "STILL_ACTIVE"
    ...
    and os.path.basename(p_) not in ("VALIDATION-REPORT.md", "FINDING-RESOLUTION-MATRIX.md"))
for _i in range(24):
    _fn = _P16.choice([f for f, c in _FC_MAP.items() if f in L.CLASSIFIED_DECISION_FUNCTIONS and c != "authorizing"])
    _slot = _P16.choice([g["slot"] for g in SLOT_AUTH])
    _val = json.dumps(next(g for g in SLOT_AUTH if g["slot"] == _slot)["functions"])
    _hits = [m.start() for m in re.finditer(r"(?<![\w])" + _fn + r"(?![\w])", _val)]
    for _h in _hits:
        _seg = next((x for x in re.split(r"(?<=[.;]) ", _val)
                     if re.search(r"(?<![\w])" + _fn + r"(?![\w])", x)), _val)
        if not any(t in _seg for t in ("DIAGNOSTIC", "diagnostic", "NON_AUTHORIZING", "non-authorizing",
                                       "PROVISIONAL_UNTIL_M3", "bare-evidence", "bare-dict")):
            _stale_bad.append(f"{_slot}:{_fn}")
rec("b1-property", f"24 seeded (demoted function x authorizing slot) pairs: a demoted name never occupies an authorizing slot except inside a clause that declares it demoted (seed {SEED + 116})",
    not _stale_bad, ";".join(sorted(set(_stale_bad))[:3]) or "no stale designation")
rec("b1-property", f"and the exhaustive scan covers every current artifact, machine and prose, with no key-name exemption list ({len(_ALL_CURRENT)} artifacts)",
    len(_ALL_CURRENT) >= 50 and "TARGET-CONTRACT.json" in _ALL_CURRENT and "SCHEMA-REGISTRY.md" in _ALL_CURRENT
    and "AUTHORITY-FUNCTION-CLASSES.json" in _ALL_CURRENT,
    f"{len(_ALL_CURRENT)} artifacts")

# ================================================================ V115-M1: receipt publication == runtime binding
_RUNTIME_FIELDS = list(L.RECEIPT_BOUND_FIELDS)
_PUB_WF = WFLAW["core_authority"]["receipt"]
_PUB_FC = FC["receipt_binding"]


def _digested_fields():
    """The field names location_receipt() ACTUALLY assembles, read out of its source by AST - so the declared tuple
    cannot simply assert something the function does not do."""
    tree = ast.parse(textwrap.dedent(inspect.getsource(L.location_receipt)))
    for node in ast.walk(tree):
        if isinstance(node, ast.Assign) and isinstance(node.targets[0], ast.Name) \
                and node.targets[0].id == "values" and isinstance(node.value, ast.Dict):
            return [k.value for k in node.value.keys]
    return []


rec("m1-receipt-parity", "V115-M1 PINNED: RUNTIME_BOUND_FIELDS == PUBLISHED_BOUND_FIELDS exactly, in both directions and in order, in BOTH machine artifacts. v1.15 published seven fields while the runtime bound ten.",
    _PUB_WF["binds"] == _RUNTIME_FIELDS and _PUB_FC["binds"] == _RUNTIME_FIELDS
    and len(_RUNTIME_FIELDS) == 10,
    f"{len(_RUNTIME_FIELDS)} fields, published == runtime")
rec("m1-receipt-parity", "V115-M1 section 16 PINNED: the declared field tuple IS what location_receipt() assembles and digests - read out of the function by AST, not taken on trust - and the function refuses RECEIPT_BINDING_DRIFT if they ever disagree",
    _digested_fields() == _RUNTIME_FIELDS
    and "RECEIPT_BINDING_DRIFT" in inspect.getsource(L.location_receipt)
    and "{f: values[f] for f in RECEIPT_BOUND_FIELDS}" in inspect.getsource(L.location_receipt)
    and "RECEIPT_BINDING_DRIFT" in L.LOCATION_ERRORS,
    ",".join(_digested_fields()))
rec("m1-receipt-parity", "V115-M1 section 15: the three fields v1.15 omitted are exactly workflow_path, workflow_sha256 and evidence_snapshot_digest, and all three are published now",
    sorted(set(_RUNTIME_FIELDS) - {"session_id", "document_path", "governed_root", "authority_version",
                                   "manifest_sha256", "document_sha256", "location_validated"})
    == ["evidence_snapshot_digest", "workflow_path", "workflow_sha256"]
    and all(f in _PUB_WF["binds"] and f in _PUB_FC["binds"]
            for f in ("workflow_path", "workflow_sha256", "evidence_snapshot_digest")))
rec("m1-receipt-parity", "V115-M1 section 19: every bound field publishes its operational semantics - source, normative, digest inclusion, live revalidation and whether the authorizing core consumes it - so a consumer can reproduce the receipt from machine authority alone",
    [f["field"] for f in _PUB_WF["bound_fields"]] == _RUNTIME_FIELDS
    and all(sorted(f) == ["consumed_by_authorizing_core", "field", "in_digest", "normative", "revalidated_live", "source"]
            for f in _PUB_WF["bound_fields"])
    and all(f["in_digest"] is True and f["normative"] is True and f["revalidated_live"] is True
            for f in _PUB_WF["bound_fields"])
    and _PUB_WF["bound_fields"] == _PUB_FC["bound_fields"])
rec("m1-receipt-parity", "the published semantics are true of the implementation: every field is inside the digest, the frozen constants are named as constants, and location_validated is a constant True rather than a caller claim",
    all(f in inspect.getsource(L.location_receipt) for f in _RUNTIME_FIELDS)
    and "FROZEN CONSTANT" in L.RECEIPT_FIELD_SEMANTICS["governed_root"]["source"]
    and "FROZEN CONSTANT" in L.RECEIPT_FIELD_SEMANTICS["authority_version"]["source"]
    and "CONSTANT True" in L.RECEIPT_FIELD_SEMANTICS["location_validated"]["source"]
    and '"location_validated": True' in inspect.getsource(L.location_receipt))
rec("m1-receipt-parity", "V115-M1 section 20: the receipt domain is UNCHANGED - documentation catching up is not a contract change - and section 21: the snapshot domain remains distinct and is published as distinct",
    L.LOCATION_RECEIPT_DOMAIN == "vidtoolz.resolveLocationReceipt.v1"
    and _PUB_WF["domain"] == L.LOCATION_RECEIPT_DOMAIN
    and _PUB_WF["snapshot_domain_is_distinct"] == L.EVIDENCE_SNAPSHOT_DOMAIN
    and L.EVIDENCE_SNAPSHOT_DOMAIN != L.LOCATION_RECEIPT_DOMAIN
    and L.LOCATION_RECEIPT_DOMAIN in L.HASH_DOMAINS and L.EVIDENCE_SNAPSHOT_DOMAIN in L.HASH_DOMAINS)

# ---- V115-M1 section 22: the negatives. Omit one field, add a false one, permute.
_omit_bad, _extra_bad, _perm_bad = [], [], []


def _parity_errors(published):
    """The exact-set law, applied to a candidate publication. This is the law the suite freezes: an omission, an
    over-claim or a reordering is a failure."""
    errs = []
    if list(published) != _RUNTIME_FIELDS:
        errs.append("ORDERED_SET_MISMATCH")
    if set(published) - set(_RUNTIME_FIELDS):
        errs.append(f"CLAIMED_NOT_BOUND: {sorted(set(published) - set(_RUNTIME_FIELDS))}")
    if set(_RUNTIME_FIELDS) - set(published):
        errs.append(f"BOUND_NOT_PUBLISHED: {sorted(set(_RUNTIME_FIELDS) - set(published))}")
    return errs


for _f in _RUNTIME_FIELDS:
    _cand = [x for x in _RUNTIME_FIELDS if x != _f]
    _e = _parity_errors(_cand)
    if not any(_f in x for x in _e if x.startswith("BOUND_NOT_PUBLISHED")):
        _omit_bad.append(_f)
rec("m1-receipt-parity", f"V115-M1 section 22 PINNED: removing ANY ONE of the {len(_RUNTIME_FIELDS)} runtime-bound fields from the publication fails the parity law - including each of the three v1.15 omitted exactly as they were",
    not _omit_bad and _parity_errors(["session_id", "document_path", "governed_root", "authority_version",
                                      "manifest_sha256", "document_sha256", "location_validated"]),
    ",".join(_omit_bad) or f"all {len(_RUNTIME_FIELDS)} omissions caught")
rec("m1-receipt-parity", "V115-M1 section 22 PINNED: the exact v1.15 published list - the seven v1.14 fields - fails the law with BOUND_NOT_PUBLISHED naming precisely workflow_path, workflow_sha256 and evidence_snapshot_digest",
    any(x == "BOUND_NOT_PUBLISHED: ['evidence_snapshot_digest', 'workflow_path', 'workflow_sha256']"
        for x in _parity_errors(["session_id", "document_path", "governed_root", "authority_version",
                                 "manifest_sha256", "document_sha256", "location_validated"])),
    ";".join(_parity_errors(["session_id", "document_path", "governed_root", "authority_version", "manifest_sha256",
                             "document_sha256", "location_validated"]))[:90])
for _fake in ("host_name", "evaluated_at", "principal", "lock_holder", "resolve_version"):
    if not any(x.startswith("CLAIMED_NOT_BOUND") for x in _parity_errors(_RUNTIME_FIELDS + [_fake])):
        _extra_bad.append(_fake)
rec("m1-receipt-parity", "V115-M1 section 22 PINNED: claiming a field the runtime does not bind also fails - the law is exact-set equality, not containment (five plausible extras tested)",
    not _extra_bad, ",".join(_extra_bad) or "every over-claim caught")
for _i in range(24):
    _perm = list(_RUNTIME_FIELDS)
    _P16.shuffle(_perm)
    if _perm != _RUNTIME_FIELDS and not _parity_errors(_perm):
        _perm_bad.append(str(_i))
rec("m1-property", f"24 seeded permutations, omissions and additions of the receipt field list: every deviation from the runtime set is caught, and only the exact ordered set passes (seed {SEED + 116})",
    not _perm_bad and not _parity_errors(_RUNTIME_FIELDS), ",".join(_perm_bad[:3]) or "exact-set law holds")

# ---- V115-M1 section 23: a consumer can do this from machine authority alone
rec("m1-receipt-parity", "V115-M1 section 23 PINNED: Hermes's package can derive the exact runtime receipt field set from frozen machine authority WITHOUT inspecting Python - two artifacts publish it, they agree with each other, and M0A-BINDING-VALUES names them and the field list",
    _M0AV["receipt_bound_fields"] == ", ".join(_RUNTIME_FIELDS)
    and _M0AV["receipt_bound_field_count"] == len(_RUNTIME_FIELDS)
    and "AUTHORITY-FUNCTION-CLASSES.json" in _M0AV["receipt_binding_artifact"]
    and "EVIDENCE-SET-WORKFLOW.json" in _M0AV["receipt_binding_artifact"]
    and "workflow_path" in _M0AV["operational_package_update_required_v116"]
    and "AUTHORITY-FUNCTION-CLASSES.json" in _M0AV["function_classification_artifact"])
rec("m1-receipt-parity", "and a consumer who recomputes the receipt from the PUBLISHED field list gets the authority's own value, which is the operational point of the fix",
    (lambda sid, doc, wf, snap: L.digest({f: {"session_id": sid, "document_path": L.canonical_document_path(sid),
                                              "workflow_path": L.canonical_workflow_path(sid),
                                              "governed_root": L.GOVERNED_ATTACHMENT_ROOT,
                                              "authority_version": L.AUTHORITY_VERSION,
                                              "manifest_sha256": _CB_ACTIVE["manifest_sha256"],
                                              "document_sha256": doc, "workflow_sha256": wf,
                                              "evidence_snapshot_digest": snap, "location_validated": True}[f]
                                          for f in _PUB_WF["binds"]}, _PUB_WF["domain"])
                                 == L.location_receipt(sid, doc, _CB_ACTIVE, workflow_sha256=wf, snapshot_digest=snap))(
        "sess-v116-parity", "a" * 64, "b" * 64, "c" * 64))

# ---- static audits (section 42)
rec("v116-static", "section 42: every current authority-function reference resolves to exactly one class, and the canonical map is the only source",
    not _claim_bad and not _auth_slot_bad and not _diag_slot_bad
    and FC["source_of_truth"].endswith("#authority_function_classes()"))
rec("v116-static", "section 42: TARGET-CONTRACT and SCHEMA-REGISTRY agree with each other and with the code on every one of the twelve decision functions",
    all((_FC_MAP[f] == "authorizing") == (f in json.dumps({k: v for k, v in _TCJ.items() if k in ("attachment_derivation", "attachment_law", "qualification_note")}))
        or _FC_MAP[f] != "authorizing"
        for f in ("derive_attachment_state_authorizing", "derive_attachment_state_for_session"))
    and "legacy bare-evidence pair" in _SRMD
    and not re.search(r"(?<![\w.])derive_attachment_state(?!_authorizing|_for_session)(?![\w])",
                      " ".join(x for x in re.split(r"(?<=[.;]) ", json.dumps([_TCJ["attachment_derivation"], _TCJ["attachment_law"], _TCJ["qualification_note"]]))
                               if "DIAGNOSTIC" not in x and "diagnostic" not in x)))
rec("v116-static", "section 42: the runtime receipt field set equals the published set, and both receipt domains are documented in CANONICALIZATION.md",
    _PUB_WF["binds"] == _RUNTIME_FIELDS
    and all(d in open(os.path.join(B, "CANONICALIZATION.md"), encoding="utf-8").read()
            for d in (L.LOCATION_RECEIPT_DOMAIN, L.EVIDENCE_SNAPSHOT_DOMAIN)))
rec("v116-static", "section 42: no active v1.15 contradiction survives - the two TARGET-CONTRACT fields, the SCHEMA-REGISTRY sentence and the seven-field receipt publication are all corrected, and each is asserted by its own pinned check above",
    all(result_of(s_, None) is not False for s_ in ("b1-classification", "m1-receipt-parity"))
    and _TCJ["version"] == L.AUTHORITY_VERSION)
rec("v116-static", "section 46: no current contradiction hides behind a precedence statement - the precedence document's blanket clause covers HISTORICAL statements only, and every CURRENT artifact is asserted correct on its own",
    "reads as its authorizing counterpart" in prec["law"]
    and "No current artifact may designate one of those four" in prec["law"]
    and not _claim_bad and not _auth_slot_bad and not SLOT_VIOLATIONS)

# ================================================================ 23. v1.17: Codex's sole v1.16 finding
# V116-B1 BLOCKER  EVIDENCE-SET-WORKFLOW.json#toctou.law still named the DIAGNOSTIC derive_attachment_state as the
#                  refusing authority; that key was absent from the canonical authorizing-slot inventory; and the
#                  v1.16 scanner missed it because an unrelated clause contained the phrase "no longer".
V116_INV = SLOTS.build_inventory(B)
_WF17 = load("EVIDENCE-SET-WORKFLOW.json")
_TOCTOU = _WF17["toctou"]["law"]
_V116B = os.path.join(os.path.dirname(B), "v1.16")

# ---- section 1: the historical reproduction, against the frozen parent
if os.path.isdir(_V116B):
    _p_wf = L.strict_load(os.path.join(_V116B, "EVIDENCE-SET-WORKFLOW.json"))
    _p_law = _p_wf["toctou"]["law"]
    _p_fc = L.strict_load(os.path.join(_V116B, "AUTHORITY-FUNCTION-CLASSES.json"))
    _p_bare = re.search(r"(?<![\w])derive_attachment_state(?!_authorizing|_for_session)(?![\w])", _p_law)
    rec("b1-toctou", "V116-B1 section 1 PINNED: in the FROZEN v1.16 bundle, EVIDENCE-SET-WORKFLOW.json#toctou.law names the bare derive_attachment_state as the refusing authority, with no diagnostic marking anywhere in the value",
        _p_bare is not None and "refuses" in _p_law and "DIAGNOSTIC" not in _p_law
        and "derive_attachment_state_authorizing" not in _p_law,
        f"...{_p_law[max(0, _p_bare.start() - 60):_p_bare.start() + 40]}..." if _p_bare else "not present")
    rec("b1-toctou", "V116-B1 section 1 PINNED: that exact key path is absent from the v1.16 canonical authorizing-slot inventory, which listed ten hand-written slots",
        "EVIDENCE-SET-WORKFLOW.json#toctou.law" not in _p_fc.get("authorizing_slots", {})
        and len(_p_fc.get("authorizing_slots", {})) == 10,
        f'{len(_p_fc.get("authorizing_slots", {}))} hand-written slots, toctou.law absent')
    _p_val = open(os.path.join(_V116B, "tools", "validate_v1_16.py"), encoding="utf-8").read()
    rec("b1-toctou", "V116-B1 section 1 PINNED: the v1.16 scanner's suppression mechanism, proved from the frozen v1.16 validator source - its marker list literally contained the bare phrase 'no longer', it split clauses only at '. ' and '; ', and in the TOCTOU sentence that phrase modifies 'the bundle's binding no longer resolves', so the offending comma-clause was never isolated and the violation was suppressed",
        '"no longer"' in _p_val
        and 'r"(?<=[.;]) "' in _p_val
        and "no longer" in _p_law
        and "no longer" in next(c for c in re.split(r"(?<=[.;]) ", _p_law)
                                if re.search(r"(?<![\w])derive_attachment_state(?!_authorizing)(?![\w])", c))
        and "no longer" not in " ".join(SLOTS.CLASS_TOKENS + SLOTS.AUTHORITY_TOKENS + SLOTS.HISTORY_TOKENS),
        "v1.16 marker list contained 'no longer'; v1.17's contains no bare negation")
    rec("b1-toctou", "V116-B1 section 1 PINNED: the v1.17 clause law isolates 'and derive_attachment_state refuses' from that same v1.16 sentence, which carries no class token - so the defect would now be caught at once",
        (lambda cs: any(re.search(r"(?<![\w])derive_attachment_state(?!_authorizing)(?![\w])", c)
                        and not any(t in c for t in SLOTS.CLASS_TOKENS) for c in cs))(SLOTS.clauses(_p_law)),
        " | ".join(SLOTS.clauses(_p_law))[:150])
else:
    rec("b1-toctou", "SKIPPED: the frozen v1.16 bundle is not present beside this one", True, "v1.16 absent")

# ---- sections 2 and 3: the corrected law
rec("b1-toctou", "V116-B1 section 2 PINNED: the current TOCTOU law names the AUTHORIZING refusal - derive_attachment_state_authorizing, with the session wrapper as the equivalent - and no longer designates the diagnostic core",
    "The AUTHORIZING refusal is tools/authority_lib.py#derive_attachment_state_authorizing" in _TOCTOU
    and "derive_attachment_state_for_session" in _TOCTOU
    and "load_governed_evidence_set" in _TOCTOU)
rec("b1-toctou", "V116-B1 section 3: the diagnostic counterpart is still named for comparison, but explicitly AS DIAGNOSTIC / NON_AUTHORIZING and in its own clause, and it is not the function designated as enforcing",
    (lambda cs: all(any(t in c for t in SLOTS.CLASS_TOKENS)
                    for c in cs if re.search(r"(?<![\w])derive_attachment_state(?!_authorizing|_for_session)(?![\w])", c)))(
        SLOTS.clauses(_TOCTOU))
    and "DIAGNOSTIC / NON_AUTHORIZING" in _TOCTOU
    and "for fixtures and diagnosis only" in _TOCTOU)
rec("b1-toctou", "V116-B1 section 2: TOCTOU SEMANTICS ARE UNCHANGED - the same record-id/content-digest law, the same two additional seals, the same runtime enforcement",
    "a BUNDLE_VERIFICATION names the exact record ids it verified" in _TOCTOU
    and "A record id is a content digest" in _TOCTOU
    and _WF17["toctou"]["additional_seals"] == [
        "prepared_content_sha256 re-checked inside add_bundle_verification",
        "evidence_set_sha256 re-checked on every _load after VERIFIED"]
    and (L.strict_load(os.path.join(_V116B, "EVIDENCE-SET-WORKFLOW.json"))["toctou"]["additional_seals"]
         == _WF17["toctou"]["additional_seals"] if os.path.isdir(_V116B) else True))

# ---- sections 4, 10, 18: the inventory is DERIVED and the TOCTOU slot is in it
_TOCTOU_SLOT = "EVIDENCE-SET-WORKFLOW.json#toctou.law"
_disc = {g["slot"]: g for g in V116_INV["authorizing_slots"]}
rec("slot-discovery", f"V116-B1 sections 4/10 PINNED: the authorizing-slot inventory is DISCOVERED from the current artifacts, not hand-written, and the TOCTOU key is in it ({len(_disc)} slots from {V116_INV['counts']['scanned_artifacts']} current artifacts; v1.16 hand-listed 10 and missed this one)",
    _TOCTOU_SLOT in _disc and set(_disc[_TOCTOU_SLOT]["classes"]) == {"authorizing"}
    and "derive_attachment_state_authorizing" in _disc[_TOCTOU_SLOT]["functions"],
    f'{_TOCTOU_SLOT} -> {_disc.get(_TOCTOU_SLOT, {}).get("functions")}')
rec("slot-discovery", "V116-B1 section 4: the canonical classes artifact no longer carries a hand-written slot dictionary at all - it publishes the DISCOVERY LAW and points at the generated inventory, so a slot cannot be omitted by forgetting to list it",
    "authorizing_slots" not in FC and "diagnostic_slots" not in FC
    and FC["slot_discovery_law"]["generated_inventory"] == "AUTHORITY-SLOT-INVENTORY.json"
    and FC["slot_discovery_law"]["generator"].startswith("tools/authority_slots.py"))
rec("slot-discovery", "V116-B1 section 5: the frozen definition is semantic - a slot is authorizing because its VALUE designates authority (authoritative/deciding/refusing/enforcing/deriving/gating/validating), never because of its key name",
    all(t in FC["slot_discovery_law"]["definition"] for t in
        ("authoritative", "deciding", "refusing", "enforcing", "deriving", "gating", "validating",
         "never a key-name allowlist"))
    and set(FC["slot_discovery_law"]["authority_tokens"]) == set(SLOTS.AUTHORITY_TOKENS)
    and set(FC["slot_discovery_law"]["class_tokens"]) == set(SLOTS.CLASS_TOKENS))
rec("slot-discovery", f"V116-B1 sections 6/16: every reference in every CURRENT artifact is classified, with the scanned set derived from precedence STATUS and published ROLE rather than a filename pattern ({V116_INV['counts']['references']} references, {V116_INV['counts']['scanned_artifacts']} scanned, {V116_INV['counts']['not_scanned']} out of scope by status or role)",
    V116_INV["counts"]["references"] > 200 and V116_INV["counts"]["scanned_artifacts"] >= 50
    and all(a["status"] == "STILL_ACTIVE" and a["role"] in SLOTS.CURRENT_ROLES for a in V116_INV["scanned_artifacts"])
    and all(a["status"] != "STILL_ACTIVE" or a["role"] not in SLOTS.CURRENT_ROLES for a in V116_INV["not_scanned"])
    and {"TARGET-CONTRACT.json", "EVIDENCE-SET-WORKFLOW.json", "PERMISSIONS.json", "SEMANTIC-VALIDATION.md",
         "PRINCIPALS.json", "M0A-BINDING-VALUES.json"} <= {a["artifact"] for a in V116_INV["scanned_artifacts"]},
    f'{V116_INV["counts"]["scanned_artifacts"]} scanned')
rec("slot-discovery", "V116-B1 sections 11/31 PINNED: ZERO violations - every reference to a demoted function in a current artifact carries a class token in its own clause (R1), and every clause that designates authority names a function of class authorizing (R2)",
    not V116_INV["violations"],
    "; ".join(f'{v["artifact"]}#{v["location"]}:{v["function"]}:{v["violation"]}'
              for v in V116_INV["violations"][:3]) or f'{V116_INV["counts"]["references"]} references, 0 violations')
rec("slot-discovery", "V116-B1 section 18: the inventory artifact is GENERATED by the validator from the current artifacts and published with artifact, key path, referenced function, authority role, function class and status per row",
    os.path.exists(os.path.join(B, "AUTHORITY-SLOT-INVENTORY.json"))
    and (lambda inv: inv["counts"] == V116_INV["counts"]
                     and all(set(r) >= {"artifact", "location", "function", "role", "function_class", "status"}
                             for r in inv["references"][:20])
                     and inv["generator"] == "tools/authority_slots.py#inventory")(
        L.strict_load(os.path.join(B, "AUTHORITY-SLOT-INVENTORY.json"))))
rec("slot-discovery", "V116-B1 section 11: every discovered authorizing slot names a function in the authorizing class, and every historical statement is recognised as historical rather than flagged",
    all(any(c == "authorizing" for c in g["classes"]) for g in V116_INV["authorizing_slots"])
    and V116_INV["counts"]["historical_statements"] > 0
    and not [r for r in V116_INV["references"] if r["role"] == SLOTS.ROLE_HISTORICAL and r["status"] != "STILL_ACTIVE"])

# ---- sections 7, 8, 9, 12, 13, 14, 15: the scanner itself, on synthetic adversarial input
_SCN = []


def _scan_text(text, artifact="SYNTHETIC.json", location="synthetic.law"):
    """Classify one synthetic value exactly as the engine classifies a real one."""
    classes = L.authority_function_classes()
    pat = SLOTS.function_pattern(L.CLASSIFIED_DECISION_FUNCTIONS)
    rows = []
    kp_role = SLOTS.keypath_role(artifact, location)
    for clause in SLOTS.clauses(text):
        clause_role = SLOTS.classify_clause(clause)
        role = clause_role if clause_role in (SLOTS.ROLE_DIAGNOSTIC, SLOTS.ROLE_HISTORICAL) else (kp_role or clause_role)
        for m in pat.finditer(clause):
            module, fn = SLOTS.normalize_reference(m.group(0))
            if fn not in L.CLASSIFIED_DECISION_FUNCTIONS or module in SLOTS.FOREIGN_MODULES:
                continue
            rows.append({"artifact": artifact, "location": location, "reference": m.group(0), "function": fn,
                         "clause": clause, "role": role, "function_class": classes.get(fn, "UNCLASSIFIED"),
                         "status": "STILL_ACTIVE", "document_role": "NORMATIVE_AUTHORITY"})
    return rows, SLOTS.violations(rows)


# section 12: the exact historical defect sentence must fail classification
_v116_sentence = ("a BUNDLE_VERIFICATION names the exact record ids it verified. A record id is a content digest, "
                  "so editing a verified preparer record changes its id, the bundle's binding no longer resolves, "
                  "and derive_attachment_state refuses.")
_r, _v = _scan_text(_v116_sentence, location="toctou.law")
rec("scanner-negation", "V116-B1 section 12 PINNED: the EXACT v1.16 TOCTOU sentence - 'binding no longer resolves, and derive_attachment_state refuses' - FAILS classification, as DEMOTED_FUNCTION_DESIGNATED_AUTHORITATIVE. This is the historical defect, frozen as a regression.",
    len(_v) == 1 and _v[0]["violation"] == "DEMOTED_FUNCTION_DESIGNATED_AUTHORITATIVE"
    and _v[0]["function"] == "derive_attachment_state"
    and _v[0]["clause"] == "and derive_attachment_state refuses",
    f'{[x["violation"] for x in _v]} in clause {_v[0]["clause"]!r}' if _v else "NOT CAUGHT")
rec("scanner-negation", "V116-B1 section 7: the phrase 'no longer' appears in NO token list - the v1.16 suppression mechanism cannot exist, because a bare negation never classifies anything",
    not any("no longer" in t for t in SLOTS.CLASS_TOKENS + SLOTS.AUTHORITY_TOKENS + SLOTS.HISTORY_TOKENS)
    and "no bare negation" in FC["slot_discovery_law"]["negation_law"].lower().replace("no bare negation", "no bare negation"))

# section 14: negation must bind only the claim it modifies
for _text, _want in (
        # a NEGATED authority claim about X is a non-authorizing declaration about X (mission section 8), while the
        # authorizing claim about Y in the next clause is unaffected
        ("derive_attachment_state is no longer authoritative; derive_attachment_state_authorizing is authoritative.",
         {"derive_attachment_state": SLOTS.ROLE_DIAGNOSTIC, "derive_attachment_state_authorizing": SLOTS.ROLE_AUTHORIZING}),
        # "no longer" next to a NON-authority verb negates nothing: this is the exact V116-B1 shape, with the
        # authorizing function named, so it must classify as a designation and pass
        ("the binding no longer resolves, and derive_attachment_state_authorizing refuses.",
         {"derive_attachment_state_authorizing": SLOTS.ROLE_AUTHORIZING}),
        ("evaluate_eligibility is no longer the evaluator; the authorizing evaluator is evaluate_eligibility_authorizing.",
         {"evaluate_eligibility": SLOTS.ROLE_DIAGNOSTIC, "evaluate_eligibility_authorizing": SLOTS.ROLE_AUTHORIZING})):
    _r, _v = _scan_text(_text)
    _roles = {r["function"]: r["role"] for r in _r}
    _ok = all(_roles.get(f) == want for f, want in _want.items())
    _SCN.append(("negation", _ok))
    rec("scanner-negation", f"V116-B1 sections 8/14: negation binds only its own clause - {_text[:64]}...",
        _ok, f"roles={_roles} violations={[x['violation'] for x in _v]}")
rec("scanner-negation", "V116-B1 section 14: an adversarial phrasing that ASSERTS the diagnostic function is authoritative is caught, even though the sentence also contains 'no longer'",
    (lambda rv: len(rv[1]) >= 1 and any(v["function"] == "derive_attachment_state" for v in rv[1]))(
        _scan_text("derive_attachment_state is no longer diagnostic; derive_attachment_state is authoritative.")),
    "caught")

# section 15: mixed clauses
_mixed = ("the DIAGNOSTIC / NON_AUTHORIZING derive_attachment_state is not authority; "
          "the current authority is derive_attachment_state_authorizing, which refuses a forbidden-location set.")
_r, _v = _scan_text(_mixed)
_roles = {r["function"]: r["role"] for r in _r}
rec("scanner-negation", "V116-B1 section 15 PINNED: a mixed clause classifies BOTH correctly - the diagnostic function as a diagnostic declaration and the authorizing function as the designation - with no violation",
    _roles.get("derive_attachment_state") == SLOTS.ROLE_DIAGNOSTIC
    and _roles.get("derive_attachment_state_authorizing") == SLOTS.ROLE_AUTHORIZING and not _v,
    f"roles={_roles}")

# section 13: an unfamiliar key must not hide a designation
for _key in ("decision_handler", "refusal_owner", "state_engine", "gatekeeper", "who_decides"):
    # two phrasings: the verb in a following clause (caught by R1, the unmarked-reference law) and the verb in the
    # SAME clause (caught by R2, the designation law). Either is a detection; both must detect.
    _r, _v = _scan_text(f"the {_key} is authority_lib.derive_attachment_state, which decides the attachment state.",
                        location=_key)
    _r2, _v2 = _scan_text(f"the {_key} is authority_lib.derive_attachment_state which decides the attachment state.",
                          location=_key)
    _ok = (len(_v) == 1 and _v[0]["violation"] in ("DEMOTED_FUNCTION_DESIGNATED_AUTHORITATIVE",
                                                   "UNMARKED_DEMOTED_REFERENCE")
           and len(_v2) == 1 and _v2[0]["violation"] == "DEMOTED_FUNCTION_DESIGNATED_AUTHORITATIVE")
    _SCN.append((f"unknown-key:{_key}", _ok))
    rec("unknown-slot", f"V116-B1 section 13 PINNED: an UNFAMILIAR key name ({_key}) whose value names the diagnostic function as deciding authority is DETECTED - the law is not hardcoded for TOCTOU",
        _ok, f'{[x["violation"] for x in _v]} / {[x["violation"] for x in _v2]}')
rec("unknown-slot", f"the unknown-key matrix is complete and every case is caught ({sum(1 for k, o in _SCN if k.startswith('unknown-key') and o)}/5)",
    all(o for k, o in _SCN if k.startswith("unknown-key")))
rec("unknown-slot", "V116-B1 section 13: the same unfamiliar key naming the AUTHORIZING function is accepted, so the rule discriminates rather than simply objecting to unfamiliar keys",
    not _scan_text("the decision_handler is authority_lib.derive_attachment_state_authorizing, which decides the "
                   "attachment state.", location="decision_handler")[1])

# section 9: reference-form normalization
_FORMS = [("derive_attachment_state", (None, "derive_attachment_state")),
          ("`derive_attachment_state`", (None, "derive_attachment_state")),
          ("authority_lib.derive_attachment_state", ("authority_lib", "derive_attachment_state")),
          ("authority_lib.py#derive_attachment_state", ("authority_lib", "derive_attachment_state")),
          ("tools/authority_lib.py#derive_attachment_state", ("authority_lib", "derive_attachment_state")),
          ("tools/authority_lib.py#derive_attachment_state(tc, es, active)", ("authority_lib", "derive_attachment_state")),
          ("evidence_authoring.derive_attachment_state", ("evidence_authoring", "derive_attachment_state")),
          ("`authority_lib.py#evaluate_eligibility`", ("authority_lib", "evaluate_eligibility"))]
_norm_bad = [f for f, want in _FORMS if SLOTS.normalize_reference(f) != want]
rec("scanner-negation", f"V116-B1 section 9: every accepted reference form normalises to one canonical (module, name) - bare, backticked, module-qualified, anchor, path-qualified and call forms ({len(_FORMS)} forms)",
    not _norm_bad, ",".join(_norm_bad[:3]) or "all normalise")
rec("scanner-negation", "V116-B1 section 9: module awareness - evidence_authoring.derive_attachment_state is the AUTHORIZING wrapper, a different symbol from the diagnostic authority_lib core, and is never judged against the authority_lib class map",
    SLOTS.normalize_reference("evidence_authoring.derive_attachment_state")[0] == "evidence_authoring"
    and "evidence_authoring" in SLOTS.FOREIGN_MODULES
    and not _scan_text("evidence_authoring.derive_attachment_state is the authorizing wrapper that decides.")[1]
    and any(r["function_class"] == "FOREIGN_MODULE_SYMBOL" for r in V116_INV["references"]))
rec("scanner-negation", "V116-B1 section 9: a longer name is never matched as a shorter one plus a suffix, so derive_attachment_state_authorizing is not read as derive_attachment_state",
    [r["function"] for r in _scan_text("derive_attachment_state_authorizing decides.")[0]] == ["derive_attachment_state_authorizing"]
    and [r["function"] for r in _scan_text("validate_transaction_set_authorizing decides.")[0]] == ["validate_transaction_set_authorizing"])
rec("scanner-negation", "V116-B1 section 7: a clause delimiter inside a dotted path, an argument list or a version number does not split a reference",
    SLOTS.clauses("authority_lib.evaluate_eligibility and authority_lib.derive_attachment_state are DIAGNOSTIC.")
    == ["authority_lib.evaluate_eligibility and authority_lib.derive_attachment_state are DIAGNOSTIC"]
    and SLOTS.clauses("f(a, b) decides.") == ["f(a, b) decides"]
    and len(SLOTS.clauses("as of v1.16.0 the law changed.")) == 1)

# ---- section 32: property tests over the scanner itself
_P17 = random.Random(SEED + 117)
_prop_bad = []
_DEMOTED = [f for f in L.CLASSIFIED_DECISION_FUNCTIONS if L.authority_function_class(f) != "authorizing"]
_AUTHZ = [f for f in L.CLASSIFIED_DECISION_FUNCTIONS if L.authority_function_class(f) == "authorizing"]
_KEYS = ["decision_handler", "law", "toctou.law", "who_refuses", "engine", "note", "qualification_note", "x_y_z"]
_FORMATS = ["{f}", "`{f}`", "authority_lib.{f}", "authority_lib.py#{f}", "tools/authority_lib.py#{f}", "{f}(a, b)"]
_VERBS = ["refuses", "decides", "is authoritative", "enforces", "gates eligibility", "validates the set"]
for _i in range(48):
    _fn = _P17.choice(_DEMOTED + _AUTHZ)
    _ref = _P17.choice(_FORMATS).format(f=_fn)
    _key = _P17.choice(_KEYS)
    _verb = _P17.choice(_VERBS)
    _neg = _P17.choice(["", "the binding no longer resolves, and ", "this is no longer the v1.12 rule, so "])
    _mark = _P17.choice(["", " (DIAGNOSTIC / NON_AUTHORIZING)"])
    _text = f"{_neg}{_ref}{_mark} {_verb}."
    _r, _v = _scan_text(_text, location=_key)
    _demoted_ref = L.authority_function_class(_fn) != "authorizing"
    _expect_violation = _demoted_ref and not _mark
    if bool(_v) != _expect_violation:
        _prop_bad.append(f"{_text!r} -> {[x['violation'] for x in _v]} (expected violation={_expect_violation})")
rec("v117-property", f"48 seeded cases over demoted/authorizing functions x six reference formats x eight key names x six authority verbs x three negation prefixes x optional class marking: a demoted designation is ALWAYS caught and a marked or authorizing one is NEVER a false positive (seed {SEED + 117})",
    not _prop_bad, "; ".join(_prop_bad[:2]) or "48/48 classified correctly")
_hist_bad = []
for _i in range(16):
    _fn = _P17.choice(_DEMOTED)
    _text = _P17.choice([f"v1.12 named {_fn} as the authority of its era.",
                         f"SUPERSEDED: {_fn} decided this in v1.13.",
                         f"the HISTORICAL rule was that {_fn} refuses."])
    if _scan_text(_text)[1]:
        _hist_bad.append(_text)
rec("v117-property", f"16 seeded HISTORICAL statements naming a demoted function are correctly not flagged (section 17), while the same sentence without a historical marker is (seed {SEED + 117})",
    not _hist_bad and bool(_scan_text(f"{_DEMOTED[0]} refuses.")[1]), "; ".join(_hist_bad[:2]) or "history preserved")

# ---- sections 20, 21: runtime and receipt unchanged
rec("v117-runtime-parity", "V116-B1 section 20 PINNED: the three runtime files expected to be untouched are BYTE-IDENTICAL to the frozen v1.16 bundle - evidence_authoring.py, a2_prepare.py and a2_verify.py",
    all(open(os.path.join(_V116B, "tools", f), "rb").read() == open(os.path.join(B, "tools", f), "rb").read()
        for f in ("evidence_authoring.py", "a2_prepare.py", "a2_verify.py")) if os.path.isdir(_V116B) else True,
    "byte-identical")
rec("v117-runtime-parity", "section 20: evidence_store.py differs from v1.16 by exactly one comment line (the validator's filename, carried by the tool rename)",
    (lambda a, b: (lambda d: len(d) == 2 and all("naming convention." in x for x in d))(
        [l for l in __import__("difflib").unified_diff(a, b, n=0)
         if (l.startswith("-") or l.startswith("+")) and not l.startswith(("---", "+++"))]))(
        open(os.path.join(_V116B, "tools", "evidence_store.py"), encoding="utf-8").read().splitlines(),
        open(os.path.join(B, "tools", "evidence_store.py"), encoding="utf-8").read().splitlines())
    if os.path.isdir(_V116B) else True)
rec("v117-runtime-parity", "section 20 PINNED: every changed line in authority_lib.py is a version pin, a lineage pin or a tool-name reference - NO runtime semantics changed in v1.17",
    (lambda diff: all(("AUTHORITY_VERSION" in l) or ("1.16.0" in l) or ("1.17.0" in l)
                      or ("a03923cd" in l) or ("459bd29e" in l) or ("ba77bfa2" in l) or ("af2c1b11" in l)
                      or ("b61f2482" in l) or ("0d4fc991" in l)
                      or ("semantic v1.1" in l) or ("build_v1_1" in l) or ("validate_v1_1" in l)
                      for l in diff))(
        [l[1:] for l in __import__("difflib").unified_diff(
            open(os.path.join(_V116B, "tools", "authority_lib.py"), encoding="utf-8").read().splitlines(),
            open(os.path.join(B, "tools", "authority_lib.py"), encoding="utf-8").read().splitlines(), n=0)
         if (l.startswith("-") or l.startswith("+")) and not l.startswith(("---", "+++"))])
    if os.path.isdir(_V116B) else True,
    "version/lineage/tool-name only")
rec("v117-runtime-parity", "section 21: the receipt parity of V115-M1 is untouched - ten runtime-bound fields, published identically in both machine artifacts, and the domain unchanged",
    list(L.RECEIPT_BOUND_FIELDS) == _WF17["core_authority"]["receipt"]["binds"]
    == FC["receipt_binding"]["binds"] and len(L.RECEIPT_BOUND_FIELDS) == 10
    and L.LOCATION_RECEIPT_DOMAIN == "vidtoolz.resolveLocationReceipt.v1"
    and _WF17["core_authority"]["receipt"]["snapshot_domain_is_distinct"] == L.EVIDENCE_SNAPSHOT_DOMAIN)

# ---- section 31: static audits
rec("v117-static", "section 31: every current function reference is classified into exactly one role, and every role is one of the four published ones",
    all(r["role"] in SLOTS.ROLES for r in V116_INV["references"])
    and V116_INV["counts"]["references"] == (len(V116_INV["authorizing_slots"]) * 0 + len(V116_INV["references"]))
    and sorted(FC["slot_discovery_law"]["roles"]) == sorted(SLOTS.ROLES))
rec("v117-static", "section 31: no diagnostic or provisional function occupies any discovered authorizing slot, and no current contradiction remains anywhere in scope",
    not V116_INV["violations"] and not _auth_slot_bad and not _diag_slot_bad)
rec("v117-static", "section 31 / section 19: no current contradiction hides behind precedence - the blanket clause names each demoted function WITH its class token in the same clause, so the clause itself cannot be read as a designation",
    (lambda law: all(any(t in c for t in SLOTS.CLASS_TOKENS)
                     for c in SLOTS.clauses(law)
                     if re.search(r"(?<![\w])(derive_attachment_state|evaluate_eligibility|validate_transaction_set|commit_eligibility)(?!_authorizing|_for_session)(?![\w])", c)))(prec["law"])
    and "AUTHORITY-SLOT-INVENTORY.json" in prec["law"])
rec("v117-static", "section 31: the published document-role vocabulary is complete and every registered artifact carries a role, so the scan scope is machine-derived rather than a filename pattern",
    set(prec["document_roles"]) == {"NORMATIVE_AUTHORITY", "CORRECTION_RECORD", "GENERATED_EVIDENCE", "HISTORICAL_INPUT"}
    and all("role" in e for e in prec["documents"])
    and all(e["role"] in prec["document_roles"] for e in prec["documents"])
    and "GENERATED_EVIDENCE" in prec["document_role_law"])
rec("v117-static", "section 31: the receipt publication remains exactly the runtime set, asserted here as well as in the inherited m1 section",
    L.receipt_binding_publication()["binds"] == list(L.RECEIPT_BOUND_FIELDS)
    and _WF17["core_authority"]["receipt"]["bound_fields"] == FC["receipt_binding"]["bound_fields"])
# ---- 21. determinism
...
CODEX_MATRIX = L.strict_load(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.16.json"))
INHERITED_V115 = L.strict_load(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.15.json"))
INHERITED_V114 = L.strict_load(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.14.json"))
INHERITED_V113 = L.strict_load(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.13.json"))
INHERITED_V112 = L.strict_load(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.12.json"))
...
rec("codex-matrix", "the inherited v1.6 finding matrix is retained unchanged as history and its seven ids are still enumerated",
    ...
    and CODEX_MATRIX["inherited_matrices"] == ["FINDING-RESOLUTION-MATRIX-v1.15.json", "FINDING-RESOLUTION-MATRIX-v1.14.json", "FINDING-RESOLUTION-MATRIX-v1.13.json", "FINDING-RESOLUTION-MATRIX-v1.12.json", "FINDING-RESOLUTION-MATRIX-v1.11.json", "FINDING-RESOLUTION-MATRIX-v1.10.json", "FINDING-RESOLUTION-MATRIX-v1.9.json", "FINDING-RESOLUTION-MATRIX-v1.8.json", "FINDING-RESOLUTION-MATRIX-v1.7.json"])
rec("codex-matrix", "Store.v5 carries NO semantic change in v1.16: evidence_store.py differs from the v1.15 bundle by exactly one comment line and every semantic constant is identical",
    (lambda a, b: (lambda da, db: len(da) == 1 and len(db) == 1
                   and da[0].lstrip("-").strip().startswith("# naming convention.")
                   and db[0].lstrip("+").strip().startswith("# naming convention."))(
        [l for l in __import__("difflib").unified_diff(a, b, n=0) if l.startswith("-") and not l.startswith("---")],
        [l for l in __import__("difflib").unified_diff(a, b, n=0) if l.startswith("+") and not l.startswith("+++")]))(
        open(os.path.join(os.path.dirname(B), "v1.15", "tools", "evidence_store.py"), encoding="utf-8").read().splitlines(),
        open(os.path.join(B, "tools", "evidence_store.py"), encoding="utf-8").read().splitlines()),
    "one comment line")
rec("codex-matrix", "the inherited v1.12 workflow matrix is retained unchanged and still enumerates V112-1..V112-4",
    sorted(f["id"] for f in INHERITED_V112["findings"]) == ["V112-1", "V112-2", "V112-3", "V112-4"])
...
rec("codex-matrix", "the ACTIVE v1.16 finding-resolution matrix enumerates exactly Codex's two v1.15 release findings",
    [f["id"] for f in CODEX_MATRIX["findings"]] == ["V115-B1", "V115-M1"])
rec("codex-matrix", "the v1.16 matrix records the 54/57 harness result, the one BLOCKER and one MERGE MAJOR, the frozen v1.15 reproduction head, and that every executable v1.15 repair was confirmed CLOSED",
    "54/57" in CODEX_MATRIX["adjudication"] and "ONE BLOCKER" in CODEX_MATRIX["adjudication"]
    and "ONE MERGE MAJOR" in CODEX_MATRIX["adjudication"] and "CLOSED" in CODEX_MATRIX["adjudication"]
    and "frozen-statement defects, not executable defects" in CODEX_MATRIX["adjudication"]
    and "459bd29e358574896f0cf69f4b0bfb347bf2902d" in CODEX_MATRIX["law"]
    and "NO RUNTIME LAW AT ALL" in CODEX_MATRIX["operational_closure"]
    and "EXACTLY ONE COMMENT LINE" in CODEX_MATRIX["operational_closure"]
    and "no positive control" in CODEX_MATRIX["operational_closure"])
rec("codex-matrix", "each v1.16 finding carries Codex's exact evidence: the key paths, the current values, the reproduction against the frozen parent and the affected gates",
    (lambda b1, m1: all(t in b1["codex_finding"] for t in ("TARGET-CONTRACT.json:5", "line 167", "SCHEMA-REGISTRY.md:8-11", "those two", "qualification_note"))
                    and "Affected gates" in b1["codex_finding"] and "frozen v1.15 bundle" in b1["prior_defect"]
                    and all(t in m1["codex_finding"] for t in ("EVIDENCE-SET-WORKFLOW.json:71-82", "authority_lib.py:302", "workflow_path", "workflow_sha256", "evidence_snapshot_digest"))
                    and "seven v1.14 fields" in m1["codex_finding"] and "(7)" in m1["prior_defect"] and "(10)" in m1["prior_defect"])(
        next(f for f in CODEX_MATRIX["findings"] if f["id"] == "V115-B1"),
        next(f for f in CODEX_MATRIX["findings"] if f["id"] == "V115-M1")))
rec("codex-matrix", "the v1.16 matrix records that the exhaustive invariant surfaced FOUR further stale statements Codex had not named, and that the provisional pair kept its class",
    "four further stale statements" in next(f for f in CODEX_MATRIX["findings"] if f["id"] == "V115-B1")["correction"]
    and "not relabelled" in next(f for f in CODEX_MATRIX["findings"] if f["id"] == "V115-B1")["correction"])
rec("codex-matrix", "the inherited v1.15 matrix is retained unchanged and still enumerates the six V114 findings against the frozen v1.14 head",
    [f["id"] for f in INHERITED_V115["findings"]] == ["V114-B1", "V114-B2", "V114-B3", "V114-B4", "V114-M1", "V114-N1"]
    and "b61f248238ec7ed60deca79aa9728ec0cde86edd" in INHERITED_V115["law"]
    and "HISTORICAL under v1.16" in INHERITED_V115["operational_closure"])
rec("codex-matrix", "the markdown matrix mirrors the machine form: both ids, every validation section they name, the exact key paths and the still-open residual",
    (lambda t_: all(f_["id"] in t_ for f_ in CODEX_MATRIX["findings"])
                and all(sec in t_ for f_ in CODEX_MATRIX["findings"] for sec in f_["validation_sections"])
                and all(k_ in t_ for k_ in ("TARGET-CONTRACT.json:5", "SCHEMA-REGISTRY.md:8-11", "EVIDENCE-SET-WORKFLOW.json:71-82", "authority_lib.py:302"))
                and "no positive control" in t_ and "one comment" in t_)(
        open(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.16.md"), encoding="utf-8").read()))
rec("codex-matrix", "the inherited v1.15 matrix still records its four-BLOCKER/one-MAJOR/one-MINOR adjudication and the essential consumed-bytes law",
    "FOUR BLOCKERs" in INHERITED_V115["adjudication"]
    and "THE EXACT BYTES VALIDATED AS GOVERNED EVIDENCE ARE THE EXACT BYTES CONSUMED" in INHERITED_V115["law"]
    and "BLOCKED THE AUTHORITY INDEFINITELY" in next(f_ for f_ in INHERITED_V115["findings"] if f_["id"] == "V114-B4")["prior_defect"])
rec("codex-matrix", "both v1.16 findings pin their reproduction against the frozen v1.15 head and carry the severities Codex assigned",
    all("frozen v1.15" in f_["prior_defect"] for f_ in CODEX_MATRIX["findings"])
    and [f_["severity"].split()[0] for f_ in CODEX_MATRIX["findings"]] == ["BLOCKER", "MERGE"])
rec("codex-matrix", "the inherited v1.14 matrix is retained unchanged and still enumerates V113-B1 against the frozen v1.13 head",
    [f["id"] for f in INHERITED_V114["findings"]] == ["V113-B1"]
    and "65895c6dedceadff012a08c1bafcdf9e92b65221" in INHERITED_V114["law"]
    and "HISTORICAL under v1.15" in INHERITED_V114["operational_closure"])
rec("codex-matrix", "the inherited v1.15 markdown matrix is retained unchanged and still mirrors its own machine form",
    (lambda t_: all(f_["id"] in t_ for f_ in INHERITED_V115["findings"])
                and "THE EXACT BYTES VALIDATED AS GOVERNED EVIDENCE ARE THE EXACT BYTES CONSUMED BY AUTHORIZING DERIVATION" in t_)(
        open(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.15.md"), encoding="utf-8").read()))
rec("codex-matrix", "the inherited v1.14 matrix still records Codex's 93/96 result and its own MINOR",
    "93 of 96" in INHERITED_V114["adjudication"]
    and "test_root_override" in INHERITED_V114["findings"][0]["prior_defect"])
rec("codex-matrix", "the inherited v1.13 governed-root matrix is retained unchanged and still enumerates V112-RP1 against the frozen v1.12 head",
    [f["id"] for f in INHERITED_V113["findings"]] == ["V112-RP1"]
    and "WORKFLOW_CONTRADICTORY" in INHERITED_V113["adjudication"]
    and "73150d00a72820b155caf357c412f46b8ec4a9ba" in INHERITED_V113["law"]
    and "HISTORICAL under v1.14" in INHERITED_V113["operational_closure"])
rec("codex-matrix", "the inherited v1.14 markdown matrix is retained unchanged and still mirrors its own machine form",
    (lambda t_: "V113-B1" in t_ and all(sec in t_ for sec in INHERITED_V114["findings"][0]["validation_sections"])
                and "semantic_mutation_plan" in t_ and "one comment line" in t_)(
        open(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.14.md"), encoding="utf-8").read()))
rec("codex-matrix", "the inherited v1.12 matrix still records the AUTHORITY_UNDERSPECIFIED classification and its protected-surface promise",
    "AUTHORITY_UNDERSPECIFIED" in INHERITED_V112["adjudication"]
    and "changes NO Store.v5 semantics" in INHERITED_V112["operational_closure"])
...
for sec, _, ok, _ in R:
    ...
    sections[sec][1] += ok
FINDINGS = {
    "V115-B1 active TARGET-CONTRACT and SCHEMA-REGISTRY authority classifications contradictory (BLOCKER)": ["b1-classification", "b1-property"],
    "V115-M1 machine-readable location-receipt binding omits three runtime-bound fields (MERGE MAJOR)": ["m1-receipt-parity", "m1-property"],
    "v1.16 static audits": ["v116-static"],
    "V114-B1 consumed evidence not bound to the validated governed bytes; a FALSE ATTACHMENT_READY under valid provenance (BLOCKER)": ["b1-consumed-evidence", "b1-positive"],
    "V114-B2 diagnostic eligibility reaching commit authority (BLOCKER)": ["b2-transaction-authority"],
    "V114-B3 active authority artifacts naming diagnostic entrypoints (BLOCKER)": ["b3-active-naming"],
    "V114-B4 governed loader following a symlinked WORKFLOW.json; FIFO hang; raw IsADirectoryError (BLOCKER)": ["b4-workflow-file"],
    "V114-M1 malformed nominal carrier leaking a raw AttributeError (MERGE MAJOR)": ["m1-malformed-carrier"],
    "V114-N1 canonicalization documentation omitting registered hash domains (MINOR)": ["hash-domain-parity"],
    "v1.15 static audits and focused property suites (fixed seed)": ["v115-static", "v115-property"],
    "V113-B1 forbidden-location evidence authorized by the CORE derivation and eligibility (BLOCKER, authorizing location bypass)": ["core-authority", "core-bypass", "core-positive", "core-callgraph", "core-selflocation", "core-static"],
    "v1.14 focused property suites (fixed seed)": ["core-property"],
    "V112-RP1 caller-selectable and symlinked governed evidence root derived ATTACHMENT_READY (BLOCKER, WORKFLOW_CONTRADICTORY)": ["root-law", "root-parity", "root-property", "root-static"],
...
# v1.17 (V116-B1 section 18): the slot inventory is GENERATED here, from the current artifacts, by the same module
# the audits use. It is machine authority for consumers and evidence for reviewers, and never hand-maintained.
with open(os.path.join(B, "AUTHORITY-SLOT-INVENTORY.json"), "w", encoding="utf-8") as _inv_f:
    json.dump(SLOTS.build_inventory(B), _inv_f, indent=2, sort_keys=False, ensure_ascii=False)
    _inv_f.write("\n")

lines = ["# VALIDATION REPORT — Resolve authority bundle v1.17", "", f"Result: **{passed}/{total} checks passed**. v1.17 is an EXTREMELY NARROW correction of the SOLE BLOCKER Codex left against v1.16: V116-B1. Codex's independent harness scored 84/85 with zero merge majors and zero minors; intake, the supplied suite, receipt parity and all 57 v1.15 functional regressions passed; merge was BLOCKED and A1 authority-ready was NO. NO RUNTIME SEMANTICS CHANGED IN v1.17: tools/evidence_authoring.py, tools/a2_prepare.py and tools/a2_verify.py are BYTE-IDENTICAL to v1.16, tools/evidence_store.py differs by one comment line, and every changed line in tools/authority_lib.py is a version pin, a lineage pin or a tool-name reference - all asserted in section v117-runtime-parity. THE DEFECT: EVIDENCE-SET-WORKFLOW.json#toctou.law said 'the bundle's binding no longer resolves, and derive_attachment_state refuses', designating the DIAGNOSTIC core function as the refusing authority in a CURRENT normative law; that key path was absent from the hand-written ten-slot authorizing inventory; and the v1.16 scanner suppressed the violation because its marker list contained the bare phrase 'no longer', which in that sentence modifies THE BINDING rather than the function's authority class, while commas were not clause delimiters so the offending clause was never isolated. All three are reproduced against the frozen v1.16 bundle and pinned. THE CORRECTION, in three parts. (1) The TOCTOU law now names the AUTHORIZING refusal, tools/authority_lib.py#derive_attachment_state_authorizing with #derive_attachment_state_for_session as the equivalent session wrapper, and names the diagnostic counterpart explicitly AS DIAGNOSTIC / NON_AUTHORIZING in its own clause; TOCTOU semantics, the record-id law and both additional seals are unchanged. (2) The hand-written slot dictionaries are GONE from AUTHORITY-FUNCTION-CLASSES.json, replaced by a frozen SLOT DISCOVERY LAW - a slot is authorizing because its VALUE designates a function as authoritative, deciding, refusing, enforcing, deriving, gating or validating, whatever its key is called - and the inventory itself is GENERATED into AUTHORITY-SLOT-INVENTORY.json by tools/authority_slots.py from the current artifacts, so a slot cannot be omitted by forgetting to list it. (3) The scanner is rebuilt: clauses break at '.', ';', ':', ',' and newline at parenthesis depth zero and only before whitespace, so an argument list, a dotted module path and a version number stay intact while an unrelated claim can no longer silence a real one; wrapped markdown prose is rejoined into logical lines first so line-wrap position cannot change a classification; a claim is read ONLY from the clause containing the reference, with signal precedence explicit class token in that clause, then structured key path, then that clause's authority verbs; NO bare negation appears in any token list, so 'no longer' classifies nothing and negation binds only the claim it modifies; every reference form - bare, backticked, module-qualified, anchor, path-qualified and call form - normalises to one canonical (module, name), and a same-named function in another module is a DIFFERENT symbol, so evidence_authoring.derive_attachment_state (the authorizing wrapper) is never judged against the authority_lib class map. The scanned artifact set is now DERIVED from precedence: every artifact is registered with a STATUS and a published ROLE (NORMATIVE_AUTHORITY, CORRECTION_RECORD, GENERATED_EVIDENCE, HISTORICAL_INPUT), so scope is machine-readable rather than a filename pattern, and the journal-state and retired-term audits are scoped the same way. Running the new engine exhaustively over all current artifacts found THREE further stale statements Codex had not named - a current designation of the provisional commit pair in SNAPSHOT-CONCURRENCY-RECOVERY.md, and the retired CAPABILITY-EVIDENCE record type still described as the qualification mechanism in CAPABILITIES.json and READ-PRIMITIVES.json - all corrected. ZERO violations now stand across every current artifact. The adversarial regressions are frozen: the exact v1.16 sentence fails classification; five unfamiliar key names (decision_handler, refusal_owner, state_engine, gatekeeper, who_decides) naming the diagnostic function as deciding authority are all detected, while the same keys naming the authorizing function are accepted; negation, mixed-clause and adversarial 'X is no longer diagnostic; X is authoritative' phrasings classify correctly; and 48 + 16 seeded property cases over six reference formats, eight key names, six authority verbs and three negation prefixes produce no false negative and no false positive. Everything else is untouched and re-run: the governed evidence byte binding, the GovernedEvidenceSet snapshot law, the authorizing/diagnostic split, the authorizing transaction and commit path, the governed root law, WORKFLOW no-follow, malformed-carrier handling, roles, A2V, the lifecycle, the constructors, BUNDLE semantics, the checked-file law, TOCTOU runtime enforcement, locking, Store.v5, cache, the schema-registry architecture, the trusted shim, raw ingestion, the exact stored chain, H0, the commit stage architecture, the capability matrix and M0B/M0C/M0D. The v1.15 residual stands and is NOT closed here: the authorizing transaction path has no positive control because no production path authors M3 write evidence. Inherited context: v1.16 is an EXTREMELY NARROW correction of the TWO release findings Codex left against v1.15: one BLOCKER (V115-B1, active TARGET-CONTRACT and SCHEMA-REGISTRY authority classifications contradictory) and one MERGE MAJOR (V115-M1, the machine-readable location-receipt binding omits three runtime-bound fields). Codex's independent harness scored 54/57, authority completeness was WORKFLOW_CONTRADICTORY, merge was BLOCKED and A1 authority-ready was NO; the candidate remained clean and unmodified, the supplied suite reproduced 2320/2320 twice byte-identically, and EVERY EXECUTABLE v1.15 REPAIR WAS CONFIRMED CLOSED. NEITHER FINDING IS AN EXECUTABLE DEFECT: both are frozen statements that disagree with the runtime, and a consumer chooses its authority by reading statements - authority selection is itself an authorizing decision, which is why a documentation contradiction is a BLOCKER. V115-B1: TARGET-CONTRACT.json:5 (qualification_note) said attachment state is derived by tools/authority_lib.py#derive_attachment_state and :167 (attachment_law) said it is computed by derive_attachment_state - both CURRENT normative contract fields, neither marking that function diagnostic, and the corrected attachment_derivation field at :169 and the diagnostic field at :242 did not erase them; and SCHEMA-REGISTRY.md:8-11 named validate_transaction_set_authorizing / commit_eligibility_authorizing as the authorizing pair and then said 'those two' are PROVISIONAL_UNTIL_M3 and non-authorizing, the exact inverse of the executable surface and a wording error introduced by v1.15 itself. The v1.15 audit passed because its invariant EXCLUDED whole key names, including law and qualification_note: an exclusion list is not an invariant. v1.16 publishes ONE canonical classification, AUTHORITY-FUNCTION-CLASSES.json, GENERATED from authority_lib.AUTHORITY_SURFACE via #authority_function_classes() - one function to one class for all twelve names in #CLASSIFIED_DECISION_FUNCTIONS, the four class definitions, the per-class inventories, the AUTHORIZING-SLOT and DIAGNOSTIC-SLOT inventories by exact artifact key path, the runtime-lookup law and the generated receipt binding - corrects the three contradictory statements at their exact key paths, and makes the invariant EXHAUSTIVE: key-name exemptions drop from twenty to seven that self-declare as a demoted-function inventory or as history, each separately reconciled with the canonical map, and the scanner now also catches MODULE-QUALIFIED references that the v1.15 lookbehind let through. Running it exhaustively surfaced FOUR MORE stale statements Codex had not named, in generated values (EVIDENCE-SET-WORKFLOW.json#read_only_journal.law, M0A-BINDING-VALUES.json#values.verification_result_law and #values.toctou_law, and the FREEZE-MANIFEST per-file note for TARGET-CONTRACT.json), all corrected. The provisional pair KEEPS its exact class and its behaviour: it was not relabelled to eliminate the contradiction. The runtime-lookup audit proves no authorizing code resolves a function by name from any artifact - no getattr on the module, no eval/exec/importlib, and the single globals() lookup in the bundle is parser_sha256() over the frozen module constant PARSER_FUNCTION_NAMES, a digest computation rather than an authority selection - so a stale document could mislead a human or an external consumer but never make the authority call a diagnostic function. V115-M1: EVIDENCE-SET-WORKFLOW.json:71-82 published receipt.binds as the seven v1.14 fields while authority_lib.py:302 location_receipt bound TEN, omitting workflow_path and workflow_sha256 (v1.15 V114-B4, which made WORKFLOW.json governed evidence) and evidence_snapshot_digest (v1.15 V114-B1, the digest of the parse of the validated bytes); a consumer recomputing the receipt from frozen machine authority would have digested seven fields and disagreed with every receipt the authority mints. v1.16 declares RECEIPT_BOUND_FIELDS in digest order and has location_receipt() ASSEMBLE its digest body FROM that tuple, refusing RECEIPT_BINDING_DRIFT if the two ever disagree, so the declared set and the digested set are one thing rather than two; #RECEIPT_FIELD_SEMANTICS publishes each field's source, normativity, digest inclusion, live revalidation and whether the authorizing core consumes it; and #receipt_binding_publication() generates the publication that BOTH machine artifacts are produced from. Parity is asserted in both directions and by AST against the function body, and the negatives are frozen: removing any one of the ten fails, the exact v1.15 seven-field list fails naming precisely the three omissions, five plausible over-claims fail because the law is exact-set equality rather than containment, 24 seeded permutations fail, and a consumer recomputing the receipt from the PUBLISHED list is asserted to obtain the authority's own value. The receipt domain vidtoolz.resolveLocationReceipt.v1 is UNCHANGED and the evidence-set snapshot domain remains distinct: documentation catching up with the runtime is not a contract change. NO RUNTIME LAW CHANGED IN v1.16 - the consumed-evidence byte binding, the immutable carrier and snapshot law, governed root enforcement, the diagnostic/authorizing API separation, the transaction/commit provenance gate, the WORKFLOW.json no-follow and file-type law, malformed-carrier handling, Store.v5 (tools/evidence_store.py differs from v1.15 by exactly one comment line), roles, A2V, the lifecycle, the constructors, BUNDLE_VERIFICATION, the checked-file law, TOCTOU, locking, cache, the schema-registry architecture, the trusted shim, strict ingestion, the exact stored chain, H0, the commit stage architecture, precedence, the capability matrix and M0B/M0C/M0D are all untouched and re-run. The v1.15 residual stands unchanged and is NOT closed here: the authorizing transaction path has no positive control because no production path authors M3 write evidence. Inherited context: v1.15 is a NARROWLY SCOPED correction of the SIX release findings Codex left against v1.14: four BLOCKERs (V114-B1 consumed evidence not bound to the validated governed bytes, V114-B2 diagnostic eligibility reaching commit authority, V114-B3 active authority artifacts naming diagnostic entrypoints, V114-B4 the governed loader following a symlinked WORKFLOW.json), one MERGE MAJOR (V114-M1 a malformed nominal carrier leaking a raw AttributeError) and one MINOR (V114-N1 canonicalization documentation omitting a registered hash domain). Authority completeness was WORKFLOW_CONTRADICTORY and merge was FAIL. Hermes's root-existence sensitivity was classified REVIEW_ENVIRONMENT_EFFECT and is NOT treated as a candidate defect. All six were reproduced against the frozen v1.14 bundle at b61f248238ec7ed60deca79aa9728ec0cde86edd before anything was changed, and two proved WORSE than reported: a FIFO at WORKFLOW.json blocked the authority indefinitely and a directory raised a raw IsADirectoryError (V114-B4), and the undocumented hash-domain gap was five domains plus two recorded at superseded versions rather than one (V114-N1). THE ONE LAW v1.15 INSTALLS: THE EXACT BYTES VALIDATED AS GOVERNED EVIDENCE ARE THE EXACT BYTES CONSUMED BY AUTHORIZING DERIVATION. V114-B1: v1.14 stored the parsed evidence set in a writable carrier slot, so provenance for document A could be presented with semantic evidence B - reproduced here in its sharpest form, where a session honestly PROVISIONED_NOT_VERIFIED returned ATTACHMENT_READY with authorizing=true under its OWN receipt and its OWN canonical document path. v1.15 makes the carrier hold the validated BYTES and no parsed object: frozen slots, no evidence_set slot, a read-only property that strict-parses those bytes afresh on every access, copy/deepcopy/pickle refused, and a new registered domain vidtoolz.resolveEvidenceSetSnapshot.v1 whose digest of the PARSED content is bound into the location receipt. authority_lib.governed_consume() is now the ONE way an authorizing path obtains semantic evidence: it validates the carrier structurally, revalidates the live location, re-reads BOTH authority files no-follow, requires the live digests to equal the minted ones AND the carrier's own bytes to digest identically, strict-parses those exact bytes, schema-validates the parse, requires its snapshot digest to equal the receipt-bound one, recomputes the receipt, and RETURNS the object the authorizing path consumes. V114-B2: commit_eligibility and validate_transaction_set were classified AUTHORIZING while consuming a bare evidence dict, and the complete committed-consistent fixture supplied that way completed all twelve stages and returned eligible=true - a commit decision, which is why v1.14 recording it as a residual was wrong. v1.15 adds validate_transaction_set_authorizing and commit_eligibility_authorizing, which require a GovernedEvidenceSet and refuse GOVERNED_EVIDENCE_REQUIRED before any stage; semantic_mutation_plan chooses its eligibility law BY TYPE, never by a caller-supplied callback; and the old pair is DEMOTED to a fourth surface class, provisional_until_m3_non_authorizing. The twelve stages, their order, the internally pinned schemas, S0/S1, journal, delta, effects, verification, conflicts and H0 are UNCHANGED - asserted by running both validators over the same legitimate governed carrier and requiring identical stage errors, so v1.15 added a gate in front of the stage laws and changed none of them. V114-B3: four current artifacts still designated the demoted functions; all four now name the authorizing entry points, the diagnostic pair appears only AS diagnostic in its own schema-pinned contract field, AUTHORITY-PRECEDENCE.json carries a blanket re-reading clause with S62, and a static audit scans EVERY current top-level artifact, machine and prose - the precedence table does not even list TARGET-CONTRACT.json or PERMISSIONS.json, so scanning only that table would have missed the defect. V114-B4: WORKFLOW.json carries authority-bearing session, location, lifecycle and seal claims and was checked only for existence, then opened through strict_load. v1.15 gives both governed files ONE authority-file law in the core and in the wrapper: lstat the entry first, refuse a symlink, require a regular file so a FIFO/socket/device/directory is never opened, check the alias, then open with O_NOFOLLOW and O_NONBLOCK and re-verify the OPEN FILE by fstat for device and inode; WORKFLOW.json's digest is bound into the receipt and re-checked on every authorizing call. Every entry-touching call in the suite is alarm-bounded so a hang can never be mistaken for a refusal. V114-M1: governed_structure_errors validates type and every field with defaults BEFORE any dereference and runs first inside governed_consume, so object.__new__, a subclass, missing/None/wrong-typed slots, a malformed receipt and copy/deepcopy/pickle all fail closed as GOVERNED_EVIDENCE_INVALID at all three public authorizing boundaries - while a genuine programming error still raises, preserving the v1.10 distinction. V114-N1: CANONICALIZATION.md now names every registered domain and section hash-domain-parity compares it with HASH_DOMAINS exactly in BOTH directions. The MANDATORY positive control runs at the REAL canonical governed root through an INTERNAL_NON_AUTHORIZING testkit seam that works in BOTH reviewer environments - creating and removing the whole tree when the root is absent, and exactly one uniquely named session directory when it is present, refusing to adopt an existing one - which is how the Hermes REVIEW_ENVIRONMENT_EFFECT is addressed without making production authority configurable: the resolver still takes no argument. It asserts ATTACHMENT_READY with authorizing=true and eligible=true with reason code ELIGIBLE, so the correction is not refuse-everything. ONE RESIDUAL IS STATED RATHER THAN CLOSED: the authorizing transaction path has no positive control, because no production path authors M3 write evidence; it is proved fail-closed and proved to apply the ordinary pre-M3 stage laws once provenance passes, and its first true positive belongs to the milestone that freezes write evidence. Nothing else is reopened: the v1.12 workflow architecture, the v1.13 governed-root law, Store.v5 (tools/evidence_store.py differs from v1.14 by exactly one comment line, the validator's filename), the role model, A2V, the lifecycle, the record constructors, BUNDLE semantics, the checked-file law, writer locking, H0, the commit STAGE architecture, the schema-registry architecture, the trusted shim, raw ingestion, the exact stored chain, precedence, the capability matrix, M0B/M0C/M0D and unrelated pre-M3 transaction design are untouched. Inherited context: v1.14 is a NARROWLY SCOPED correction of the ONE BLOCKER Codex's independent harness found in v1.13: V113-B1, the authorizing location-check bypass. Hermes's v1.13 operational review passed, and Codex passed 93 of 96 checks with one BLOCKER and one MINOR. What failed: v1.13 enforced the governed-location law in tools/evidence_authoring.py, one layer above the authority, while authority_lib.derive_attachment_state and authority_lib.evaluate_eligibility - the two functions v1.13's own AUTHORITY_SURFACE declared AUTHORIZING - still accepted a bare evidence-set dict and knew nothing about where it came from. A complete, mechanically perfect workflow authored under a sandbox root, the exact document the wrapper refuses with NOT_PRODUCTION_ROOT, returned ATTACHMENT_READY from the core derivation and eligible=true from the core eligibility gate. That is the same class as V112-RP1 one level down: a law enforced in exactly one place is a property of that place, not of the authority. Reproduced against the frozen v1.13 bundle at 65895c6dedceadff012a08c1bafcdf9e92b65221 before anything was changed, and pinned as a single check that asserts the wrapper's refusal, the diagnostic ATTACHMENT_READY and the authorizing refusal together, so the difference between v1.13 and v1.14 is visible rather than claimed. v1.14 moves location reconciliation INTO the authorizing core. authority_lib.load_governed_evidence_set(session_id, active) is the ONLY door: it takes a session id and the active authority and nothing else, derives the canonical path from the frozen constant itself, classifies the root, session and document entries with lstat before anything is resolved, reads the document only from the path it computed, compares the document's own recorded location and session identity against where it was found, requires the recorded authority_root to be the frozen constant and written_under_production_root to be true, re-checks the bytes against the digest sealed at VERIFIED, and mints a location receipt under the registered hash domain vidtoolz.resolveLocationReceipt.v1. It returns a GovernedEvidenceSet whose __init__ refuses without a module-private token and which has __slots__ and no instance dict, so a dict, a namespace object, a subclass or a hand-computed receipt cannot fabricate provenance. derive_attachment_state_authorizing and evaluate_eligibility_authorizing, plus their by-session-id forms, accept ONLY that object and call governed_provenance_errors FIRST, which re-derives every location fact from the live filesystem; precedence inside an authorizing call is therefore LOCATION AUTHORITY -> ATTACHMENT SEMANTICS -> ELIGIBILITY, and a forbidden-location evidence set never reaches the point where readiness is computed. The eligibility gate has no attachment-state parameter and recomputes the state itself, so no precomputed or diagnostic state can bootstrap it, and it is fail-closed with reason code LOCATION_AUTHORITY_INVALID. derive_attachment_state and evaluate_eligibility are DEMOTED to a new AUTHORITY_SURFACE key, diagnostic_non_authorizing, and say NON-AUTHORIZING in their own docstrings; their behaviour is unchanged, because the whole inherited suite depends on the semantic derivation. The authoring layer now delegates to the core's single implementation of the root resolver, the session-id law and the directory-entry trust check, keeping its earlier checks as DEFENCE IN DEPTH rather than as the only guard. The MANDATORY positive control runs at the REAL canonical governed root - created 0700 only when absent, and removed unconditionally in a finally - and asserts ATTACHMENT_READY with authorizing=true and eligible=true with reason code ELIGIBLE, so the correction is not merely refuse-everything. The MINOR is also closed: EVIDENCE-SET-WORKFLOW.json no longer claims the authoring API accepts a root parameter. One residual is RECORDED rather than fixed, honestly: the M3 composed path (validate_transaction_set -> semantic_mutation_plan) still consumes the diagnostic eligibility. It is PROVISIONAL_UNTIL_M3, refuses without a mutation plan and a complete S0 snapshot, and cannot yield M0A probe eligibility; pre-M3 transaction authority is deliberately outside v1.14's scope. Nothing else is reopened: the v1.12/v1.13 workflow, principals, A2V, the lifecycle, the record constructors, record_id, the envelope law, BUNDLE_VERIFICATION semantics, the TOCTOU law, the checked-file law, locking, Store.v5 (tools/evidence_store.py differs from v1.13 by exactly one comment line, the validator's filename), cache, the schema registry, the shim, strict ingestion, H0, commit authority, precedence, the matrix architecture and M0B/M0C/M0D are untouched. Inherited context: v1.13 is an EXTREMELY NARROW correction of the ONE defect Codex's runtime-parity oracle found in v1.12: V112-RP1, classification WORKFLOW_CONTRADICTORY. Hermes's v1.12 operational review passed on the intended path - lifecycle, positive runtime-only E2E, canonical records, 9/9 checked files, role separation, TOCTOU detection, NO_RERUN_RISK, zero Resolve/EKA contact - and Codex's intake and semantic delta also passed. What failed was runtime parity: the v1.12 production APIs and CLIs accepted arbitrary AND SYMLINKED evidence roots, and a complete workflow under such a forbidden root still derived ATTACHMENT_READY, contradicting the frozen law that there is exactly one governed attachment evidence root. The cause was that every authorizing function took root=None and both CLIs took --evidence-root, introduced deliberately as a test convenience and placed on the AUTHORIZING surface, while governed_root(root) applied abspath to the caller value and never classified the entry, so a symlink was followed rather than refused. Reproduced against the frozen v1.12 bundle at 73150d00 before anything was changed. v1.13 freezes GOVERNED_ATTACHMENT_ROOT and GOVERNED_ROOT_MODE in authority_lib, gives the resolver NO ARGUMENT, removes every root-selecting parameter from the authorizing surface and from both CLIs, renames the preparer's Resolve-library option to --library-root so no production CLI carries an option literally called --root, classifies the root and session directory entries with lstat BEFORE any resolution (no .resolve() anywhere; realpath only as a later alias check), re-establishes the governed location on EVERY authorizing load rather than only at creation, requires the document's recorded authority_root to equal the frozen constant and written_under_production_root to be true at the authorizing derivation, and moves scratch roots into tools/evidence_authoring_testkit.py, an INTERNAL_NON_AUTHORIZING module that neither CLI imports, that refuses to sandbox the frozen root, and whose documents the authorizing derivation refuses. The sandbox changes the root PREFIX ONLY: every trust, lifecycle, principal, TOCTOU and freshness check runs unchanged on the same code, so no test path can establish a state production location policy forbids. Nothing that passed the v1.12 operational review is reopened: principal roles, A2V semantics, the OPEN/PREPARED/VERIFIED lifecycle, the record constructors, the BUNDLE_VERIFICATION content, the TOCTOU law, the checked-file law, Store.v5 (tools/evidence_store.py differs from v1.12 by exactly one comment line and by nothing else), cache, shim, strict ingestion, H0, commit authority, precedence, the capability matrix and M0B/M0C/M0D are untouched. Inherited context: v1.12 is a NARROWLY SCOPED correction of the v1.11 pre-M0A workflow gap, classified AUTHORITY_UNDERSPECIFIED by the architectural adjudication of the failed A2 repair. v1.11 froze the resolveEvidenceSet schema, the record_id law, make_record(), envelope validation, all three record shapes and the attachment derivation - and all of it was demonstrated correct - but froze no governed persistence location for the evidence-set document, no writer, no permission law, no lifecycle and no production constructor, so ATTACHMENT_READY was reachable only from fixtures and fixture/runtime parity was false. That was the single reason A2 could not lawfully reach ATTACHMENT_READY. V112-1 freezes ONE governed persistence topology under QUALIFICATION_EVIDENCE_ROOT with a path law, atomic replace, an O_EXCL writer lock and a self-location check, keeping Store.v5 and the evidence set as two deliberately unmerged container models. V112-2 freezes the OPEN -> PREPARED -> VERIFIED lifecycle with (state, role) write grants, VERIFIED granting nothing to anyone, a prepared seal, a verified seal and a governed staleness law. V112-3 freezes a role-bound principal registry with actor disjointness and per-role write grants, stamped by the tool for the one role it implements, and the derivation now requires both principals registered under exactly their role and of DISTINCT actors. V112-4 makes tools/a2_verify.py the ONE production path that authors a BUNDLE_VERIFICATION, named A2V and governed as a distinct human authorization, computing the result, both principals, both verified record ids, the manifest digest and the nine pinned-file digests inside the verifier itself; the derivation binds all of it and only PASS satisfies the gate. The mandatory positive end-to-end runs with production tools only and no fixture synthesis, 28 negative end-to-end cases are refused, and a parity test asserts the production record field sets are identical to the fixture field sets for all four record types. A principal is a role-bound operational label, NOT an authenticated identity: no key, no signature, no certificate, and a human who runs both tools is still one human - stated in THREAT-MODEL.md, not closed. Protected and unchanged: Store.v5 semantics, Boundary.v1, Inventory.v4, marker cardinality, mode law, content addressing, the caches, the schema-registry architecture, the trusted shim, strict raw ingestion, the exact stored chain, H0, commit authority, precedence, the capability matrix and the M0B/M0C/M0D phase design. Inherited context: v1.11 is an EXTREMELY NARROW correction of the four release-relevant findings Codex left open against v1.10, whose supplied suite passed 1911/1911 twice and deterministically: F110-A the boundary receipt contained normative fields (schema, evidence_store_version, authority_version, platform_scope, created_at) that could be consistently rewritten with a recomputed boundary_sha256 and still authorize the next operation; F110-B six normative inventory header fields (record_count, entry_count, total_bytes, attempt_count, self_path, finalized_marker_path) were written, digested, published as derived and never compared, and an extra semantic top-level field survived verification; F110-C marker/record reconciliation checked semantic derivation but not exact cardinality, so two attempt ids over the same session, layer, logical identity and bytes produced two valid markers for one content-addressed record and finalize() and verify() both passed; F110-D nine public authority-bearing methods returned live session data after the session directory had been replaced, and inventory() and finalization_marker() masked the substitution with SESSION_NOT_FINALIZED. All four were reproduced against the frozen v1.10 bundle at 9c6643430d2dc66741afe9852668d8e6e19c0630 before anything was changed, and F110-D was broader than reported. v1.11 classifies every BOUNDARY.json field with exactly one provenance class and independently reconciles every normative one (created_at is deliberately INFORMATIONAL_NON_AUTHORIZING, because this store governs no independent creation-event authority); freezes the exact BOUNDARY, INVENTORY and FINALIZATION top-level field vocabularies and recomputes all six missing inventory values from the independent model and the frozen store layout; enforces the marker/record bijection with exact cardinality in both directions, refusing the second write before any ambiguous state is persisted; and publishes the explicit public authority-bearing method inventory with the frozen error precedence BOUNDARY > SESSION_STATE > MODEL > READ. Codex's PARTIAL filesystem-normalization observation was NOT one of the four release findings and is deliberately out of scope: the frozen FS_* vocabulary and fs() behaviour are retained and re-run unchanged. Inherited context: v1.10 is a narrowly scoped correction of the five evidence-store findings Codex left open against v1.9, whose v1.7 and v1.8 regressions all passed 211/211: S110-1 the session boundary lived only in process memory, so a replaced session directory or the same basename under another root verified clean; S110-2 several normative inventory and finalization fields were recorded and then trusted, including the marker store and authority versions; S110-3 a foreign attempt marker planted on disk was accepted through write, finalize and verify; S110-4 the session directory was outside mode authority and modes were only checked after finalization; S110-5 a raw PermissionError could leak from the public authority path. All five were reproduced against the frozen v1.9 bundle first. v1.10 persists a SESSION_BOUNDARY receipt as governed evidence and binds root and session identity into the session identity, rebuilds the expected model from independent sources so the inventory is only ever compared to it, reconciles stored attempt markers and records as a bijection, derives every mode from entry kind and enforces it continuously from creation, and normalizes every filesystem condition into a frozen refusal class. Inherited context: v1.9 is an extremely narrow correction of the four evidence-store integrity defects Codex left open against v1.8: S19-1 a symlinked session root was accepted because the path was resolved before any trust boundary existed; S19-2 the inventory reconciled bytes but not semantic identity, so a renamed or cross-session directory still validated; S19-3 attempt uniqueness was digest-bound rather than bound to (session, layer, logical identity, attempt id); S19-4 chmod after finalization was undetected. All four were reproduced against the frozen v1.8 bundle before anything was changed, and two were worse than reported. v1.9 establishes the root trust boundary by lstat before any resolution, freezes a session identity tuple and the basename-is-session-id path law, derives every record name from its semantics, keys attempt uniqueness on the canonical tuple, and records POSIX file type and permission mode as frozen metadata; verification PASS now requires missing, unexpected, changed AND semantic_mismatch all zero. Mode and file-type claims are POSIX-scoped to the reference host. Inherited context: v1.8 was an extremely narrow correction of the one defect class Codex left open against v1.7, evidence-store integrity and competing store authority: ES-1, the verifier ignored unexpected files in a finalized session root (independent attack STORE-14), and ES-2, a competing EvidenceRoot surface in the capture shim built filesystem paths from caller strings and escaped the evidence root. v1.8 makes finalization a CLOSED WORLD whose inventory records every governed file and directory and whose verification requires missing == 0, unexpected == 0 and changed == 0; and it leaves exactly one EVIDENCE_STORE_AUTHORIZING implementation, tools/evidence_store.py, whose write API takes a layer, an attempt id, a logical identity and bytes, never a path. Everything v1.7 established is retained and re-run as regressions. Inherited context: v1.7 corrects Codex's final forensic adjudication of v1.6: four BLOCKERs (C16-B1 content-insensitive authority caches, C16-B2 caller-supplied schema validators, C16-B3 unbound capture-shim trust, C16-B4 chain substitution), three M0A MAJORs (C16-M1 duplicate receiver paths, C16-M2 absent strict byte ingestion, C16-M3 non-executable evidence store) and the operational binding closure. Every authority cache is keyed by content digests plus the authority version and warm results are asserted equal to cold ones; the authorizing entry points take no schema-validator parameter and resolve schemas from the pinned SCHEMA-REGISTRY.json; a capture qualifies only under the trusted shim pinned by TRUSTED-SHIM.json, which the active authority must also name; a promoted row must resolve the EXACT stored raw capture, derived record, current review and current reviewed refreeze by digest, with multiplicity a CONFLICT unless explicit supersession leaves one; identity claims validate receiver-path, handle-token and attempt-id uniqueness before any index is built; raw evidence enters only through the strict byte boundary and every record carries a strict-parse receipt; and the append-only evidence store is executable code this suite attacks in a temporary directory. Layers: raw parse -> schema -> evidence-binding (envelope, active manifest, record laws) -> attachment -> capability (RAW_CAPABILITY_CAPTURE -> reference re-parser -> REVIEW_DECISION -> REFREEZE_RECORD -> content-bound ACTIVE matrix) -> snapshot (observation model, raw-capture field provenance, occurrence uniqueness) -> semantic (binding, membership, mandatory checkpoint, readback->S1, derived verification over the protected surface with named exclusions) -> eligibility (incl. the H0/S0 early write gate) -> linked-set / composed authorization (validate_transaction_set, commit_eligibility, both with a MANDATORY schema validator). A finding may be covered by more than one section, so the per-finding counts overlap; the total is not proof. Every layered fixture records its expected failure layer; a check that raises is a FAIL. Order-independence: seeded permutations (seed {SEED}, {PERMS} per subject). The bypass audit proves no exported validator can skip S0, S1, schema validation, capability provenance, delta derivation or expected-effect validation, and that no parameter default makes schema enforcement optional. The raw-capability sections execute the REFERENCE capture shim against fake in-process objects (no Resolve) and prove that a probe-authored interpretation has zero authority. The exact-manifest section binds against the real FREEZE-MANIFEST.json sha without printing it. Offline; no Resolve. Node conformance = M1. SCHEMA-VALID != AUTHORIZED TO MUTATE; passing proves internal consistency of the authority documents only. The total count is not proof: see the per-finding coverage below.", "", f"Layered fixture layers: {json.dumps(layer_counts, sort_keys=True)}", "", "| Finding | Sections | Checks | Passed |", "|---|---|---|---|"]
