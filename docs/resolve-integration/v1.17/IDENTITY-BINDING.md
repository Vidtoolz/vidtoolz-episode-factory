...
## v1.6: what may be recorded about `GetUniqueId`, and when

Three separate M0 claims — **A** the getter is callable on the observed objects, **B** the ids are unique within one no-mutation read pass, **C** the ids are stable across ≥3 consecutive no-mutation passes of the same session — are derived from individual raw captures by `authority_lib.identity_claims` and recorded as `IDENTITY_UNIQUENESS_OBSERVATION` / `IDENTITY_STABILITY_OBSERVATION`. `current_read_unique` and `current_read_stable` may be recorded here as OBSERVED **for the qualification library** only after those observations exist and the identity row is refrozen.

Survival across append, replace, delete, take finalization, save/reopen and duplicate/DRT round trips is **not** an M0 claim and is never inferred from A, B or C; it stays with M3 probes P6/P15 and is promoted per operation. Until then the cross-check fields of the tuple above remain mandatory, and verification keeps failing closed on duplicate ids regardless of any claim.
