...
**Registered hash domains added after v1.6.** `vidtoolz.resolveReadPrimitiveAuthority.v1`,
`vidtoolz.resolveEvidenceSet.v1`, `vidtoolz.resolveContentKey.v1`, `vidtoolz.resolveSchemaRegistry.v1`,
`vidtoolz.resolveTrustedCaptureShim.v1`, `vidtoolz.resolveShimAllowlist.v1`, `vidtoolz.resolveRawIngestReceipt.v1`,
`vidtoolz.resolveIdentityClaimSet.v1`, `vidtoolz.resolveStoredChain.v1`, `vidtoolz.resolveRefreezeBlock.v1`; the
evidence-store domains `vidtoolz.resolveEvidenceSession.v4`, `vidtoolz.resolveEvidenceInventory.v4`,
`vidtoolz.resolveEvidenceSessionIdentity.v2`, `vidtoolz.resolveEvidenceRecordKey.v1`,
`vidtoolz.resolveEvidenceAttemptKey.v1` and `vidtoolz.resolveEvidenceBoundary.v1`; and the governed-location domains
`vidtoolz.resolveLocationReceipt.v1` (v1.14 — the location receipt: session id, both canonical authority-file paths,
the frozen governed root, the authority version, the active manifest, both file digests, the parsed-snapshot digest
and the validation result) and `vidtoolz.resolveEvidenceSetSnapshot.v1` (v1.15 — the canonical digest of a governed
document's PARSED semantic content, which is what binds the object an authorizing derivation consumes to the exact
bytes that passed governed validation).

The domain registry stays closed: an unregistered domain raises. **This list is the closed list, and it is now
machine-checked**: `validate_v1_15.py` section `hash-domain-parity` compares it against `authority_lib.HASH_DOMAINS`
exactly, in both directions, so a domain can never again exist in code and be missing here (V114-N1, where
`vidtoolz.resolveLocationReceipt.v1` and six evidence-store domains were absent or recorded at superseded versions
while the document still claimed a closed list).
