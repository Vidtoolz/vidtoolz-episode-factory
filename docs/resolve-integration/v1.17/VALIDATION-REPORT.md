...
| R7 F15-07 / M-PRECEDENCE transaction lifecycle precedence | precedence, precedence-supersession, precedence-retired-terms | 237 | 237 |
...
| precedence | 146 | 146 |
| precedence-retired-terms | 27 | 27 |
...
| evidence-set-binding-negative | attached-evidence-raised-fake-parse-block | PASS | 07ffafc825d9: capture carries forbidden interpretation field 'parse'; 07ffafc825d9: capture carries forbidden interpretation field 'result' |
| evidence-set-schema | attached-evidence-raw-tampered | PASS |  |
| evidence-set-binding-negative | attached-evidence-raw-tampered | PASS | 140118360555: capture raw_digest does not re-hash; 159d62253488: capture raw_digest does not re-hash |
...
| evidence-set-binding-negative | attached-fatal-probe-failure | PASS | 351ba8b10444: capture binding library_root disagrees with envelope/contract/active authority; 351ba8b10444: capture binding library_uuid disagrees with envelope/contract/active authority |
...
| evidence-set-binding-negative | invalid-bundle-other-manifest-not-historical | PASS | 475c7b044f4e: BUNDLE_VERIFICATION bound to manifest 999999999999 != active reviewed manifest |
| evidence-set-schema | invalid-connection-other-authority | PASS |  |
| evidence-set-binding-negative | invalid-connection-other-authority | PASS | cefa34f61bb8: CONNECTION_OBSERVATION bound to authority 1.5.0 != active 1.17.0 |
| evidence-set-schema | invalid-connection-wrong-manifest | PASS |  |
| evidence-set-binding-negative | invalid-connection-wrong-manifest | PASS | 3567822e0f62: CONNECTION_OBSERVATION bound to manifest 999999999999 != active reviewed manifest |
...
| evidence-set-binding-negative | invalid-provisioning-without-root | PASS | 28863b69d0de: PROVISIONING_RECORD: envelope.library_root required at level LIBRARY |
...
| evidence-set-binding-negative | invalid-retired-capability-evidence-type | PASS | c5e6d1cd77cb: record_type CAPABILITY_EVIDENCE is RETIRED in v1.6 (capability evidence is RAW_CAPABILITY_CAPTURE + derived result + review) |
| evidence-set-schema | invalid-tampered-record | PASS |  |
| evidence-set-binding-negative | invalid-tampered-record | PASS | 70519fb3ebcf: record_id does not match content digest |
...
| evidence-ref-negative | uppercase sha | PASS | M3_AUTHORIZATION: reference is not a sha256 ('6D538CACFFDECEAF58483AF227F94AC1370D960E7F92F27C390A3D7E5ED14FA7') |
| evidence-ref-negative | unlinked sha | PASS | M3_AUTHORIZATION: referenced record 999999999999 not in evidence set |
| evidence-ref-negative | wrong type | PASS | M3_AUTHORIZATION: record 6d538cacffde has type PROVISIONING_RECORD |
| evidence-ref-negative | short hex | PASS | M3_AUTHORIZATION: reference is not a sha256 ('6d538cacffdeceaf58483af227f94ac1370d960e7f92f27c390a3d7e5ed14fa') |
...
| attachment-derived | attached-evidence-raised-fake-parse-block -> CONFLICT | PASS | got CONFLICT; failures=['EVIDENCE_SET_INVALID', "07ffafc825d9: capture carries forbidden interpretation field 'parse'"] conflicts=[] |
| attachment-derived | attached-evidence-raw-tampered -> CONFLICT | PASS | got CONFLICT; failures=['EVIDENCE_SET_INVALID', '140118360555: capture raw_digest does not re-hash'] conflicts=[] |
...
| attachment-derived | attached-fatal-probe-failure -> CONFLICT | PASS | got CONFLICT; failures=['EVIDENCE_SET_INVALID', '351ba8b10444: capture binding library_root disagrees with envelope/contract/active authority'] conflicts=[] |
...
| attachment-derived | invalid-bundle-other-manifest-not-historical -> CONFLICT | PASS | got CONFLICT; failures=['EVIDENCE_SET_INVALID', '475c7b044f4e: BUNDLE_VERIFICATION bound to manifest 999999999999 != active reviewed manifest'] conflicts=[] |
| attachment-derived | invalid-connection-other-authority -> CONFLICT | PASS | got CONFLICT; failures=['EVIDENCE_SET_INVALID', 'cefa34f61bb8: CONNECTION_OBSERVATION bound to authority 1.5.0 != active 1.17.0'] conflicts=[] |
| attachment-derived | invalid-connection-wrong-manifest -> CONFLICT | PASS | got CONFLICT; failures=['EVIDENCE_SET_INVALID', '3567822e0f62: CONNECTION_OBSERVATION bound to manifest 999999999999 != active reviewed manifest'] conflicts=[] |
| attachment-derived | invalid-provisioning-without-root -> CONFLICT | PASS | got CONFLICT; failures=['EVIDENCE_SET_INVALID', '28863b69d0de: PROVISIONING_RECORD: envelope.library_root required at level LIBRARY'] conflicts=[] |
| attachment-derived | invalid-retired-capability-evidence-type -> CONFLICT | PASS | got CONFLICT; failures=['EVIDENCE_SET_INVALID', 'c5e6d1cd77cb: record_type CAPABILITY_EVIDENCE is RETIRED in v1.6 (capability evidence is RAW_CAPABILITY_CAPTURE + derived result + review)'] conflicts=[] |
| attachment-derived | invalid-tampered-record -> CONFLICT | PASS | got CONFLICT; failures=['EVIDENCE_SET_INVALID', '70519fb3ebcf: record_id does not match content digest'] conflicts=[] |
...
| refreeze-law | a refreeze from another session is refused | PASS | GetCurrentDatabase: raw capture from another probe/session |
| refreeze-law | a refreeze from another probe run is refused | PASS | GetCurrentDatabase: raw capture from another probe/session |
...
| fixture-schema-negative | plan-authority-version-old | PASS | const:authority_version:'1.17.0' was expected |
| fixture-schema-positive | plan-digest-not-of-body | PASS |  |
| fixture-semantic-negative | plan-digest-not-of-body | PASS | plan_digest does not match plan body digest; operation APPEND not eligible: ["JOURNAL_PREPARED (JOURNAL_PREPARED: record f8f5f134e451 plan_digest='<SHA256>' != '<SHA256>')", "PLAN_VALIDATED (PLAN_VALIDATION: record b62fc |
| fixture-schema-positive | plan-duplicate-operation-id | PASS |  |
| fixture-semantic-negative | plan-duplicate-operation-id | PASS | operation ids must be unique and non-empty; operation APPEND not eligible: ["JOURNAL_PREPARED (JOURNAL_PREPARED: record f8f5f134e451 plan_digest='<SHA256>' != '<SHA256>')", "PLAN_VALIDATED (PLAN_VALIDATION: record b62fc5 |
| fixture-schema-positive | plan-eligible-without-prepared-journal | PASS |  |
| fixture-eligibility-negative | plan-eligible-without-prepared-journal | PASS | operation APPEND not eligible: ['JOURNAL_PREPARED (JOURNAL_PREPARED: referenced record f8f5f134e451 not in evidence set)', 'PLAN_VALIDATED (PLAN_VALIDATION: referenced record b62fc572a88d not in evidence set)', 'GUARD_CU |
...
| fixture-semantic-negative | plan-operation-set-digest-mismatch | PASS | operation_set_digest does not match operations; operation APPEND not eligible: ["JOURNAL_PREPARED (JOURNAL_PREPARED: record f8f5f134e451 plan_digest='<SHA256>' != '<SHA256>')", "PLAN_VALIDATED (PLAN_VALIDATION: record b6 |
| fixture-schema-positive | plan-plan-validation-record-fail | PASS |  |
| fixture-eligibility-negative | plan-plan-validation-record-fail | PASS | operation APPEND not eligible: ['PLAN_VALIDATED (PLAN_VALIDATION: referenced record b62fc572a88d not in evidence set)'] |
| fixture-schema-positive | plan-read-class-with-append | PASS |  |
| fixture-semantic-negative | plan-read-class-with-append | PASS | RESOLVE_READ permission class cannot carry write operations; operation APPEND not eligible: ["JOURNAL_PREPARED (JOURNAL_PREPARED: record f8f5f134e451 plan_digest='<SHA256>' != '<SHA256>')", "PLAN_VALIDATED (PLAN_VALIDATI |
| fixture-schema-positive | plan-selector-target-mismatch | PASS |  |
| fixture-semantic-negative | plan-selector-target-mismatch | PASS | operation APPEND not eligible: ["JOURNAL_PREPARED (JOURNAL_PREPARED: record f8f5f134e451 plan_digest='<SHA256>' != '<SHA256>')", "PLAN_VALIDATED (PLAN_VALIDATION: record b62fc572a88d plan_digest='<SHA256>' != '<SHA256>') |
| fixture-schema-positive | plan-session-not-current | PASS |  |
| fixture-semantic-negative | plan-session-not-current | PASS | plan session_id != evidence set current_session_id (execution session mismatch); operation APPEND not eligible: ["JOURNAL_PREPARED (JOURNAL_PREPARED: record f8f5f134e451 plan_digest='<SHA256>' != '<SHA256>')", "PLAN_VALI |
| fixture-schema-positive | plan-stale-guard | PASS |  |
| fixture-semantic-negative | plan-stale-guard | PASS | operation APPEND not eligible: ["JOURNAL_PREPARED (JOURNAL_PREPARED: record f8f5f134e451 plan_digest='<SHA256>' != '<SHA256>')", "PLAN_VALIDATED (PLAN_VALIDATION: record b62fc572a88d plan_digest='<SHA256>' != '<SHA256>') |
...
| linked-set-negative | linked-set-effect-not-specified-claims-verified | PASS | plan: operation SET_PROPERTIES not eligible: ["JOURNAL_PREPARED (JOURNAL_PREPARED: record f8f5f134e451 plan_digest='<SHA256>' != '<SHA256>')", "PLAN_VALIDATED (PLAN_VALIDATION: record b62fc572a88d plan_digest='<SHA256>'  |
...
| eligibility | m0-deny-ref-wrong-record-type | PASS | eligible=False state=ATTACHED_READ_ONLY tr=PROJECT_TIMELINE failed=['READ_ONLY_JOURNAL_OPEN (READ_ONLY_JOURNAL: record 70519fb3ebcf has type CONNECTION_OBSERVATION)'] codes=['PREREQUISITES_FAILED'] |
...
| eligibility | m3-append-deny-authorization-old-authority | PASS | eligible=False state=ATTACHED_READ_ONLY tr=PROJECT_TIMELINE failed=['MIKKO_M3_AUTHORIZATION (M3_AUTHORIZATION: referenced record 6dfd2b6c6dec not in evidence set)', 'TARGET_STATE_SCRATCH_WRITE_READY (no M3_AUTHORIZATION  |
...
| eligibility | m3-save-deny-ids-unavailable | PASS | eligible=False state=ATTACHED_READ_ONLY tr=PROJECT failed=['M2_EXIT_EVIDENCE (MILESTONE_EXIT: referenced record ba7a73d779f6 not in evidence set)', 'MIKKO_M3_AUTHORIZATION (M3_AUTHORIZATION: referenced record 6dfd2b6c6de |
...
| eligibility | m3-append-deny-h0-minimal-m0 | PASS | eligible=False state=SCRATCH_WRITE_READY tr=PROJECT_TIMELINE failed=["JOURNAL_PREPARED (JOURNAL_PREPARED: record f8f5f134e451 plan_digest='<SHA256>' != '<SHA256>')", "GUARD_CURRENT (H0_PROFILE: snapshot profile 'MINIMAL_ |
| eligibility-request-schema | m3-append-deny-h0-full-timeline-read | PASS | request schema-valid=True |
| eligibility | m3-append-deny-h0-full-timeline-read | PASS | eligible=False state=SCRATCH_WRITE_READY tr=PROJECT_TIMELINE failed=["JOURNAL_PREPARED (JOURNAL_PREPARED: record f8f5f134e451 plan_digest='<SHA256>' != '<SHA256>')", "GUARD_CURRENT (H0_PROFILE: snapshot profile 'FULL_TIM |
| eligibility-request-schema | m3-append-deny-h0-incomplete-write-precheck | PASS | request schema-valid=True |
| eligibility | m3-append-deny-h0-incomplete-write-precheck | PASS | eligible=False state=SCRATCH_WRITE_READY tr=PROJECT_TIMELINE failed=["JOURNAL_PREPARED (JOURNAL_PREPARED: record f8f5f134e451 plan_digest='<SHA256>' != '<SHA256>')", "GUARD_CURRENT (H0_INCOMPLETE: guarded snapshot is not |
| eligibility-request-schema | m3-append-deny-h0-candidate-provenance | PASS | request schema-valid=True |
| eligibility | m3-append-deny-h0-candidate-provenance | PASS | eligible=False state=SCRATCH_WRITE_READY tr=PROJECT_TIMELINE failed=["JOURNAL_PREPARED (JOURNAL_PREPARED: record f8f5f134e451 plan_digest='<SHA256>' != '<SHA256>')", 'GUARD_CURRENT (H0_PROVENANCE: Folder.GetUniqueId is C |
| eligibility-request-schema | m3-append-deny-h0-other-matrix | PASS | request schema-valid=True |
| eligibility | m3-append-deny-h0-other-matrix | PASS | eligible=False state=SCRATCH_WRITE_READY tr=PROJECT_TIMELINE failed=["JOURNAL_PREPARED (JOURNAL_PREPARED: record f8f5f134e451 plan_digest='<SHA256>' != '<SHA256>')", 'GUARD_CURRENT (H0_MATRIX: guarded snapshot collected  |
| eligibility-request-schema | m3-append-deny-h0-other-target | PASS | request schema-valid=True |
| eligibility | m3-append-deny-h0-other-target | PASS | eligible=False state=SCRATCH_WRITE_READY tr=PROJECT_TIMELINE failed=["JOURNAL_PREPARED (JOURNAL_PREPARED: record f8f5f134e451 plan_digest='<SHA256>' != '<SHA256>')", 'GUARD_CURRENT (H0_TARGET: guarded snapshot identity d |
...
| eligibility | m3-append-deny-plan-validation-from-helper | PASS | eligible=False state=SCRATCH_WRITE_READY tr=PROJECT_TIMELINE failed=["PLAN_VALIDATED (PLAN_VALIDATION: record bb63e6e467a3 validator='tools/authority_lib.py#semantic_mutation_plan' != 'tools/authority_lib.py#validate_tra |
...
| precedence | the v1.5 through v1.16 changelogs and finding matrices are HISTORICAL; the v1.17 ones are active | PASS |  |
...
| precedence | the v1.17 slot-discovery supersession S64 is recorded, the v1.17 records and the generated inventory are active, and the document version tracks the authority version | PASS |  |
| precedence | S64 names the v1.16 statement it replaces, the generated artifacts and tooling that now hold the law, and Codex's finding | PASS |  |
| precedence | the v1.17 supersession table in the markdown mirrors the machine form, keeps S63 standing, and restates that precedence is not a band-aid | PASS |  |
| precedence | every artifact is registered with a status AND a published role, and the four roles are defined - so the audit scope is machine-derived rather than a filename pattern (V116-B1 sections 16/17) | PASS |  |
| precedence | THREAT-MODEL.md records the v1.17 audit-fragility residuals and restates the still-open authorizing-transaction control | PASS |  |
...
| precedence | AUTHORITY-PRECEDENCE.json carries the blanket clause that re-reads every earlier statement's demoted function names as their authorizing counterparts, and v1.17 makes that clause itself clause-local: every demoted name in it carries its class token in the same clause (V114-B3, V116-B1) | PASS |  |
...
| manifest | the parent block records the v1.16 branch HEAD and its semantic commit, which for v1.16 are the same commit because its bundle and registration were committed together (section 29) | PASS |  |
...
| b3-active-naming | the audit actually scanned every CURRENT artifact, machine and prose, not a sample (55 documents) | PASS | 55 scanned |
...
| b1-classification | V115-B1 section 9 / V116-B1 section 11 PINNED: every DISCOVERED authorizing slot names a function whose canonical class is authorizing (35 slots discovered from 57 current artifacts, not a hand-written list) | PASS | 35 authorizing slots clean |
| b1-classification | V115-B1 section 10: every slot that DECLARES a class actually declares one about a demoted function, and every dedicated diagnostic/provisional inventory names ONLY demoted functions (58 declaring slots) | PASS | declaring slots clean |
| b1-classification | V115-B1 section 8 PINNED: every current machine artifact that classifies a function agrees with the canonical map and with the code - one function, one current authority class (98 claims reconciled) | PASS | 98 claims consistent |
| b1-classification | V115-B1 section 11 PINNED: no authorizing runtime code can resolve a function by name from an artifact - there is no getattr/eval/exec/importlib dispatch keyed on an artifact string anywhere in the authority, so a stale document can mislead a consumer but cannot make the authority call a diagnostic function | PASS | the only name lookup is parser_sha256 over a frozen module constant |
| b1-classification | and the artifacts the authority DOES read at runtime are consumed for values, never for function names: the target contract, permissions, read primitives and capabilities are data inputs to statically bound functions | PASS |  |
| b1-classification | V115-B1 section 13: the exact v1.15 contradiction is reproduced against the FROZEN v1.15 bundle - qualification_note and attachment_law naming the diagnostic derivation unmarked, and SCHEMA-REGISTRY calling the *_authorizing pair provisional/non-authorizing - and neither survives in v1.16 | PASS | frozen v1.15 contradiction confirmed present |
| b1-classification | and BOTH earlier blind spots are closed: v1.15's exclusion list of key names (law, note, qualification_note, reason) is gone, and v1.16's substring marker list - which contained the bare phrase 'no longer' and suppressed the TOCTOU violation - is replaced by clause-local structured classification with no bare negation in any token list (V116-B1) | PASS | both invariants are now derived |
...
| b1-property | and the exhaustive scan covers every current artifact, machine and prose, with no key-name exemption list (55 artifacts) | PASS | 55 artifacts |
...
| b1-toctou | V116-B1 section 1 PINNED: in the FROZEN v1.16 bundle, EVIDENCE-SET-WORKFLOW.json#toctou.law names the bare derive_attachment_state as the refusing authority, with no diagnostic marking anywhere in the value | PASS | ...hanges its id, the bundle's binding no longer resolves, and derive_attachment_state refuses.... |
| b1-toctou | V116-B1 section 1 PINNED: that exact key path is absent from the v1.16 canonical authorizing-slot inventory, which listed ten hand-written slots | PASS | 10 hand-written slots, toctou.law absent |
| b1-toctou | V116-B1 section 1 PINNED: the v1.16 scanner's suppression mechanism, proved from the frozen v1.16 validator source - its marker list literally contained the bare phrase 'no longer', it split clauses only at '. ' and '; ', and in the TOCTOU sentence that phrase modifies 'the bundle's binding no longer resolves', so the offending comma-clause was never isolated and the violation was suppressed | PASS | v1.16 marker list contained 'no longer'; v1.17's contains no bare negation |
| b1-toctou | V116-B1 section 1 PINNED: the v1.17 clause law isolates 'and derive_attachment_state refuses' from that same v1.16 sentence, which carries no class token - so the defect would now be caught at once | PASS | a BUNDLE_VERIFICATION names the exact record ids it verified / A record id is a content digest / so editing a verified preparer record changes its id  |
| b1-toctou | V116-B1 section 2 PINNED: the current TOCTOU law names the AUTHORIZING refusal - derive_attachment_state_authorizing, with the session wrapper as the equivalent - and no longer designates the diagnostic core | PASS |  |
| b1-toctou | V116-B1 section 3: the diagnostic counterpart is still named for comparison, but explicitly AS DIAGNOSTIC / NON_AUTHORIZING and in its own clause, and it is not the function designated as enforcing | PASS |  |
| b1-toctou | V116-B1 section 2: TOCTOU SEMANTICS ARE UNCHANGED - the same record-id/content-digest law, the same two additional seals, the same runtime enforcement | PASS |  |
| slot-discovery | V116-B1 sections 4/10 PINNED: the authorizing-slot inventory is DISCOVERED from the current artifacts, not hand-written, and the TOCTOU key is in it (35 slots from 57 current artifacts; v1.16 hand-listed 10 and missed this one) | PASS | EVIDENCE-SET-WORKFLOW.json#toctou.law -> ['derive_attachment_state_authorizing', 'derive_attachment_state_for_session', 'load_governed_evidence_set'] |
| slot-discovery | V116-B1 section 4: the canonical classes artifact no longer carries a hand-written slot dictionary at all - it publishes the DISCOVERY LAW and points at the generated inventory, so a slot cannot be omitted by forgetting to list it | PASS |  |
| slot-discovery | V116-B1 section 5: the frozen definition is semantic - a slot is authorizing because its VALUE designates authority (authoritative/deciding/refusing/enforcing/deriving/gating/validating), never because of its key name | PASS |  |
| slot-discovery | V116-B1 sections 6/16: every reference in every CURRENT artifact is classified, with the scanned set derived from precedence STATUS and published ROLE rather than a filename pattern (285 references, 57 scanned, 46 out of scope by status or role) | PASS | 57 scanned |
| slot-discovery | V116-B1 sections 11/31 PINNED: ZERO violations - every reference to a demoted function in a current artifact carries a class token in its own clause (R1), and every clause that designates authority names a function of class authorizing (R2) | PASS | 285 references, 0 violations |
| slot-discovery | V116-B1 section 18: the inventory artifact is GENERATED by the validator from the current artifacts and published with artifact, key path, referenced function, authority role, function class and status per row | PASS |  |
| slot-discovery | V116-B1 section 11: every discovered authorizing slot names a function in the authorizing class, and every historical statement is recognised as historical rather than flagged | PASS |  |
| scanner-negation | V116-B1 section 12 PINNED: the EXACT v1.16 TOCTOU sentence - 'binding no longer resolves, and derive_attachment_state refuses' - FAILS classification, as DEMOTED_FUNCTION_DESIGNATED_AUTHORITATIVE. This is the historical defect, frozen as a regression. | PASS | ['DEMOTED_FUNCTION_DESIGNATED_AUTHORITATIVE'] in clause 'and derive_attachment_state refuses' |
| scanner-negation | V116-B1 section 7: the phrase 'no longer' appears in NO token list - the v1.16 suppression mechanism cannot exist, because a bare negation never classifies anything | PASS |  |
| scanner-negation | V116-B1 sections 8/14: negation binds only its own clause - derive_attachment_state is no longer authoritative; derive_attac... | PASS | roles={'derive_attachment_state': 'DIAGNOSTIC_DECLARATION', 'derive_attachment_state_authorizing': 'AUTHORIZING_DESIGNATION'} violations=[] |
| scanner-negation | V116-B1 sections 8/14: negation binds only its own clause - the binding no longer resolves, and derive_attachment_state_auth... | PASS | roles={'derive_attachment_state_authorizing': 'AUTHORIZING_DESIGNATION'} violations=[] |
| scanner-negation | V116-B1 sections 8/14: negation binds only its own clause - evaluate_eligibility is no longer the evaluator; the authorizing... | PASS | roles={'evaluate_eligibility': 'DIAGNOSTIC_DECLARATION', 'evaluate_eligibility_authorizing': 'AUTHORIZING_DESIGNATION'} violations=[] |
| scanner-negation | V116-B1 section 14: an adversarial phrasing that ASSERTS the diagnostic function is authoritative is caught, even though the sentence also contains 'no longer' | PASS | caught |
| scanner-negation | V116-B1 section 15 PINNED: a mixed clause classifies BOTH correctly - the diagnostic function as a diagnostic declaration and the authorizing function as the designation - with no violation | PASS | roles={'derive_attachment_state': 'DIAGNOSTIC_DECLARATION', 'derive_attachment_state_authorizing': 'AUTHORIZING_DESIGNATION'} |
| unknown-slot | V116-B1 section 13 PINNED: an UNFAMILIAR key name (decision_handler) whose value names the diagnostic function as deciding authority is DETECTED - the law is not hardcoded for TOCTOU | PASS | ['UNMARKED_DEMOTED_REFERENCE'] / ['DEMOTED_FUNCTION_DESIGNATED_AUTHORITATIVE'] |
| unknown-slot | V116-B1 section 13 PINNED: an UNFAMILIAR key name (refusal_owner) whose value names the diagnostic function as deciding authority is DETECTED - the law is not hardcoded for TOCTOU | PASS | ['UNMARKED_DEMOTED_REFERENCE'] / ['DEMOTED_FUNCTION_DESIGNATED_AUTHORITATIVE'] |
| unknown-slot | V116-B1 section 13 PINNED: an UNFAMILIAR key name (state_engine) whose value names the diagnostic function as deciding authority is DETECTED - the law is not hardcoded for TOCTOU | PASS | ['UNMARKED_DEMOTED_REFERENCE'] / ['DEMOTED_FUNCTION_DESIGNATED_AUTHORITATIVE'] |
| unknown-slot | V116-B1 section 13 PINNED: an UNFAMILIAR key name (gatekeeper) whose value names the diagnostic function as deciding authority is DETECTED - the law is not hardcoded for TOCTOU | PASS | ['UNMARKED_DEMOTED_REFERENCE'] / ['DEMOTED_FUNCTION_DESIGNATED_AUTHORITATIVE'] |
| unknown-slot | V116-B1 section 13 PINNED: an UNFAMILIAR key name (who_decides) whose value names the diagnostic function as deciding authority is DETECTED - the law is not hardcoded for TOCTOU | PASS | ['UNMARKED_DEMOTED_REFERENCE'] / ['DEMOTED_FUNCTION_DESIGNATED_AUTHORITATIVE'] |
| unknown-slot | the unknown-key matrix is complete and every case is caught (5/5) | PASS |  |
| unknown-slot | V116-B1 section 13: the same unfamiliar key naming the AUTHORIZING function is accepted, so the rule discriminates rather than simply objecting to unfamiliar keys | PASS |  |
| scanner-negation | V116-B1 section 9: every accepted reference form normalises to one canonical (module, name) - bare, backticked, module-qualified, anchor, path-qualified and call forms (8 forms) | PASS | all normalise |
| scanner-negation | V116-B1 section 9: module awareness - evidence_authoring.derive_attachment_state is the AUTHORIZING wrapper, a different symbol from the diagnostic authority_lib core, and is never judged against the authority_lib class map | PASS |  |
| scanner-negation | V116-B1 section 9: a longer name is never matched as a shorter one plus a suffix, so derive_attachment_state_authorizing is not read as derive_attachment_state | PASS |  |
| scanner-negation | V116-B1 section 7: a clause delimiter inside a dotted path, an argument list or a version number does not split a reference | PASS |  |
| v117-property | 48 seeded cases over demoted/authorizing functions x six reference formats x eight key names x six authority verbs x three negation prefixes x optional class marking: a demoted designation is ALWAYS caught and a marked or authorizing one is NEVER a false positive (seed 20261026) | PASS | 48/48 classified correctly |
| v117-property | 16 seeded HISTORICAL statements naming a demoted function are correctly not flagged (section 17), while the same sentence without a historical marker is (seed 20261026) | PASS | history preserved |
| v117-runtime-parity | V116-B1 section 20 PINNED: the three runtime files expected to be untouched are BYTE-IDENTICAL to the frozen v1.16 bundle - evidence_authoring.py, a2_prepare.py and a2_verify.py | PASS | byte-identical |
| v117-runtime-parity | section 20: evidence_store.py differs from v1.16 by exactly one comment line (the validator's filename, carried by the tool rename) | PASS |  |
| v117-runtime-parity | section 20 PINNED: every changed line in authority_lib.py is a version pin, a lineage pin or a tool-name reference - NO runtime semantics changed in v1.17 | PASS | version/lineage/tool-name only |
| v117-runtime-parity | section 21: the receipt parity of V115-M1 is untouched - ten runtime-bound fields, published identically in both machine artifacts, and the domain unchanged | PASS |  |
| v117-static | section 31: every current function reference is classified into exactly one role, and every role is one of the four published ones | PASS |  |
| v117-static | section 31: no diagnostic or provisional function occupies any discovered authorizing slot, and no current contradiction remains anywhere in scope | PASS |  |
| v117-static | section 31 / section 19: no current contradiction hides behind precedence - the blanket clause names each demoted function WITH its class token in the same clause, so the clause itself cannot be read as a designation | PASS |  |
| v117-static | section 31: the published document-role vocabulary is complete and every registered artifact carries a role, so the scan scope is machine-derived rather than a filename pattern | PASS |  |
| v117-static | section 31: the receipt publication remains exactly the runtime set, asserted here as well as in the inherited m1 section | PASS |  |
...
| stored-chain | attached-evidence-duplicate-current-review: refused, and the refusal names the substituted link | PASS | REVIEW: 2 current candidates (945c00b74a9b, da5c4a0d3724); an explicit supersession is required and insertion order never decides; REVIEW_CONFLICT: no single cu |
...
| stored-chain | attached-evidence-duplicate-current-refreeze: refused, and the refusal names the substituted link | PASS | REFREEZE: 2 current candidates (939ff577e691, a5c794cc4fdd); an explicit supersession is required and insertion order never decides; REFREEZE_CONFLICT: no singl |
...
| evidence-boundary-fields | F110-A pinned: receipt field authority_version (FROZEN_CONSTANT) rewritten with a recomputed boundary_sha256 refuses the next canonical operation. v1.10 accepted schema, evidence_store_version, authority_version and platform_scope. | PASS | BOUNDARY_RECEIPT_INVALID: authority_version='1.10.0' is not the frozen FROZEN_CONSTANT '1.17.0' of this ev |
...
| codex-matrix | V115-B1: correction, negative fixture and expected layer are named, and its validation sections all pass | PASS | 23 checks; failing=[] |
...
| m0a-binding | published authority_version is the value this bundle actually implements | PASS | published=1.17.0 actual=1.17.0 |
...
| m0a-binding | published schema_registry_sha256 is the value this bundle actually implements | PASS | published=1a031050891f66f94d559083 actual=1a031050891f66f94d559083 |
