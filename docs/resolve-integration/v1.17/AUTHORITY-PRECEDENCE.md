...
## v1.9 supersessions (S48–S49)

Both replace a v1.8 statement that Codex's adjudication showed to be unenforceable as written. The
machine-readable form is `AUTHORITY-PRECEDENCE.json`.

| id | superseded v1.8 statement | new authority | why |
|---|---|---|---|
| S48 | An evidence root and a session directory are located by resolving the caller-supplied path (os.path.realpath) and then confining every component … | EVIDENCE-ROOT.md#v19; tools/evidence_store.py#RootBoundary | Codex v1.8 BLOCKER S19-1: resolving first destroys the evidence needed to refuse. A symlinked evidence root was resolved away and a session was created, finalized and verified through i… |
| S49 | A finalized evidence session is verified as a closed world of paths, digests, byte counts and directories, with attempt uniqueness keyed on the a… | EVIDENCE-ROOT.md#v19; tools/evidence_store.py#session_identity; tools/evidence_store.py#record_key; tools/evidence_store.py#attempt_key | Codex v1.8 BLOCKERs S19-2 and S19-3 and MAJOR S19-4: bytes alone are not identity. A renamed session directory verified clean, one attempt id produced records in two layers, and chmod a… |

`EVIDENCE-ROOT.md` remains the single normative evidence-store document and gains the v1.9 laws.
`CHANGELOG-v1.8.md` and `FINDING-RESOLUTION-MATRIX-v1.8.md` become HISTORICAL; `CHANGELOG-v1.9.md` and
`FINDING-RESOLUTION-MATRIX-v1.9.md` are the active correction record.

## v1.10 supersessions (S50–S51)

Both replace a v1.9 statement that Codex's independent result showed to be unenforceable as written. The
machine-readable form is `AUTHORITY-PRECEDENCE.json`.

| id | superseded v1.9 statement | new authority | why |
|---|---|---|---|
| S50 | The evidence root trust boundary is established by lstat at open time and held in process memory as a RootBoundary, whose device and inode a… | EVIDENCE-ROOT.md#v110; tools/evidence_store.py#create_session; tools/evidence_store.py#check_boundary | Codex v1.9 BLOCKER S110-1: a boundary that exists only in memory cannot survive the process that built it. Replacing a finalized session directory with another of the same basename and … |
| S51 | A finalized session is verified by comparing the stored inventory against the filesystem, with stored attempt markers, directory entries and… | EVIDENCE-ROOT.md#v110; tools/evidence_store.py#expected_model; INVENTORY-FIELD-PROVENANCE.json | Codex v1.9 BLOCKERs S110-2 and S110-3 and MAJOR S110-4: recording a field and then trusting it is not verification. Finalization-marker store and authority versions were ignored, a fore… |

`EVIDENCE-ROOT.md` remains the single normative evidence-store document and gains the v1.10 laws.
`CHANGELOG-v1.9.md` and `FINDING-RESOLUTION-MATRIX-v1.9.md` become HISTORICAL; `CHANGELOG-v1.10.md` and
`FINDING-RESOLUTION-MATRIX-v1.10.md` are the active correction record.

## v1.11 supersessions (S52-S55)

All four replace a v1.10 statement that Codex's independent result showed to be unenforceable as written. The
machine-readable form is `AUTHORITY-PRECEDENCE.json`.

| id | superseded v1.10 statement | new authority | why |
|---|---|---|---|
| S52 | The persisted SESSION_BOUNDARY receipt is authoritative once its own boundary_sha256 recomputes over its stored bytes and its field set is canonical (v1.10 S50). | EVIDENCE-ROOT.md#v111; tools/evidence_store.py#BOUNDARY_PROVENANCE; tools/evidence_store.py#_boundary_receipt; INVENTORY-FIELD-PROVENANCE.json#boundary_fields | Codex v1.10 BLOCKER F110-A: digest consistency is not semantic truth. Five receipt fields were stored and digested but never compared outside the receipt, so rewriting one and recomputing boundary_sha256 left the next put_raw() authorized. |
| S53 | The stored inventory is verified by comparing its recorded entries, directories and nine normative header fields against the independently recomputed model, and additional top-level fields are tolerated (v1.10 S51). | EVIDENCE-ROOT.md#v111; tools/evidence_store.py#INVENTORY_FIELDS; tools/evidence_store.py#verify_summary; INVENTORY-FIELD-PROVENANCE.json#inventory_field_set | Codex v1.10 BLOCKER F110-B: six normative header fields were written, digested, published as derived and never compared, and an extra semantic top-level field survived verification. |
| S54 | Stored attempt markers and records reconcile as a bijection: every marker resolves to a record whose semantic key recomputes from the marker tuple, and every record has such a marker (v1.10 L7). | EVIDENCE-ROOT.md#v111; tools/evidence_store.py#expected_model; tools/evidence_store.py#_put; INVENTORY-FIELD-PROVENANCE.json#marker_cardinality_law | Codex v1.10 BLOCKER F110-C: the claim was one-to-one but only one direction was enforced. Two attempt ids over the same session, layer, logical identity and bytes produced two valid markers for one record, and finalize() and verify() both passed. |
| S55 | check_boundary() is called before every authorizing operation, and read and model accessors evaluate their own preconditions (v1.10 L3). | EVIDENCE-ROOT.md#v111; tools/evidence_store.py#PUBLIC_AUTHORITY_METHODS; tools/evidence_store.py#ERROR_PRECEDENCE; INVENTORY-FIELD-PROVENANCE.json#boundary_dominance_law | Codex v1.10 MERGE MAJOR / M0A MAJOR F110-D: nine public authority-bearing surfaces returned live data after session replacement, and inventory() and finalization_marker() masked the substitution with SESSION_NOT_FINALIZED. |

`EVIDENCE-ROOT.md` remains the single normative evidence-store document and gains the v1.11 laws.
`CHANGELOG-v1.10.md` and `FINDING-RESOLUTION-MATRIX-v1.10.md` become HISTORICAL; `CHANGELOG-v1.11.md` and
`FINDING-RESOLUTION-MATRIX-v1.11.md` are the active correction record.

## v1.12 supersessions (S56-S59)

All four replace a v1.11 statement that was CORRECT about form and silent about operation. The classification was
`AUTHORITY_UNDERSPECIFIED`: derivation correctness and record-form specification were complete and demonstrated, and
the failure was operational authority completeness. Machine form: `AUTHORITY-PRECEDENCE.json`.

| id | superseded v1.11 statement | new authority | why |
|---|---|---|---|
| S56 | the evidence-set document is defined by its schema and consumed by the derivation; where it lives, who writes it and whether it has a lifecycle are unspecified | EVIDENCE-SET-WORKFLOW.json; evidence_authoring.py#set_path, #_load | V112-1: with no governed location or writer, an implementer had to invent the container mapping, and two A2 repairs failed the same way. |
| S57 | the evidence-set document has no state | EVIDENCE-SET-WORKFLOW.json#lifecycle; authority_lib.py#EVIDENCE_SET_STATE_GRANTS | V112-2: nothing sequenced preparer against verifier, so a verifier could only attest to editable inputs. |
| S58 | BUNDLE_VERIFICATION independence is `verifier != prepared_by` over two caller-supplied strings | PRINCIPALS.json; authority_lib.py#principal_errors, #may_write; evidence_authoring.py#_authorize | V112-3: any differing string satisfied the gate and no tool owned the write. |
| S59 | a BUNDLE_VERIFICATION's presence, manifest binding and string inequality satisfy the attachment gate | a2_verify.py; evidence_authoring.py#add_bundle_verification; authority_lib.py#derive_attachment_state (v1.12 era; now SUPERSEDED by S61/S62 in favour of #derive_attachment_state_authorizing) | V112-4: no production law permitted an independent verifier to author it, and no positive state was reachable without fixtures. |

`EVIDENCE-ROOT.md` is unchanged: v1.12 touches no Store.v5 semantics. `PRINCIPALS.json` joins the trust roots and
`EVIDENCE-SET-WORKFLOW.json` becomes the single normative pre-M0A workflow document. `CHANGELOG-v1.11.md` and
`FINDING-RESOLUTION-MATRIX-v1.11.md` become HISTORICAL; `CHANGELOG-v1.12.md` and
`FINDING-RESOLUTION-MATRIX-v1.12.md` are the active correction record.

## v1.13 supersession (S60)

One statement, replacing the v1.12 persistence law that contradicted its own implementation. Machine form:
`AUTHORITY-PRECEDENCE.json`.

| id | superseded v1.12 statement | new authority | why |
|---|---|---|---|
| S60 | the document lives at one governed location; the authoring API accepts a root parameter so the suite can use a temporary directory, and the document records which root it was written under (v1.12 S56) | `tools/authority_lib.py#GOVERNED_ATTACHMENT_ROOT`; `tools/evidence_authoring.py#governed_attachment_root`, `#require_governed_root`, `#require_governed_session_dir`; `tools/evidence_authoring_testkit.py` | Codex v1.12 runtime-parity FAIL, WORKFLOW_CONTRADICTORY, V112-RP1: the one-root law lived in prose while every authorizing function took `root=` and both CLIs took `--evidence-root`, so an arbitrary OR SYMLINKED root derived ATTACHMENT_READY. A test convenience had been placed on the authorizing surface. |

`EVIDENCE-SET-WORKFLOW.json` remains the single normative pre-M0A workflow document and gains the governed-root,
entry-trust, location-recheck, refusal-code and test-override laws. `EVIDENCE-ROOT.md` is unchanged: v1.13 touches no
Store.v5 semantics, and `tools/evidence_store.py` is byte-identical to v1.12. `CHANGELOG-v1.12.md` and
`FINDING-RESOLUTION-MATRIX-v1.12.md` become HISTORICAL; `CHANGELOG-v1.13.md` and
`FINDING-RESOLUTION-MATRIX-v1.13.md` are the active correction record.


## v1.14 supersession (S61)

One statement, replacing the half of the v1.13 location law that lived only in the authoring wrapper. Machine form:
`AUTHORITY-PRECEDENCE.json`.

| id | superseded v1.13 statement | new authority | why |
|---|---|---|---|
| S61 | attachment readiness and eligibility are decided by `authority_lib.derive_attachment_state` and `authority_lib.evaluate_eligibility` — the two functions the v1.13 authority surface declared AUTHORIZING — over whatever evidence set the caller supplies, and the governed-location law is enforced one layer up in the authoring wrapper (v1.13 S60) | `tools/authority_lib.py#load_governed_evidence_set`, `#GovernedEvidenceSet`, `#governed_provenance_errors`, `#derive_attachment_state_authorizing`, `#evaluate_eligibility_authorizing`, `#location_errors`, `#location_receipt`; `tools/evidence_authoring.py#derive_attachment_state` | Codex v1.13 BLOCKER V113-B1: a document the wrapper refused with `NOT_PRODUCTION_ROOT` was handed straight to `authority_lib.derive_attachment_state` and returned `ATTACHMENT_READY`, and to `authority_lib.evaluate_eligibility` and returned `eligible: true`. Location authority did not dominate the authorizing call graph. |

S60 is not withdrawn: the frozen root, the argument-free resolver, the lstat-first entry trust and the
INTERNAL_NON_AUTHORIZING testkit all stand. S61 says where that law is *enforced*. After v1.14 the only door to an
authorizing answer is `load_governed_evidence_set`, which mints a `GovernedEvidenceSet` that nothing else can
construct; `derive_attachment_state_authorizing` and `evaluate_eligibility_authorizing` accept nothing else and
re-verify the location receipt against the real filesystem before they read a single record; and the two old
functions are declared DIAGNOSTIC / NON_AUTHORIZING on the authority surface. Precedence within an authorizing call
is therefore **location authority → attachment semantics → eligibility**, in that order and never the reverse.

`EVIDENCE-SET-WORKFLOW.json` remains the single normative pre-M0A workflow document and gains a `core_authority`
block naming the authorizing entry points, the diagnostic ones and the provenance law. `EVIDENCE-ROOT.md` is
unchanged: v1.14 touches no Store.v5 semantics, and `tools/evidence_store.py` differs from v1.13 by exactly one
comment line (the validator's filename, carried by the tool rename). `CHANGELOG-v1.13.md` and
`FINDING-RESOLUTION-MATRIX-v1.13.md` become HISTORICAL; `CHANGELOG-v1.14.md` and
`FINDING-RESOLUTION-MATRIX-v1.14.md` are the active correction record.

## v1.15 supersession (S62)

One statement, replacing the half of the v1.14 governed-carrier law that bound the wrong thing, and the v1.7 commit
designation that survived the v1.14 demotion. Machine form: `AUTHORITY-PRECEDENCE.json`.

| id | superseded v1.14 statement | new authority | why |
|---|---|---|---|
| S62 | the governed carrier holds the parsed evidence set in a writable slot and every authorizing use revalidates the live digest and receipt, so stale possession or raw evidence cannot bootstrap authority (v1.14 S61); and `commit_eligibility` → `validate_transaction_set` over a bare evidence set is the only authorizing commit path (v1.7 S44) | `tools/authority_lib.py#GovernedEvidenceSet`, `#governed_consume`, `#governed_structure_errors`, `#evidence_snapshot_digest`, `#governed_file_errors`, `#read_governed_file`, `#validate_transaction_set_authorizing`, `#commit_eligibility_authorizing`; `tools/evidence_authoring.py#_load`; `CANONICALIZATION.md`; `TARGET-CONTRACT.json`; `PERMISSIONS.json`; `TARGET-ATTACHMENT-GATE.md`; `ELIGIBILITY.md` | Codex v1.14: **V114-B1** the writable slot let provenance for document A be presented with semantic evidence B, deriving a **false `ATTACHMENT_READY` under A's own receipt**; **V114-B2** the bare-dict commit pair was classified authorizing and returned `eligible: true` for a fixture with no provenance; **V114-B3** four current artifacts still designated the demoted functions; **V114-B4** `WORKFLOW.json` was read through `strict_load`, following a symlink, hanging on a FIFO; **V114-M1** `object.__new__` leaked a raw `AttributeError`; **V114-N1** the closed hash-domain list omitted the receipt domain. |

The essential law S62 installs: **the exact bytes validated as governed evidence are the exact bytes consumed by
authorizing derivation.** The carrier is immutable and holds bytes, not a parsed object; `governed_consume` re-reads
and reparses those bytes on every authorizing call, compares the live digests with the minted ones *and* with the
carrier's own bytes, schema-validates the parse and checks its canonical snapshot digest against the one bound into
the receipt; both authority files receive one no-follow file law; malformed carriers fail closed as
`GOVERNED_EVIDENCE_INVALID`; the authorizing transaction and commit entry points require governed provenance and
route plan-time eligibility through `evaluate_eligibility_authorizing`.

S61 is not withdrawn: location authority still lives in the core, and the diagnostic pair stays demoted. S62 says
*what* the authorizing core consumes, and extends the same treatment to the transaction and commit paths.

Where an earlier statement in this document names one of the four bare-evidence functions as the authority of its
era — the DIAGNOSTIC / NON_AUTHORIZING `authority_lib.py#derive_attachment_state`; the DIAGNOSTIC / NON_AUTHORIZING
`authority_lib.py#evaluate_eligibility`; the PROVISIONAL_UNTIL_M3 non-authorizing
`authority_lib.py#validate_transaction_set`; the PROVISIONAL_UNTIL_M3 non-authorizing
`authority_lib.py#commit_eligibility` — that name now reads as its **authorizing counterpart**. The four bare-evidence functions are retained — the first two as
`DIAGNOSTIC / NON_AUTHORIZING` (v1.14 S61), the second two as `PROVISIONAL_UNTIL_M3 / NON-AUTHORIZING` (S62) — and no
current artifact may designate one for an authority decision. `EVIDENCE-ROOT.md` is unchanged: v1.15 touches no
Store.v5 semantics, and `tools/evidence_store.py` differs from v1.14 by exactly one comment line (the validator's
filename, carried by the tool rename). `CHANGELOG-v1.14.md` and `FINDING-RESOLUTION-MATRIX-v1.14.md` become
HISTORICAL; `CHANGELOG-v1.15.md` and `FINDING-RESOLUTION-MATRIX-v1.15.md` are the active correction record.

## v1.16 supersession (S63)

One statement, replacing the part of the v1.15 classification law that lived only in code plus hand-written prose,
and the receipt binding that was published as seven fields while ten were bound. Machine form:
`AUTHORITY-PRECEDENCE.json`.

| id | superseded v1.15 statement | new authority | why |
|---|---|---|---|
| S63 | the four authority classes are declared in code and every current artifact was corrected to agree with them (v1.15 S62); and `EVIDENCE-SET-WORKFLOW.json` publishes the location-receipt binding as the seven fields v1.14 bound | `AUTHORITY-FUNCTION-CLASSES.json`; `tools/authority_lib.py#authority_function_classes`, `#CLASSIFIED_DECISION_FUNCTIONS`, `#RECEIPT_BOUND_FIELDS`, `#RECEIPT_FIELD_SEMANTICS`, `#receipt_binding_publication`; `TARGET-CONTRACT.json`; `SCHEMA-REGISTRY.md`; `EVIDENCE-SET-WORKFLOW.json` | Codex v1.15: **V115-B1** two current normative `TARGET-CONTRACT` fields (`qualification_note`, `attachment_law`) still named the diagnostic derivation as the computing authority, unmarked, and `SCHEMA-REGISTRY.md:8-11` said the `*_authorizing` pair was `PROVISIONAL_UNTIL_M3`/non-authorizing — the inverse of the executable surface; the v1.15 audit missed both because its invariant exempted whole key names. **V115-M1** the published receipt binding omitted `workflow_path`, `workflow_sha256` and `evidence_snapshot_digest`, which the runtime binds. |

S62 is not withdrawn: the consumed-evidence law, the four classes and the demotions all stand. S63 changes *how* they
are published — **generated from the executable surface instead of restated by hand** — and closes the two current
artifacts that still disagreed.

**Precedence is not a band-aid for current inconsistency.** The blanket clause in this document's `law` re-reads
demoted function names in *earlier, superseded* statements only. Every CURRENT artifact is asserted correct on its own
by `validate_v1_16.py` sections `b1-classification` and `m1-receipt-parity`, with an exhaustive invariant that no
longer exempts `law`, `note`, `qualification_note` or `reason` — the exemptions that let the v1.15 contradiction pass
its own audit.

`EVIDENCE-ROOT.md` is unchanged: v1.16 touches no Store.v5 semantics, and `tools/evidence_store.py` differs from
v1.15 by exactly one comment line (the validator's filename, carried by the tool rename). The location-receipt hash
domain `vidtoolz.resolveLocationReceipt.v1` and the evidence-set snapshot domain are **unchanged and remain
distinct**: documentation catching up with the runtime is not a contract change. `CHANGELOG-v1.15.md` and
`FINDING-RESOLUTION-MATRIX-v1.15.md` become HISTORICAL; `CHANGELOG-v1.16.md` and
`FINDING-RESOLUTION-MATRIX-v1.16.md` are the active correction record.

## v1.17 supersession (S64)

One statement, replacing the hand-written slot inventory and the phrase-presence scanner that let a current
contradiction escape. Machine form: `AUTHORITY-PRECEDENCE.json`.

| id | superseded v1.16 statement | new authority | why |
|---|---|---|---|
| S64 | the canonical authorizing-slot inventory is the hand-written list of ten key paths in `AUTHORITY-FUNCTION-CLASSES.json`, and a current artifact's function references are audited by scanning each sentence for any of a list of demotion phrases (v1.16 S63) | `AUTHORITY-SLOT-INVENTORY.json`; `tools/authority_slots.py`; `AUTHORITY-FUNCTION-CLASSES.json`; `EVIDENCE-SET-WORKFLOW.json`; `CAPABILITIES.json`; `READ-PRIMITIVES.json`; `SNAPSHOT-CONCURRENCY-RECOVERY.md`; `SEMANTIC-VALIDATION.md`; `THREAT-MODEL.md` | Codex v1.16 sole BLOCKER **V116-B1**: `EVIDENCE-SET-WORKFLOW.json#toctou.law` designated the diagnostic core as the refusing authority, that key path was absent from the hand-written inventory, and the scanner suppressed the violation because an unrelated clause contained the phrase "no longer". A hand-written inventory cannot notice a slot nobody listed; a phrase-presence test cannot tell what a negation modifies. |

S63 is not withdrawn: the four classes, the canonical class map generated from `AUTHORITY_SURFACE`, and the generated
receipt binding all stand. S64 changes **how slots are found and how references are classified** — both derived, both
machine-checked — and corrects the TOCTOU designation itself.

**Precedence is still not a band-aid.** The blanket re-reading clause in this document's `law` applies to superseded
statements only, and now names each demoted function **with its class token in the same clause**, so the clause cannot
itself be read as a designation. Every current artifact is asserted correct on its own by `validate_v1_17.py`
sections `b1-toctou`, `slot-discovery`, `scanner-negation`, `unknown-slot`, `v117-property` and `v117-static`.

**No runtime semantics changed in v1.17.** `tools/evidence_authoring.py`, `tools/a2_prepare.py` and
`tools/a2_verify.py` are byte-identical to v1.16; `tools/evidence_store.py` differs by one comment line; every
changed line in `tools/authority_lib.py` is a version pin, a lineage pin or a tool-name reference — asserted in
section `v117-runtime-parity`. The location-receipt binding of S63 is untouched: ten runtime-bound fields, published
identically. `CHANGELOG-v1.16.md` and `FINDING-RESOLUTION-MATRIX-v1.16.md` become HISTORICAL; `CHANGELOG-v1.17.md`
and `FINDING-RESOLUTION-MATRIX-v1.17.md` are the active correction record.
