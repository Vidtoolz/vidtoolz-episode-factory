#!/usr/bin/env python3
"""Build FREEZE-MANIFEST.json for bundle v1.4 with machine-readable status, authority class, byte counts and lineage flags vs v1.3.
Run after validate_v1_4.py (so VALIDATION-REPORT.md is current): python3 -B tools/build_manifest.py
...
PARENT_HEAD = "1c9e090fcc8978e6e9b8f5dd538849448044d26b"
PARENT_MANIFEST = "ad1e6bfcbcb41d79c230795d64e031438e8e39a194f640daca68532d798a55dc"
...
parent = L.strict_load(os.path.join(V13, "FREEZE-MANIFEST.json"))
...
NOTES = {
    ...
    "SEMANTIC-VALIDATION.md": ("reference validators incl. validate_transaction_set / verify_transaction are not runtime-qualified; APPEND expected-effect law is provisional", "M3"),
    ...
    "tools/fixture_evidence.py": ("TOOL: mints fixture evidence against the PLACEHOLDER manifest sha and re-mints against the real manifest sha inside the validator; never an authority", None),
    ...
    "fixtures/evidence/ACTIVE-AUTHORITY-PLACEHOLDER.json": ("fixture evidence is minted against this PLACEHOLDER manifest sha; validate_v1_4.py proves placeholder-bound records do not derive against the real FREEZE-MANIFEST.json sha and that re-minted records do", None),
}
...
def authority_class(p):
    ...
    if p.startswith("tools/"):
        return "TOOL"
    if p in ("ADJUDICATION-FREEZE-CONTRACT.md", "AUTHORITY-ARCHITECTURE-ADJUDICATION-M0-M3.md", "CHANGELOG-v1.1.md", "FINDING-RESOLUTION-MATRIX.md", "CHANGELOG-v1.2.md", "FINDING-RESOLUTION-MATRIX-v1.2.md", "CHANGELOG-v1.3.md", "FINDING-RESOLUTION-MATRIX-v1.3.md"):
...
for p in files:
    ...
    if st == "PROVISIONAL_UNTIL_M3":
        e["blocked_until"] = "M3"
    entries.append(e)
ext = {"v1_3_parent_manifest": os.path.join(V13, "FREEZE-MANIFEST.json"), "v1_2_grandparent_manifest": os.path.join(V12, "FREEZE-MANIFEST.json"), "v1_1_manifest": os.path.join(V11, "FREEZE-MANIFEST.json"), "v1_0_manifest": os.path.join(V10, "FREEZE-MANIFEST.json"), "doc_authority": os.path.join(os.path.dirname(os.path.dirname(B)), "DOC-AUTHORITY.md")}
exth = {k: {"path": os.path.relpath(v, B), "sha256": sha(v), "bytes": os.path.getsize(v)} for k, v in ext.items()}
assert exth["v1_3_parent_manifest"]["sha256"] == PARENT_MANIFEST, exth["v1_3_parent_manifest"]
assert exth["v1_2_grandparent_manifest"]["sha256"] == "69e1caecf9ba9bd16875b7625c58902e3e5f613372165c8d11d80ff48939b5e6"
...
m = {"schema": "vidtoolz.resolveFreezeManifest.v1.4", "bundle": "docs/resolve-integration/v1.4", "version": "1.4.0", "frozen_at": "2026-09-09", "prepared_by": "Claude Code (Fable 5.1)", "approval_status": "CANDIDATE_FOR_INDEPENDENT_REVIEW", "approver": None,
     "parent": {"version": "1.3.0", "branch": "docs/resolve-authority-freeze-v1.3", "head": PARENT_HEAD, "manifest_sha256": PARENT_MANIFEST, "immutable": True,
                "grandparent": {"version": "1.2.0", "branch": "docs/resolve-authority-freeze-v1.2", "head": "c6d1284c6f395f5b21a6d14f8c2873bccfc065bb", "manifest_sha256": "69e1caecf9ba9bd16875b7625c58902e3e5f613372165c8d11d80ff48939b5e6",
                                "great_grandparent": {"version": "1.1.0", "branch": "docs/resolve-authority-freeze-v1.1", "head": "47ddb225335b8c85ca5255b1de86ff508e0e8e91", "manifest_sha256": "83d8a307098cdc18931f6f09de2ce782afa0bdef94ec8540c2a143910cd448c5",
                                                      "great_great_grandparent": {"version": "1.0.0", "branch": "docs/resolve-authority-freeze-v1", "head": "e2874f6f67f5adb3a5ced0e0e138c370e38995cb", "manifest_sha256": "d9cd54780e0a123ae2045ac575bbc41638039ef3e878f1b608afbce3989a8821", "baseline": "f30e4543b0af17d047f803ae9044aef859ef13bc"}}}},
     ...
     "rules": ["a file whose sha256 or byte count differs from this manifest is not the frozen version", "v1.0, v1.1, v1.2 and v1.3 bytes and manifests are never modified; v1.4 supersedes by reference", "SCHEMA-VALID != AUTHORIZED TO MUTATE; PERMISSION DECLARATION != ELIGIBILITY; PROBE_ALLOWED != QUALIFIED_READ; CANDIDATE_CAPABILITY_EVIDENCE != QUALIFIED_READ; attachment state is derived from a coherent, current, envelope-bound evidence set against the ACTIVE reviewed manifest, never declared; contradictory evidence is CONFLICT", "a BUNDLE_VERIFICATION, MILESTONE_EXIT, M3_AUTHORIZATION or REFREEZE_RECORD counts only when bound to this manifest's sha256 and authority version 1.4.0; records for other manifests are historical", "mutation-capable schemas remain PROVISIONAL_UNTIL_M3; verification is derived from S0/S1 (verify_transaction) and never declared; no adapter mutation implementation exists; no Resolve write was executed; M0 has not begun", "PERMISSIONS.json is default DENY; unknown milestone/operation/scope/prerequisite is denied; shared_library_allowed is never true; every entry declares a target_requirement", "nothing in this bundle is human approval of any run, gate or publication"],
