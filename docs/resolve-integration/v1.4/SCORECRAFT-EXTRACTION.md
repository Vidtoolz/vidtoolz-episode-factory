# Implementation lineage — Scorecraft extraction map v1.4 (FROZEN_NOW)

Active extraction targets are the **v1.4** authorities (`resolveTargetContract.v1.4`, `resolveSnapshot.v1.4`, `TARGET-ATTACHMENT-GATE.md`, `ELIGIBILITY.md`, `SEMANTIC-VALIDATION.md`). References to earlier schema versions in `../v1/`, `../v1.1/`, `../v1.2/`, `../v1.3/` are historical only.
...
|---|---|---|
| explicit target contract shape (`normalizeTarget`) | QUALIFIED_PATTERN | PRESERVE as concept → `resolveTargetContract.v1.4` (attachment state derived from envelope-bound, current evidence against the active manifest, not declared) |
...
| readback | EXPERIMENTAL | REWRITE for `resolveSnapshot.v1.4` (all tracks, provenance variants, observation status on every timeline/project/track/item field, capability-coupled coverage, failure ledger, no rounding, plural APIs) |
...
| `source_timeline_untouched` assertion | unsafe | REJECT as proof; replace by the derived S0→S1 delta (`verify_transaction`) |
