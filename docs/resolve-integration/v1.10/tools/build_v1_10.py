...
Run from the bundle dir: python3 -B tools/build_v1_10.py
...
tc["schema"] = "vidtoolz.resolveTargetContract.v1.10"; tc["version"] = "1.10.0"
...
tcs["properties"]["version"] = {"type": "string", "pattern": "^1\\.10\\.[0-9]+$"}
...
rp["schema"] = "vidtoolz.resolveReadPrimitives.v1.10"; rp["version"] = "1.10.0"
...
caps["schema"] = "vidtoolz.resolveCapabilityMatrix.v1.10"; caps["version"] = "1.10.0"
...
caps_hyp = copy.deepcopy(caps); caps_hyp["version"] = "1.10.0-HYPOTHETICAL"; caps_hyp["qualification_note"] = "HYPOTHETICAL_NOT_AUTHORITY: illustrates a future reviewed refreeze (rows QUALIFIED_READ citing raw/derived/review digests of fixture captures); never load as CAPABILITIES.json"
...
policy = {"target_contract_sha256": sha_of("TARGET-CONTRACT.json"), "timebase_sha256": sha_of("TIMEBASE.json"), "track_policy_sha256": sha_of("schemas/resolveTrackPolicy.v1.json"), "capabilities_version": "1.10.0", "capability_matrix_sha256": CAPS_SHA, "collector_version": "0.0.0-fixture", "canonicalization_version": "1.5"}
policy_hyp = dict(policy, capabilities_version="1.10.0-HYPOTHETICAL", capability_matrix_sha256=HYP_SHA)
...
perms["schema"] = "vidtoolz.resolvePermissions.v1.10"; perms["version"] = "1.10.0"
...
matrix = load("MILESTONE-MATRIX.json"); matrix["schema"] = "vidtoolz.resolveMilestoneMatrix.v1.10"; matrix["version"] = "1.10.0"; dump("MILESTONE-MATRIX.json", matrix)
...
h0_other_matrix = copy.deepcopy(s0); h0_other_matrix["policy"] = dict(policy_hyp, capability_matrix_sha256=CAPS_SHA, capabilities_version="1.10.0"); resign(h0_other_matrix)
...
p = copy.deepcopy(plan_m3); p["authority_version"] = "1.4.0"; fixture("plan-authority-version-old", "schema", "provisional/resolveMutationPlan", p, expect_contains="1.10.0")
...
man_schema["properties"]["schema"] = {"const": "vidtoolz.resolveFreezeManifest.v1.10"}; man_schema["properties"]["bundle"] = {"const": "docs/resolve-integration/v1.10"}; man_schema["properties"]["version"] = {"const": "1.10.0"}
man_schema["properties"]["parent"] = {"type": "object", "additionalProperties": False, "required": ["version", "branch", "head", "semantic_head", "manifest_sha256", "immutable", "ancestors"], "properties": {
    "version": {"const": "1.9.0"}, "branch": {"const": "docs/resolve-authority-freeze-v1.9"}, "head": {"const": "863fe4fb5944935bad51a318a50b25ed748be22d"}, "semantic_head": {"const": "863fe4fb5944935bad51a318a50b25ed748be22d"}, "manifest_sha256": {"const": "5aac34dae6c2ec9842aaa3499d7c20ffd104f65966ba7cb8b0c79d32d42eb9fe"}, "immutable": {"const": True},
    "ancestors": {"type": "array", "minItems": 9, "items": {"type": "object", "additionalProperties": False, "required": ["version", "branch", "head", "manifest_sha256"], "properties": {"version": {"type": "string"}, "branch": {"type": "string"}, "head": {"type": "string", "pattern": "^[0-9a-f]{40}$"}, "manifest_sha256": SHA, "baseline": {"type": "string"}}}}}}
...
CODEX_FINDINGS = [
    {"id": "C16-B1", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_10.py section cache-mutation: nine in-place mutations of a qualified chain (raw->RAISED, primitive spec, review->REJECT, refreeze un-reviewed, matrix row demoted, stored derived edited, host_name, build, matrix evidence reference), each asserting warm == cold and NOT QUALIFIED, plus parser replacement, callable-set mutation and cross-evidence-set contamination.",
    ...
    {"id": "C16-B2", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_10.py section validator-injection: five injected validators x (positional, keyword) x both entry points, five validator-named keywords, the diagnostic surface, and one changed byte of one pinned schema file making the whole registry untrusted.",
    ...
    {"id": "C16-B3", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_10.py section shim-trust: wrong sha, stale build, unknown shim, same version with changed code, other codec, other raw schema, a method outside the trusted allowlist, an active authority missing or misnaming the trusted shim, and a whole chain over a wrong-shim capture.",
    ...
    {"id": "C16-M1", "severity": "MAJOR",
     ...
     "negative_fixture": "validate_v1_10.py section identity-duplicates: duplicate path, path with two handle tokens, path with two identity values, duplicate attempt id, two paths returning one id, and a pass-set mismatch.",
    ...
    {"id": "C16-M2", "severity": "MAJOR",
     ...
     "negative_fixture": "validate_v1_10.py section raw-ingestion: thirteen malformed frames each asserting its closed RAW_INGEST_CODES refusal, plus records with a missing, mis-bound or non-strict receipt.",
    ...
    {"id": "C16-M3", "severity": "MAJOR",
     ...
     "negative_fixture": "validate_v1_10.py section evidence-store: session re-creation, caller-declared digest mismatch, attempt-id reuse, path traversal, unknown layer, symlinked session, write after finalization, re-finalization, replaced record bytes, tampered HASHES, removed record, unfinalized session, partially written record, mismatched pinned authority, and a static audit that no code path opens a record for writing, truncates, renames over one or unlinks one.",
...
S110_FINDINGS = [
    {"id": "S110-1", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_10.py section evidence-boundary-receipt: session directory replaced by another with the same basename, same-basename session under an alternate root, boundary receipt mutation and deletion, root replaced by symlink and by another directory, plus randomized boundary-replacement property tests.",
    ...
    {"id": "S110-2", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_10.py sections evidence-recomputed-model and evidence-finalization-fields: the seven-field finalization attack matrix, inventory header mutation, directory duplicate and conflicting-kind cases, and the provenance table assertion that no normative field is trusted from the inventory itself.",
    ...
    {"id": "S110-3", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_10.py section evidence-attempt-derivation: foreign marker planted while ACTIVE, duplicate attempt id, orphaned record, marker field mutation property tests, and the cross-session ACTIVE insertion matrix.",
    ...
    {"id": "S110-4", "severity": "MAJOR",
     ...
     "negative_fixture": "validate_v1_10.py section evidence-mode-authority: ACTIVE chmod of session directory, layer directory, manifest, record, attempt marker and journal each blocking the next canonical operation; the finalized chmod matrix retained; randomized permission-bit property tests in both states.",
    ...
    {"id": "S110-5", "severity": "MINOR",
     ...
     "negative_fixture": "validate_v1_10.py section evidence-fs-errors: missing path, permission denied, symlink loop, wrong file type, name too long, cross-device, and concurrent disappearance, each asserting a frozen class and no raw OSError.",
...
S19_FINDINGS = [
    {"id": "S19-1", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_10.py section evidence-root-trust: symlinked evidence root and symlinked session root refused at create, open, finalize and verify; root replaced by a symlink and by another directory between operations; non-directory root.",
    ...
    {"id": "S19-2", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_10.py sections evidence-session-identity and evidence-semantic-inventory: renamed session, cross-session copies of the whole session, the inventory alone, the marker alone, one record and the manifest; wrong layer; wrong logical identity; tampered internal session id.",
    ...
    {"id": "S19-3", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_10.py section evidence-attempt-tuple: the full collision table plus RAW attempt reused in DERIVED, DERIVED reused in HUMAN_REVIEW, one digest under two logical identities, one record's bytes in two layers, one attempt id across two sessions, and duplicate attempt keys in an inventory.",
    ...
    {"id": "S19-4", "severity": "MAJOR",
     ...
     "negative_fixture": "validate_v1_10.py section evidence-mode-authority: chmod of a record, a layer directory, the session manifest, the inventory and the finalization marker; a regular file replaced by another filesystem type.",
...
ES_FINDINGS = [
    ...
    {"id": "ES-2", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_10.py sections evidence-single-authority (static audit: exactly one EVIDENCE_STORE_AUTHORIZING module, EvidenceRoot absent, no storage primitives in the shim) and evidence-traversal (the exact legacy attack plus twelve traversal, absolute-path, separator, symlink, dot, empty and over-long identifier shapes against every write API, each proving no file appears outside the session root).",
...
dump("FINDING-RESOLUTION-MATRIX-v1.10.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.10", "authority_version": L.AUTHORITY_VERSION,
                                             ...
                                             "law": "each finding names the v1.9 defect, the v1.10 correction, at least one negative fixture, the layer the attack must fail at, and the validate_v1_10.py sections that prove it. validate_v1_10.py refuses to pass unless every named section exists and every check in it passes.",
                                             "operational_closure": "the v1.8 evidence-store matrix (S19-1..S19-4) is retained as FINDING-RESOLUTION-MATRIX-v1.9.json and its corrections are re-run as v1.10 regressions; this file is the ACTIVE matrix.",
...
dump("FINDING-RESOLUTION-MATRIX-v1.9.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.10", "authority_version": L.AUTHORITY_VERSION,
                                             ...
                                             "law": "each finding names the v1.8 defect, the v1.9 correction, at least one negative fixture, the layer the attack must fail at, and the validate_v1_10.py sections that prove it. validate_v1_10.py refuses to pass unless every named section exists and every check in it passes.",
...
dump("FINDING-RESOLUTION-MATRIX-v1.8.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.10", "authority_version": L.AUTHORITY_VERSION,
                                             ...
                                             "law": "each finding names the v1.7 defect, the v1.8 correction, at least one negative fixture, the layer the attack must fail at, and the validate_v1_10.py sections that prove it. validate_v1_10.py refuses to pass unless every named section exists and every check in it passes.",
...
dump("FINDING-RESOLUTION-MATRIX-v1.7.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.10", "authority_version": L.AUTHORITY_VERSION,
                                             ...
                                             "law": "each finding names the v1.6 defect, the v1.7 correction, at least one negative fixture, the layer the attack must fail at, and the validate_v1_10.py sections that prove it. validate_v1_10.py refuses to pass unless every named section exists and every check in it passes.",
                                             "operational_closure": "C16-OP1 (Hermes M0A cannot bind to v1.6 because the trusted shim, parser, spec and raw identities are not operationally closed) is closed by M0A-BINDING-VALUES.json, which publishes every value with no placeholder and is verified against the running code by validate_v1_10.py section m0a-binding.",
...
# ============================================================ section 8: the inventory field provenance table
PROVENANCE = {
    "schema": "vidtoolz.resolveInventoryFieldProvenance.v1.10",
    "authority_version": L.AUTHORITY_VERSION,
    "law": "every NORMATIVE inventory and finalization field is classified by the source the verifier derives it from. No normative field may be classified TRUSTED_FROM_INVENTORY_ITSELF: the stored inventory is evidence that is compared against an independently recomputed model, never the model. validate_v1_10.py asserts this table is complete and that no normative field is self-asserted.",
...
prov_schema = S("vidtoolz.resolveInventoryFieldProvenance.v1.10", "Inventory field provenance v1.10 (no normative field is self-asserted)",
                {"schema": {"const": "vidtoolz.resolveInventoryFieldProvenance.v1.10"}, "authority_version": {"const": L.AUTHORITY_VERSION},
...
dump("M0A-BINDING-VALUES.json", {
    ...
    "authority_version": L.AUTHORITY_VERSION,
    "purpose": "The exact identities an M0A probe package must record and be bound to under authority v1.7. Codex's v1.6 operational finding was that Hermes could not bind its M0A package because these values were not operationally closed. Every value here is the value the running code produces; validate_v1_10.py section m0a-binding re-derives each one and fails if any differs or reads as a placeholder.",
