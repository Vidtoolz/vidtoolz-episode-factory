#!/usr/bin/env python3
"""Build FREEZE-MANIFEST.json for bundle v1.7 with machine-readable status, authority class, byte counts and lineage flags vs v1.6.
Run after validate_v1_10.py (so VALIDATION-REPORT.md is current): python3 -B tools/build_manifest.py
...
PARENT_HEAD = "863fe4fb5944935bad51a318a50b25ed748be22d"            # v1.9 branch HEAD that v1.10 was cut from
PARENT_SEMANTIC_HEAD = "863fe4fb5944935bad51a318a50b25ed748be22d"   # v1.9 committed its bundle and its DOC-AUTHORITY registration together, so this IS the head
PARENT_MANIFEST = "5aac34dae6c2ec9842aaa3499d7c20ffd104f65966ba7cb8b0c79d32d42eb9fe"
...
parent = L.strict_load(os.path.join(V19, "FREEZE-MANIFEST.json"))
...
NOTES = {
    ...
    "SEMANTIC-VALIDATION.md": ("reference validators incl. commit_eligibility / validate_transaction_set / verify_transaction are not runtime-qualified; the APPEND expected-effect law, the APPEND_VERIFY profile, the mandatory checkpoint and the H0 early write gate are provisional", "M3"),
    ...
    "fixtures/evidence/ACTIVE-AUTHORITY-PLACEHOLDER.json": ("fixture evidence is minted against this PLACEHOLDER manifest sha; validate_v1_10.py proves placeholder-bound records do not derive against the real FREEZE-MANIFEST.json sha and that re-minted records do", None),
...
m = {"schema": "vidtoolz.resolveFreezeManifest.v1.10", "bundle": "docs/resolve-integration/v1.10", "version": "1.10.0", "frozen_at": "2026-09-09", "prepared_by": "Claude Code (Fable 5.1)", "approval_status": "CANDIDATE_FOR_INDEPENDENT_REVIEW", "approver": None,
     "parent": {"version": "1.9.0", "branch": "docs/resolve-authority-freeze-v1.9", "head": PARENT_HEAD, "semantic_head": PARENT_SEMANTIC_HEAD, "manifest_sha256": PARENT_MANIFEST, "immutable": True,
                "ancestors": [
