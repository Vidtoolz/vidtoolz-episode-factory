...
Run: python3 -B tools/validate_v1_10.py
"""
import copy
import glob
import hashlib
...
rec("precedence", "the v1.5 through v1.9 changelogs and finding matrices are HISTORICAL; the v1.10 ones are active", all(st_.get(d_) == "HISTORICAL" for d_ in ("CHANGELOG-v1.5.md", "FINDING-RESOLUTION-MATRIX-v1.5.md", "CHANGELOG-v1.6.md", "FINDING-RESOLUTION-MATRIX-v1.6.md", "CHANGELOG-v1.7.md", "FINDING-RESOLUTION-MATRIX-v1.7.md", "CHANGELOG-v1.8.md", "FINDING-RESOLUTION-MATRIX-v1.8.md", "CHANGELOG-v1.9.md", "FINDING-RESOLUTION-MATRIX-v1.9.md")) and st_.get("CHANGELOG-v1.10.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.10.md") == "STILL_ACTIVE")
...
rec("precedence", "the v1.10 evidence-store supersessions S50-S51 are recorded and the v1.10 changelog and matrix are active", sorted(s_["id"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.10.0") == ["S50", "S51"] and st_.get("CHANGELOG-v1.10.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.10.md") == "STILL_ACTIVE" and st_.get("CHANGELOG-v1.9.md") == "HISTORICAL" and st_.get("EVIDENCE-ROOT.md") == "STILL_ACTIVE")
...
if os.path.exists(man_path):
    ...
    parent = L.strict_load(os.path.join(os.path.dirname(B), "v1.9", "FREEZE-MANIFEST.json"))
    rec("manifest", "parent (v1.9) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.9", "FREEZE-MANIFEST.json")) == "5aac34dae6c2ec9842aaa3499d7c20ffd104f65966ba7cb8b0c79d32d42eb9fe")
    rec("manifest", "grandparent (v1.8) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.8", "FREEZE-MANIFEST.json")) == "727bcaad6ee49f1439eb84b33bf36cb11d2c94d41165b4b092f728a167626d86")
    rec("manifest", "the parent block records the v1.9 branch HEAD and its semantic commit, which for v1.9 are the same commit because its bundle and registration were committed together (section 38)", man["parent"]["head"] == "863fe4fb5944935bad51a318a50b25ed748be22d" and man["parent"]["semantic_head"] == "863fe4fb5944935bad51a318a50b25ed748be22d" and man["parent"]["manifest_sha256"] == "5aac34dae6c2ec9842aaa3499d7c20ffd104f65966ba7cb8b0c79d32d42eb9fe")
    rec("manifest", "v1.0-v1.9 bundles are byte-identical to their frozen manifests (history preserved)", all(all(sha(os.path.join(os.path.dirname(B), v_, e_["path"])) == e_["sha256"] for e_ in L.strict_load(os.path.join(os.path.dirname(B), v_, "FREEZE-MANIFEST.json"))["files"] if os.path.exists(os.path.join(os.path.dirname(B), v_, e_["path"]))) for v_ in ("v1", "v1.1", "v1.2", "v1.3", "v1.4", "v1.5", "v1.6", "v1.7", "v1.8", "v1.9")))
    ...
    rec("manifest", "validation tools listed as TOOL", all(any(e["path"] == p and e["authority_class"] == "TOOL" for e in man["files"]) for p in ("tools/authority_lib.py", "tools/validate_v1_10.py", "tools/build_v1_10.py", "tools/build_manifest.py", "tools/fixture_evidence.py", "tools/capture_shim_reference.py")))
...
try:
    ...
    for _label, _mut in (("a mutated root inode", lambda d: d.update(root_inode=d["root_inode"] + 1)),
        ...
        rec("evidence-boundary-receipt", f"a boundary receipt with {_label} is refused", _e.code in ("ROOT_IDENTITY_MISMATCH", "SESSION_BOUNDARY_CHANGED", "SESSION_PATH_MISMATCH", "ROOT_REPLACED"), _e.code)
        os.chmod(_bp, 0o600)
...
CODEX_MATRIX = L.strict_load(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.10.json"))
...
rec("codex-matrix", "the inherited v1.6 finding matrix is retained unchanged as history and its seven ids are still enumerated",
    ...
    and CODEX_MATRIX["inherited_matrices"] == ["FINDING-RESOLUTION-MATRIX-v1.9.json", "FINDING-RESOLUTION-MATRIX-v1.8.json", "FINDING-RESOLUTION-MATRIX-v1.7.json"])
rec("codex-matrix", "the ACTIVE v1.10 finding-resolution matrix enumerates exactly the five Codex v1.9 evidence-store ids",
    sorted(f["id"] for f in CODEX_MATRIX["findings"]) == ["S110-1", "S110-2", "S110-3", "S110-4", "S110-5"])
