#!/usr/bin/env python3
"""Build the v1.19 manifest with final external bytes and exact v1.18 lineage.
...
m = {"schema": "vidtoolz.resolveFreezeManifest.v1.10", "bundle": "docs/resolve-integration/v1.19", "version": "1.19.0", "frozen_at": "2026-09-10", "prepared_by": "Claude Code (Opus 5)", "approval_status": "CANDIDATE_FOR_INDEPENDENT_REVIEW", "approver": None,
     ...
     "rules": ["a file whose sha256 or byte count differs from this manifest is not the frozen version", "v1.0 through v1.18 bytes and manifests are never modified; v1.19 supersedes by reference", "SCHEMA-VALID != AUTHORIZED TO MUTATE; PERMISSION DECLARATION != ELIGIBILITY; PROBE_ALLOWED != QUALIFIED_READ; A CAPTURE != EVIDENCE THAT SOMETHING WORKS; attachment state is derived from a coherent, current, envelope-bound evidence set against the ACTIVE reviewed manifest, never declared; contradictory evidence is CONFLICT", "a BUNDLE_VERIFICATION, MILESTONE_EXIT, M3_AUTHORIZATION or REFREEZE_RECORD counts only when bound to this manifest's sha256 and authority version 1.19.0; records for other manifests are historical", "mutation-capable schemas remain PROVISIONAL_UNTIL_M3; verification is derived from S0/S1 over the protected surface (verify_transaction) and never declared; commit_eligibility_authorizing -> validate_transaction_set_authorizing over a GovernedEvidenceSet is the ONLY authorizing commit path (v1.15 V114-B2; the bare-dict commit_eligibility/validate_transaction_set pair is PROVISIONAL_UNTIL_M3 and NON-AUTHORIZING, and every per-document helper is INTERNAL_NON_AUTHORIZING); capability meaning is DERIVED by the versioned reference parser from an immutable facts-only raw capture (never from a probe-written flag), the ACTIVE capability authority is the matrix's own content digest, a degraded H0 can never be a write precondition, and schema enforcement in the composed path is mandatory; no adapter mutation implementation exists; no Resolve write was executed; M0 has not begun",
...
# v1.19 release lineage: inherited ancestors are immutable; current parent is exact v1.18.
_p17 = L.strict_load(os.path.join(os.path.dirname(B), 'v1.18', 'FREEZE-MANIFEST.json'))
_out = os.path.join(B, 'FREEZE-MANIFEST.json')
_m18 = m
_m18['parent'] = dict(_p17['parent'])
_m18['parent'].update(version='1.18.0', branch='docs/resolve-authority-freeze-v1.18', head='5d9dbba7c9f136caf694debe177f61168e5c4271', semantic_head='5d9dbba7c9f136caf694debe177f61168e5c4271', manifest_sha256='80b108e4e142841af38827d3b6170c1260c543740f21b03ebb3d934278723ae9', immutable=True)
...
_m18['external_pins']['v1_17_parent_manifest']={'path':'../v1.18/FREEZE-MANIFEST.json','sha256':'80b108e4e142841af38827d3b6170c1260c543740f21b03ebb3d934278723ae9','bytes':os.path.getsize(os.path.join(os.path.dirname(B),'v1.18/FREEZE-MANIFEST.json'))}
