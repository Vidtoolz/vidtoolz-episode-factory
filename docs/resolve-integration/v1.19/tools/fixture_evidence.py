...
PRIMITIVE_SPECS = {
    ...
    "GetTimelineCount": _spec("Project", "int"), "GetTimelineByIndex": _spec("Project", "object", arg_types=("int",), nullable=True), "GetCurrentTimeline": _spec("Project", "object", nullable=True), "GetProjectLastModifiedTime": _spec("ProjectManager", "int", arg_types=("str",)),
    ...
    "GetClipEnabled": _spec("TimelineItem", "bool"), "GetMediaPoolItem": _spec("TimelineItem", "object", nullable=True), "GetClipProperty": _spec("MediaPoolItem", "dict", shape_rule="NON_EMPTY"), "TimelineItem.GetProperties": _spec("TimelineItem", "dict", shape_rule="NON_EMPTY"),
}
assert len(PRIMITIVE_SPECS) == 47, len(PRIMITIVE_SPECS)


ARG_SAMPLE = {"str": {"GetProjectLastModifiedTime": PROJ, "GetMarkerByCustomData": "vidtoolz:resolve:binding:v1:epoch-fixture-1:b-001:o-1"}, "int": {}, "bool": {}, "float": {}}
...
OK_VALUES = {
    ...
    "GetTimelineCount": lambda s: 1, "GetTimelineByIndex": lambda s, i: FakeObj("tl"), "GetCurrentTimeline": lambda s: FakeObj("tl"), "GetProjectLastModifiedTime": lambda s, projectName: 1788868200,
...
# v1.12: fixtures use the SAME registered principals and the SAME contract pinned-file digest set the production
# verifier computes. A fixture cannot satisfy the gate with an arbitrary verifier string any more than a real run can.
PREPARER_PRINCIPAL = L.PRINCIPAL_REGISTRY["PREPARER"][0]
VERIFIER_PRINCIPAL = L.PRINCIPAL_REGISTRY["VERIFIER"][0]
PINNED_DIGESTS = dict(L.strict_load(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "TARGET-CONTRACT.json"))["resolve"]["pins"])
...
class Mint:
    ...
    def bundle(self, verified_provisioning_id=None, verified_launch_recipe_id=None, **kw):
        """v1.12: the workflow binding is part of the record. verified_launch_recipe_id defaults to the id the
        matching self.launch() produces, so a fixture set is bound exactly as a production document is."""
        vp = verified_provisioning_id or self.prov["record_id"]
        vl = verified_launch_recipe_id or self.launch()["record_id"]
        body = {"authority_version": self.AV, "manifest_sha256": self.M,
                "verifier": VERIFIER_PRINCIPAL, "prepared_by": PREPARER_PRINCIPAL, "historical": False,
                "verification_result": "PASS",
                "verifier_principal": VERIFIER_PRINCIPAL, "preparer_principal": PREPARER_PRINCIPAL,
                "verified_provisioning_id": vp, "verified_launch_recipe_id": vl,
                "pinned_file_digests": dict(PINNED_DIGESTS)}
        env = kw.pop("env", {})
        body.update(kw)
...
def ES(records, current_session=S_CUR, evaluated_at=EVALUATED_AT):
    return {"schema": L.EVIDENCE_SET_SCHEMA_ID, "current_session_id": current_session, "evaluated_at": evaluated_at, "records": {r["record_id"]: r for r in records}}
...
def base_sets(m, probe_methods, caps_hyp, parent_caps_sha):
    ...
    sets = {
        ...
        # v1.12: self-verification now means BOTH the v1.11 free-text pair AND the registered principals collapse to
        # the preparer, so the fixture exercises the inequality rule and the role rule at once.
        "ready-self-verified-bundle": ES([prov, m.bundle(verifier=PREPARER_PRINCIPAL, verifier_principal=PREPARER_PRINCIPAL), launch, roj]),
