...
def run(bundle,rec):
    ...
    def check(id,ok,detail=''):rec('v119-release-scanner',id,bool(ok),detail)
    ...
    finding_map=json.loads((b/'FINDING-RESOLUTION-MATRIX-v1.19.json').read_text())
    matrix_errors=S.L.internal_schema_errors('resolveFindingResolutionMatrix',finding_map)
    check('active-finding-map-schema',not matrix_errors and {r['id'] for r in finding_map['findings']}=={'V118-M'+str(i) for i in range(1,5)}|{'V118-N'+str(i) for i in range(1,4)},matrix_errors)
    sel=RELEASE.MARKER.search(doc)
    for id,mutant in [
        ('zero-current',doc[:sel.start()]+doc[sel.end():]),
        ('duplicate-selector',doc+'\n'+sel[0]),
        ('old-current',doc.replace('"version":"1.19.0"','"version":"1.13.0"')),
        ('missing-registration',doc.replace('**v1.19 registration** (current):','')),
    ...
    manifest=json.loads((b/'FREEZE-MANIFEST.json').read_text());parent=json.loads((b.parent/'v1.18/FREEZE-MANIFEST.json').read_text())
    ...
    with tempfile.TemporaryDirectory(prefix='v119-publication-tests-') as td:
        t=pathlib.Path(td);dest=t/'docs/resolve-integration/v1.19';dest.mkdir(parents=True)
        # Parent is read-only source input; copy only the registration needed by independent universe derivation.
        (dest.parent/'v1.18').mkdir();shutil.copyfile(b.parent/'v1.18/AUTHORITY-PRECEDENCE.json',dest.parent/'v1.18/AUTHORITY-PRECEDENCE.json')
        for f in b.iterdir():
    ...
    for name in ['evidence_authoring.py','a2_prepare.py','a2_verify.py','evidence_store.py']:
        check('runtime-byte-'+name,(b/'tools'/name).read_bytes()==(b.parent/'v1.18/tools'/name).read_bytes())
    def semantics(path):
    ...
    check('runtime-authority-ast',semantics(b/'tools/authority_lib.py')==semantics(b.parent/'v1.18/tools/authority_lib.py'))
    check('receipt-ten-fields',len(S.L.RECEIPT_BOUND_FIELDS)==10)
    for v in ['v1']+['v1.'+str(i) for i in range(1,19)]:
