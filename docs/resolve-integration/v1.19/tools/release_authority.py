...
VERSION = '1.19.0'
PARENT_HEAD = '5d9dbba7c9f136caf694debe177f61168e5c4271'
PARENT_MANIFEST = '80b108e4e142841af38827d3b6170c1260c543740f21b03ebb3d934278723ae9'
...
def registration_errors(text, version=VERSION):
    ...
    if {v for v,s in rows}!={'1.'+str(i)+'.0' for i in range(20)}:errors.append('VERSION_UNIVERSE')
...
def external_errors(bundle, manifest=None):
    ...
    if m.get('version')!=VERSION or m.get('bundle')!='docs/resolve-integration/v1.19':errors.append('MANIFEST_CURRENT_VERSION')
...
def expected_documents(bundle):
    ...
    b=pathlib.Path(bundle);parent=json.loads((b.parent/'v1.18/AUTHORITY-PRECEDENCE.json').read_text())
    expected={e['path']:(e['status'],e['role']) for e in parent['documents']}
    for name in expected:
        if name in ['CHANGELOG-v1.18.md','FINDING-RESOLUTION-MATRIX-v1.18.md','FINDING-RESOLUTION-MATRIX-v1.18.json']:
            expected[name]=('HISTORICAL','HISTORICAL_INPUT')
    for name in ['CHANGELOG-v1.19.md','FINDING-RESOLUTION-MATRIX-v1.19.md','FINDING-RESOLUTION-MATRIX-v1.19.json']:
