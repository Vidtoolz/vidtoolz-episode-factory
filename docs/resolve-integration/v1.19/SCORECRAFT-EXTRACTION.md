...
| `source_timeline_untouched` assertion | unsafe | REJECT as proof; replace by the derived S0→S1 delta over the protected surface (`verify_transaction`, committed only via `commit_eligibility_authorizing`) |
