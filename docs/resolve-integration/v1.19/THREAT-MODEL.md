...
| **Lower-level validator misuse (direct commit)** | `schema_validate=None` was the default for the composed path and for the then-authorizing `commit_eligibility` (F15-04; that function is PROVISIONAL_UNTIL_M3 non-authorizing since v1.15 V114-B2) | the schema validator is a **required** parameter of both authorizing entry points; a call without it refuses instead of reporting the schema stage complete; every per-document helper is `INTERNAL_NON_AUTHORIZING` and refuses partial input; the bypass audit checks names, docstrings, signatures, defaults and behaviour | a caller could still pass a permissive validator; the manifest pins the schemas the validator must load |
...
the PROVISIONAL_UNTIL_M3 non-authorizing `validate_transaction_set`, the M3 composed path. That path cannot yield M0A probe
...
## v1.15: what the authority actually consumes

v1.14's table is accurate about *where* the location check runs and silent about *what* the check protects. Codex
attacked the gap: it kept a legitimate governed document, its digest and its receipt exactly as minted, and replaced
only the parsed object the carrier was holding. Every control passed, and the authority derived readiness from
evidence that had never been at the governed location. In the sharpest form, a session that was honestly
`PROVISIONED_NOT_VERIFIED` returned **`ATTACHMENT_READY` under its own receipt and its own canonical document path**.
**A validated byte stream and a consumed object are two different things, and only the first was ever checked.**

| attack | v1.14 | v1.15 control | residual, stated |
|---|---|---|---|
| keep the receipt, replace the carrier's parsed evidence | **accepted; false ATTACHMENT_READY** | the carrier holds bytes, not a parsed object; it is frozen (`__setattr__`/`__delattr__` refuse); `evidence_set` is a read-only property that reparses those bytes on each access | `object.__setattr__` can still force a field — every such attempt is caught by the digest bindings below, which is a refusal, not an exclusion |
| mutate the dict the carrier handed you | **accepted** | there is no shared dict: each property read returns a fresh parse | none known |
| force bytes, digest, snapshot and receipt onto another session's carrier | not applicable | `governed_consume` re-reads the live file and requires the live digest to equal the minted digest **and** the carrier's own bytes to digest identically, then recomputes the receipt over all of it | an attacker who can also rewrite the governed file to match is an attacker with write access to governed evidence, which this model never claimed to stop |
| feed a diagnostic `eligible: true` into the commit path | **accepted; the now PROVISIONAL_UNTIL_M3 non-authorizing `commit_eligibility` returned eligible=true** | `commit_eligibility_authorizing` / `validate_transaction_set_authorizing` require a `GovernedEvidenceSet` and refuse `GOVERNED_EVIDENCE_REQUIRED` before any stage; plan-time eligibility is evaluated by the authorizing gate, chosen by type, never by a caller-supplied callback | the bare-dict pair remains callable for fixtures and is classified `PROVISIONAL_UNTIL_M3 / NON-AUTHORIZING`; its results carry no provenance and no authorizing flag |
| follow the active contract's own binding to pick a function | **it named the diagnostic one** | every current artifact designates only authorizing entry points, and a static audit scans every current artifact in both machine and prose form | a reviewer reading a HISTORICAL changelog will still see the old names; those documents are marked HISTORICAL in `AUTHORITY-PRECEDENCE.json` |
| redirect `WORKFLOW.json` to foreign bytes | **accepted; the loader minted provenance from them** | one no-follow file law for both authority files: lstat first, symlink refused, regular file required, `O_NOFOLLOW`+`O_NONBLOCK` open re-verified by `fstat` for device and inode; the workflow digest is bound into the receipt and re-checked on every call | as v1.13/v1.14: this detects replacement on the next authorizing call; it is not a TOCTOU guarantee for the filesystem beneath the checks |
| hang the authority with a FIFO at an authority path | **blocked forever on `open()`** | the entry is classified before anything is opened, so a FIFO is `WORKFLOW_NOT_A_REGULAR_FILE` and never opened; the suite bounds every entry-touching call with an alarm so "refuses" and "hangs" cannot be confused | a FIFO planted by an actor with write access to the governed directory is still that actor's write access |
| hand a malformed carrier to a public boundary | **raw `AttributeError`** | `governed_structure_errors` validates type and every field with defaults before any dereference; `object.__new__`, missing/None/wrong-typed slots, a subclass, copy, deepcopy and pickle all fail closed as `GOVERNED_EVIDENCE_INVALID` | deliberate, and narrow: only malformed *authority input* is normalised. A genuine internal programming error still raises, preserving the v1.10 distinction |

**What v1.15 does not claim.** No cryptographic location proof and no TOCTOU guarantee (both restated from v1.14).
No defence against an actor with write access to the governed evidence itself, to the bundle's own code, or to the
running interpreter — `object.__setattr__` and `ctypes` live inside that boundary, and the digest bindings are what
turn such tampering into a refusal rather than a bypass. No claim that the diagnostic or provisional functions are
unreachable: only that they are not authority, which all four surface classes now state in machine form. And no
claim that the authorizing transaction path has a *positive* control yet: there is no production path that authors
M3 write evidence, so `validate_transaction_set_authorizing` is proved fail-closed and proved to apply the ordinary
pre-M3 stage laws once provenance passes, but its first true positive belongs to the milestone that freezes write
evidence. That is stated here rather than left for a reviewer to discover.

## v1.16: what a consumer reads

v1.15 made the executable authority correct and left two frozen statements pointing the other way. Codex's finding
is not that the code can be tricked — it cannot, and the independent harness confirmed that — but that **authority
selection is itself an authorizing decision, and a consumer makes it by reading documents.** A conforming external
consumer that followed `TARGET-CONTRACT.json#attachment_law` would have called the diagnostic derivation; one that
followed `SCHEMA-REGISTRY.md:8-11` would have believed the authorizing transaction pair was non-authorizing; and one
that recomputed the location receipt from `EVIDENCE-SET-WORKFLOW.json` would have digested seven fields where the
authority digests ten, and disagreed with every receipt the authority mints.

| attack / failure | v1.15 | v1.16 control | residual, stated |
|---|---|---|---|
| follow a current contract field to pick the deriving function | **two fields named the diagnostic one, unmarked** | every field describing attachment/eligibility/gate authority names an authorizing entry point; the bare function appears only in a field whose own name and text declare it diagnostic | a reviewer reading a HISTORICAL changelog still sees old names; those documents are marked HISTORICAL in `AUTHORITY-PRECEDENCE.json` |
| believe a normative document's classification over the code | **`SCHEMA-REGISTRY.md` inverted it** | one canonical machine map, `AUTHORITY-FUNCTION-CLASSES.json`, **generated** from `AUTHORITY_SURFACE`; every current artifact is compared against it, and the comparison has no key-name exemption list | prose can still be written badly; what changed is that the check now reads every current field, so a new contradiction fails validation instead of passing it |
| recompute the receipt from published machine authority | **seven fields published, ten bound** | the publication is generated from `RECEIPT_BOUND_FIELDS`, which is the tuple `location_receipt()` itself assembles and digests; parity is asserted in both directions and by AST against the function body | a consumer that hard-codes a field list from an older bundle is outside this control; the field count and list are published in `M0A-BINDING-VALUES.json` for exactly that reason |
| drift the declared field set away from the digested one later | not detectable | `location_receipt()` refuses `RECEIPT_BINDING_DRIFT` if its assembled body and the declared tuple disagree, and the validator reads the digested names out of the function by AST | this is an internal-consistency guard, raised not normalised: it protects against a future edit, not against an attacker |
| hide a current contradiction behind precedence | possible in principle | the blanket re-reading clause applies to superseded statements only; every current artifact is asserted correct on its own | none known |

**What v1.16 does not claim.** It does not change any runtime law: not the consumed-evidence byte binding, not the
carrier model, not the governed root, not the file-entry law, not the malformed-carrier handling, not the transaction
provenance gate, not Store.v5. It does not claim that documentation can be made self-enforcing — only that every
current classification statement is now compared, exhaustively and by machine, against one generated source of truth,
and that the receipt a consumer can reconstruct from frozen machine authority is the receipt the authority mints. And
the residual v1.15 recorded still stands: the authorizing transaction path has no positive control until some
milestone freezes M3 write evidence.

## v1.17: what an audit can miss

v1.16 fixed the three contradictions Codex named and published a generated class map — and then missed an eleventh
authorizing slot in its own bundle. The reason is worth stating precisely, because it is a property of *checks*, not
of authority: the audit asked "does the sentence containing this reference contain any demotion phrase?", and the
sentence contained the words "no longer" — attached to *the bundle's binding*, not to the function's class. **A test
that reads a phrase instead of a claim will eventually read the wrong claim.** And the slot inventory was
hand-written, so a slot nobody thought to list could not be noticed at all.

| failure mode | v1.16 | v1.17 control | residual, stated |
|---|---|---|---|
| a current law names the diagnostic function as the refusing authority | **`toctou.law` did exactly that** | the TOCTOU law names `derive_attachment_state_authorizing` (session wrapper equivalent), and the diagnostic counterpart only in a clause that declares it diagnostic; semantics unchanged | prose can still be written badly — what changed is that a badly written designation now fails validation |
| an authority designation hides in a key nobody enumerated | **the ten-slot list could not see it** | slots are **discovered** from what values SAY: authoritative / deciding / refusing / enforcing / deriving / gating / validating, whatever the key is called; five unfamiliar key names are pinned as regressions | a designation phrased with a verb outside the frozen token list would be recorded as a MENTION; the token list is published in `AUTHORITY-FUNCTION-CLASSES.json#slot_discovery_law` so a reviewer can attack it directly |
| an unrelated negation suppresses a real finding | **"no longer" did** | no bare negation appears in any token list; classification is read only from the reference's own clause, and clauses break at commas at paren depth zero | a single clause that both declares a class and designates authority resolves as a declaration; that ordering is published as `signal_precedence` |
| line wrapping changes a classification | possible | wrapped markdown is rejoined into logical lines before clause splitting | table rows, list items and headings stay separate units by design |
| a reference form escapes the scan | dotted/module-qualified forms did | six forms normalise to one canonical `(module, name)`; a same-named function in another module is a different symbol | a form nobody anticipated would be missed; the accepted forms are published |
| generated evidence is mistaken for law | excluded by filename | scope is derived from precedence **status + published role**; `GENERATED_EVIDENCE` is out of scope because it quotes the suite's own checks, and that exclusion is published rather than hidden in a regex | a document mis-registered as `GENERATED_EVIDENCE` would escape the scan; every artifact's role is now in `AUTHORITY-PRECEDENCE.json` for review |

**What v1.17 does not claim.** It changes no runtime law at all — three runtime files are byte-identical to v1.16 —
so every threat statement above this section stands unaltered. It does not claim that documentation can be made
self-enforcing, only that a designation must now survive a clause-local, structured, module-aware check whose
vocabulary and scope are both published. And the v1.15 residual is unchanged and still open: the authorizing
transaction path has no positive control until some milestone freezes M3 write evidence.
