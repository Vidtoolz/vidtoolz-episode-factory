# Implementation lineage — Scorecraft extraction map v1.1 (FROZEN_NOW)
...
- **The production driver (`scripts/scorecraft-resolve-production-driver.py`) has no library/database guard.** Guards exist only in the hardlink **fixture** (`scorecraft-resolve-hardlink-fixture.py:72-73`: Disk + "Local Database" + empty library + `VIDTOOLZ_RESOLVE_HARDLINK_GATE_` prefix).
...
|---|---|---|
| explicit target contract shape (`normalizeTarget`) | QUALIFIED_PATTERN | PRESERVE as concept → `resolveTargetContract.v1.1` |
...
| database/library guard | missing in driver | REWRITE as mandatory in every path (`TARGET-CONTRACT` refusal) |
| readback | EXPERIMENTAL | REWRITE for `resolveSnapshot.v1.1` (all tracks, provenance variants, ids, no rounding, plural APIs) |
...
| `source_timeline_untouched` assertion | unsafe | REJECT as proof; replace by protected-state comparator |
