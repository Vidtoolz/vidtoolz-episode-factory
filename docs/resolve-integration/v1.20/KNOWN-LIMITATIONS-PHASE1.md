...
| F-120-09 | frozen VALIDATION-REPORT.md carried environment-bound values (temp-root path, raw inventory total_bytes), so validation rewrote a frozen file across scratch environments | CLOSED in the final repair: environment scrub (<TMPDIR>, <REPO>, <CWD>), outcome-only inventory detail, section `report-determinism`; byte-identical under short/medium/long roots |
