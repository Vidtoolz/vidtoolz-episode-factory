...
| F-120-04 | validator positive controls write transient self-test sessions under the shared governed evidence root; concurrent validators on one host interfere (false-positive leftover assertion; absent-root create/remove race) | DOCUMENTED operating constraint: one Resolve authority validator per host at a time (README.md, RELEASE-AUTHORITY.md); the testkit creates and removes only its own uniquely named session and refuses to adopt an existing one; root isolation or an advisory lock BEFORE any successor that changes the testkit |
...
The three P2 findings of the v1.20 independent review (F-120-01 parent-pin byte count, F-120-02 manifest rule version, F-120-03 stale inherited v1.19 matrix) are CLOSED at generator level in this repair candidate; see `FINDING-RESOLUTION-MATRIX-v1.20.md` and `CHANGELOG-v1.20.md`.
