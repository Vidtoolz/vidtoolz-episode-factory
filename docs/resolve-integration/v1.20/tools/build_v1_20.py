...
# v1.20 repair (F-120-03). The parent's finding matrices are INHERITED from the frozen parent bundle, never regenerated
# from embedded literals: e65ed5a4 dumped a stale v1.17 literal (one finding, V116-B1) under the v1.19 name, so the seven
# accepted v1.19 findings vanished from every machine-readable matrix. Each inherited copy is byte-asserted against the
# parent's FREEZE-MANIFEST entry, its finding ids are asserted, only `authority_version` is re-stamped with this bundle's
# version (the convention every inherited matrix in this bundle follows), and nothing else is touched.
PARENT_BUNDLE = os.path.join(os.path.dirname(B), "v1.19")
PARENT_MANIFEST_ENTRIES = {e["path"]: e for e in L.strict_load(os.path.join(PARENT_BUNDLE, "FREEZE-MANIFEST.json"))["files"]}


def inherit_parent_matrix(name, expected_ids):
    src = os.path.join(PARENT_BUNDLE, name)
    ent = PARENT_MANIFEST_ENTRIES[name]
    with open(src, "rb") as f:
        data = f.read()
    assert hashlib.sha256(data).hexdigest() == ent["sha256"] and len(data) == ent["bytes"], ("parent matrix differs from the frozen parent manifest", name)
    doc = L.strict_loads(data.decode("utf-8"))
    assert sorted(x["id"] for x in doc["findings"]) == sorted(expected_ids), (name, [x["id"] for x in doc["findings"]])
    doc["authority_version"] = L.AUTHORITY_VERSION
    dump(name, doc)


inherit_parent_matrix("FINDING-RESOLUTION-MATRIX-v1.19.json", ["V118-M1", "V118-M2", "V118-M3", "V118-M4", "V118-N1", "V118-N2", "V118-N3"])
inherit_parent_matrix("FINDING-RESOLUTION-MATRIX-v1.18.json", ["V117-I1", "V117-S1", "V117-S2", "V117-S3", "V117-S4", "V117-S5"])
inherit_parent_matrix("FINDING-RESOLUTION-MATRIX-v1.17.json", ["V116-B1"])
...
frm_schema = S("vidtoolz.resolveFindingResolutionMatrix.v1.10", "Finding resolution matrix v1.7 (Codex v1.6 adjudication)",
               {"schema": {"const": "vidtoolz.resolveFindingResolutionMatrix.v1.10"}, "authority_version": {"const": L.AUTHORITY_VERSION}, "adjudication": {"type": "string"}, "law": {"type": "string"}, "operational_closure": {"type": "string"},
                ...
                "findings": {"type": "array", "minItems": 1, "items": {"type": "object", "additionalProperties": False, "required": ["id", "severity", "codex_finding", "correction", "negative_fixture", "expected_layer", "validation_sections"],
                                                                       "properties": {"id": {"pattern": "^(C16-(B|M)[0-9]|ES-[0-9]|S19-[0-9]|S110-[0-9]|F110-[A-D]|V112-(RP1|[1-4])|V113-B1|V114-(B[1-4]|M1|N1)|V115-(B1|M1)|V116-B1|V117-(I1|S[1-5])|V118-(M[1-4]|N[1-3])|V119-F[1-5]|P1-(F(07|08|09|10|11|12)|R0[1-5])|F-120-0[1-9])$"}, "severity": {"type": "string", "pattern": "^(BLOCKER|MAJOR|MINOR|MERGE MAJOR|OPERATIONAL)( / M0A MAJOR)?( \\(.+\\))?$"}, "codex_finding": {"type": "string", "minLength": 20}, "v16_defect": {"type": "string", "minLength": 20}, "prior_defect": {"type": "string", "minLength": 20}, "correction": {"type": "string", "minLength": 20}, "negative_fixture": {"type": "string", "minLength": 20}, "expected_layer": {"type": "string", "minLength": 3}, "validation_sections": {"type": "array", "minItems": 1, "items": {"type": "string"}}}}}},
...
# v1.20 repair (F-120-06): PHASE1-SOURCE-PIN.json validates against a registered schema and is machine-verified against
# resolve-control/** by validate_v1_20.py section phase1-pin.
HEX8 = {"type": "string", "pattern": "^[0-9a-f]{8}$"}
HEX40 = {"type": "string", "pattern": "^[0-9a-f]{40}$"}
pin_schema = S("vidtoolz.resolveControlPlaneSourcePin.v1", "Resolve Control Plane Phase 1 source pin (v1.20)",
               {"schema": {"const": "vidtoolz.resolveControlPlaneSourcePin.v1"}, "authority_version": AV, "component": {"type": "string", "minLength": 1}, "protocol": {"const": "vrc.v1"},
                "implementation_commit": HEX8, "implementation_commit_full": HEX40, "git_tree_sha1_resolve_control": HEX40, "worker_sha256": SHA, "file_count": POSINT,
                "files": {"type": "array", "minItems": 1, "items": {"type": "object", "additionalProperties": False, "required": ["path", "sha256", "bytes"], "properties": {"path": {"type": "string", "pattern": "^resolve-control/[^/].*$"}, "sha256": SHA, "bytes": NONNEG}}},
                "law": {"type": "string", "minLength": 20},
                "governance": {"type": "object", "additionalProperties": False, "required": ["scope_adjudication_commit", "a3_provisioning_commit", "records_path", "acceptance_of_parent_v1_19"],
                               "properties": {"scope_adjudication_commit": HEX8, "a3_provisioning_commit": HEX8, "records_path": {"type": "string", "minLength": 1}, "acceptance_of_parent_v1_19": {"type": "string", "minLength": 1}}},
                "write_authority": {"const": "NONE"}, "hermes_facade": {"const": "ABSENT"}},
               ["schema", "authority_version", "component", "protocol", "implementation_commit", "implementation_commit_full", "git_tree_sha1_resolve_control", "worker_sha256", "file_count", "files", "law", "governance", "write_authority", "hermes_facade"])
dump("schemas/resolveControlPlaneSourcePin.schema.json", pin_schema)
