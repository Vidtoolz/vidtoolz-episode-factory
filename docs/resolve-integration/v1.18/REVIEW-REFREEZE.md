...
`active.capability_matrix_sha256` **is** `digest(matrix object, vidtoolz.resolveCapabilityMatrix.v1)`. Every authorizing entry point first checks that the matrix object it was handed is that content (`active_authority_errors` → `CAPABILITY_MATRIX_NOT_ACTIVE`). Passing an invented matrix — even a schema-valid one, even with plausible promotion metadata, even citing the digest of the real zero-qualified matrix — qualifies nothing: the digest does not match, and `primitive_status`, `callable_method_set`, `provenance_errors`, `evaluate_eligibility_authorizing` and `validate_transaction_set_authorizing` all refuse - as do their DIAGNOSTIC / PROVISIONAL_UNTIL_M3 counterparts (Codex F15-02).
...
## v1.7: the exact stored chain, and how multiplicity resolves (Codex C16-B4)

v1.6 recomputed the derived result and compared digests, which closes forgery of the value but not substitution of the
artifact. Three gaps are closed here; the normative law is `STORED-CHAIN.md`.

1. **A claimed derived artifact must exist.** If the chain names a `DERIVED_CAPABILITY_RESULT`, that exact stored
   record must resolve by digest **and** by record id, belong to the named raw capture, be bound to the active parser
   and primitive spec, and be byte-equal to the recomputation. A missing one is `DERIVED_ARTIFACT_MISSING` and is
   never replaced by a fresh recomputation, because the review was written about the artifact, not about the
   arithmetic.
2. **A review is bound to one derived artifact.** It must reference the exact stored derived digest and raw digest.
   Review A cannot serve derived B even when both recompute to SUCCESS.
3. **Multiplicity is a refusal, not a selection.** Among the current candidate reviews, or the current reviewed
   refreeze records, exactly one must survive. A record retires another only by naming its record id in `supersedes`.
   Zero is `_NONE`, more than one is `_CONFLICT`, and a supersession cycle is `_CONFLICT`. Insertion order, array
   order, map order and record age never decide anything.

A promoted matrix row now binds the whole chain: the raw capture, the stored derived record, the review, the refreeze
**block** digest, the probe and session, the trusted shim identity, the reference parser and the primitive spec. The
row binds the refreeze block rather than the refreeze record's id because that record's id depends on the successor
matrix digest, which depends on the rows; binding the block is the same guarantee without the cycle.
