...
VERSION = '1.18.0'
PARENT_HEAD = '24ead748eec4bce46dc2e945d24ad403fc9472b3'
PARENT_MANIFEST = '5dff9c98f1086ea6a4edfab0073770597554ab338e276ed88125e94796eb91af'
...
def registration_errors(text, version=VERSION):
    ...
    if {v for v,s in rows}!={'1.'+str(i)+'.0' for i in range(19)}:errors.append('VERSION_UNIVERSE')
...
def external_errors(bundle, manifest=None):
    ...
    if m.get('version')!=VERSION or m.get('bundle')!='docs/resolve-integration/v1.18':errors.append('MANIFEST_CURRENT_VERSION')
...
def expected_documents(bundle):
    ...
    b=pathlib.Path(bundle);parent=json.loads((b.parent/'v1.17/AUTHORITY-PRECEDENCE.json').read_text())
    expected={e['path']:(e['status'],e['role']) for e in parent['documents']}
    for name in expected:
        if name in ['CHANGELOG-v1.17.md','FINDING-RESOLUTION-MATRIX-v1.17.md','FINDING-RESOLUTION-MATRIX-v1.17.json']:
            expected[name]=('HISTORICAL','HISTORICAL_INPUT')
    for name in ['CHANGELOG-v1.18.md','FINDING-RESOLUTION-MATRIX-v1.18.md','FINDING-RESOLUTION-MATRIX-v1.18.json']:
