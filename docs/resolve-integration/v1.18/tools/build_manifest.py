#!/usr/bin/env python3
"""Build the v1.18 manifest with final external bytes and exact v1.17 lineage.
Run after validate_v1_18.py (so VALIDATION-REPORT.md is current): python3 -B tools/build_manifest.py
...
PARENT_HEAD = "a03923cd4b09e8e11d40e8cf13819f8df763063a"            # inherited v1.16 lineage input; final v1.17 parent is applied below
...
NOTES = {
    ...
    "fixtures/evidence/ACTIVE-AUTHORITY-PLACEHOLDER.json": ("fixture evidence is minted against this PLACEHOLDER manifest sha; validate_v1_18.py proves placeholder-bound records do not derive against the real FREEZE-MANIFEST.json sha and that re-minted records do", None),
...
m = {"schema": "vidtoolz.resolveFreezeManifest.v1.10", "bundle": "docs/resolve-integration/v1.18", "version": "1.18.0", "frozen_at": "2026-09-10", "prepared_by": "Claude Code (Opus 5)", "approval_status": "CANDIDATE_FOR_INDEPENDENT_REVIEW", "approver": None,
     ...
     "rules": ["a file whose sha256 or byte count differs from this manifest is not the frozen version", "v1.0 through v1.17 bytes and manifests are never modified; v1.18 supersedes by reference", "SCHEMA-VALID != AUTHORIZED TO MUTATE; PERMISSION DECLARATION != ELIGIBILITY; PROBE_ALLOWED != QUALIFIED_READ; A CAPTURE != EVIDENCE THAT SOMETHING WORKS; attachment state is derived from a coherent, current, envelope-bound evidence set against the ACTIVE reviewed manifest, never declared; contradictory evidence is CONFLICT", "a BUNDLE_VERIFICATION, MILESTONE_EXIT, M3_AUTHORIZATION or REFREEZE_RECORD counts only when bound to this manifest's sha256 and authority version 1.18.0; records for other manifests are historical", "mutation-capable schemas remain PROVISIONAL_UNTIL_M3; verification is derived from S0/S1 over the protected surface (verify_transaction) and never declared; commit_eligibility_authorizing -> validate_transaction_set_authorizing over a GovernedEvidenceSet is the ONLY authorizing commit path (v1.15 V114-B2; the bare-dict commit_eligibility/validate_transaction_set pair is PROVISIONAL_UNTIL_M3 and NON-AUTHORIZING, and every per-document helper is INTERNAL_NON_AUTHORIZING); capability meaning is DERIVED by the versioned reference parser from an immutable facts-only raw capture (never from a probe-written flag), the ACTIVE capability authority is the matrix's own content digest, a degraded H0 can never be a write precondition, and schema enforcement in the composed path is mandatory; no adapter mutation implementation exists; no Resolve write was executed; M0 has not begun",
...
# v1.18 release lineage: inherited ancestors are immutable; current parent is exact v1.17.
_p17 = L.strict_load(os.path.join(os.path.dirname(B), 'v1.17', 'FREEZE-MANIFEST.json'))
_out = os.path.join(B, 'FREEZE-MANIFEST.json')
_m18 = m
_m18['parent'] = dict(_p17['parent'])
_m18['parent'].update(version='1.17.0', branch='docs/resolve-authority-freeze-v1.17', head='24ead748eec4bce46dc2e945d24ad403fc9472b3', semantic_head='24ead748eec4bce46dc2e945d24ad403fc9472b3', manifest_sha256='5dff9c98f1086ea6a4edfab0073770597554ab338e276ed88125e94796eb91af', immutable=True)
...
_m18['external_pins']['v1_17_parent_manifest']={'path':'../v1.17/FREEZE-MANIFEST.json','sha256':'5dff9c98f1086ea6a4edfab0073770597554ab338e276ed88125e94796eb91af','bytes':os.path.getsize(os.path.join(os.path.dirname(B),'v1.17/FREEZE-MANIFEST.json'))}
...
print('FINAL_V118_MANIFEST_SHA256',sha(_out))
