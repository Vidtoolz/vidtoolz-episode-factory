...
Run from the bundle dir: python3 -B tools/build_v1_18.py
...
tc["schema"] = "vidtoolz.resolveTargetContract.v1.10"; tc["version"] = "1.18.0"
...
tcs["properties"]["version"] = {"type": "string", "pattern": "^1\\.18\\.[0-9]+$"}
...
rp["schema"] = "vidtoolz.resolveReadPrimitives.v1.10"; rp["version"] = "1.18.0"
...
caps["schema"] = "vidtoolz.resolveCapabilityMatrix.v1.10"; caps["version"] = "1.18.0"
...
caps_hyp = copy.deepcopy(caps); caps_hyp["version"] = "1.18.0-HYPOTHETICAL"; caps_hyp["qualification_note"] = "HYPOTHETICAL_NOT_AUTHORITY: illustrates a future reviewed refreeze (rows QUALIFIED_READ citing raw/derived/review digests of fixture captures); never load as CAPABILITIES.json"
...
policy = {"target_contract_sha256": sha_of("TARGET-CONTRACT.json"), "timebase_sha256": sha_of("TIMEBASE.json"), "track_policy_sha256": sha_of("schemas/resolveTrackPolicy.v1.json"), "capabilities_version": "1.18.0", "capability_matrix_sha256": CAPS_SHA, "collector_version": "0.0.0-fixture", "canonicalization_version": "1.5"}
policy_hyp = dict(policy, capabilities_version="1.18.0-HYPOTHETICAL", capability_matrix_sha256=HYP_SHA)
...
perms["schema"] = "vidtoolz.resolvePermissions.v1.10"; perms["version"] = "1.18.0"
...
matrix = load("MILESTONE-MATRIX.json"); matrix["schema"] = "vidtoolz.resolveMilestoneMatrix.v1.10"; matrix["version"] = "1.18.0"; dump("MILESTONE-MATRIX.json", matrix)
...
h0_other_matrix = copy.deepcopy(s0); h0_other_matrix["policy"] = dict(policy_hyp, capability_matrix_sha256=CAPS_SHA, capabilities_version="1.18.0"); resign(h0_other_matrix)
...
p = copy.deepcopy(plan_m3); p["authority_version"] = "1.4.0"; fixture("plan-authority-version-old", "schema", "provisional/resolveMutationPlan", p, expect_contains="1.18.0")
...
man_schema["properties"]["schema"] = {"const": "vidtoolz.resolveFreezeManifest.v1.10"}; man_schema["properties"]["bundle"] = {"const": "docs/resolve-integration/v1.18"}; man_schema["properties"]["version"] = {"const": "1.18.0"}
man_schema["properties"]["parent"] = {"type": "object", "additionalProperties": False, "required": ["version", "branch", "head", "semantic_head", "manifest_sha256", "immutable", "ancestors"], "properties": {
    "version": {"const": "1.17.0"}, "branch": {"const": "docs/resolve-authority-freeze-v1.17"}, "head": {"const": "24ead748eec4bce46dc2e945d24ad403fc9472b3"}, "semantic_head": {"const": "24ead748eec4bce46dc2e945d24ad403fc9472b3"}, "manifest_sha256": {"const": "5dff9c98f1086ea6a4edfab0073770597554ab338e276ed88125e94796eb91af"}, "immutable": {"const": True},
    "ancestors": {"type": "array", "minItems": 17, "items": {"type": "object", "additionalProperties": False, "required": ["version", "branch", "head", "manifest_sha256"], "properties": {"version": {"type": "string"}, "branch": {"type": "string"}, "head": {"type": "string", "pattern": "^[0-9a-f]{40}$"}, "manifest_sha256": SHA, "baseline": {"type": "string"}}}}}}
...
CODEX_FINDINGS = [
    {"id": "C16-B1", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_18.py section cache-mutation: nine in-place mutations of a qualified chain (raw->RAISED, primitive spec, review->REJECT, refreeze un-reviewed, matrix row demoted, stored derived edited, host_name, build, matrix evidence reference), each asserting warm == cold and NOT QUALIFIED, plus parser replacement, callable-set mutation and cross-evidence-set contamination.",
    ...
    {"id": "C16-B2", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_18.py section validator-injection: five injected validators x (positional, keyword) x both entry points, five validator-named keywords, the diagnostic surface, and one changed byte of one pinned schema file making the whole registry untrusted.",
    ...
    {"id": "C16-B3", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_18.py section shim-trust: wrong sha, stale build, unknown shim, same version with changed code, other codec, other raw schema, a method outside the trusted allowlist, an active authority missing or misnaming the trusted shim, and a whole chain over a wrong-shim capture.",
    ...
    {"id": "C16-M1", "severity": "MAJOR",
     ...
     "negative_fixture": "validate_v1_18.py section identity-duplicates: duplicate path, path with two handle tokens, path with two identity values, duplicate attempt id, two paths returning one id, and a pass-set mismatch.",
    ...
    {"id": "C16-M2", "severity": "MAJOR",
     ...
     "negative_fixture": "validate_v1_18.py section raw-ingestion: thirteen malformed frames each asserting its closed RAW_INGEST_CODES refusal, plus records with a missing, mis-bound or non-strict receipt.",
    ...
    {"id": "C16-M3", "severity": "MAJOR",
     ...
     "negative_fixture": "validate_v1_18.py section evidence-store: session re-creation, caller-declared digest mismatch, attempt-id reuse, path traversal, unknown layer, symlinked session, write after finalization, re-finalization, replaced record bytes, tampered HASHES, removed record, unfinalized session, partially written record, mismatched pinned authority, and a static audit that no code path opens a record for writing, truncates, renames over one or unlinks one.",
...
S110_FINDINGS = [
    {"id": "S110-1", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_18.py section evidence-boundary-receipt: session directory replaced by another with the same basename, same-basename session under an alternate root, boundary receipt mutation and deletion, root replaced by symlink and by another directory, plus randomized boundary-replacement property tests.",
    ...
    {"id": "S110-2", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_18.py sections evidence-recomputed-model and evidence-finalization-fields: the seven-field finalization attack matrix, inventory header mutation, directory duplicate and conflicting-kind cases, and the provenance table assertion that no normative field is trusted from the inventory itself.",
    ...
    {"id": "S110-3", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_18.py section evidence-attempt-derivation: foreign marker planted while ACTIVE, duplicate attempt id, orphaned record, marker field mutation property tests, and the cross-session ACTIVE insertion matrix.",
    ...
    {"id": "S110-4", "severity": "MAJOR",
     ...
     "negative_fixture": "validate_v1_18.py section evidence-mode-authority: ACTIVE chmod of session directory, layer directory, manifest, record, attempt marker and journal each blocking the next canonical operation; the finalized chmod matrix retained; randomized permission-bit property tests in both states.",
    ...
    {"id": "S110-5", "severity": "MINOR",
     ...
     "negative_fixture": "validate_v1_18.py section evidence-fs-errors: missing path, permission denied, symlink loop, wrong file type, name too long, cross-device, and concurrent disappearance, each asserting a frozen class and no raw OSError.",
...
S19_FINDINGS = [
    {"id": "S19-1", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_18.py section evidence-root-trust: symlinked evidence root and symlinked session root refused at create, open, finalize and verify; root replaced by a symlink and by another directory between operations; non-directory root.",
    ...
    {"id": "S19-2", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_18.py sections evidence-session-identity and evidence-semantic-inventory: renamed session, cross-session copies of the whole session, the inventory alone, the marker alone, one record and the manifest; wrong layer; wrong logical identity; tampered internal session id.",
    ...
    {"id": "S19-3", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_18.py section evidence-attempt-tuple: the full collision table plus RAW attempt reused in DERIVED, DERIVED reused in HUMAN_REVIEW, one digest under two logical identities, one record's bytes in two layers, one attempt id across two sessions, and duplicate attempt keys in an inventory.",
    ...
    {"id": "S19-4", "severity": "MAJOR",
     ...
     "negative_fixture": "validate_v1_18.py section evidence-mode-authority: chmod of a record, a layer directory, the session manifest, the inventory and the finalization marker; a regular file replaced by another filesystem type.",
...
ES_FINDINGS = [
    ...
    {"id": "ES-2", "severity": "BLOCKER",
     ...
     "negative_fixture": "validate_v1_18.py sections evidence-single-authority (static audit: exactly one EVIDENCE_STORE_AUTHORIZING module, EvidenceRoot absent, no storage primitives in the shim) and evidence-traversal (the exact legacy attack plus twelve traversal, absolute-path, separator, symlink, dot, empty and over-long identifier shapes against every write API, each proving no file appears outside the session root).",
...
V112_FINDINGS = [
    {"id": "V112-1", "severity": "BLOCKER (AUTHORITY_UNDERSPECIFIED)",
     ...
     "negative_fixture": "validate_v1_18.py section workflow-persistence: forbidden-location and absoluteness assertions, seven path-escape session ids, a copied document refused SET_LOCATION_MISMATCH, a held lock refusing a second writer LOCK_HELD, lock release, SET_EXISTS and SET_NOT_FOUND.",
    ...
    {"id": "V112-4", "severity": "BLOCKER (AUTHORITY_UNDERSPECIFIED)",
     ...
     "negative_fixture": "validate_v1_18.py section workflow-e2e-positive: the MANDATORY end-to-end runtime path using only production tools derives ATTACHMENT_READY, plus the same path driven through both CLIs; section workflow-e2e-negative: 28 cases, none of which may derive ATTACHMENT_READY; section workflow-parity: the production record field sets are asserted identical to the fixture field sets for all four record types, so no positive state is reachable from a shape production cannot build.",
...
V115_FINDINGS = [
    ...
    {"id": "V114-M1", "severity": "MERGE MAJOR (malformed nominal carrier leaking a raw exception)",
     ...
     "negative_fixture": "validate_v1_18.py section m1-malformed-carrier: a ten-case matrix run against all THREE public authorizing boundaries (attachment derivation, eligibility, commit eligibility) requiring a frozen refusal and no raw exception from each; the exact Codex reproduction pinned separately with its GOVERNED_EVIDENCE_INVALID code; the structure-validator-runs-first source assertion; the programming-error distinction; and the frozen-vocabulary membership of all four new codes; section v115-property: seeded malformed-slot and wrong-type carriers.",
...
dump("FINDING-RESOLUTION-MATRIX-v1.18.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.10", "authority_version": L.AUTHORITY_VERSION,
...
dump("FINDING-RESOLUTION-MATRIX-v1.15.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.10", "authority_version": L.AUTHORITY_VERSION,
                                              ...
                                              "law": "each of the six findings names the v1.14 defect as reproduced against the frozen v1.14 bundle at b61f248238ec7ed60deca79aa9728ec0cde86edd, its root cause, the v1.15 correction, the negative and property fixtures, and the validate_v1_18.py sections that prove it. validate_v1_18.py refuses to pass unless every named section exists and every check in it passes. The essential v1.15 law is THE EXACT BYTES VALIDATED AS GOVERNED EVIDENCE ARE THE EXACT BYTES CONSUMED BY AUTHORIZING DERIVATION.",
...
dump("FINDING-RESOLUTION-MATRIX-v1.14.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.10", "authority_version": L.AUTHORITY_VERSION,
                                              ...
                                              "law": "the single finding names the v1.13 defect as reproduced against the frozen v1.13 bundle at 65895c6dedceadff012a08c1bafcdf9e92b65221, its root cause, the v1.14 correction, the negative and property fixtures, and the validate_v1_18.py sections that prove it. validate_v1_18.py refuses to pass unless every named section exists and every check in it passes.",
...
dump("FINDING-RESOLUTION-MATRIX-v1.13.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.10", "authority_version": L.AUTHORITY_VERSION,
                                              ...
                                              "law": "the single finding names the v1.12 defect as reproduced against the frozen v1.12 bundle at 73150d00a72820b155caf357c412f46b8ec4a9ba, its root cause, the v1.13 correction, the negative and property fixtures, and the validate_v1_18.py sections that prove it. validate_v1_18.py refuses to pass unless every named section exists and every check in it passes.",
...
dump("FINDING-RESOLUTION-MATRIX-v1.12.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.10", "authority_version": L.AUTHORITY_VERSION,
                                              ...
                                              "law": "each finding names the v1.11 gap, the v1.12 correction, at least one negative fixture, the layer the attack must fail at, and the validate_v1_18.py sections that prove it. validate_v1_18.py refuses to pass unless every named section exists and every check in it passes.",
...
dump("FINDING-RESOLUTION-MATRIX-v1.10.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.10", "authority_version": L.AUTHORITY_VERSION,
                                             ...
                                             "law": "each finding names the v1.9 defect, the v1.10 correction, at least one negative fixture, the layer the attack must fail at, and the validate_v1_18.py sections that prove it. validate_v1_18.py refuses to pass unless every named section exists and every check in it passes.",
...
dump("FINDING-RESOLUTION-MATRIX-v1.9.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.10", "authority_version": L.AUTHORITY_VERSION,
                                             ...
                                             "law": "each finding names the v1.8 defect, the v1.9 correction, at least one negative fixture, the layer the attack must fail at, and the validate_v1_18.py sections that prove it. validate_v1_18.py refuses to pass unless every named section exists and every check in it passes.",
...
dump("FINDING-RESOLUTION-MATRIX-v1.8.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.10", "authority_version": L.AUTHORITY_VERSION,
                                             ...
                                             "law": "each finding names the v1.7 defect, the v1.8 correction, at least one negative fixture, the layer the attack must fail at, and the validate_v1_18.py sections that prove it. validate_v1_18.py refuses to pass unless every named section exists and every check in it passes.",
...
dump("FINDING-RESOLUTION-MATRIX-v1.7.json", {"schema": "vidtoolz.resolveFindingResolutionMatrix.v1.10", "authority_version": L.AUTHORITY_VERSION,
                                             ...
                                             "law": "each finding names the v1.6 defect, the v1.7 correction, at least one negative fixture, the layer the attack must fail at, and the validate_v1_18.py sections that prove it. validate_v1_18.py refuses to pass unless every named section exists and every check in it passes.",
                                             "operational_closure": "C16-OP1 (Hermes M0A cannot bind to v1.6 because the trusted shim, parser, spec and raw identities are not operationally closed) is closed by M0A-BINDING-VALUES.json, which publishes every value with no placeholder and is verified against the running code by validate_v1_18.py section m0a-binding.",
...
frm_schema = S("vidtoolz.resolveFindingResolutionMatrix.v1.10", "Finding resolution matrix v1.7 (Codex v1.6 adjudication)",
               {"schema": {"const": "vidtoolz.resolveFindingResolutionMatrix.v1.10"}, "authority_version": {"const": L.AUTHORITY_VERSION}, "adjudication": {"type": "string"}, "law": {"type": "string"}, "operational_closure": {"type": "string"},
                ...
                "findings": {"type": "array", "minItems": 2, "items": {"type": "object", "additionalProperties": False, "required": ["id", "severity", "codex_finding", "correction", "negative_fixture", "expected_layer", "validation_sections"],
                                                                       "properties": {"id": {"pattern": "^(C16-(B|M)[0-9]|ES-[0-9]|S19-[0-9]|S110-[0-9]|V117-(I1|S[1-5]))$"}, "severity": {"enum": ["BLOCKER", "MAJOR", "MERGE MAJOR", "OPERATIONAL"]}, "codex_finding": {"type": "string", "minLength": 20}, "v16_defect": {"type": "string", "minLength": 20}, "prior_defect": {"type": "string", "minLength": 20}, "correction": {"type": "string", "minLength": 20}, "negative_fixture": {"type": "string", "minLength": 20}, "expected_layer": {"type": "string", "minLength": 3}, "validation_sections": {"type": "array", "minItems": 1, "items": {"type": "string"}}}}}},
...
dump("PRINCIPALS.json", {
    ...
    "role_separation": "no ACTOR may appear under two roles; authority_lib.role_separation_errors() is asserted empty by validate_v1_18.py",
...
# fact rather than restating a claim, and validate_v1_18.py compares all of them against it exhaustively - no
...
dump("AUTHORITY-FUNCTION-CLASSES.json", {
    ...
    # v1.18 (V116-B1): the hardcoded ten-slot and four-slot dictionaries are GONE. Codex found an eleventh
    ...
    # validate_v1_18.py through tools/authority_slots.py, from the current artifacts themselves.
    "slot_discovery_law": {
...
dump("EVIDENCE-SET-WORKFLOW.json", {
    ...
    "core_authority": {
        ...
        # semantics, and validate_v1_18.py compares the two sets in both directions.
...
PROVENANCE = {
    ...
    "authority_version": L.AUTHORITY_VERSION,
    "law": "every NORMATIVE inventory, boundary and finalization field is classified by the source the verifier derives it from. No normative field may be classified TRUSTED_FROM_INVENTORY_ITSELF: the stored inventory is evidence that is compared against an independently recomputed model, never the model. validate_v1_18.py asserts this table is complete, that no normative field is self-asserted, and (v1.11) that every field the table classifies as reconciled is ACTUALLY compared by tools/evidence_store.py against a source outside the document it appears in.",
...
dump("M0A-BINDING-VALUES.json", {
    ...
    "authority_version": L.AUTHORITY_VERSION,
    "purpose": "The exact identities an M0A probe package must record and be bound to under authority v1.7. Codex's v1.6 operational finding was that Hermes could not bind its M0A package because these values were not operationally closed. Every value here is the value the running code produces; validate_v1_18.py section m0a-binding re-derives each one and fails if any differs or reads as a placeholder.",
