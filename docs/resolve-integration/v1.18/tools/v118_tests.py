...
def independent_references(bundle):
    ...
    names=set(S.L.CLASSIFIED_DECISION_FUNCTIONS)
    def walk(v,path=''):
        if isinstance(v,dict):
            for k,x in v.items():yield from walk(x,path+'.'+k if path else k)
...
def run(bundle,rec):
    ...
    def check(id,ok,detail=''):rec('v118-release-scanner',id,bool(ok),detail)
    ...
    finding_map=json.loads((b/'FINDING-RESOLUTION-MATRIX-v1.18.json').read_text())
    matrix_errors=S.L.internal_schema_errors('resolveFindingResolutionMatrix',finding_map)
    check('active-finding-map-schema',not matrix_errors and {r['id'] for r in finding_map['findings']}=={'V117-I1'}|{'V117-S'+str(i) for i in range(1,6)},matrix_errors)
    sel=RELEASE.MARKER.search(doc)
    for id,mutant in [
        ('zero-current',doc[:sel.start()]+doc[sel.end():]),
        ('duplicate-selector',doc+'\n'+sel[0]),
        ('old-current',doc.replace('"version":"1.18.0"','"version":"1.13.0"')),
        ('missing-registration',doc.replace('**v1.18 registration** (current):','')),
    ...
    manifest=json.loads((b/'FREEZE-MANIFEST.json').read_text());parent=json.loads((b.parent/'v1.17/FREEZE-MANIFEST.json').read_text())
    ...
    check('independent-role-equality',all(indroles.get((r['artifact'],r['location'],r['module'],r['function'],r['occurrence']))==r['role'] for r in actual['references']))
    indslots={(r['artifact'],r['location']) for r in independent if r['authority_role']==S.ROLE_AUTHORIZING}
    check('independent-slot-equality',indslots=={(r['artifact'],r['location']) for r in inv['authorizing_slots']})
    ...
    with tempfile.TemporaryDirectory(prefix='v118-publication-tests-') as td:
        t=pathlib.Path(td);dest=t/'docs/resolve-integration/v1.18';dest.mkdir(parents=True)
        # Parent is read-only source input; copy only the registration needed by independent universe derivation.
        (dest.parent/'v1.17').mkdir();shutil.copyfile(b.parent/'v1.17/AUTHORITY-PRECEDENCE.json',dest.parent/'v1.17/AUTHORITY-PRECEDENCE.json')
        for f in b.iterdir():
    ...
    for name in ['evidence_authoring.py','a2_prepare.py','a2_verify.py','evidence_store.py']:
        check('runtime-byte-'+name,(b/'tools'/name).read_bytes()==(b.parent/'v1.17/tools'/name).read_bytes())
    def semantics(path):
    ...
    check('runtime-authority-ast',semantics(b/'tools/authority_lib.py')==semantics(b.parent/'v1.17/tools/authority_lib.py'))
    check('receipt-ten-fields',len(S.L.RECEIPT_BOUND_FIELDS)==10)
    for v in ['v1']+['v1.'+str(i) for i in range(1,18)]:
