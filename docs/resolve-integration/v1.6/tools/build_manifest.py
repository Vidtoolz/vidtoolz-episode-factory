#!/usr/bin/env python3
"""Build FREEZE-MANIFEST.json for bundle v1.6 with machine-readable status, authority class, byte counts and lineage flags vs v1.5.
Run after validate_v1_6.py (so VALIDATION-REPORT.md is current): python3 -B tools/build_manifest.py
...
PARENT_HEAD = "9d944e01358b4b88d6ee4c9993a4db4084ba23ad"
PARENT_MANIFEST = "a40c6954fbe7f72d48eaf3957c0ac036c471a7a92496e277660530386e7aba5a"
...
parent = L.strict_load(os.path.join(V15, "FREEZE-MANIFEST.json"))
...
def authority_class(p):
    ...
    if p.startswith("tools/"):
        return "TOOL"
    if p in ("ADJUDICATION-FREEZE-CONTRACT.md", "AUTHORITY-ARCHITECTURE-ADJUDICATION-M0-M3.md", "CHANGELOG-v1.1.md", "FINDING-RESOLUTION-MATRIX.md", "CHANGELOG-v1.2.md", "FINDING-RESOLUTION-MATRIX-v1.2.md", "CHANGELOG-v1.3.md", "FINDING-RESOLUTION-MATRIX-v1.3.md", "CHANGELOG-v1.4.md", "FINDING-RESOLUTION-MATRIX-v1.4.md", "CHANGELOG-v1.5.md", "FINDING-RESOLUTION-MATRIX-v1.5.md"):
...
for p in files:
    ...
    if st == "PROVISIONAL_UNTIL_M3":
        e["blocked_until"] = "M3"
    entries.append(e)
ext = {"v1_5_parent_manifest": os.path.join(V15, "FREEZE-MANIFEST.json"), "v1_4_manifest": os.path.join(V14, "FREEZE-MANIFEST.json"), "v1_3_manifest": os.path.join(V13, "FREEZE-MANIFEST.json"), "v1_2_manifest": os.path.join(V12, "FREEZE-MANIFEST.json"), "v1_1_manifest": os.path.join(V11, "FREEZE-MANIFEST.json"), "v1_0_manifest": os.path.join(V10, "FREEZE-MANIFEST.json"), "doc_authority": os.path.join(os.path.dirname(os.path.dirname(B)), "DOC-AUTHORITY.md")}
exth = {k: {"path": os.path.relpath(v, B), "sha256": sha(v), "bytes": os.path.getsize(v)} for k, v in ext.items()}
assert exth["v1_5_parent_manifest"]["sha256"] == PARENT_MANIFEST, exth["v1_5_parent_manifest"]
...
m = {"schema": "vidtoolz.resolveFreezeManifest.v1.6", "bundle": "docs/resolve-integration/v1.6", "version": "1.6.0", "frozen_at": "2026-09-09", "prepared_by": "Claude Code (Fable 5.1)", "approval_status": "CANDIDATE_FOR_INDEPENDENT_REVIEW", "approver": None,
     "parent": {"version": "1.5.0", "branch": "docs/resolve-authority-freeze-v1.5", "head": PARENT_HEAD, "manifest_sha256": PARENT_MANIFEST, "immutable": True,
                "ancestors": [
     ...
     "rules": ["a file whose sha256 or byte count differs from this manifest is not the frozen version", "v1.0, v1.1, v1.2, v1.3, v1.4 and v1.5 bytes and manifests are never modified; v1.6 supersedes by reference", "SCHEMA-VALID != AUTHORIZED TO MUTATE; PERMISSION DECLARATION != ELIGIBILITY; PROBE_ALLOWED != QUALIFIED_READ; A CAPTURE != EVIDENCE THAT SOMETHING WORKS; attachment state is derived from a coherent, current, envelope-bound evidence set against the ACTIVE reviewed manifest, never declared; contradictory evidence is CONFLICT", "a BUNDLE_VERIFICATION, MILESTONE_EXIT, M3_AUTHORIZATION or REFREEZE_RECORD counts only when bound to this manifest's sha256 and authority version 1.6.0; records for other manifests are historical", "mutation-capable schemas remain PROVISIONAL_UNTIL_M3; verification is derived from S0/S1 over the protected surface (verify_transaction) and never declared; commit_eligibility -> validate_transaction_set is the ONLY authorizing commit path (every per-document helper is INTERNAL_NON_AUTHORIZING); capability meaning is DERIVED by the versioned reference parser from an immutable facts-only raw capture (never from a probe-written flag), the ACTIVE capability authority is the matrix's own content digest, a degraded H0 can never be a write precondition, and schema enforcement in the composed path is mandatory; no adapter mutation implementation exists; no Resolve write was executed; M0 has not begun", "PERMISSIONS.json is default DENY; unknown milestone/operation/scope/prerequisite is denied; shared_library_allowed is never true; every entry declares a target_requirement", "nothing in this bundle is human approval of any run, gate or publication"],
