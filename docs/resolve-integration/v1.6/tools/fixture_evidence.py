...
class Mint:
    ...
    def raw_rec(self, capture, session=None, seq=5, build=None, resolve_version=None):
        ...
        return self.R("RAW_CAPABILITY_CAPTURE", env=env, capture=capture, raw_capture_sha256=capture["raw_digest"])
    ...
    def review(self, capture, spec, decision="ACCEPT", reviewer=REVIEWER, rationale="raw capture inspected; derived result agrees with documented expectation", derived_sha=None, raw_sha=None, parser_version=None, parser_sha=None, spec_sha=None, method=None, receiver_class=None, probe_id=None, session_id=None):
        d = L.derive_capability_result(capture, spec, ENV, {"authority_version": self.AV, "manifest_sha256": self.M})
        return self.R("REVIEW_DECISION", raw_capture_sha256=raw_sha or capture["raw_digest"], derived_result_sha256=derived_sha or d["derived_result_sha256"], parser_version=parser_version or L.PARSER_VERSION, parser_sha256=parser_sha or L.parser_sha256(), primitive_spec_sha256=spec_sha or L.primitive_spec_digest(self.rp), method=method or capture["method"], receiver_class=receiver_class or capture["receiver"]["class"], probe_id=probe_id or capture["probe_id"], session_id=session_id or capture["session_id"], reviewer=reviewer, decision=decision, rationale=rationale, reviewed_at=ts(5))
...
def ES(records, current_session=S_CUR, evaluated_at=EVALUATED_AT):
    return {"schema": "vidtoolz.resolveEvidenceSet.v1.6", "current_session_id": current_session, "evaluated_at": evaluated_at, "records": {r["record_id"]: r for r in records}}
...
def base_sets(m, probe_methods, caps_hyp, parent_caps_sha):
    ...
    def variant(behaviour, decision="ACCEPT", **capkw):
        ...
        recs = [m.raw_rec(cs[x]) for x in probe_methods] + [m.review(cs[x], specs[x], decision=decision) for x in probe_methods]
        return cs, recs
    ...
    caps_timeout_accept = [m.raw_rec(slow_c["GetVersionString"]), m.review(slow_c["GetVersionString"], specs["GetVersionString"])] + [r for r in caps_ok if not (r["record_type"] in ("RAW_CAPABILITY_CAPTURE", "REVIEW_DECISION") and (r.get("capture") or r).get("method", r.get("method")) == "GetVersionString")]
    ...
    caps_raised_fake_parse = [m.raw_rec(reseal(raised_c[x], **fake_parse)) for x in probe_methods] + [m.review(reseal(raised_c[x], **fake_parse), specs[x]) for x in probe_methods] + [refz_ok]
    ...
    caps_wrong_build = [m.raw_rec(wb_c[x], session=S_OLD, build=7, resolve_version="21.0.3.0007") for x in probe_methods] + [m.review(wb_c[x], specs[x]) for x in probe_methods] + [refz_ok]
    ...
    caps_wrong_receiver = [m.raw_rec(wr_c[x]) for x in probe_methods] + [m.review(wr_c[x], specs[x], receiver_class=wr_c[x]["receiver"]["class"]) for x in probe_methods] + [refz_ok]
    caps_raw_tampered = [m.raw_rec(tamper(caps_ok_c[x], returned=dict(caps_ok_c[x].get("returned") or {"python_type": "str", "value": {"$t": "str", "v": "x"}}, python_type="tampered"))) for x in probe_methods] + list(reviews_ok.values()) + [refz_ok]
    caps_no_raw = list(reviews_ok.values()) + [refz_ok]
    caps_review_other_raw = raws_ok + [m.review(caps_ok_c[x], specs[x], raw_sha=L.sha256_text("some other capture " + x)) for x in probe_methods] + [refz_ok]
    caps_review_stale_parser = raws_ok + [m.review(caps_ok_c[x], specs[x], parser_version="vidtoolz.resolveProbeParser.v0", parser_sha=L.sha256_text("old parser")) for x in probe_methods] + [refz_ok]
    caps_review_stale_spec = raws_ok + [m.review(caps_ok_c[x], specs[x], spec_sha=L.sha256_text("old spec")) for x in probe_methods] + [refz_ok]
    caps_reviewer_is_operator = raws_ok + [m.review(caps_ok_c[x], specs[x], reviewer=OPERATOR) for x in probe_methods] + [refz_ok]
    caps_review_reject = raws_ok + [m.review(caps_ok_c[x], specs[x], decision="REJECT", rationale="operator asked to reject") for x in probe_methods] + [refz_ok]
    caps_refreeze_stale_parser = raws_ok + list(reviews_ok.values()) + [m.refz(caps_hyp, parent_caps_sha, parser_sha=L.sha256_text("old parser"))]
    caps_refreeze_stale_spec = raws_ok + list(reviews_ok.values()) + [m.refz(caps_hyp, parent_caps_sha, spec_sha=L.sha256_text("old spec"))]
    caps_refreeze_other_session = raws_ok + list(reviews_ok.values()) + [m.refz(caps_hyp, parent_caps_sha, session=S_OLD)]
    caps_refreeze_unreviewed = raws_ok + list(reviews_ok.values()) + [m.refz(caps_hyp, parent_caps_sha, reviewed=False)]
    caps_refreeze_unlisted_raw = raws_ok + list(reviews_ok.values()) + [m.refz(caps_hyp, parent_caps_sha, promoted_raw_capture_sha256=[])]
    caps_old_refreeze = raws_ok + list(reviews_ok.values()) + [m.refz(caps_hyp, parent_caps_sha, caps_sha="0" * 64)]
    caps_old_authority = raws_ok + list(reviews_ok.values()) + [m.refz(caps_hyp, parent_caps_sha, authority_version="1.5.0")]
    caps_no_refreeze = raws_ok + list(reviews_ok.values())
