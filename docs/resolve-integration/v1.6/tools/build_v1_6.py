#!/usr/bin/env python3
"""Generate v1.6 machine artifacts as the correction of v1.5 (Codex F15-01..07, Claude M-RAW/M-H0/M-PRECEDENCE/M-ID + minors,
prototype and Hermes M0A compatibility findings). Deterministic; offline; no Resolve; fake receivers only.
Run from the bundle dir: python3 -B tools/build_v1_6.py
Inputs: the v1.5 instances/schemas seeded in this directory (byte-identical copies of the frozen v1.5 bundle)."""
import copy
import hashlib
import json
import os
...
tc["schema"] = "vidtoolz.resolveTargetContract.v1.6"; tc["version"] = "1.6.0"
...
tcs["$id"] = "vidtoolz.resolveTargetContract.v1.6"; tcs["title"] = "Resolve target contract v1.6 (attachment gate; derived state; envelope law)"
tcs["properties"]["schema"] = {"const": "vidtoolz.resolveTargetContract.v1.6"}
tcs["properties"]["version"] = {"type": "string", "pattern": "^1\\.6\\.[0-9]+$"}
...
rp["schema"] = "vidtoolz.resolveReadPrimitives.v1.6"; rp["version"] = "1.6.0"
...
rp_schema["$id"] = "vidtoolz.resolveReadPrimitives.v1.6"; rp_schema["properties"]["schema"] = {"const": "vidtoolz.resolveReadPrimitives.v1.6"}
...
rp_schema["required"] = sorted(set(rp_schema["required"]) | {"primitive_spec_sha256", "primitive_spec_law"})
...
caps["schema"] = "vidtoolz.resolveCapabilityMatrix.v1.6"; caps["version"] = "1.6.0"
...
cap_schema["$id"] = "vidtoolz.resolveCapabilityMatrix.v1.6"; cap_schema["title"] = "Capability matrix v1.6 (content-bound; raw-capture evidence entries)"
cap_schema["properties"]["schema"] = {"const": "vidtoolz.resolveCapabilityMatrix.v1.6"}
...
er["properties"].update({"product": STR_OR_NULL, "receiver_class": {"enum": list(L.RECEIVER_CLASSES)}, "probe_id": STR_OR_NULL, "raw_capture_sha256": SHA, "derived_result_sha256": SHA, "review_decision_sha256": SHA, "parser_version": {"const": L.PARSER_VERSION}, "parser_sha256": SHA, "primitive_spec_sha256": SHA, "result": {"enum": ["UNQUALIFIED_PRIOR_VERSION", None]}, "observed_result": {"type": "string"}})
...
er["allOf"] = [{"if": {"properties": {"result": {"const": None}}}, "then": {"required": ["receiver_class", "raw_capture_sha256", "derived_result_sha256", "review_decision_sha256", "parser_version", "parser_sha256", "primitive_spec_sha256", "product"], "not": {"required": ["observed_result"]}}}]
...
V16E = {"host": HOST, "product": F.PRODUCT, "resolve_version": "21.1.0", "build": 14, "run_ref": "probe", "method": "GetVersionString", "receiver_class": "Resolve", "evidence_path": "/x", "evidence_sha256": "a" * 64, "version_match": True, "reviewed_refreeze_version": "1.6.0", "probe_id": "probe-0001", "raw_capture_sha256": "b" * 64, "derived_result_sha256": "c" * 64, "review_decision_sha256": "d" * 64, "parser_version": L.PARSER_VERSION, "parser_sha256": L.parser_sha256(), "primitive_spec_sha256": rp["primitive_spec_sha256"], "result": None}
...
caps_hyp = copy.deepcopy(caps); caps_hyp["version"] = "1.6.0-HYPOTHETICAL"; caps_hyp["qualification_note"] = "HYPOTHETICAL_NOT_AUTHORITY: illustrates a future reviewed refreeze (rows QUALIFIED_READ citing raw/derived/review digests of fixture captures); never load as CAPABILITIES.json"
...
CLAIMS_S = {"type": "object", "additionalProperties": False, "required": ["claim_A_callable", "claim_B_unique_within_pass", "claim_C_stable_across_passes", "passes", "items", "duplicates", "null_paths", "raw_capture_sha256s", "derived_result_sha256s", "not_claimed"], "properties": {"claim_A_callable": {"type": "boolean"}, "claim_B_unique_within_pass": {"type": "boolean"}, "claim_C_stable_across_passes": {"type": "boolean"}, "passes": NONNEG, "items": NONNEG, "duplicates": {"type": "array", "items": {"type": "string"}}, "null_paths": {"type": "array", "items": {"type": "string"}}, "raw_capture_sha256s": {"type": "array", "items": SHA}, "derived_result_sha256s": {"type": "array", "items": SHA}, "not_claimed": {"type": "array", "items": {"type": "string"}}}}
...
RECORD_RULES = [
    ...
    rec_schema("RAW_CAPABILITY_CAPTURE", ["capture", "raw_capture_sha256"], {"capture": CAPTURE_S, "raw_capture_sha256": SHA}),
...
ev_schema = S("vidtoolz.resolveEvidenceSet.v1.6", "Linked evidence set (content-addressed, envelope-bound records)", {"schema": {"const": "vidtoolz.resolveEvidenceSet.v1.6"}, "current_session_id": STR_OR_NULL, "evaluated_at": {"type": "string", "minLength": 20}, "records": {"type": "object", "propertyNames": {"pattern": "^[a-f0-9]{64}$"}, "additionalProperties": {"type": "object", "required": ["record_type", "record_id", "envelope", "recorded_at"], "properties": record_common, "allOf": RECORD_RULES}}}, ["schema", "current_session_id", "evaluated_at", "records"],
...
req_schema["$id"] = "vidtoolz.resolveEligibilityRequest.v1.6"; req_schema["properties"]["schema"] = {"const": "vidtoolz.resolveEligibilityRequest.v1.6"}
...
snap_schema = S("vidtoolz.resolveSnapshot.v1.6", "Canonical Resolve readback snapshot v1.6", {
    "schema": {"const": "vidtoolz.resolveSnapshot.v1.6"}, "collector_version": {"type": "string", "minLength": 1}, "canonicalization_version": {"const": "1.5"}, "target_epoch": {"type": "string", "minLength": 1}, "resolve_build": {"type": "string", "minLength": 1},
...
policy = {"target_contract_sha256": sha_of("TARGET-CONTRACT.json"), "timebase_sha256": sha_of("TIMEBASE.json"), "track_policy_sha256": sha_of("schemas/resolveTrackPolicy.v1.json"), "capabilities_version": "1.6.0", "capability_matrix_sha256": CAPS_SHA, "collector_version": "0.0.0-fixture", "canonicalization_version": "1.5"}
policy_hyp = dict(policy, capabilities_version="1.6.0-HYPOTHETICAL", capability_matrix_sha256=HYP_SHA)
...
dump("fixtures/canonicalization/vectors.json", {"schema": "vidtoolz.resolveCanonicalizationVectors.v1.6", "spec": "CANONICALIZATION.md v1.6", "digest_rule": "sha256(utf8(domain) + 0x0A + canonical_bytes)", "registered_domains": sorted(L.HASH_DOMAINS), "reference_implementation": "tools/authority_lib.py", "vectors": vectors})
...
dump("fixtures/canonicalization/rejections.json", {"schema": "vidtoolz.resolveCanonicalizationRejections.v1.6", "cases": cases})

# ============================================================ PERMISSIONS v1.5 (unchanged law; schema/version bump) + ELIGIBILITY cases
perms = load("PERMISSIONS.json")
perms["schema"] = "vidtoolz.resolvePermissions.v1.6"; perms["version"] = "1.6.0"
...
perm_schema["$id"] = "vidtoolz.resolvePermissions.v1.6"; perm_schema["properties"]["schema"] = {"const": "vidtoolz.resolvePermissions.v1.6"}
...
matrix = load("MILESTONE-MATRIX.json"); matrix["schema"] = "vidtoolz.resolveMilestoneMatrix.v1.6"; matrix["version"] = "1.6.0"; dump("MILESTONE-MATRIX.json", matrix)
probes = load("M3-PROBES.json"); probes["schema"] = "vidtoolz.resolveM3Probes.v1.6"; dump("M3-PROBES.json", probes)


def REQ(m, op, project=None, timeline=None, refs=None, plan_target=None, **kw):
    r = {"schema": "vidtoolz.resolveEligibilityRequest.v1.6", "milestone": m, "operation": op, "scope": SCOPE, "refs": refs or {}}
...
h0_other_matrix = copy.deepcopy(s0); h0_other_matrix["policy"] = dict(policy_hyp, capability_matrix_sha256=CAPS_SHA, capabilities_version="1.6.0"); resign(h0_other_matrix)
...
p = copy.deepcopy(plan_m3); p["authority_version"] = "1.4.0"; fixture("plan-authority-version-old", "schema", "provisional/resolveMutationPlan", p, expect_contains="1.6.0")
...
man_schema["$id"] = "vidtoolz.resolveFreezeManifest.v1.6"; man_schema["title"] = "Freeze manifest v1.6"
man_schema["properties"]["schema"] = {"const": "vidtoolz.resolveFreezeManifest.v1.6"}; man_schema["properties"]["bundle"] = {"const": "docs/resolve-integration/v1.6"}; man_schema["properties"]["version"] = {"const": "1.6.0"}
man_schema["properties"]["parent"] = {"type": "object", "additionalProperties": False, "required": ["version", "branch", "head", "manifest_sha256", "immutable", "ancestors"], "properties": {
    "version": {"const": "1.5.0"}, "branch": {"const": "docs/resolve-authority-freeze-v1.5"}, "head": {"const": "9d944e01358b4b88d6ee4c9993a4db4084ba23ad"}, "manifest_sha256": {"const": "a40c6954fbe7f72d48eaf3957c0ac036c471a7a92496e277660530386e7aba5a"}, "immutable": {"const": True},
    "ancestors": {"type": "array", "minItems": 5, "items": {"type": "object", "additionalProperties": False, "required": ["version", "branch", "head", "manifest_sha256"], "properties": {"version": {"type": "string"}, "branch": {"type": "string"}, "head": {"type": "string", "pattern": "^[0-9a-f]{40}$"}, "manifest_sha256": SHA, "baseline": {"type": "string"}}}}}}
dump("schemas/resolveFreezeManifest.schema.json", man_schema)
print("build_v1_6: OK; layered fixtures:", FIXTURE_COUNT[0], "; evidence sets:", len(EVSETS))
