# SEMANTIC-VALIDATION.md — schema validity is not authorization (v1.2, FROZEN_NOW as doctrine; validators NOT runtime-qualified)

**SCHEMA-VALID != AUTHORIZED TO MUTATE.** JSON Schema proves shape. Authority requires a second layer, the *semantic validator*, implemented as a reference in `tools/authority_lib.py` and exercised by `tools/validate_v1_2.py` against layered fixtures only (parse → schema → semantic → eligibility; every positive semantic fixture must first pass its schema). It has not run against Resolve and is not runtime-qualified.
...
## v1.2 strengthening
- Mutation plan: rejects `RESOLVE_READ` with any write op; selector/target mismatch; missing `h0_guard_digest` for guard-required ops; stale guard; per-operation eligibility via `evaluate_eligibility` (non-dry-run); payload digest used as guard.
- Journal: rejects repeated or non-contiguous sequence numbers, transaction-id change mid-chain, broken hash chain, illegal transitions (table includes `RECOVERY_RECONCILING` → `NOT_APPLIED | COMMITTED_RECOVERED | ORPHANED_PARTIAL | ORPHANED_AMBIGUOUS`, and `ABORTED`), reused `operation_id` on APPLIED, records after a terminal state.
- Commit eligibility: never a naked terminal string; requires journal identity and chain head, a `VERIFIED` verification result linked by digest, matching guard digest, empty `unresolved_conflicts`, and required evidence digests.
- Every fixture states its expected failure layer: `none | parse | schema | semantic | eligibility`.
