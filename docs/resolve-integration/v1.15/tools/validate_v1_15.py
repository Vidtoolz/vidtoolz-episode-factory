...
Run: python3 -B tools/validate_v1_15.py
"""
import ast
import copy
import glob
...
rec("precedence", "the v1.5 through v1.14 changelogs and finding matrices are HISTORICAL; the v1.15 ones are active", all(st_.get(d_) == "HISTORICAL" for d_ in ("CHANGELOG-v1.5.md", "FINDING-RESOLUTION-MATRIX-v1.5.md", "CHANGELOG-v1.6.md", "FINDING-RESOLUTION-MATRIX-v1.6.md", "CHANGELOG-v1.7.md", "FINDING-RESOLUTION-MATRIX-v1.7.md", "CHANGELOG-v1.8.md", "FINDING-RESOLUTION-MATRIX-v1.8.md", "CHANGELOG-v1.9.md", "FINDING-RESOLUTION-MATRIX-v1.9.md", "CHANGELOG-v1.10.md", "FINDING-RESOLUTION-MATRIX-v1.10.md", "CHANGELOG-v1.11.md", "FINDING-RESOLUTION-MATRIX-v1.11.md", "CHANGELOG-v1.12.md", "FINDING-RESOLUTION-MATRIX-v1.12.md", "CHANGELOG-v1.13.md", "FINDING-RESOLUTION-MATRIX-v1.13.md", "CHANGELOG-v1.14.md", "FINDING-RESOLUTION-MATRIX-v1.14.md")) and st_.get("CHANGELOG-v1.15.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.15.md") == "STILL_ACTIVE")
...
rec("precedence", "the v1.15 consumed-evidence supersession S62 is recorded, the v1.15 changelog and matrix are active, and the document version tracks the authority version", [s_["id"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.15.0"] == ["S62"] and st_.get("CHANGELOG-v1.15.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.15.md") == "STILL_ACTIVE" and prec["version"] == L.AUTHORITY_VERSION)
...
if os.path.exists(man_path):
    ...
    parent = L.strict_load(os.path.join(os.path.dirname(B), "v1.14", "FREEZE-MANIFEST.json"))
    rec("manifest", "parent (v1.14) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.14", "FREEZE-MANIFEST.json")) == "0d4fc9915923d17ef6ec469cc01fb164b4d2ee1001fc7983d557bf4c9d9032b8")
    rec("manifest", "grandparent (v1.13) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.13", "FREEZE-MANIFEST.json")) == "436e12c8616ea6704fef2dbd0e14517cf33092aec1a7ff064dd4b10fb3dad573")
    ...
    rec("manifest", "the parent block records the v1.14 branch HEAD and its semantic commit, which for v1.14 are the same commit because its bundle and registration were committed together (section 44)", man["parent"]["head"] == "b61f248238ec7ed60deca79aa9728ec0cde86edd" and man["parent"]["semantic_head"] == "b61f248238ec7ed60deca79aa9728ec0cde86edd" and man["parent"]["manifest_sha256"] == "0d4fc9915923d17ef6ec469cc01fb164b4d2ee1001fc7983d557bf4c9d9032b8" and man["parent"]["version"] == "1.14.0" and {a["version"] for a in man["parent"]["ancestors"]} >= {"1.13.0", "1.12.0"})
    rec("manifest", "v1.0-v1.14 bundles are byte-identical to their frozen manifests (history preserved)", all(all(sha(os.path.join(os.path.dirname(B), v_, e_["path"])) == e_["sha256"] for e_ in L.strict_load(os.path.join(os.path.dirname(B), v_, "FREEZE-MANIFEST.json"))["files"] if os.path.exists(os.path.join(os.path.dirname(B), v_, e_["path"]))) for v_ in ("v1", "v1.1", "v1.2", "v1.3", "v1.4", "v1.5", "v1.6", "v1.7", "v1.8", "v1.9", "v1.10", "v1.11", "v1.12", "v1.13", "v1.14")))
...
try:
    ...
    rec("core-static", "the published core_authority precedence matches the code: location authority, then attachment semantics, then eligibility",
        ...
        and WFLAW["core_authority"]["receipt"]["binds"] == ["session_id", "document_path", "governed_root", "authority_version", "manifest_sha256", "document_sha256", "location_validated"])
        ...
        if path.endswith(".json") and path != "AUTHORITY-PRECEDENCE.json":
            ...
            def _walk(node, keypath):
                if isinstance(node, dict):
                    ...
                    if any(t in " ".join(keypath).lower() for t in
                           ("diagnostic", "provisional", "non_authorizing", "non-authorizing", "historical",
                            "demoted", "inherited", "codex_finding", "prior_defect", "correction",
                            "negative_fixture", "residual", "operational_closure", "law", "rules", "note",
                            "qualification_note", "reason", "adjudication")):
                        return
...
CODEX_MATRIX = L.strict_load(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.15.json"))
...
rec("codex-matrix", "the inherited v1.6 finding matrix is retained unchanged as history and its seven ids are still enumerated",
    ...
    and CODEX_MATRIX["inherited_matrices"] == ["FINDING-RESOLUTION-MATRIX-v1.14.json", "FINDING-RESOLUTION-MATRIX-v1.13.json", "FINDING-RESOLUTION-MATRIX-v1.12.json", "FINDING-RESOLUTION-MATRIX-v1.11.json", "FINDING-RESOLUTION-MATRIX-v1.10.json", "FINDING-RESOLUTION-MATRIX-v1.9.json", "FINDING-RESOLUTION-MATRIX-v1.8.json", "FINDING-RESOLUTION-MATRIX-v1.7.json"])
rec("codex-matrix", "Store.v5 carries NO semantic change in v1.15: evidence_store.py differs from the v1.14 bundle by exactly one comment line and every semantic constant is identical",
    (lambda a, b: (lambda da, db: len(da) == 1 and len(db) == 1
        ...
        [l for l in __import__("difflib").unified_diff(a, b, n=0) if l.startswith("+") and not l.startswith("+++")]))(
        open(os.path.join(os.path.dirname(B), "v1.14", "tools", "evidence_store.py"), encoding="utf-8").read().splitlines(),
...
rec("codex-matrix", "the ACTIVE v1.15 finding-resolution matrix enumerates exactly Codex's six v1.14 release findings",
    [f["id"] for f in CODEX_MATRIX["findings"]] == ["V114-B1", "V114-B2", "V114-B3", "V114-B4", "V114-M1", "V114-N1"])
rec("codex-matrix", "the v1.15 matrix records the four-BLOCKER/one-MAJOR/one-MINOR adjudication, the frozen v1.14 reproduction head, the essential law and the honest residual",
    "FOUR BLOCKERs" in CODEX_MATRIX["adjudication"] and "MERGE MAJOR" in CODEX_MATRIX["adjudication"]
    and "REVIEW_ENVIRONMENT_EFFECT" in CODEX_MATRIX["adjudication"]
    and "b61f248238ec7ed60deca79aa9728ec0cde86edd" in CODEX_MATRIX["law"]
    and "THE EXACT BYTES VALIDATED AS GOVERNED EVIDENCE ARE THE EXACT BYTES CONSUMED" in CODEX_MATRIX["law"]
...
rec("codex-matrix", "every v1.15 finding pins its reproduction against the frozen v1.14 head and names a severity Codex assigned",
    all("b61f2482" in f_["prior_defect"] or "frozen v1.14" in f_["prior_defect"] for f_ in CODEX_MATRIX["findings"])
    and [f_["severity"].split()[0] for f_ in CODEX_MATRIX["findings"]] == ["BLOCKER", "BLOCKER", "BLOCKER", "BLOCKER", "MERGE", "MINOR"])
rec("codex-matrix", "the two findings whose reproduction proved WIDER than reported say so (V114-B4's FIFO hang and raw IsADirectoryError; V114-N1's five-domain gap)",
    "BLOCKED THE AUTHORITY INDEFINITELY" in next(f_ for f_ in CODEX_MATRIX["findings"] if f_["id"] == "V114-B4")["prior_defect"]
    and "WIDER than reported" in next(f_ for f_ in CODEX_MATRIX["findings"] if f_["id"] == "V114-N1")["prior_defect"])
...
rec("codex-matrix", "the markdown matrix mirrors the machine form: all six ids, every validation section they name, and the stated residual",
