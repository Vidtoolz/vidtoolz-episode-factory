...
(then named `validate_transaction_set` / `commit_eligibility`; since v1.15 the authorizing pair is
`validate_transaction_set_authorizing` / `commit_eligibility_authorizing` and those two are PROVISIONAL_UNTIL_M3 and
non-authorizing), so that no default could make schema enforcement optional. That closed the wrong half of the
