...
# V112-RP1 section 5: the safe-basename law, retained from v1.12 and stated explicitly. The first character must be
# alphanumeric and the rest [A-Za-z0-9._-], max 128, which rejects: "" (empty), "/" and "\\" (separators), ".." and
# "." (traversal), a leading dot, NUL, absolute syntax, "~", drive letters, whitespace and anything over 128 bytes.
...
def canonical_set_dir(session_id):
    """The canonical governed directory under the FROZEN authority root, independent of any sandbox. Every
    authorizing operation compares the document's actual location against this."""
    _session_id(session_id)
    return os.path.join(L.GOVERNED_ATTACHMENT_ROOT, session_id)
...
def _require_trusted_dir(p, symlink_code, notdir_code, missing_code, want_mode=None):
    st = _lstat_or_none(p)
    if st is None:
        raise AuthoringError(missing_code, p)
    if stat.S_ISLNK(st.st_mode):
        raise AuthoringError(symlink_code, f"{p} is a symlink; the governed path is never followed through one")
    if not stat.S_ISDIR(st.st_mode):
        raise AuthoringError(notdir_code, f"{p} is not a directory ({stat.S_IFMT(st.st_mode):#o})")
    if want_mode is not None and stat.S_IMODE(st.st_mode) != want_mode:
        raise AuthoringError("ROOT_MODE_INVALID", f"{p} mode {stat.S_IMODE(st.st_mode):#o} != {want_mode:#o}")
    return st
...
def require_governed_root():
    ...
    # belt and braces AFTER the entry classification, never before it: no alias, mount or symlink anywhere in the chain
    if os.path.realpath(root) != root:
        raise AuthoringError("ROOT_NOT_GOVERNED",
                             f"{root} resolves to {os.path.realpath(root)}; an aliased governed root is refused")
    return root
...
def require_governed_session_dir(session_id, must_exist=True):
    ...
    if st is None:
        ...
        return d
    if stat.S_ISLNK(st.st_mode):
        raise AuthoringError("SESSION_SYMLINK_REFUSED",
                             f"{d} is a symlink; a symlinked session directory is never authority")
    if not stat.S_ISDIR(st.st_mode):
        raise AuthoringError("SESSION_NOT_A_DIRECTORY", d)
    if os.path.realpath(d) != d:
        raise AuthoringError("SESSION_PATH_NOT_GOVERNED",
                             f"{d} resolves to {os.path.realpath(d)}; an aliased session directory is refused")
    return d
