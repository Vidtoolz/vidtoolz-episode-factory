...
Run: python3 -B tools/validate_v1_13.py
"""
import copy
import glob
import hashlib
...
rec("precedence", "the v1.5 through v1.12 changelogs and finding matrices are HISTORICAL; the v1.13 ones are active", all(st_.get(d_) == "HISTORICAL" for d_ in ("CHANGELOG-v1.5.md", "FINDING-RESOLUTION-MATRIX-v1.5.md", "CHANGELOG-v1.6.md", "FINDING-RESOLUTION-MATRIX-v1.6.md", "CHANGELOG-v1.7.md", "FINDING-RESOLUTION-MATRIX-v1.7.md", "CHANGELOG-v1.8.md", "FINDING-RESOLUTION-MATRIX-v1.8.md", "CHANGELOG-v1.9.md", "FINDING-RESOLUTION-MATRIX-v1.9.md", "CHANGELOG-v1.10.md", "FINDING-RESOLUTION-MATRIX-v1.10.md", "CHANGELOG-v1.11.md", "FINDING-RESOLUTION-MATRIX-v1.11.md", "CHANGELOG-v1.12.md", "FINDING-RESOLUTION-MATRIX-v1.12.md")) and st_.get("CHANGELOG-v1.13.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.13.md") == "STILL_ACTIVE")
...
rec("precedence", "the v1.13 governed-root supersession S60 is recorded and the v1.13 changelog and matrix are active", [s_["id"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.13.0"] == ["S60"] and st_.get("CHANGELOG-v1.13.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.13.md") == "STILL_ACTIVE" and st_.get("CHANGELOG-v1.12.md") == "HISTORICAL" and st_.get("EVIDENCE-ROOT.md") == "STILL_ACTIVE")
...
if os.path.exists(man_path):
    ...
    parent = L.strict_load(os.path.join(os.path.dirname(B), "v1.12", "FREEZE-MANIFEST.json"))
    rec("manifest", "parent (v1.12) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.12", "FREEZE-MANIFEST.json")) == "e5eace278a1b6130971a38f55ef207620cdc97d7b3872d11ce9ab8909e4014fb")
    rec("manifest", "grandparent (v1.11) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.11", "FREEZE-MANIFEST.json")) == "0c47f0cefdd63b971489a46274e3b782927f2c6ad26e4251de21860abc50c44e")
    ...
    rec("manifest", "the parent block records the v1.12 branch HEAD and its semantic commit, which for v1.12 are the same commit because its bundle and registration were committed together (section 38)", man["parent"]["head"] == "73150d00a72820b155caf357c412f46b8ec4a9ba" and man["parent"]["semantic_head"] == "73150d00a72820b155caf357c412f46b8ec4a9ba" and man["parent"]["manifest_sha256"] == "e5eace278a1b6130971a38f55ef207620cdc97d7b3872d11ce9ab8909e4014fb")
    rec("manifest", "v1.0-v1.12 bundles are byte-identical to their frozen manifests (history preserved)", all(all(sha(os.path.join(os.path.dirname(B), v_, e_["path"])) == e_["sha256"] for e_ in L.strict_load(os.path.join(os.path.dirname(B), v_, "FREEZE-MANIFEST.json"))["files"] if os.path.exists(os.path.join(os.path.dirname(B), v_, e_["path"]))) for v_ in ("v1", "v1.1", "v1.2", "v1.3", "v1.4", "v1.5", "v1.6", "v1.7", "v1.8", "v1.9", "v1.10", "v1.11", "v1.12")))
...
try:
    ...
    rec("root-law", "V112-RP1 section 6: the root is classified by lstat BEFORE anything is resolved, and realpath is only ever a later alias check",
        (lambda src: src.index("_require_trusted_dir") < src.index("realpath")
                     and "os.lstat" in inspect.getsource(AUTH_MOD._lstat_or_none)
                     and "lstat" in inspect.getsource(AUTH_MOD._require_trusted_dir)
                     and ".resolve()" not in inspect.getsource(AUTH_MOD))(inspect.getsource(AUTH_MOD.require_governed_root)))
    ...
    rec("root-static", "no symlink-follow-before-check: the module never calls .resolve() and realpath appears only after lstat classification",
        ".resolve()" not in _AS
        and all(min(inspect.getsource(f).find(t) for t in ("_require_trusted_dir", "_lstat_or_none") if inspect.getsource(f).find(t) >= 0)
                < inspect.getsource(f).index("realpath")
                for f in (AUTH_MOD.require_governed_root, AUTH_MOD.require_governed_session_dir)
                if "realpath" in inspect.getsource(f)))
...
CODEX_MATRIX = L.strict_load(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.13.json"))
...
rec("codex-matrix", "the inherited v1.6 finding matrix is retained unchanged as history and its seven ids are still enumerated",
    ...
    and CODEX_MATRIX["inherited_matrices"] == ["FINDING-RESOLUTION-MATRIX-v1.12.json", "FINDING-RESOLUTION-MATRIX-v1.11.json", "FINDING-RESOLUTION-MATRIX-v1.10.json", "FINDING-RESOLUTION-MATRIX-v1.9.json", "FINDING-RESOLUTION-MATRIX-v1.8.json", "FINDING-RESOLUTION-MATRIX-v1.7.json"])
rec("codex-matrix", "Store.v5 carries NO semantic change in v1.13: evidence_store.py differs from the v1.12 bundle by exactly one comment line and every semantic constant is identical",
    (lambda a, b: (lambda da, db: len(da) == 1 and len(db) == 1
        ...
        [l for l in __import__("difflib").unified_diff(a, b, n=0) if l.startswith("+") and not l.startswith("+++")]))(
        open(os.path.join(os.path.dirname(B), "v1.12", "tools", "evidence_store.py"), encoding="utf-8").read().splitlines(),
...
rec("codex-matrix", "the ACTIVE v1.13 finding-resolution matrix enumerates exactly the one governed-root finding",
    [f["id"] for f in CODEX_MATRIX["findings"]] == ["V112-RP1"])
rec("codex-matrix", "the v1.13 matrix records the WORKFLOW_CONTRADICTORY classification, the frozen v1.12 reproduction head and the protected-surface promise",
    "WORKFLOW_CONTRADICTORY" in CODEX_MATRIX["adjudication"]
    and "73150d00a72820b155caf357c412f46b8ec4a9ba" in CODEX_MATRIX["law"]
    and "EXACTLY ONE COMMENT LINE" in CODEX_MATRIX["operational_closure"]
    and "73150d00" in CODEX_MATRIX["findings"][0]["prior_defect"])
