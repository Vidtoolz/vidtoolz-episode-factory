#!/usr/bin/env python3
"""Build FREEZE-MANIFEST.json for bundle v1.11 with machine-readable status, authority class, byte counts and lineage flags vs v1.10.
Run after validate_v1_14.py (so VALIDATION-REPORT.md is current): python3 -B tools/build_manifest.py
...
PARENT_HEAD = "65895c6dedceadff012a08c1bafcdf9e92b65221"            # v1.13 branch HEAD that v1.14 was cut from
PARENT_SEMANTIC_HEAD = "65895c6dedceadff012a08c1bafcdf9e92b65221"   # v1.13 committed its bundle and its DOC-AUTHORITY registration together, so this IS the head
PARENT_MANIFEST = "436e12c8616ea6704fef2dbd0e14517cf33092aec1a7ff064dd4b10fb3dad573"
...
parent = L.strict_load(os.path.join(V113, "FREEZE-MANIFEST.json"))
...
NOTES = {
    ...
    "fixtures/evidence/ACTIVE-AUTHORITY-PLACEHOLDER.json": ("fixture evidence is minted against this PLACEHOLDER manifest sha; validate_v1_14.py proves placeholder-bound records do not derive against the real FREEZE-MANIFEST.json sha and that re-minted records do", None),
...
m = {"schema": "vidtoolz.resolveFreezeManifest.v1.10", "bundle": "docs/resolve-integration/v1.14", "version": "1.14.0", "frozen_at": "2026-09-10", "prepared_by": "Claude Code (Opus 5)", "approval_status": "CANDIDATE_FOR_INDEPENDENT_REVIEW", "approver": None,
     "parent": {"version": "1.13.0", "branch": "docs/resolve-authority-freeze-v1.13", "head": PARENT_HEAD, "semantic_head": PARENT_SEMANTIC_HEAD, "manifest_sha256": PARENT_MANIFEST, "immutable": True,
                "ancestors": [
     ...
     "rules": ["a file whose sha256 or byte count differs from this manifest is not the frozen version", "v1.0 through v1.13 bytes and manifests are never modified; v1.14 supersedes by reference", "SCHEMA-VALID != AUTHORIZED TO MUTATE; PERMISSION DECLARATION != ELIGIBILITY; PROBE_ALLOWED != QUALIFIED_READ; A CAPTURE != EVIDENCE THAT SOMETHING WORKS; attachment state is derived from a coherent, current, envelope-bound evidence set against the ACTIVE reviewed manifest, never declared; contradictory evidence is CONFLICT", "a BUNDLE_VERIFICATION, MILESTONE_EXIT, M3_AUTHORIZATION or REFREEZE_RECORD counts only when bound to this manifest's sha256 and authority version 1.14.0; records for other manifests are historical", "mutation-capable schemas remain PROVISIONAL_UNTIL_M3; verification is derived from S0/S1 over the protected surface (verify_transaction) and never declared; commit_eligibility -> validate_transaction_set is the ONLY authorizing commit path (every per-document helper is INTERNAL_NON_AUTHORIZING); capability meaning is DERIVED by the versioned reference parser from an immutable facts-only raw capture (never from a probe-written flag), the ACTIVE capability authority is the matrix's own content digest, a degraded H0 can never be a write precondition, and schema enforcement in the composed path is mandatory; no adapter mutation implementation exists; no Resolve write was executed; M0 has not begun",
