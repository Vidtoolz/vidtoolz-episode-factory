...
Run: python3 -B tools/validate_v1_14.py
"""
import ast
import copy
import glob
...
rec("precedence", "the v1.5 through v1.13 changelogs and finding matrices are HISTORICAL; the v1.14 ones are active", all(st_.get(d_) == "HISTORICAL" for d_ in ("CHANGELOG-v1.5.md", "FINDING-RESOLUTION-MATRIX-v1.5.md", "CHANGELOG-v1.6.md", "FINDING-RESOLUTION-MATRIX-v1.6.md", "CHANGELOG-v1.7.md", "FINDING-RESOLUTION-MATRIX-v1.7.md", "CHANGELOG-v1.8.md", "FINDING-RESOLUTION-MATRIX-v1.8.md", "CHANGELOG-v1.9.md", "FINDING-RESOLUTION-MATRIX-v1.9.md", "CHANGELOG-v1.10.md", "FINDING-RESOLUTION-MATRIX-v1.10.md", "CHANGELOG-v1.11.md", "FINDING-RESOLUTION-MATRIX-v1.11.md", "CHANGELOG-v1.12.md", "FINDING-RESOLUTION-MATRIX-v1.12.md", "CHANGELOG-v1.13.md", "FINDING-RESOLUTION-MATRIX-v1.13.md")) and st_.get("CHANGELOG-v1.14.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.14.md") == "STILL_ACTIVE")
...
rec("precedence", "the v1.14 authorizing-location supersession S61 is recorded, the v1.14 changelog and matrix are active, and the v1.13 pair is HISTORICAL", [s_["id"] for s_ in prec["superseded_statements"] if s_["effective_version"] == "1.14.0"] == ["S61"] and st_.get("CHANGELOG-v1.14.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.14.md") == "STILL_ACTIVE" and st_.get("FINDING-RESOLUTION-MATRIX-v1.13.md") == "HISTORICAL" and prec["version"] == L.AUTHORITY_VERSION)
...
if os.path.exists(man_path):
    ...
    parent = L.strict_load(os.path.join(os.path.dirname(B), "v1.13", "FREEZE-MANIFEST.json"))
    rec("manifest", "parent (v1.13) manifest sha pinned", sha(os.path.join(os.path.dirname(B), "v1.13", "FREEZE-MANIFEST.json")) == "436e12c8616ea6704fef2dbd0e14517cf33092aec1a7ff064dd4b10fb3dad573")
    ...
    rec("manifest", "the parent block records the v1.13 branch HEAD and its semantic commit, which for v1.13 are the same commit because its bundle and registration were committed together (section 38)", man["parent"]["head"] == "65895c6dedceadff012a08c1bafcdf9e92b65221" and man["parent"]["semantic_head"] == "65895c6dedceadff012a08c1bafcdf9e92b65221" and man["parent"]["manifest_sha256"] == "436e12c8616ea6704fef2dbd0e14517cf33092aec1a7ff064dd4b10fb3dad573")
    rec("manifest", "v1.0-v1.13 bundles are byte-identical to their frozen manifests (history preserved)", all(all(sha(os.path.join(os.path.dirname(B), v_, e_["path"])) == e_["sha256"] for e_ in L.strict_load(os.path.join(os.path.dirname(B), v_, "FREEZE-MANIFEST.json"))["files"] if os.path.exists(os.path.join(os.path.dirname(B), v_, e_["path"]))) for v_ in ("v1", "v1.1", "v1.2", "v1.3", "v1.4", "v1.5", "v1.6", "v1.7", "v1.8", "v1.9", "v1.10", "v1.11", "v1.12", "v1.13")))
...
try:
    ...
    rec("root-law", "the production authoring path refuses outright when the frozen root is absent, so no evidence can appear anywhere else by default",
        _try_auth(lambda: AUTH_MOD.create_evidence_set(_rp_sess("prod"), _PREP)).code in ("ROOT_NOT_FOUND", "ROOT_NOT_GOVERNED"),
        _try_auth(lambda: AUTH_MOD.create_evidence_set(_rp_sess("prod2"), _PREP)).code)
    ...
    rec("root-parity", "no fixture or test path can establish a state production location policy forbids: the sandboxed document is mechanically sound yet the AUTHORIZING derivation refuses it outright",
        _par_mech == "ATTACHMENT_READY" and _par_auth.code in ("NOT_PRODUCTION_ROOT", "ROOT_NOT_FOUND"),
...
try:
    ...
    rec("core-authority", "it has __slots__ and no dict, so provenance fields cannot be grafted onto an instance after the fact",
        set(L.GovernedEvidenceSet.__slots__) == {"evidence_set", "session_id", "document_path", "governed_root",
                                                 "authority_version", "manifest_sha256", "document_sha256", "receipt"}
    ...
    rec("core-bypass", "V113-B1 section 9: location authority strictly DOMINATES attachment semantics - the authorizing path checks provenance BEFORE it computes readiness, rather than computing readiness and retracting it afterwards",
        (lambda a, b: a.index("governed_provenance_errors") < a.index("derive_attachment_state(tc")
                      and b.index("governed_provenance_errors") < b.index("evaluate_eligibility(perms"))(
    ...
    rec("core-selflocation", "V113-B1 section 21: the core loader independently compares the document's own recorded location and session identity against the canonical path it actually loaded from, and requires the frozen authority root plus written_under_production_root",
        all(t in inspect.getsource(L.load_governed_evidence_set) for t in
            ("SELF_LOCATION_MISMATCH", "authority_root", "written_under_production_root", "canonical_session_dir",
             "evidence_set_sha256", "NOT_PRODUCTION_ROOT")))
    rec("core-selflocation", "the loader reads the document ONLY from the canonical path it computed, never from a caller-supplied path",
        ...
        and not re.search(r"open\((?!doc\b|wfp\b)", inspect.getsource(L.load_governed_evidence_set)))
    ...
    rec("core-static", "section 32: authorizing eligibility cannot accept a raw or precomputed attachment state - it has no state parameter and recomputes from the governed document",
        ...
        and "governed.evidence_set" in inspect.getsource(L.evaluate_eligibility_authorizing))
    ...
    rec("core-static", "section 32: the authorizing derivation checks the canonical location itself and does not delegate that to a wrapper",
        all(t in inspect.getsource(L.derive_attachment_state_authorizing) for t in
            ("governed_provenance_errors", "authorizing", "LOCATION_AUTHORITY_INVALID"))
        and all(t in inspect.getsource(L.governed_provenance_errors) for t in
                ("location_errors", "canonical_document_path", "location_receipt", "GOVERNED_ATTACHMENT_ROOT")))
    ...
    rec("core-static", "EVIDENCE-SET-WORKFLOW.json publishes the v1.14 core_authority block: the only door, the provenance object, the four authorizing entry points, the two demoted ones, the receipt and the fail-closed law",
        (lambda ca: ca["only_door"].startswith("tools/authority_lib.py#load_governed_evidence_set")
                    and sorted(ca["authorizing_entrypoints"]) == ["derive_attachment_state_authorizing", "derive_attachment_state_for_session", "evaluate_eligibility_authorizing", "evaluate_eligibility_for_session"]
                    ...
                    and "semantic_mutation_plan" in ca["residual"])(WFLAW["core_authority"]))
...
CODEX_MATRIX = L.strict_load(os.path.join(B, "FINDING-RESOLUTION-MATRIX-v1.14.json"))
...
rec("codex-matrix", "the inherited v1.6 finding matrix is retained unchanged as history and its seven ids are still enumerated",
    ...
    and CODEX_MATRIX["inherited_matrices"] == ["FINDING-RESOLUTION-MATRIX-v1.13.json", "FINDING-RESOLUTION-MATRIX-v1.12.json", "FINDING-RESOLUTION-MATRIX-v1.11.json", "FINDING-RESOLUTION-MATRIX-v1.10.json", "FINDING-RESOLUTION-MATRIX-v1.9.json", "FINDING-RESOLUTION-MATRIX-v1.8.json", "FINDING-RESOLUTION-MATRIX-v1.7.json"])
rec("codex-matrix", "Store.v5 carries NO semantic change in v1.14: evidence_store.py differs from the v1.13 bundle by exactly one comment line and every semantic constant is identical",
    (lambda a, b: (lambda da, db: len(da) == 1 and len(db) == 1
        ...
        [l for l in __import__("difflib").unified_diff(a, b, n=0) if l.startswith("+") and not l.startswith("+++")]))(
        open(os.path.join(os.path.dirname(B), "v1.13", "tools", "evidence_store.py"), encoding="utf-8").read().splitlines(),
...
rec("codex-matrix", "the ACTIVE v1.14 finding-resolution matrix enumerates exactly the one authorizing-location finding",
    [f["id"] for f in CODEX_MATRIX["findings"]] == ["V113-B1"])
rec("codex-matrix", "the v1.14 matrix records Codex's 93/96 result, the frozen v1.13 reproduction head, the demotion and the protected-surface promise",
    "93 of 96" in CODEX_MATRIX["adjudication"]
    and "BLOCKER" in CODEX_MATRIX["adjudication"] and "MINOR" in CODEX_MATRIX["adjudication"]
    and "65895c6dedceadff012a08c1bafcdf9e92b65221" in CODEX_MATRIX["law"]
    and "EXACTLY ONE COMMENT LINE" in CODEX_MATRIX["operational_closure"]
    and "pre-M3 transaction authority" in CODEX_MATRIX["operational_closure"]
    and "65895c6d" in CODEX_MATRIX["findings"][0]["prior_defect"]
    and "DEMOTED" in CODEX_MATRIX["findings"][0]["correction"])
rec("codex-matrix", "the v1.14 finding states the defect in the same terms Codex reported it, and names the MINOR it also closes",
    all(t in CODEX_MATRIX["findings"][0]["codex_finding"] for t in ("derive_attachment_state", "evaluate_eligibility", "ATTACHMENT_READY", "eligible=true"))
    and "test_root_override" in CODEX_MATRIX["findings"][0]["prior_defect"]
    and "NOT_PRODUCTION_ROOT" in CODEX_MATRIX["findings"][0]["prior_defect"])
...
rec("codex-matrix", "the markdown matrix mirrors the machine form: the one id, the seven validation sections and the scoped M3 residual",
    (lambda t_: "V113-B1" in t_ and all(sec in t_ for sec in CODEX_MATRIX["findings"][0]["validation_sections"])
