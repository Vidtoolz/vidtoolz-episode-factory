...
// ---- the committed failed fixture stays frozen as historical evidence ----

test("es-v04-acceptance: the committed v0.9.3 fixture stays hash-frozen with its real-import discrepancy", () => {
  ...
  const manifest = JSON.parse(fs.readFileSync(path.join(pkg, acceptance.FILES.manifest), "utf8"));
  assert.equal(manifest.planner_version, "0.9.3");
  const frozenHash = crypto.createHash("sha256")
    .update(fs.readFileSync(path.join(pkg, "earth-studio", "earth-studio.esp")))
    .digest("hex");
  assert.equal(frozenHash, "7754a178b63c60a9f7ccdf3c5313200501a0ac6463016b850fe628da667c3e41");
  const checks = acceptance.runSemanticChecks(pkg);
  assert.equal(checks.ok, true, checks.checks.filter((c) => !c.ok).map((c) => `${c.name}: ${c.detail}`).join("; "));
  const status = acceptance.computeStatus(pkg);
  assert.equal(status.state, "IMPORT_DISCREPANCY_REPORTED");
  assert.equal(status.observation.complete, true);
  assert.equal(status.observation.accepted, false);
  assert.deepEqual(status.observation.discrepancies, ["zoom.correct", "tilt.correct"]);
});
