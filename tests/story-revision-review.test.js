...
function inputFor(env, v1, v2, extra = {}) {
  return {
    ...
    source_version_id: v1.id, candidate_version_id: v2.id,
...
test('SRR2: wrong parent blocks', () => {
  const env = mkEnv(); const { v1 } = mkVersions(env, S1, S2);
  // create an unrelated second lineage in a separate project root — parent cannot match
  const env2 = mkEnv();
  const other = versions.createVersion(env2.dataRoot, env2.project, S2, env2.config, { central_claim: 'Local beats cloud.' });
  // point review at env1 data root but candidate from env2 lineage: load must fail or parent mismatch → BLOCKED
  const out = srr.buildReview({ script_builder_root: SB_ROOT, data_root: env.dataRoot, project_id: 'p1',
    source_version_id: v1.id, candidate_version_id: 'nonexistent-version-id',
    change_rationales: [] });
  assert.equal(out.state, 'BLOCKED');
});

test('SRR3: expected content hash mismatch blocks', () => {
  const env = mkEnv(); const { v1, v2 } = mkVersions(env, S1, S2);
  const out = srr.buildReview(inputFor(env, v1, v2, { over: { expected_source_content_hash: sha('wrong') } }));
  assert.equal(out.state, 'BLOCKED');
});
...
test('SRR13: stale Research Result blocks candidate binding', () => {
  ...
  result.claim.temporal = { temporal_class: 'CURRENT_FACT', as_of: '2026-01-01T00:00:00Z', freshness_policy: { MAX_AGE_DAYS: 1 } };
...
test('SRR16: exact human exception surfaces without changing Research verdict', () => {
  const result = mkRoot([mkResult({ recommendation: 'RESEARCH_MORE' })]).results[0];
  const script = 'exact test script';
  const exception = {
    ...
    research_result_ref: { result_id: result.result_id, result_revision: result.result_revision,
      result_digest_sha256: result.result_digest_sha256 },
    script_usage_ref: { script_version_id: 'v', script_content_hash: sha(script),
      binding_id: 'b1', assertion_text_sha256: sha('a') },
  ...
  const out = rrv.validateHumanException(exception, {
    current_result_ref: exception.research_result_ref,
    current_script_usage_ref: exception.script_usage_ref,
    current_exception_bytes: exceptionBytes() });
...
test('SRR17: exception drift blocks', () => {
  ...
  const e = { schema_version: 1, artifact_type: 'research-human-exception',
    ...
    script_usage_ref: { script_version_id: 'v', script_content_hash: sha('s'), binding_id: 'b', assertion_text_sha256: sha('a') },
    reason: 'r', acknowledged_risks: ['r'],
    approval_binding: { artifact_path: 'x', artifact_sha256: sha('s'), commit: 'T', approved_by: 'T', approved_at: AS_OF, scope: 'T' } };
  const out = rrv.validateHumanException(e, { current_result_ref: { ...e.research_result_ref, result_digest_sha256: result.result_digest_sha256 },
    current_script_usage_ref: e.script_usage_ref, current_exception_bytes: rrv.exceptionApprovalBytes(JSON.parse(JSON.stringify(e))) });
...
test('SRR18: two claims in one sentence stay distinct', () => {
  ...
  const doc = mkBindingsDoc([mkBinding(r1, { assertion: 'Cloud costs monthly.' }), mkBinding(r2, { assertion: 'Local needs hardware.' })]);
  const out = srr.buildReview(inputFor(env, v1, v2, { research: {
...
test('SRR22: capped diff flags human attention', () => {
  const env = mkEnv();
  const big1 = Array.from({ length: 200 }, (_, i) => ({ order: i + 1, type: 'dialogue', beat: 'b', dialogue: `Line ${i} one.` }));
  const big2 = Array.from({ length: 200 }, (_, i) => ({ order: i + 1, type: 'dialogue', beat: 'b', dialogue: `Line ${i} two.` }));
  const { v1, v2 } = mkVersions(env, big1, big2);
  const out = srr.buildReview(inputFor(env, v1, v2));
  assert.ok(out.bundle.diff_summary.truncated || out.bundle.diff_summary.added > 0);
  assert.ok(['NEEDS_HUMAN_DECISION', 'READY_FOR_STORY_REVIEW'].includes(out.state));
});
