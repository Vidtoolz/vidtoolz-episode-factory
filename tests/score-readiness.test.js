...
test("readiness: approved plan without candidates is ready-to-render, not Resolve-ready", () => {
  ...
  assert.match(st.readiness.resolve_ready_requires, /technical audio QC \+ exact-byte human listening approval \+ explicit final selection \+ hash-checked Resolve copy/, "Resolve readiness requires production, listening, and selection gates");
});
