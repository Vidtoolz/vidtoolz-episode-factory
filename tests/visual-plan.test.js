...
test('HVP-T85 required-beat derivation creates one beat per Story section in order', () => { const beats = vp.deriveRequiredBeats({ project_id: 'p1', version_id: 'v1', sections: [{ id: 's1', order: 1 }, { id: 's2', order: 2 }] }); assert.deepEqual(beats.map((beat) => beat.section_id), ['s1', 's2']); });
test('HVP-T86 derived beat IDs are unique Visual Plan ULIDs', () => { const beats = vp.deriveRequiredBeats({ project_id: 'p1', version_id: 'v1', sections: [{ id: 's1', order: 1 }, { id: 's2', order: 2 }] }); assert.equal(new Set(beats.map((beat) => beat.canonical_beat_id)).size, 2); assert.ok(beats.every((beat) => /^visual-beat-[0-9ABCDEFGHJKMNPQRSTVWXYZ]{26}$/.test(beat.canonical_beat_id))); });
test('HVP-T87 derived beat alias binds exact Script Builder section', () => { const [beat] = vp.deriveRequiredBeats({ project_id: 'p1', version_id: 'v1', sections: [{ id: 's1', order: 1 }] }); assert.deepEqual(beat.aliases, [{ namespace: 'vidtoolz-script-builder/section', id: 's1' }]); });
test('HVP-T88 derived beat provenance binds exact project/version/section', () => { const [beat] = vp.deriveRequiredBeats({ project_id: 'p1', version_id: 'v1', sections: [{ id: 's1', order: 1 }] }); assert.deepEqual(beat.source_provenance, { source_system: 'vidtoolz-script-builder', source_id: 'p1/v1/s1' }); });
test('HVP-T89 duplicate Story section IDs fail derivation', () => assert.throws(() => vp.deriveRequiredBeats({ project_id: 'p1', version_id: 'v1', sections: [{ id: 's1', order: 1 }, { id: 's1', order: 2 }] }), (error) => error.code === 'STORY_SECTION_ID_DUPLICATE'));
test('HVP-T90 missing or empty Story sections fail derivation', () => { assert.throws(() => vp.deriveRequiredBeats({ project_id: 'p1', version_id: 'v1' }), (error) => error.code === 'STORY_SECTIONS_REQUIRED'); assert.throws(() => vp.deriveRequiredBeats({ project_id: 'p1', version_id: 'v1', sections: [] }), (error) => error.code === 'STORY_SECTIONS_REQUIRED'); });
test('HVP-T91 noncanonical Story section order fails derivation', () => assert.throws(() => vp.deriveRequiredBeats({ project_id: 'p1', version_id: 'v1', sections: [{ id: 's2', order: 2 }] }), (error) => error.code === 'STORY_SECTION_ORDER_INVALID'));
test('HVP-T92 missing Story section identity fails derivation', () => assert.throws(() => vp.deriveRequiredBeats({ project_id: 'p1', version_id: 'v1', sections: [{ order: 1 }] }), (error) => error.code === 'STORY_SECTION_ID_INVALID'));
test('HVP-T93 malformed Story identity fails derivation', () => assert.throws(() => vp.deriveRequiredBeats({ project_id: '', version_id: 'v1', sections: [{ id: 's1', order: 1 }] }), (error) => error.code === 'STORY_IDENTITY_INVALID'));
test('HVP-T94 repeat derivation preserves bindings but mints fresh opaque IDs', () => { const story = { project_id: 'p1', version_id: 'v1', sections: [{ id: 's1', order: 1 }] }; const a = vp.deriveRequiredBeats(story); const b = vp.deriveRequiredBeats(story); assert.notEqual(a[0].canonical_beat_id, b[0].canonical_beat_id); assert.deepEqual(a[0].aliases, b[0].aliases); assert.deepEqual(a[0].source_provenance, b[0].source_provenance); });
test('HVP-T95 canonical beat ID is not content- or section-derived', () => { const [beat] = vp.deriveRequiredBeats({ project_id: 'p1', version_id: 'v1', sections: [{ id: 'section-semantic-name', order: 1, dialogue: 'content' }] }); assert.equal(beat.canonical_beat_id.includes('section-semantic-name'), false); assert.equal(beat.canonical_beat_id.includes('content'), false); });
test('HVP-T96 infographic cannot claim still-generation mode', () => { const p = makePlan(); p.shots[0].media_type = 'INFOGRAPHIC'; p.shots[0].generation_mode = 'STILL'; p.shots[0].generation_requirements.artifact_class = 'infographic'; p.shots[0].generation_requirements.generation_mode = 'STILL'; refresh(p); expectCode(validate(p), 'MEDIA_GENERATION_MODE_MISMATCH'); });
test('HVP-T97 text graphic uses deterministic non-generation mode', () => { const p = makePlan(); p.shots[0].media_type = 'TEXT_GRAPHIC'; p.shots[0].generation_mode = 'NOT_APPLICABLE'; p.shots[0].generation_requirements.artifact_class = 'text_graphic'; p.shots[0].generation_requirements.generation_mode = 'NOT_APPLICABLE'; p.prompts[0].prompt_type = 'TEXT_GRAPHIC'; p.prompts[0].shot_intent_digest_sha256 = vp.shotIntentDigest(p.shots[0]); refresh(p); assert.equal(validate(p).ok, true); });
