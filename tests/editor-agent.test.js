...
test('ED35 stale human acceptance cannot elevate successor',()=>{const {fx,out:first}=runFx();const approval=ep.createEditApprovalBinding(first.edit_plan,{approver:{type:'HUMAN',id:'TEST_HUMAN'},approved_at:NOW,scope:'test acceptance'},fx.options);fx.task.action='revise_edit';fx.task.previous_edit_plan=first.edit_plan;fx.task.human_edit_approval=approval;fx.task.edit_plan_spec.timeline.output_class='REV2';const second=editor.run(fx.task,fx.options);assert.equal(second.human_acceptance.state,'INVALID_OR_STALE');assert.equal(second.state,'ROUGH_CUT_READY_FOR_QC');});
