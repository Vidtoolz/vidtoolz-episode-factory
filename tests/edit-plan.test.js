...
test('EP human edit approval exact binding verifies', () => { const { plan, ctx } = build(); const approval = ep.createEditApprovalBinding(plan, { approver: { type: 'HUMAN', id: 'TEST_HUMAN' }, approved_at: NOW, scope: 'rough-cut-human-acceptance' }, ctx); assert.equal(ep.verifyEditApprovalBinding(plan, approval, ctx).ok, true); });
test('EP plan mutation stales human approval', () => { const { plan, ctx } = build(); const approval = ep.createEditApprovalBinding(plan, { approver: { type: 'HUMAN', id: 'TEST_HUMAN' }, approved_at: NOW, scope: 'rough-cut-human-acceptance' }, ctx); const next = ep.createSuccessorEditPlan(plan, (p) => { p.timeline.output_class = 'revision'; }, ctx); assert.equal(ep.verifyEditApprovalBinding(next, approval, ctx).ok, false); });
